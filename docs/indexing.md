...
> This article is based on XLA documentation on indexing

TODO: The urls should be changed with ZKX ones.
...
For a broadcast from `tensor<20xu32>` to `tensor<10x20x30xu32>`

```c
p0 = u32[20] parameter(0)
bc0 = u32[10, 20, 30] broadcast(p0), dimensions={1}
```

the indexing map from the output to input is `(i, j, k) -> (j)` for
`i in [0, 10]`, `j in [0, 20]` and `k in [0, 30]`.

## Motivation

ZKX GPU uses several bespoke solutions to reason about coalescing, operand
utilization, and tiling schemes (more details below). The goal of indexing
analysis is to provide a reusable component for such use cases. Indexing
analysis is built on MLIR's Affine Map infrastructure and adds HLO semantics.
...
Operand utilization in ZKX indicates how much each input of the instruction is
used assuming its output is fully used. Indexing analysis allows to compute
utilization precisely.

### Tiling

A tile/slice is a hyper-rectangular subset of a tensor parameterized by offsets,
...
is already a [library] that does it for dot. Tile propagation can be made more
generic and robust if it is expressed via indexing maps.

## Function and Domain

The indexing map is a function **f**(**x**) = **f**(**d**, **r**, **rt**) that
maps a multi-index **d** of a tensor `A` to elements/ranges of tensor `B`. The
parameter **r** refers to the ranges of indices of the dimensions that are
present in tensor `B`, but not in tensor `A`​. The parameter **rt** refers to
the runtime values, e.g. indices for a gather op.

For example, if we have a reduction from `tensor<2x4x8x16xu32>` to
`tensor<4x8xu32>`, then the indexing map from the 2D output to the 4D input is
...
computations. ZKX already depends on MLIR, so we use [mlir::AffineMap] instead
of writing a yet-another symbolic arithmetic library.
...
`AffineMap` has two types of parameters: _dimensions_ and _symbols_. The
_dimensions_ correspond to the dimension variables _d_, while _symbols_
correspond to the range variables _r_ and RT variables _rt_. `AffineMap` does
not contain any metadata about ranges of the dimensions, so we have to provide
this data ourselves.
...
`dim_vars_` encode the **inclusive** box constraints for the dimension variables
**d** of the indexing map, which usually coincide with the shape of the output
tensor for ops like transpose, reduce, elementwise, dot, but there are some
exceptions like [HloConcatenateInstruction].

`range_vars_` encode possible values that **r** parameters can take.

`rt_vars_` encode the feasible values in runtime. For example, if the offset is
...
```c++
  p0 = u32[10, 20] parameter(0)
  p1 = u32[10, 20] parameter(1)
  add = u32[10, 20] add(p0, p1)
```
...
Those are the symbols that represent ranges of values. For example, in this
particular case every element of input with index `d0` is mapped to a 10x1x30
slice of the output.
...
```c++
operand = u32[33,76,70] parameter(0)
indices = s32[1806,2] parameter(1)
gather = u32[1806,7,8,4] gather(operand, indices),
  offset_dims={1,2,3},
...
```c+
p0 = u32[3, 12288, 6, 128] parameter(0)
transpose = u32[3, 6, 128, 12288] transpose(p0), dimensions={0, 2, 3, 1}
```
...
Indexing map for reverse changes the reverted dimensions to
`upper_bound(d_i) - d_i`:

```c+
p0 = u32[1, 17, 9, 9] parameter(0)
reverse = u32[1, 17, 9, 9] reverse(p0), dimensions={1, 2}
```
...
```c+
p0 = u32[256,10] parameter(0)
p0_init = u32[] constant(-inf)
p1 = s32[256,10] parameter(1)
p1_init = s32[] constant(0)
reduce = (u32[10], s32[10]) reduce(p0, p1, p0_init, p1_init),
...
```c+
p0 = u32[10, 20, 50] parameter(0)
slice = u32[5, 3, 25] slice(u32[10, 20, 50] p0),
...
```c+
p0 = u32[4,8] parameter(0)
reshape = u32[2, 4, 4] reshape(p0)
```

This reshape can be represented as a composition of collapse shape of
`tensor<4x8xu32>` to `tensor<32xu32>` and then a shape expansion to
`tensor<2x4x4xu32>`.
...
```c+
p0 = u32[4, 8, 12] parameter(0)
reshape = u32[32, 3, 4] reshape(p0)
```

This reshape can be represented as a composition of two reshapes. The first one
collapses the outermost dimensions `tensor<4x8x12xu32>` to `tensor<32x12xu32>`
and the second one expand the innermost dimension `tensor<32x12xu32>` into
`tensor<32x3x4xu32>`.
...
A bitcast op can be represented as a [sequence of transpose-reshape-transpose].
...
```c+
p0 = u32[2, 5, 7] parameter(0)
p1 = u32[2, 11, 7] parameter(1)
p2 = u32[2, 17, 7] parameter(2)
ROOT concat = u32[2, 33, 7] concatenate(u32[2, 5, 7] p0, u32[2, 11, 7] p1, u32[2, 17, 7] p2), dimensions={1}
```
...
```c+
p0 = u32[4, 128, 256] parameter(0)
p1 = u32[4, 256, 64] parameter(1)
dot = u32[4, 128, 64] dot(p0, p1),
...
p1 = u32[] parameter(1)
pad = u32[12, 16] pad(p0, p1), padding=1_4_1x4_8_0
```

The padding config `1_4_1x4_8_0` denotes
...
```c+
f {
  p0 = u32[1000, 1000] parameter(0)
  transpose_p0 = u32[1000, 1000]{0, 1} transpose(p0), dimensions={1, 0}
  ROOT a0 = u32[1000, 1000] add(p0, transpose_p0)
}
```

The output-to-input indexing maps for `p0` will be `(d0, d1) -> (d0, d1)` and
`(d0, d1) -> (d1, d0)`. It means that to compute one element of the output we
might need to read the input parameter twice.

### One input, deduplicated indexing map

**TODO(chokobole)**: change this image with the `u32` one.
...
```c+
f {
  p0 = u32[20, 10, 50] parameter(0)
  lhs_transpose_1 = u32[10, 20, 50] transpose(p0), dimensions={1, 0, 2}
  lhs_e = u32[10, 20, 50] exponential(lhs_transpose_1)
  lhs_transpose_2 = u32[10, 50, 20] transpose(lhs_e), dimensions={0, 2, 1}
  rhs_transpose_1 = u32[50, 10, 20] transpose(p0), dimensions={2, 1, 0}
  rhs_log = u32[50, 10, 20] exponential(rhs_transpose_1)
  rhs_transpose_2 = u32[10, 50, 20] transpose(rhs_log), dimensions={1, 0, 2}
  ROOT add = u32[10, 50, 20] add(lhs_transpose_2, rhs_transpose_2)
}
```
...
1. `(d0, d1) -> (d0 + d1 floordiv 16, d1 mod 16)` for **d** in
   `[0, 6] x [0, 14]` becomes `(d0, d1) -> (d0, d1)`
1. `(d0, d1, d2) -> ((100d0 + 10d1 + d2) floorDiv 100, ((100d0 + 10d1 + d2) mod 100) floordiv 10, d2 mod 10)`
   for `di in [0, 9]` becomes `(d0, d1, d2) -> (d0, d1, d2)`.
1. `(d0, d1, d2) -> ((16d0 + 4d1 + d2) floordiv 8, (16d0 + 4d1 + d2) mod 8)` for
   `d_i in [0, 9]` becomes
   `(d0, d1, d2) -> (2d0 + (4d1 + d2) floordiv 8,(4d1 + d2) mod 8)`.
1. `(d0, d1) -> (-(-11d0 - d1 + 109) floordiv 11 + 9)` for **d** in
   `[0, 9] x [0, 10]` becomes `(d0, d1) -> (d0)`.

Indexing map simplifier allows us to understand that some of the chained
reshapes in HLO cancel each other.

```c+
p0 = u32[10, 10, 10] parameter(0)
reshape1 = u32[50, 20] reshape(p0)
reshape2 = u32[10, 10, 10] reshape(reshape1)
```
...
1. Constraints that are always satisfied, e.g. `d0 + s0 in [0, 20]` for
   `d0 in [0, 5]` and `s0 in [1, 3]` are eliminated.
1. Affine expressions in the constraints are optimized as the indexing affine
   map above.

For more examples see [indexing_map_test.cc].

**TODO(chokobole)**: Some of the links below should be changed with ZKX ones.
...
[gather_simplifier.h]: https://github.com/openxla/xla/blob/main/xla/hlo/transforms/simplifiers/gather_simplifier.h
[hloconcatenateinstruction]: https://github.com/openxla/stablehlo/blob/main/docs/spec.md#concatenate
[indexing_map_test.cc]: https://github.com/openxla/xla/blob/main/xla/hlo/analysis/indexing_map_test.cc
