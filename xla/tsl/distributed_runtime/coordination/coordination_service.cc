...
constexpr absl::Duration kServiceToClientTimeout = absl::Seconds(10);
...
class CoordinationServiceStandaloneImpl : public CoordinationServiceInterface {
 ...
 private:
  ...
  BarrierCallback ConnectAfterBarrierPasses(std::string_view task_name,
  ...
  class ErrorPollingState {
   public:
    ...
    // service.
    bool IsTaskPolling(std::string_view task_name) const {
    ...
    void RemoveTask(const CoordinatedTask& task, std::string_view reason);
  ...
  class TaskState {
   public:
    ...
    explicit TaskState(std::string_view task) { task_name_ = task; }
...
CoordinationServiceStandaloneImpl::ConnectAfterBarrierPasses(
    std::string_view task_name, uint64_t incarnation, StatusCallback done) {
...
void CoordinationServiceStandaloneImpl::PropagateError(
  ...
  call_opts.SetTimeout(kServiceToClientTimeout);
...
bool CoordinationServiceStandaloneImpl::InitializeBarrier(
  ...
  barrier->deadline_in_micros =
      Env::Default()->NowMicros() + (absl::ToInt64Microseconds(timeout));
