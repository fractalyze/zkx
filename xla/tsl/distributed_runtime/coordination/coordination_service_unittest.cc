...
TestDevice CreateTestDevice(std::string_view name, int local_id = 0) {
  TestDevice device;
...
TEST_F(CoordinateTwoTasksTest, TestTaskHeartbeatTimeout) {
  ...
  Env::Default()->Sleep(2 * kHeartbeatTimeout);
...
TEST_F(CoordinateTwoTasksTest, ReportedErrorCanPropagateThroughErrorPolling) {
  ...
  TF_ASSERT_OK(coord_service_->ReportTaskError(
      task_1_, absl::InternalError("test_error")));
...
TEST_F(CoordinateTwoTasksTest, InsertKeyValue_Duplicate_Fail) {
  EnableCoordinationService();
  TF_ASSERT_OK(coord_service_->InsertKeyValue("key0", "original_value"));
...
TEST_F(CoordinateTwoTasksTest, Reset_HeartbeatsAreAcceptedForAGracePeriod) {
  ...
  // period.
  Env::Default()->Sleep(3 * kHeartbeatTimeout);
  EXPECT_THAT(coord_service_->RecordHeartbeat(task_0_, incarnation_0_),
              StatusIs(absl::StatusCode::kInvalidArgument));
}
...
TEST_F(CoordinateTwoTasksTest,
  ...
  Env::Default()->Sleep(absl::Seconds(1));
...
TEST(CoordinationServiceTest, RecoverableAndNonRecoverableTasks) {
  ...
  // interactions.
  TF_ASSERT_OK(coord_service->ResetTask(task_0));
  TF_ASSERT_OK(coord_service->RegisterTask(task_0, /*incarnation=*/1));
  coord_service->PollForErrorAsync(
      task_0, [&s0](const absl::Status& status) { s0 = status; });
...
TEST_F(CoordinateTwoTasksTest, DoNotAllowPollForErrorAfterReset) {
  ...
  absl::Status s;
  TF_ASSERT_OK(coord_service_->RegisterTask(task_0_, incarnation_0_));
  TF_ASSERT_OK(coord_service_->ResetTask(task_0_));
