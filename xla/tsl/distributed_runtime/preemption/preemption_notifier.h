...
#define REGISTER_PREEMPTION_NOTIFIER_UNIQ_HELPER(counter, notifier_type_name, \
                                                 factory_fn)                  \
  static bool static_preemption_notifier_##counter ABSL_ATTRIBUTE_UNUSED =    \
      []() {                                                                  \
...
class PreemptionNotifier {
 public:
  ...
  static std::unique_ptr<PreemptionNotifier> CreatePreemptionNotifier(
    ...
    const std::unordered_map<std::string, PreemptionNotifierFactory>*
        factories = GetPreemptionNotifierFactories();
 ...
 private:
  ...
  Env* env_;  // Not owned.
  absl::Mutex mu_;
  absl::Time death_time_ ABSL_GUARDED_BY(mu_) = absl::InfinitePast();
  std::vector<PreemptTimeCallback> callbacks_ ABSL_GUARDED_BY(mu_);
};
