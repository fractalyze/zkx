...
class Call : public UntypedCall<Service> {
 public:
  ...
  using EnqueueFunction = void (GrpcService::*)(
      ...
      grpc::ServerAsyncResponseWriter<ResponseMessage>*, grpc::CompletionQueue*,
  ...
  static void EnqueueRequestForMethod(
      GrpcService* grpc_service, grpc::ServerCompletionQueue* cq, int method_id,
      HandleRequestFunction handle_request_function, bool supports_cancel) {
    auto call = new Call<Service, GrpcService, RequestMessage, ResponseMessage>(
        handle_request_function);
    if (supports_cancel) {
      call->RegisterCancellationHandler();
 ...
 private:
  ...
  absl::Mutex mu_;
  std::function<void()> cancel_callback_ ABSL_GUARDED_BY(mu_);
};
