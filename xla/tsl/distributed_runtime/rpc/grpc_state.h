...
// has dependencies on many TensorFlow-specific abstractions.
...
class RPCState : public GrpcClientCQTag {
 public:
  RPCState(
      ...
      std::function<bool(grpc::ByteBuffer*, Response*)> parse_proto_fn =
          [](grpc::ByteBuffer* src, Response* dst) {
            return GrpcMaybeParseProto(src, dst);
          })
      : RPCState(
            ...
            (call_opts != nullptr ? call_opts->GetTimeout()
                                  : absl::ZeroDuration()),
            max_retries, target, parse_proto_fn) {}
  ...
  RPCState(
      ...
      thread::ThreadPool* threadpool, bool fail_fast, absl::Duration timeout,
  ...
  void StartCall() {
    ...
    if (timeout_ > absl::ZeroDuration()) {
      context_->set_deadline(gpr_time_from_millis(
          absl::ToInt64Milliseconds(timeout_), GPR_TIMESPAN));
    }
    if (call_opts_) {
  ...
  void OnCompleted(bool ok) override {
    ...
    if (++num_retries_ <= max_retries_ &&
      ...
      ComputeRetryBackoff(/*min_backoff=*/absl::Milliseconds(1),
                          /*max_backoff=*/absl::Seconds(1));
      Env::Default()->SchedClosureAfter(retry_backoff_,
                                        [this]() { StartCall(); });
    } else {
      ...
      absl::StrAppend(&error_msg, ":\n:", context_->debug_error_string());
      s = errors::internal::CreateWithUpdatedMessage(s, error_msg);
 ...
 private:
  void ComputeRetryBackoff(absl::Duration min_backoff,
                           absl::Duration max_backoff) {
    constexpr float kBackoffBase = 1.3;
    if (retry_backoff_ < absl::ZeroDuration()) {
      retry_backoff_ = min_backoff;
    } else {
      retry_backoff_ *= kBackoffBase;
      if (retry_backoff_ > max_backoff) {
        retry_backoff_ = max_backoff;
      }
    }
  }
  ...
  size_t max_retries_;
  absl::Duration retry_backoff_ = absl::Milliseconds(-1);
