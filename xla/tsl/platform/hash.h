...
#ifndef XLA_TSL_PLATFORM_HASH_H_
#define XLA_TSL_PLATFORM_HASH_H_
...
//
// The second type argument is only used for SFINAE below.
...
struct hash<std::string_view> {
  size_t operator()(std::string_view sp) const {
    return static_cast<size_t>(Hash64(sp.data(), sp.size()));
  }
};
using StringPieceHasher = ::tsl::hash<std::string_view>;
...
#endif  // XLA_TSL_PLATFORM_HASH_H_
