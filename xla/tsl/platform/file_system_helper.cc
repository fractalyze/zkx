...
std::vector<std::string> AllDirectoryPrefixes(std::string_view d) {
  ...
  while (!dir.empty()) {
    ...
    std::string_view new_dir(io::Dirname(dir));
...
inline int GetFirstGlobbingEntry(const std::vector<std::string>& dirs) {
  int i = 0;
...
absl::Status GetMatchingPaths(FileSystem* fs, Env* env,
  ...
  while (!expand_queue.empty()) {
    ...
    auto handle_level = [&fs, &results, &dirs, &expand_queue,
      ...
      const std::pair<std::string, int>& queue_item = expand_queue.at(i);
