...
template <typename InnerMatcher>
internal_status::IsOkAndHoldsMatcher<std::decay_t<InnerMatcher>> IsOkAndHolds(
    InnerMatcher&& inner_matcher) {
  return internal_status::IsOkAndHoldsMatcher<std::decay_t<InnerMatcher>>(
      std::forward<InnerMatcher>(inner_matcher));
}
...
internal_status::StatusIsMatcher StatusIs(absl::StatusCode code,
                                          MessageMatcher message_matcher) {
  return internal_status::StatusIsMatcher(code, std::move(message_matcher));
}
...
// Remove this specialization when tensorflow::Status is absl::Status
template <>
inline internal_status::StatusIsMatcher StatusIs(absl::StatusCode code) {
  return StatusIs(code, ::testing::_);
}
