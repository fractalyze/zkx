...
constexpr int kMaxAggregatedStatusMessageSize = 8 * 1024;
constexpr int kMaxAttachedLogMessageSize = 512;
...
class StatusLogSink : public zkx::base::LogSink {
 public:
  ...
  void enable() {
    absl::call_once(flag_, [this] {
      ...
      if (const char* num_msgs_str =
              getenv("ZKX_WORKER_NUM_FORWARDED_LOG_MESSAGES")) {
        if (!absl::SimpleAtoi(num_msgs_str, &num_messages_)) {
          LOG(WARNING) << "Failed to parse env variable "
                          "ZKX_WORKER_NUM_WARNING_ERROR_LOG_IN_STATUS="
  ...
  void GetMessages(std::vector<std::string>* logs) ABSL_LOCKS_EXCLUDED(mu_) {
  ...
  void Send(const zkx::base::LogEntry& entry) override
 ...
 private:
  ...
  int num_messages_ = 0;
  std::deque<std::string> messages_ ABSL_GUARDED_BY(mu_);
};
...
std::unordered_map<std::string, absl::Cord> StatusGroup::GetPayloads() const {
  ...
  auto capture_payload = [&payloads](std::string_view key,
