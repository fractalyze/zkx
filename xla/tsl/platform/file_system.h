...
#include "xla/tsl/platform/cord.h"
...
class FileSystem {
 public:
  ...
  virtual std::string_view Basename(std::string_view path) const;
  ...
  /// paths with respect to the actual working directory.  That is, this is
  /// purely string manipulation, completely independent of process state.
  std::string CleanPath(std::string_view path) const;
  ...
  bool IsAbsolutePath(std::string_view path) const;
  ...
  /// empty.
  void ParseURI(std::string_view remaining, std::string_view* scheme,
                std::string_view* host, std::string_view* path) const;
...
class WrappedFileSystem : public FileSystem {
 public:
  ...
  std::string_view Basename(std::string_view path) const override {
...
class RandomAccessFile {
 public:
  ...
  /// filesystem.
  virtual absl::Status Name(std::string_view* result) const {
  ...
  /// On non-OK returned status: `[0..n]` bytes have been stored in
  /// `*result`.
  ///
  ...
  virtual absl::Status Read(uint64_t offset, size_t n, std::string_view* result,
...
class WritableFile {
 public:
  ...
  virtual absl::Status Append(std::string_view data) = 0;
