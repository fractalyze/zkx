...
absl::StatusOr<std::string> ProtoToHumanReadableJson(
  ...
  auto status =
      google::protobuf::util::MessageToJsonString(proto, &result, json_options);
  if (!status.ok()) {
    // Convert error_msg google::protobuf::StringPiece to
    // tsl::StringPiece.
    auto error_msg = status.message();
    return absl::InternalError(
        absl::StrCat("Could not convert proto to JSON string: ",
                     std::string_view(error_msg.data(), error_msg.length())));
  }
...
absl::Status HumanReadableJsonToProto(const std::string& str,
  ...
  if (!status.ok()) {
    ...
    auto error_msg = status.message();
    return absl::InternalError(
        absl::StrCat("Could not convert JSON string to proto: ",
                     std::string_view(error_msg.data(), error_msg.length())));
  }
