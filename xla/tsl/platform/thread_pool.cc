...
struct EigenEnvironment {
  using EnvThread = tsl::Thread;

  struct TaskImpl {
    std::function<void()> fn;
    // TODO(chokobole): Uncomment this. Dependency: Context, Tracer
    // Context context;
    // uint64_t trace_id;
  };
  ...
  struct Task {
    ...
    explicit Task(std::function<void()> fn) : f(TaskImpl{std::move(fn)}) {}
  ...
  EigenEnvironment(tsl::Env* env, const tsl::ThreadOptions& thread_options,
  ...
  Task CreateTask(std::function<void()> f) {
    ...
    // }
    // TODO(chokobole): Do we need Context?.
...
ThreadPool::ThreadPool(tsl::Env* env, std::string_view name, int num_threads)
    : ThreadPool(env, tsl::ThreadOptions(), name, num_threads, true, nullptr) {}

ThreadPool::ThreadPool(tsl::Env* env, const tsl::ThreadOptions& thread_options,
                       std::string_view name, int num_threads)
...
ThreadPool::ThreadPool(tsl::Env* env, const tsl::ThreadOptions& thread_options,
  ...
  eigen_threadpool_.reset(new Eigen::ThreadPoolTempl<EigenEnvironment>(
      ...
      EigenEnvironment(env, thread_options, absl::StrCat("zkx_", name))));
...
void ThreadPool::ParallelForFixedBlockSizeScheduling(
  ...
  absl::BlockingCounter counter(num_shards_used);
...
void ThreadPool::ParallelFor(int64_t total, int64_t cost_per_unit,
                             const std::function<void(int64_t, int64_t)>& fn) {
  CHECK_GE(total, 0);
  CHECK_EQ(total, int64_t{static_cast<Eigen::Index>(total)});
