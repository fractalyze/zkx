...
const int64_t kInt64min = static_cast<int64_t>(~0x7FFFFFFFFFFFFFFFll);
...
TEST(safe_strto32, Int32s) {
  ...
  EXPECT_EQ(true, absl::SimpleAtoi(std::string_view("123", 1), &result));
  EXPECT_EQ(1, result);
  EXPECT_EQ(true, absl::SimpleAtoi(std::string_view(" -123", 4), &result));
  EXPECT_EQ(-12, result);
  EXPECT_EQ(false, absl::SimpleAtoi(std::string_view(nullptr, 0), &result));
}
...
TEST(safe_strtou32, UInt32s) {
  ...
  // Check that the StringPiece's length is respected.
  EXPECT_TRUE(absl::SimpleAtoi(std::string_view("123", 1), &result));
  EXPECT_EQ(1, result);
  EXPECT_TRUE(absl::SimpleAtoi(std::string_view(" 123", 3), &result));
  EXPECT_EQ(12, result);
  EXPECT_FALSE(absl::SimpleAtoi(std::string_view(nullptr, 0), &result));
}
