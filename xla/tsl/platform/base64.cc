...
absl::Status DecodeThreeChars(const char* codes, char* result) {
  ...
  if (ABSL_PREDICT_FALSE((packed & 0xFF000000) != 0)) {
...
absl::Status Base64Decode(absl::string_view data, T* decoded) {
  ...
  if (ABSL_PREDICT_FALSE(remain == 1)) {
...
template absl::Status Base64Decode<std::string>(std::string_view data,
                                                std::string* decoded);
template absl::Status Base64Encode<std::string>(std::string_view source,
                                                std::string* encoded);
template absl::Status Base64Encode<std::string>(std::string_view source,
                                                bool with_padding,
                                                std::string* encoded);
