...
class RandomAccessFilePosix : public RandomAccessFile {
 private:
 ...
 public:
  ...
  absl::Status Name(std::string_view* result) const override {
    *result = filename_;
    return absl::OkStatus();
  }

  absl::Status Read(uint64_t offset, size_t n, std::string_view* result,
    ...
    while (n > 0 && s.ok()) {
      ...
      if (r > 0) {
        ...
        s = errors::IOError(filename_, errno);
      }
    }
    *result = std::string_view(scratch, dst - scratch);
    return s;
  }
  ...
  absl::Status Read(uint64_t offset, size_t n,
    ...
    absl::Cord tmp_cord = absl::MakeCordFromExternal(
        std::string_view(static_cast<char*>(scratch), tmp.size()),
        [scratch](std::string_view) { delete[] scratch; });
...
class WritableFilePosix : public WritableFile {
 ...
 public:
  ...
  absl::Status Append(std::string_view data) override {
    ...
    if (r != data.size()) {
      return errors::IOError(filename_, errno);
    }
    return absl::OkStatus();
  }
  ...
  absl::Status Close() override {
    if (file_ == nullptr) {
      return errors::IOError(filename_, EBADF);
    }
    absl::Status result;
    if (fclose(file_) != 0) {
      result = errors::IOError(filename_, errno);
    }
    file_ = nullptr;
    return result;
  }
...
class ReadOnlyMemoryRegionPosix : public ReadOnlyMemoryRegion {
 public:
  ReadOnlyMemoryRegionPosix(const void* address, uint64_t length)
      : address_(address), length_(length) {}
  ~ReadOnlyMemoryRegionPosix() override {
...
absl::Status FileSystemPosix::NewRandomAccessFile(
  ...
  if (fd < 0) {
    s = errors::IOError(fname, errno);
  } else {
    result->reset(new RandomAccessFilePosix(translated_fname, fd));
  }
  return s;
}
...
absl::Status FileSystemPosix::NewWritableFile(
  ...
  if (f == nullptr) {
    ...
    result->reset(new WritableFilePosix(translated_fname, f));
  }
  return s;
}
...
absl::Status FileSystemPosix::NewReadOnlyMemoryRegionFromFile(
  ...
  if (fd < 0) {
    ...
    if (address == MAP_FAILED) {
      ...
      result->reset(new ReadOnlyMemoryRegionPosix(address, st.st_size));
    }
    if (close(fd) < 0) {
      s = errors::IOError(fname, errno);
    }
...
absl::Status FileSystemPosix::GetChildren(std::string_view dir,
  ...
  if (d == nullptr) {
    return errors::IOError(dir, errno);
  }
  struct dirent* entry;
  while ((entry = readdir(d)) != nullptr) {
    std::string_view basename = entry->d_name;
...
absl::Status FileSystemPosix::DeleteFile(std::string_view fname,
  ...
  if (unlink(TranslateName(fname).c_str()) != 0) {
    result = errors::IOError(fname, errno);
  }
  return result;
}
...
absl::Status FileSystemPosix::CreateDir(std::string_view name,
  ...
  if (mkdir(translated.c_str(), 0755) != 0) {
    return errors::IOError(name, errno);
  }
  return absl::OkStatus();
}
...
absl::Status FileSystemPosix::DeleteDir(std::string_view name,
  ...
  if (rmdir(TranslateName(name).c_str()) != 0) {
    result = errors::IOError(name, errno);
  }
  return result;
}
...
absl::Status FileSystemPosix::RenameFile(std::string_view src,
  ...
  if (rename(TranslateName(src).c_str(), TranslateName(target).c_str()) != 0) {
    result = errors::IOError(src, errno);
  }
  return result;
}
...
absl::Status FileSystemPosix::CopyFile(std::string_view src,
  ...
  if (stat(translated_src.c_str(), &sbuf) != 0) {
    return errors::IOError(src, errno);
  }
  ...
  if (target_fd < 0) {
    close(src_fd);
    return errors::IOError(target, errno);
  }
  int rc = 0;
  off_t offset = 0;
  ...
  if (rc < 0) {
    result = errors::IOError(target, errno);
  }
