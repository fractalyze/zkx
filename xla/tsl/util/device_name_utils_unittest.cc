...
static bool IsCSHelper(std::string_view pattern, std::string_view actual) {
...
static bool IsSpecHelper(std::string_view pattern, std::string_view actual) {
  DeviceNameUtils::ParsedName p, a;
  EXPECT_TRUE(DeviceNameUtils::ParseFullName(pattern, &p));
  EXPECT_TRUE(DeviceNameUtils::ParseFullName(actual, &a));
...
static void MergeDevNamesHelperImpl(const std::string& name_a,
  ...
  EXPECT_TRUE(DeviceNameUtils::MergeDevNames(&target_a, Name(name_b),
                                             allow_soft_placement)
                  .ok());
  DeviceNameUtils::ParsedName target_b = Name(name_b);
  EXPECT_TRUE(DeviceNameUtils::MergeDevNames(&target_b, Name(name_a),
                                             allow_soft_placement)
                  .ok());
...
static void MergeOverrideHelper(const std::string& target,
  ...
  EXPECT_TRUE(
      DeviceNameUtils::MergeOverrideDevNames(&parsed_target, Name(name)).ok());
...
TEST(DeviceNameUtilsTest, CanonicalizeDeviceName) {
  std::string canonical_name;
  {
    ...
    EXPECT_TRUE(DeviceNameUtils::CanonicalizeDeviceName(
                    "/job:foo/replica:10/task:0/device:CPU:1", basename,
                    &canonical_name)
                    .ok());
    EXPECT_EQ("/job:foo/replica:10/task:0/device:CPU:1", canonical_name);
    EXPECT_TRUE(DeviceNameUtils::CanonicalizeDeviceName(
                    "/job:foo/task:0/replica:10/device:CPU:1", basename,
                    &canonical_name)
                    .ok());
    EXPECT_EQ("/job:foo/replica:10/task:0/device:CPU:1", canonical_name);
    EXPECT_TRUE(
        DeviceNameUtils::CanonicalizeDeviceName(
            ...
            .ok());
    EXPECT_EQ("/job:foo/replica:10/task:0/device:CPU:1", canonical_name);
    EXPECT_TRUE(DeviceNameUtils::CanonicalizeDeviceName("CPU:0", basename,
                                                        &canonical_name)
                    .ok());
