...
bool IsJobName(std::string_view in) {
  return !in.empty() && IsAlpha(in.front()) &&
         std::all_of(in.begin(), in.end(), IsAlphaNumOrUnderscore);
}

bool ConsumePrefix(std::string_view* in, std::string* out,
                   std::string_view prefix_terminators) {
...
std::string DeviceName(const std::string& job, int replica, int task,
...
bool DeviceNameUtils::ParseFullName(std::string_view fullname, ParsedName* p) {
  p->Clear();
  if (fullname == "/") {
    return true;
  }
...
bool DeviceNameUtils::ParseFullOrLocalName(std::string_view fullname,
                                           ParsedName* p) {
...
absl::Status DeviceNameUtils::CanonicalizeDeviceName(
    std::string_view fullname, std::string_view basename,
  ...
  *canonical_name = "";
  ...
  if (!(parsed_basename.has_job && parsed_basename.has_replica &&
    ...
    return absl::InvalidArgumentError(absl::StrCat("Basename: ", basename,
                                                   " should be fully "
                                                   "specified."));
  }
...
// static
bool DeviceNameUtils::IsSameAddressSpace(std::string_view src,
                                         std::string_view dst) {
  ParsedName x;
  ParsedName y;
...
// static
std::string DeviceNameUtils::LocalName(std::string_view type, int id) {
...
// '/device:type:id').
std::string LegacyLocalName(std::string_view type, int id) {
...
// static
std::string DeviceNameUtils::LocalName(std::string_view fullname) {
  ParsedName x;
...
// static
bool DeviceNameUtils::ParseLocalName(std::string_view name, ParsedName* p) {
...
// static
bool DeviceNameUtils::SplitDeviceName(std::string_view name, std::string* task,
                                      std::string* device) {
  ParsedName pn;
