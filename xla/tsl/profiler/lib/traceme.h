...
==============================================================================*/
#ifndef XLA_TSL_PROFILER_LIB_TRACEME_H_
#define XLA_TSL_PROFILER_LIB_TRACEME_H_
...
class TraceMe {
 public:
  ...
  explicit TraceMe(const char* raw, int level = 1,
                   uint64_t filter_mask = kTraceMeDefaultFilterMask)
      : TraceMe(std::string_view(raw), level, filter_mask) {}
  ...
  // Returns the activity ID, which is used to stop the activity.
  static int64_t ActivityStart(
      std::string_view name, int level = 1,
      uint64_t filter_mask = kTraceMeDefaultFilterMask) {
    if (ABSL_PREDICT_FALSE(TraceMeRecorder::Active(level) &&
                           TraceMeRecorder::CheckFilter(filter_mask))) {
      int64_t activity_id = TraceMeRecorder::NewActivityId();
  ...
  static int64_t ActivityStart(
      ...
      uint64_t filter_mask = kTraceMeDefaultFilterMask) {
    return ActivityStart(std::string_view(name), level, filter_mask);
  }
  ...
  static int64_t NewActivityId() { return TraceMeRecorder::NewActivityId(); }
...
#endif  // XLA_TSL_PROFILER_LIB_TRACEME_H_
