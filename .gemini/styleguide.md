...
- When using a **raw pointer** (`T*`) in **class or struct members**, explicitly document ownership.
- If the pointer is **not owned**, add the comment `// not owned`.
...
class Prover {
 ...
 private:
  // not owned
  Context* ctx_;
  std::unique_ptr<Engine> engine_;  // owned
};
```
...
- Tests must be deterministic and runnable independently.
- Add benchmarks for performance-critical code paths when appropriate.
