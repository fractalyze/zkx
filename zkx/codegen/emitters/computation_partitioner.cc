...
PartitionedComputation::PartitionedComputation(
  ...
  std::vector<HloInstruction*> pre_order =
  ...
  for (auto [instr_index, instr] : llvm::enumerate(pre_order)) {
    ...
    if (!is_root && users_have_consistent_indexing && all_users_elementwise) {
      ...
      // `transpose` and `sub` to be in the same function.
      ids[instr_index] = ids[instr_indices[instr->users().front()]];
  ...
  for (auto&& [function_id, instructions] : llvm::enumerate(functions)) {
    ...
    const Shape* first_root_shape = nullptr;
  ...
  for (const Subgraph& subgraph : subgraphs_) {
    for (const HloInstruction* instruction : subgraph.instructions) {
...
PartitionedComputation::Subgraph PartitionedComputation::Subgraph::ForEpilogue(
  ...
  const HloComputation* computation = epilogue.heroes.front()->parent();
  PartitionedComputation::Subgraph subgraph;
  subgraph.name = llvm_ir::SanitizeFunctionName(absl::StrCat(
      computation->name(), "__epilogue__",
      absl::StrJoin(epilogue.roots, "_",
                    [](std::string* out, const HloInstruction* root) {
                      absl::StrAppend(out, root->name());
                    })));
  subgraph.roots = epilogue.roots;

  int index = 0;
  for (const HloInstruction* hero : epilogue.heroes) {
  ...
  auto visit = [&](const HloInstruction* instruction, auto& self) -> void {
  ...
  visit(computation->root_instruction(), visit);
...
PartitionedComputations::PartitionedComputations(
    ...
    const std::vector<EpilogueSpecification>& epilogues)
    : fusion_(fusion) {
  ...
  for (const EpilogueSpecification& epilogue : epilogues) {
    ...
    for (const HloInstruction* instruction : epilogue.heroes) {
...
PartitionedComputations::DeclareFunctions(mlir::ModuleOp module) const {
  ...
  auto create_funcs =
      [&](absl::Span<const PartitionedComputation::Subgraph> subgraphs) {
        for (const PartitionedComputation::Subgraph& subgraph : subgraphs) {
  ...
  for (const PartitionedComputation& computation : partitioned_computations_) {
...
mlir::func::FuncOp CreateSubgraphMlirFunction(
  ...
  const HloComputation* computation = subgraph.roots.front()->parent();
  llvm::SmallVector<mlir::Type> parameter_types;
  llvm::SmallVector<mlir::Type> result_types;

  auto element_type = [&](const Shape& shape) {
    return mlir_utils::PrimitiveTypeToMlirType(shape.element_type(),
                                               b.getContext());
  };

  for (const HloInstruction* root : subgraph.roots) {
    for (mlir::Type ty :
         mlir_utils::ShapeToMlirTypes(root->shape(), b.getContext())) {
  ...
  if (computation->IsFusionComputation() || computation->IsEntryComputation()) {
    for (const HloInstruction* param : computation->parameter_instructions()) {
      parameter_types.push_back(
          mlir_utils::ShapeToMlirTensorType(param->shape(), b.getContext()));
    ...
    for (int64_t size : subgraph.index_ranges) {
      ...
      arg_attrs.emplace_back(mlir::DictionaryAttr::get(
          b.getContext(),
          {b.getNamedAttr("zkx.range", b.getIndexArrayAttr({0, size - 1}))}));
    }
  ...
  mlir::FunctionType ty = b.getFunctionType(parameter_types, result_types);
