...
==============================================================================*/
#ifndef ZKX_CODEGEN_EMITTERS_COMPUTATION_PARTITIONER_H_
#define ZKX_CODEGEN_EMITTERS_COMPUTATION_PARTITIONER_H_
...
//
// There are hooks to customize this partitioning:
// `is_subgraph_root`: Forces the cluster to start a new subgraph at a given
// instruction. The instruction is guaranteed to be in a different subgraph
// than its users.
class PartitionedComputation {
 ...
 private:
  const HloComputation* computation_;  // not owned
...
class PartitionedComputations {
 ...
 private:
  ...
  const HloComputation* fusion_;  // not owned
...
//   (element type, ...) -> element type(s)
//
// Subgraphs of fusions will also have range (zkx.range = [lower_bound,
...
#endif  // ZKX_CODEGEN_EMITTERS_COMPUTATION_PARTITIONER_H_
