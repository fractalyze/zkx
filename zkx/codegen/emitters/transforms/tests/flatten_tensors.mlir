...
func.func @pure_call(%arg0: tensor<10x24xf32>) -> tensor<10x24xf32> {
  %updated_tensor = zkx.pure_call @update(%arg0)
...
// CHECK-NEXT:    zkx.pure_call @update(%[[TENSOR]])
...
func.func @atomic_rmw(%in: tensor<2x4xf32>, %i: index, %j: index)
    -> (tensor<2x4xf32>) {
  %ret = zkx.atomic_rmw %in[%i, %j] : tensor<2x4xf32> {
    ^bb0(%current : f32):
...
// CHECK: #[[$MAP:.+]] = #zkx.indexing_map<"(d0, d1) -> (d0 * 4 + d1), domain: d0 in [0, 1], d1 in [0, 3]">
// CHECK-LABEL: func.func @atomic_rmw(
// CHECK-SAME:      %[[TENSOR:.*]]: tensor<8xf32>, %[[I:.*]]: index,
// CHECK-SAME:      %[[J:.*]]: index) -> tensor<8xf32> {
// CHECK:         %[[INDEX:.*]] = zkx.apply_indexing #[[$MAP]]
// CHECK-SAME:      (%[[I]], %[[J]])
// CHECK:         zkx.atomic_rmw %[[TENSOR]][%[[INDEX]]] : tensor<8xf32>
...
#map = #zkx.indexing_map<"(d0, d1) -> ((d1 * 128 + d0) floordiv 36), domain: d0 in [0, 127], d1 in [0, 393749]">
#map1 = #zkx.indexing_map<"(d0, d1) -> (((d1 * 128 + d0) floordiv 9) mod 4), domain: d0 in [0, 127], d1 in [0, 393749]">
#map2 = #zkx.indexing_map<"(d0, d1) -> ((d1 * 128 + d0) mod 9), domain: d0 in [0, 127], d1 in [0, 393749]">
func.func @if_op(%arg0: tensor<4000x4x9xf32>, %arg1: tensor<1400x1xi32>,
    ...
    %bl_x = gpu.block_id  x {zkx.range = [0 : index, 393749 : index]}
    %0 = zkx.apply_indexing #map(%th_x, %bl_x)
    ...
    %3 = scf.if %2 -> (tensor<4000x4x9xf32>) {
        %4 = zkx.apply_indexing #map1(%th_x, %bl_x)
        %5 = zkx.apply_indexing #map2(%th_x, %bl_x)
        %elem = tensor.extract %arg2[%0, %c0, %4, %5] : tensor<1400x1x4x9xf32>
        %atomic_rmw = zkx.atomic_rmw %arg3[%1, %4, %5] : tensor<4000x4x9xf32> {
        ^bb0(%arg4: f32):
...
func.func @index_switch(%arg0: tensor<2x3xf32>, %arg1: tensor<2x3xf32>,
  ...
  %block_id_y = gpu.block_id  y {zkx.range = [0 : index, 1 : index]}
...
// TODO(chokobole): Enable this. Dependency: zkx_cpu.load
// func.func @cpu_load(%arg0: !zkx_cpu.call_frame) -> tensor<10x10x4xf32> {
//   %0 = zkx_cpu.load %arg0, 0 : tensor<10x10x4xf32>
//   return %0 : tensor<10x10x4xf32>
// }
// C HECK-LABEL: func.func @cpu_load
// C HECK-SAME: -> tensor<400xf32>
// C HECK-NOT:  builtin.unrealized_conversion_cast
...
// }
// C HECK: FlattenTensorsPass failed to converge
...
func.func @if_op_vector(%arg0: vector<8x1xf32>, %value: f32)
  ...
  %c32 = arith.constant 32 : index
  %thread_id_x = gpu.thread_id  x {zkx.range = [0 : index, 127 : index]}
  %block_id_x = gpu.block_id  x {zkx.range = [0 : index, 599 : index]}
