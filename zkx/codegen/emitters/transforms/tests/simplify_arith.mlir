// RUN: emitters_opt %s -split-input-file -zkx-simplify-arith -cse \
// RUN:   -canonicalize | FileCheck %s

module {
  func.func @unknown(%arg0: index {zkx.range = [0 : index, 42 : index]}) -> i1 {
...
module {
  func.func @true(%arg0: index {zkx.range = [12 : index, 42 : index]}) -> i1 {
...
module {
  func.func @false(%arg0: index {zkx.range = [12 : index, 42 : index]}) -> i1 {
    %c5 = arith.constant 5 : index
...
module {
  func.func @rhs_range(%arg0: index {zkx.range = [12 : index, 42 : index]}) -> i1 {
...
module {
  func.func @both_range(%arg0: index {zkx.range = [12 : index, 42 : index]},
                        %arg1: index {zkx.range = [63 : index, 100 : index]}) -> i1 {
...
module {
  func.func @minsi_lhs(%arg0: index {zkx.range = [12 : index, 42 : index]},
                       %arg1: index {zkx.range = [63 : index, 100 : index]}) -> index {
...
module {
  func.func @minsi_rhs(%arg0: index {zkx.range = [12 : index, 42 : index]},
                       %arg1: index {zkx.range = [63 : index, 100 : index]}) -> index {
...
module {
  func.func @maxsi_lhs(%arg0: index {zkx.range = [12 : index, 42 : index]},
                       %arg1: index {zkx.range = [63 : index, 100 : index]}) -> index {
...
module {
  func.func @maxsi_rhs(%arg0: index {zkx.range = [12 : index, 42 : index]},
                       %arg1: index {zkx.range = [63 : index, 100 : index]}) -> index {
...
module {
  func.func @maxsi_add(%arg0: index {zkx.range = [102 : index, 142 : index]},
                       %arg1: index {zkx.range = [63 : index, 100 : index]}) -> index {
...
module {
  func.func @minsi_add(%arg0: index {zkx.range = [102 : index, 142 : index]},
                       %arg1: index {zkx.range = [63 : index, 100 : index]}) -> index {
    %add = arith.addi %arg0, %arg1 : index
...
func.func @refine_constraints(%tensor: tensor<100xf32>) -> tensor<100xf32> {
  ...
  %loop = scf.for %i = %c0 to %c3 step %c1
    ...
    %0 = zkx.apply_indexing #zkx.indexing_map<"(d0) -> (d0 mod 4),"
      "domain: d0 in [0, 9]">(%i)
...
func.func @refine_constraints_for_symbol(%arg0: tensor<2400000x9xf32>,
  ...
  %c42_f32 = arith.constant 42.0 : f32
  %th_x = gpu.thread_id  x {zkx.range = [0 : index, 127 : index]}
  %bl_x = gpu.block_id  x {zkx.range = [0 : index, 575 : index]}
  %0 = scf.for %i = %c0 to %c73 step %c1 iter_args(%arg3 = %arg1)
      -> (tensor<2400000x9xf32>) {
    %2 = scf.for %j = %c0 to %c4 step %c1 iter_args(%arg5 = %arg3)
        -> (tensor<2400000x9xf32>) {
      %3 = zkx.apply_indexing #map(%th_x, %bl_x)[%i, %j]
      %4 = zkx.apply_indexing #map1(%th_x, %bl_x)[%j]
...
// CHECK: #[[$MAP:.*]] = #zkx.indexing_map<"(d0, d1, d2, d3) -> (d2 * 32768 + (d0 * 4 + d1 * 512 + d3) floordiv 9),
...
func.func @dus(%arg0: tensor<20x30xf32>, %arg1: tensor<5x6xf32>, %arg2: i32, %arg3: i32, %arg4: tensor<20x30xf32>) -> tensor<20x30xf32> {
  ...
  %zkx_loop = zkx.loop (%thread_id_x, %thread_id_y, %thread_id_z, %block_id_x, %block_id_y, %block_id_z)[%i] -> (%ra, %rb) in #map iter_args(%iter = %arg4) -> (tensor<20x30xf32>) {
    ...
    zkx.yield %inserted : tensor<20x30xf32>
  }
  return %zkx_loop : tensor<20x30xf32>
}

// CHECK-LABEL: func.func @dus
// CHECK: arith.minsi
// CHECK-SAME: zkx.range = [-9223372036854775808 : index, 15 : index]
// CHECK: arith.maxsi
// CHECK-SAME: zkx.range = [0 : index, 15 : index]
// CHECK: arith.minsi
// CHECK-SAME: zkx.range = [-9223372036854775808 : index, 24 : index]
// CHECK: arith.maxsi
// CHECK-SAME: zkx.range = [0 : index, 24 : index]
// CHECK: zkx.loop
// CHECK: arith.addi
// CHECK-SAME: zkx.range = [0 : index, 19 : index]
// CHECK: arith.addi
// CHECK-SAME: zkx.range = [0 : index, 29 : index]
...
// CHECK: arith.select
// CHECK-SAME: zkx.range = [0 : index, 2147483647 : index]
...
module {
  func.func @annotate_range_abs_index(%v: i32 {zkx.range = [-31 : i32, 17 : i32]}) -> index {
    %c0_i32 = arith.constant 0 : i32
    %0 = arith.cmpi sge, %v, %c0_i32 : i32
    %1 = arith.subi %c0_i32, %v : i32
    %2 = arith.select %0, %v, %1 : i32
...
// CHECK-LABEL: @annotate_range_abs
// CHECK: arith.select
// CHECK-SAME: zkx.range = [0 : index, 31 : index]
// CHECK-NEXT: arith.index_cast
// CHECK-SAME: zkx.range = [0 : index, 31 : index]
...
module {
  func.func @annotate_range_abs_index(%v: i32 {zkx.range = [-5 : i32, 3 : i32]}) -> index {
    %c0_i32 = arith.constant 0 : i32
    %0 = arith.cmpi sge, %v, %c0_i32 : i32
    %1 = arith.subi %c0_i32, %v : i32
    %2 = arith.select %0, %v, %1 : i32
...
// CHECK-LABEL: @annotate_range_abs
// CHECK: arith.select
// CHECK-SAME: zkx.range = [0 : index, 5 : index]
// CHECK-NEXT: arith.index_cast
// CHECK-SAME: zkx.range = [0 : index, 5 : index]
