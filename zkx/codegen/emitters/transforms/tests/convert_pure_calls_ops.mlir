// RUN: emitters_opt %s -zkx-convert-pure-call-ops | FileCheck %s
// RUN: emitters_opt %s -cse -zkx-convert-pure-call-ops \
...
func.func @caller() -> f32 {
  ...
  %call0 = zkx.pure_call @callee() : () -> (f32)
  %v = scf.for %i = %c0 to %c10 step %c1 iter_args(%r = %call0) -> f32 {
    %call1 = zkx.pure_call @callee() : () -> (f32)
...
func.func @arg_caller() -> f32 {
  ...
  %call = zkx.pure_call @arg_callee(%cst0, %cst1) : (f32, f32) -> (f32)
  return %call : f32
}
