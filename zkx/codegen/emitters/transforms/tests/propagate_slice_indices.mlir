// RUN: emitters_opt %s -split-input-file -zkx-propagate-slice-indices | FileCheck %s
...
module {
  ...
  func.func @stores(%arg0: tensor<17xf32> {zkx.invariant, zkx.slice_index = 0},
                    %arg1: tensor<43xf32> {zkx.slice_index = 1}) -> tensor<43xf32>
                    attributes { zkx.entry } {
...
// CHECK-DAG: @tensorarg(%{{.*}}: tensor<43xf32> {zkx.invariant, zkx.slice_index = 0 : i64}, %{{.*}}: index)
// CHECK-DAG: @tensorcall(%{{.*}}: tensor<43xf32> {zkx.invariant, zkx.slice_index = 0 : i64}, %{{.*}}: index)
// CHECK-DAG: @stores(%{{.*}}: tensor<17xf32> {zkx.invariant, zkx.slice_index = 0 : i64}, %{{.*}}: tensor<43xf32> {zkx.slice_index = 1 : i64})
