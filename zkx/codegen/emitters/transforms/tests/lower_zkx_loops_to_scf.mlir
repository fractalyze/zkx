// RUN: emitters_opt %s -zkx-lower-zkx-loops-to-scf  \
// RUN: --split-input-file | FileCheck %s

#map = #zkx.indexing_map<"(d0)[s0, s1] -> (s0 + 1, s1 - 1),"
  "domain: d0 in [0, 3], s0 in [0, 1024], s1 in [0, 32], s0 + s1 in [0, 90]">

func.func @loop_op(%input: tensor<1024x32xf32>, %init: f32, %dim: index) -> (f32) {
  %sum = zkx.loop (%dim)[%i, %j] -> (%ra, %rb)
      in #map iter_args(%sum_ = %init) -> (f32) {
    %t = tensor.extract %input[%ra, %rb] : tensor<1024x32xf32>
    %add = arith.addf %sum_, %t : f32
    zkx.yield %add : f32
  } {zkx.range = [0 : index, 42 : index]}
  func.return %sum : f32
}

// CHECK-DAG: #[[$MAP:.*]] = #zkx.indexing_map<"(d0)[s0, s1] -> (s0 + s1),
// CHECK-DAG: #[[$MAPA:.*]] = #zkx.indexing_map<"(d0)[s0, s1] -> (s0 + 1),
// CHECK-DAG: #[[$MAPB:.*]] = #zkx.indexing_map<"(d0)[s0, s1] -> (s1 - 1),
...
// CHECK:        %[[INDEX:.*]] = zkx.apply_indexing
...
// CHECK:          %[[RA:.*]] = zkx.apply_indexing #[[$MAPA]](%[[DIM]])[%[[I]], %[[J]]]
// CHECK:          %[[RB:.*]] = zkx.apply_indexing #[[$MAPB]](%[[DIM]])[%[[I]], %[[J]]]
