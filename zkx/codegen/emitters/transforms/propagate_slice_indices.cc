    ...
    for (auto func : getOperation().getOps<mlir::func::FuncOp>()) {
      ...
      for (int i = 0; i < func.getNumArguments(); ++i) {
        ...
        if (mlir::isa<mlir::RankedTensorType>(func.getArgument(i).getType())) {
          if (mlir::Attribute index = entry.getArgAttr(i, "zkx.slice_index")) {
            func.setArgAttr(i, "zkx.slice_index", index);
          }
          if (mlir::Attribute invariant =
                  entry.getArgAttr(i, "zkx.invariant")) {
            func.setArgAttr(i, "zkx.invariant", invariant);
          }
        } else {
          break;
        }
