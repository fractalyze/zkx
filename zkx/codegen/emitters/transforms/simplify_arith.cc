...
struct RewriteCmpI : OpRewritePattern<CmpIOp> {
  ...
  LogicalResult matchAndRewrite(CmpIOp op,
    ...
    std::optional<Interval> rhs = GetRange(op.getRhs());
    std::optional<Interval> lhs = GetRange(op.getLhs());
    if (!lhs || !rhs) {
...
struct RewriteMaxSi : OpRewritePattern<mlir::arith::MaxSIOp> {
  ...
  LogicalResult matchAndRewrite(mlir::arith::MaxSIOp op,
    ...
    if (Interval::ComparisonResult lhs_ge_rhs = lhs->Ge(*rhs);
        lhs_ge_rhs == true) {
      rewriter.replaceOp(op, op.getLhs());
    } else if (Interval::ComparisonResult rhs_ge_lhs = rhs->Ge(*lhs);
               rhs_ge_lhs == true) {
...
struct RewriteMinSi : OpRewritePattern<mlir::arith::MinSIOp> {
  ...
  LogicalResult matchAndRewrite(mlir::arith::MinSIOp op,
    ...
    if (Interval::ComparisonResult lhs_le_rhs = lhs->Le(*rhs);
        lhs_le_rhs == true) {
      rewriter.replaceOp(op, op.getLhs());
    } else if (Interval::ComparisonResult rhs_le_lhs = rhs->Le(*lhs);
               rhs_le_lhs == true) {
      rewriter.replaceOp(op, op.getRhs());
    } else {
      return rewriter.notifyMatchFailure(op, "not equal to lhs or rhs");
...
mlir::Value FindNarrowestValueInChain(mlir::Value value) {
  ...
  if (defining_op) {
    ...
    if (first_trunc &&
        mlir::cast<mlir::IntegerType>(first_trunc.getType()).getWidth() <=
            mlir::cast<mlir::IntegerType>(defining_op.getType()).getWidth()) {
      return first_trunc;
    }
    return defining_op;
  }
...
std::optional<Interval> GetSelectRange(mlir::Operation* sel) {
  ...
  Interval result{
      0, static_cast<int64_t>(
             (1ull << (mlir::cast<mlir::IntegerType>(x.getType()).getWidth() -
                       1)) -
             1)};
...
void AnnotateRanges(mlir::func::FuncOp func) {
  func->walk([](mlir::Operation* op) {
    ...
    auto get_range = [](mlir::Value value) -> Interval {
      std::optional<Interval> range = GetRange(value);
      if (range) {
        return *range;
      }
    ...
    if (mlir::isa<mlir::arith::MaxSIOp, mlir::arith::MinSIOp,
      ...
      Interval lhs_range = get_range(op->getOperand(0));
      Interval rhs_range = get_range(op->getOperand(1));
...
struct RefineConstraints : public OpRewritePattern<ApplyIndexingOp> {
  ...
  LogicalResult matchAndRewrite(ApplyIndexingOp indexing_op,
    ...
    for (mlir::OpOperand& operand : indexing_op->getOpOperands()) {
      std::optional<Interval> range = GetIVRange(operand.get());
      if (!range) {
        continue;
      }
