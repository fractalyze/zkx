...
struct RewriteVectorExtract : OpRewritePattern<vector::ExtractOp> {
  using OpRewritePattern::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(vector::ExtractOp op,
    ...
    auto vector_1D = rewriter
                         ...
                         .getResult(0);
    rewriter.replaceOpWithNewOp<vector::ExtractOp>(op, vector_1D, linear_index);
    return mlir::success();
  }
};
...
struct RewriteVectorInsert : OpRewritePattern<vector::InsertOp> {
  using OpRewritePattern::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(vector::InsertOp op,
    ...
    auto vector_1D = b.create<UnrealizedConversionCastOp>(
                         ...
                         .getResult(0);
    auto new_insert = b.create<vector::InsertOp>(op.getValueToStore(),
                                                 vector_1D, linear_index);
    auto cast_to_orig_type = b.create<UnrealizedConversionCastOp>(
...
// Checks if the value is produced by an unrealized conversion cast from a 1D
// tensor or vector to ND. If so, returns the 1D tensor or vector.
