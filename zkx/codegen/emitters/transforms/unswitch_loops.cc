...
struct UnswitchLoop : mlir::OpRewritePattern<mlir::scf::ForOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (!op.isDefinedOutsideOfLoop(if_op.getCondition())) {
      return rewriter.notifyMatchFailure(
          op, "condition is defined outside of the loop");
    }
    ...
    mlir::OpBuilder else_builder =
        new_if.getElseBodyBuilder(rewriter.getListener());
