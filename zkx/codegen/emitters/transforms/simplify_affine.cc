...
int Distance(ImplicitLocOpBuilder& builder, Value a) {
  ...
  mlir::Block* parent = a.getParentBlock();
  int distance = 0;
...
struct RewriteAffineApply : OpRewritePattern<AffineApplyOp> {
  ...
  LogicalResult matchAndRewrite(AffineApplyOp op,
    ...
    AffineExpr result_expr = indexing_map.GetAffineMap().getResult(0);
    ...
    Value result = ExpressionEvaluator(b, indexing_map.GetDimensionCount(),
                                       op->getOperands())
