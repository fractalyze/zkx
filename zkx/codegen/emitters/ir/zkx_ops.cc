...
std::optional<Interval> GetRange(Value value) {
  ...
  } else if (auto cst = value.getDefiningOp<arith::ConstantIndexOp>()) {
    return {{cst.value(), cst.value()}};
  } else if (value.getDefiningOp()) {
    return attr_to_range(value.getDefiningOp()->getAttr("zkx.range"));
  }
  ...
  mlir::Operation* parent = bbarg.getParentBlock()->getParentOp();
  if (auto func_op = mlir::dyn_cast<mlir::func::FuncOp>(parent)) {
    return attr_to_range(func_op.getArgAttr(bbarg.getArgNumber(), "zkx.range"));
  }
  return GetIVRange(value);
}

std::optional<Interval> GetIVRange(Value iv) {
  ...
  if (auto loop_op = mlir::dyn_cast<zkx::LoopOp>(parent)) {
...
ParseResult ApplyIndexingOp::parse(OpAsmParser& parser,
  ...
  if (failed(parser.parseAttribute(indexing_map_attr, "indexing_map_attr",
                                   result.attributes))) {
    return failure();
  }
  ...
  if (succeeded(parser.parseOptionalLParen())) {
    if (failed(parseOperands(parser, &operands)) ||
  ...
  if (failed(parser.resolveOperands(operands, index_type, result.operands)) ||
      failed(parser.parseOptionalAttrDict(result.attributes))) {
    return failure();
  }
  AffineMap map = indexing_map_attr.getIndexingMap().GetAffineMap();
...
LogicalResult ApplyIndexingOp::fold(
...
void AtomicRMWOp::build(OpBuilder& builder, OperationState& result,
  ...
  build(builder, result, tensor, ivs,
        mlir::cast<RankedTensorType>(tensor.getType()).getElementType());
}
...
LogicalResult AtomicRMWOp::verify() {
  ...
  mlir::Operation* body_terminator = getBody()->getTerminator();
...
struct FoldConstantDimensions : public mlir::OpRewritePattern<LoopOp> {
  ...
  LogicalResult matchAndRewrite(LoopOp loop_op,
    ...
    for (auto [operand, dim_var] :
      ...
      std::optional<Interval> range = GetRange(operand.get());
...
void SetBackendKind(MLIRContext* context, mlir::func::FuncOp fn,
                    BackendKind backend_kind) {
  fn->setAttr(BackendKindAttr::name,
              BackendKindAttr::get(context, backend_kind));
}

std::optional<BackendKind> GetBackendKind(mlir::func::FuncOp fn) {
  auto backend_attr = fn->getAttrOfType<BackendKindAttr>(BackendKindAttr::name);
  if (!backend_attr) {
    return std::nullopt;
  }
...
#include "zkx/codegen/emitters/ir/zkx_ops.cc.inc"
