...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> ParseMlirModuleString(
  ...
  registry.insert<mlir::func::FuncDialect, ZkxDialect>();
  context->appendDialectRegistry(registry);
  context->loadDialect<mlir::func::FuncDialect, ZkxDialect>();
...
class ZKXOpsTest : public HloTestBase {
 public:
...
TEST_F(ZKXOpsTest, GetConstraintsForVariables) {
...
TEST_F(ZKXOpsTest, GetConstraintsForVariablesEmpty) {
  auto map = *ParseIndexingMap(R"(
...
TEST_F(ZKXOpsTest, BackendKindGetAndSet) {
  ...
  ASSERT_TRUE(func);
  EXPECT_EQ(GetBackendKind(func), BackendKind::kCpu);

  // Test that overwriting the attribute works.
  SetBackendKind(&mlir_context_, func, BackendKind::kGpu);
  EXPECT_EQ(GetBackendKind(func), BackendKind::kGpu);
