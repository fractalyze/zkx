...
struct HloRunnerConfig {
  ...
  bool run_zkx_backend_only = false;
  ...
  bool zkx_dump_as_text = false;
  bool zkx_dump_as_proto = false;
  ...
  // float gpu_client_mem_fraction = zkx::GpuAllocatorConfig{}.memory_fraction;
...
void PreprocessFlags(HloRunnerConfig& opts) {
...
absl::StatusOr<FunctionalHloRunner::ModuleArgumentMode> ArgumentModeFromString(
...
RawCompileOptionsFromFlags(const HloRunnerConfig& opts) {
  ...
  out.hlo_passes_mode =
      opts.run_zkx_backend_only
          ? FunctionalHloRunner::HloPassesMode::kRunZKXBackendOnly
  ...
  out.zkx_dump_to = opts.zkx_dump_to;
  out.zkx_text_dump_mode =
      opts.zkx_dump_as_text
          ? FunctionalHloRunner::ZkxTextDumpMode::kDumpAsText
          : FunctionalHloRunner::ZkxTextDumpMode::kNotDumpAsText;
  out.zkx_proto_dump_mode =
      opts.zkx_dump_as_proto
          ? FunctionalHloRunner::ZkxProtoDumpMode::kDumpAsProto
          : FunctionalHloRunner::ZkxProtoDumpMode::kNotDumpAsProto;
  out.zkx_gpu_dump_xspace_to = opts.zkx_gpu_dump_xspace_to;
  return out;
}

absl::Status RunMultihostHloRunner(int argc, char** argv,
  ...
  // clang-format off
  // TODO(chokobole): Uncomment this. Dependency: HloRunnerConfig::gpu_client_mem_fraction
  // clang-format on
  ...
  if (opts.device_type_str == "gpu") {
    // clang-format off
    // TODO(chokobole): Uncomment this. Dependency: GetPjRtEnvironmentForGpu, GPURunnerProfiler
    // clang-format on
    ...
    // // Create a GPURunnerProfiler to profile GPU executions to save xspace
    // data
    // // to disk.
    // if (env.client != nullptr && !opts.zkx_gpu_dump_xspace_to.empty()) {
    //   TF_ASSIGN_OR_RETURN(auto profiler,
    //                       GPURunnerProfiler::Create(opts.zkx_gpu_dump_xspace_to,
    //                                                 /*keep_xspace=*/false));
    //   running_options.profiler = profiler.get();
    // }
    return absl::UnimplementedError("GPU support is not implemented yet");
...
int main(int argc, char** argv) {
  ...
  std::vector<tsl::Flag> flag_list = {
      ...
      tsl::Flag("run_zkx_backend_only", &opts.run_zkx_backend_only,
                "Call only ZKX's RunBackend during the compilation. "
      ...
      tsl::Flag("zkx_dump_to", &opts.zkx_dump_to,
                "A directory to dump zkx debug data to."),
      tsl::Flag("zkx_dump_as_text", &opts.zkx_dump_as_text,
                "Whether to dump zkx debug data as text."),
      tsl::Flag("zkx_dump_as_proto", &opts.zkx_dump_as_proto,
                "Whether to dump zkx debug data as protobuf."),
      ...
      //           "of (0.0, 1.0). Same as ZKX_CLIENT_MEM_FRACTION in the Python "
      ...
      tsl::Flag("zkx_gpu_dump_xspace_to", &opts.zkx_gpu_dump_xspace_to,
  ...
  // TODO(chokobole): Uncomment this. Dependency: InitMain
  // tsl::port::InitMain(kUsageString.c_str(), &argc, &argv);
  if (!parse_ok) {
    LOG(QFATAL) << kUsageString;
  }
  absl::Status s = zkx::RunMultihostHloRunner(argc, argv, opts);
  if (!s.ok()) {
    std::cerr << s;
    return 1;
  }
