...
absl::StatusOr<Literal> MakeFakeLiteralWithSameValue(const Shape& shape,
  ...
  return absl::InvalidArgumentError(
      absl::StrFormat("Unsupported type for fake literal generation: %s",
                      ShapeUtil::HumanString(shape)));
}
...
absl::StatusOr<ExecutionOptions> FunctionalHloRunner::LoadExecutionOptions(
  ...
  TF_RETURN_IF_ERROR(tsl::ReadTextOrBinaryProto(tsl::Env::Default(), path,
                                                &execution_options));
...
absl::StatusOr<CompileOptions> FunctionalHloRunner::CreateCompileOptions(
    ...
    int num_nodes, std::shared_ptr<KeyValueStoreInterface> kv_store) {
  ...
  if (task_id == 0) {
    // Overwrite zkx_dump_to only if it's not empty, to preserve `zkx_dump_to`
    // from parsed ZKX_FLAGS env (already populated in debug_options).
    // clang-format off
    // TODO(chokobole): Uncomment this. Dependency: DebugOption::zkx_dump_to, DebugOption::zkx_dump_hlo_as_text, DebugOption::zkx_dump_hlo_as_proto
    // clang-format on
    // if (!raw_options.zkx_dump_to.empty()) {
    //   debug_options.set_zkx_dump_to(raw_options.zkx_dump_to);
    // }
    // debug_options.set_zkx_dump_hlo_as_text(raw_options.zkx_text_dump_mode ==
    //                                        ZkxTextDumpMode::kDumpAsText);
    // debug_options.set_zkx_dump_hlo_as_proto(raw_options.zkx_proto_dump_mode
    // ==
    //                                         ZkxProtoDumpMode::kDumpAsProto);
  }
  switch (raw_options.hlo_passes_mode) {
    case HloPassesMode::kRunZKXBackendOnly:
      build_options.set_run_backend_only(true);
      break;
    case HloPassesMode::kDisableAllHloPasses:
      // clang-format off
      // TODO(chokobole): Uncomment this. Dependency: DebugOption::zkx_disable_all_hlo_passes
      // clang-format on
      // debug_options.set_zkx_disable_all_hlo_passes(true);
      break;
...
FunctionalHloRunner::CreateExecutableBuildOptionsFromExecutionOptions(
  ...
  if (execution_options.has_debug_options()) {
    *build_options.mutable_debug_options() = execution_options.debug_options();
    // TODO(chokobole): Uncomment this. Dependency: DebugOptions::zkx_dump_to
    // build_options.mutable_debug_options()->set_zkx_dump_to("");
  }
...
absl::Status FunctionalHloRunner::DumpOutput(
    ...
    std::string_view dump_output_to, int task_id, OutputFormat output_format) {
...
FunctionalHloRunner::LoadHloModuleAndArguments(std::string_view hlo_file,
...
FunctionalHloRunner::LoadAndRun(PjRtClient& client,
  ...
  for (int i = 0; i < hlo_module_and_arguments.arguments.size(); ++i) {
    loaded_arguments[client.devices()[i]->global_device_id().value()] =
...
FunctionalHloRunner::ReadModuleFromHloTextFile(std::string_view hlo_file) {
  std::string hlo_string;
  TF_RETURN_IF_ERROR(
      tsl::ReadFileToString(tsl::Env::Default(), hlo_file, &hlo_string));
  return ParseAndReturnUnverifiedModule(
      hlo_string, {}, HloParserOptions().set_keep_module_auto_layouts(true));
}

absl::StatusOr<std::unique_ptr<HloModule>>
FunctionalHloRunner::ReadModuleFromBinaryProtoFile(std::string_view hlo_file) {
  HloProto proto;
  TF_RETURN_IF_ERROR(
      tsl::ReadBinaryProto(tsl::Env::Default(), hlo_file, &proto));
  return HloProtoToModule(proto.hlo_module());
}

absl::StatusOr<std::unique_ptr<HloModule>>
FunctionalHloRunner::ReadModuleFromTextProtoFile(std::string_view hlo_file) {
  HloProto proto;
  TF_RETURN_IF_ERROR(tsl::ReadTextProto(tsl::Env::Default(), hlo_file, &proto));
  return HloProtoToModule(proto.hlo_module());
}
...
absl::StatusOr<std::unique_ptr<HloModule>>
FunctionalHloRunner::ReadModuleFromString(std::string_view hlo_text) {
...
absl::Status FunctionalHloRunner::PrepareHloModuleForCompilation(
  ...
  // TODO(chokobole): Uncomment this. Dependency: HloControlFlowFlattening
  ...
  //   // debug_options().zkx_disable_hlo_passes().
...
// StableHLO mlir::Module or to ZkxComputation and calls the compile_function
...
absl::StatusOr<std::unique_ptr<R>> ConvertAndCallCompiler(
  ...
  if (compile_as_stablehlo) {
    // TODO(chokobole): Uncomment this. Dependency: ConvertHloToStablehlo
    ...
    ZkxComputation computation(hlo_module->ToProto());
...
FunctionalHloRunner::CreateArgumentsOnDevice(
  ...
  for (int i = 0; i < num_addressable_devices; ++i) {
    ...
    LiteralVec& argument_literals = per_device_argument_literals
...
FunctionalHloRunner::CreateUninitializedArgumentsOnDevice(
  ...
  for (int i = 0; i < static_cast<int>(addressable_devices.size()); ++i) {
    ...
    argument_shapes_per_device[device->global_device_id().value()] =
  ...
  for (int i = 0; i < static_cast<int>(addressable_devices.size()); ++i) {
    ...
    CHECK(argument_shapes_per_device.contains(
        device->global_device_id().value()));
    const std::vector<Shape>& argument_shapes =
        argument_shapes_per_device.at(device->global_device_id().value());
    ...
    for (const Shape& shape : argument_shapes) {
      if (running_options.log_input_output()) {
        LOG(INFO) << "device_id=" << device->global_device_id().value()
...
FunctionalHloRunner::CopyArgumentsToDevice(
  ...
  for (const std::shared_ptr<const PjRtLayout>& pjrt_layout :
       executable_parameter_pjrt_layouts) {
    executable_parameter_layouts.push_back(pjrt_layout->zkx_layout());
  }
  ...
  for (int i = 0; i < num_addressable_devices; ++i) {
    ...
    int curr_device_id = curr_device->global_device_id().value();
    ...
    int source_device_id = source_device->global_device_id().value();
...
FunctionalHloRunner::FetchAndLogOutput(
  ...
  for (PjRtDevice* device : GetLocalDevices(client)) {
    if (device->global_device_id().value() == 0) {
  ...
  for (int i = 0; i < output_buffers.size(); ++i) {
    ...
    const int device_id =
        output_buffers[i][0]->device()->global_device_id().value();
  ...
  if (module_output_mode == ModuleOutputMode::kReturnOutputs ||
    ...
    if (log_output) {
      for (const PjRtDevice* device : GetLocalDevices(client)) {
        int device_id = device->global_device_id().value();
        if (module_output_mode == ModuleOutputMode::kReturnDevice0Outputs &&
            device_id != 0) {
          continue;
        }
