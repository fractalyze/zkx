...
void SetDefaultLayoutToContainer(T* minor_to_major) {
  // The default ZKX layout is major-to-minor (dim 0 is major).
  // For more information on ZKX layouts, see:
  ...
  if (src.IsTuple()) {
    ...
    if (src.has_layout()) {
      if (src.rank() != dst->rank()) {
        return absl::InvalidArgumentError(
            "cannot copy layout from shape: ranks differs");
      }
      TF_RETURN_IF_ERROR(
...
// static
Layout LayoutUtil::MakeDescendingLayout(int64_t rank) {
...
// static
Layout LayoutUtil::MakeAscendingLayout(int64_t rank) {
  std::vector<int64_t> layout(rank);
...
// static
Layout LayoutUtil::MakeLayoutFromMajorToMinor(
...
// static
Layout LayoutUtil::GetDefaultLayoutForRank(int64_t rank) {
...
// static
Shape LayoutUtil::GetWithDefaultLayout(const Shape& shape) {
  Shape copy(shape);
...
// static
absl::Status LayoutUtil::ValidateLayoutInShape(const Shape& shape,
                                               bool allow_missing_layouts) {
  if (shape.IsTuple()) {
    // Tuple shape.
    if (shape.has_layout()) {
      return absl::InvalidArgumentError("tuple should not have a layout field");
    }
    ...
    if (!shape.has_layout()) {
      ...
      return absl::InvalidArgumentError(absl::StrFormat(
          "shape %s does not have a layout", ShapeUtil::HumanString(shape)));
    }
    ...
    if (shape.has_layout()) {
      return absl::InvalidArgumentError(
          absl::StrFormat("shape of primitive type %s should not have a layout",
...
// static
absl::Status LayoutUtil::ValidateLayoutForShape(const Layout& layout,
                                                const Shape& shape) {
  if (shape.IsTuple()) {
    return absl::InvalidArgumentError(
        "a single Layout is not valid for tuple shapes");
  }
  ...
  if (layout.minor_to_major_size() != shape.rank()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("layout minor_to_major field contains %d elements, "
      ...
      return absl::InvalidArgumentError(absl::StrFormat(
          ...
          absl::StrJoin(dim_unique, ", ",
                        [](std::string* out, bool dim_unique) {
                          absl::StrAppend(out, base::BoolToString(dim_unique));
                        }),
          shape.ShortDebugString()));
    }
  }
      ...
      return absl::InvalidArgumentError(absl::StrFormat(
          ...
          absl::StrJoin(dim_ordered, ", ",
                        [](std::string* out, bool dim_ordered) {
                          absl::StrAppend(out, base::BoolToString(dim_ordered));
                        }),
          shape.ShortDebugString()));
    }
  }
  ...
  if (LayoutUtil::IsSparse(layout)) {
    ...
    for (const auto& tile : layout.tiles()) {
      if (tile.dimensions().empty() ||
        ...
        return absl::InvalidArgumentError(absl::StrFormat(
            "layout has invalid tiles: %s", shape.ShortDebugString()));
      }
    }
  }
...
// static
void LayoutUtil::ClearTiles(Shape* shape) {
...
// static
bool LayoutUtil::IsDenseArray(const Shape& shape) {
  return shape.IsArray() && (!shape.has_layout() || IsDense(shape.layout()));
}

// static
bool LayoutUtil::IsSparseArray(const Shape& shape) {
  return shape.IsArray() && shape.has_layout() && IsSparse(shape.layout());
}

// static
bool LayoutUtil::IsCOOArray(const Shape& shape) {
  return shape.IsArray() && shape.has_layout() && IsCOO(shape.layout());
}

// static
bool LayoutUtil::IsCSRArray(const Shape& shape) {
  return shape.IsArray() && shape.rank() == 2 && shape.has_layout() &&
         IsCSR(shape.layout());
}

// static
bool LayoutUtil::IsCSCArray(const Shape& shape) {
  return shape.IsArray() && shape.rank() == 2 && shape.has_layout() &&
         IsCSC(shape.layout());
}

// static
bool LayoutUtil::IsDense(const Layout& layout) {
...
// static
bool LayoutUtil::IsSparse(const Layout& layout) { return !IsDense(layout); }

// static
bool LayoutUtil::IsCOO(const Layout& layout) {
  if ((layout.dim_level_types_size() == 0) ||
...
// static
bool LayoutUtil::IsCSR(const Layout& layout) {
...
// static
bool LayoutUtil::IsCSC(const Layout& layout) {
...
// static
bool LayoutUtil::HasAnyLayout(const Shape& shape) {
  if (shape.IsTuple()) {
    // Tuple shape: all subshapes must have a layout.
...
// static
bool LayoutUtil::HasCustomElementSizeInBits(const Shape& shape) {
  if (shape.IsTuple()) {
    return absl::c_any_of(shape.tuple_shapes(),
...
// static
std::vector<int64_t> LayoutUtil::MakeLogicalToPhysical(const Layout& layout) {
...
// static
void LayoutUtil::PrintHumanString(Printer* printer, const Layout& layout) {
  layout.Print(printer);
}

// static
std::string LayoutUtil::HumanString(const Layout& layout) {
...
// static
bool LayoutUtil::LayoutsInShapesEqual(const Shape& lhs, const Shape& rhs,
                                      std::optional<Layout::Equal> equal) {
  if (lhs.IsTuple()) {
...
// static
bool LayoutUtil::AreDimensionsConsecutive(const Layout& layout,
...
// static
Layout LayoutUtil::MoveDimToMajor(const Layout& layout, int64_t dim) {
...
// static
int64_t LayoutUtil::LinearIndex(const Shape& shape,
  ...
  for (int64_t minor = 0; minor < rank; minor++) {
    ...
    if (minor < tile.dimensions().size()) {
      ...
      tile_multiplier *=
          tsl::MathUtil::CeilOfRatio(shape_dim_size, tile_dim_size);
...
// static
int64_t LayoutUtil::MemorySpace(const Shape& shape) {
  return shape.has_layout() ? shape.layout().memory_space()
                            : Layout::kDefaultMemorySpace;
}

// static
DimLevelType LayoutUtil::GetDimLevelType(const Layout& layout, int64_t dim) {
  if (layout.dim_level_types_size() == 0) {
    return DIM_DENSE;
  }
...
// static
bool LayoutUtil::DimUnique(const Layout& layout, int64_t dim) {
  if (layout.dim_unique_size() == 0) {
    return true;
  }
...
// static
bool LayoutUtil::DimOrdered(const Layout& layout, int64_t dim) {
  if (layout.dim_ordered_size() == 0) {
    return true;
  }
...
// static
int64_t LayoutUtil::MaxSplitSize(const Shape& shape, int64_t dim) {
...
// static
int64_t LayoutUtil::MaxElementsInPerSplit(const Shape& shape) {
  CHECK(shape.IsArray()) << ShapeUtil::HumanString(shape);
