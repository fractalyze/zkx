...
#include "zkx/autotune_result_wrapper.h"
...
// static
absl::StatusOr<AutotuneResultWrapper> AutotuneResultWrapper::FromKeyAndValue(
    OpaqueKey key, OpaqueValue value) {
  AutotuneResults key_proto;
  if (!key_proto.ParseFromString(key)) {
    return absl::InvalidArgumentError("Could not parse the provided key");
  }
  if (key_proto.results().empty()) {
    return absl::InvalidArgumentError("Key has no results");
  }

  AutotuneResults::Entry value_entry;
  if (!value_entry.ParseFromString(value)) {
    return absl::InvalidArgumentError("Could not parse the provided value");
  }
