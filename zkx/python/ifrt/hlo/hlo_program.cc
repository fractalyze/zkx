...
absl::StatusOr<std::unique_ptr<HloProgram>> HloProgram::FromBytes(
    std::string_view bytes, std::unique_ptr<mlir::MLIRContext> context) {
  if (context == nullptr) {
    context = std::make_unique<mlir::MLIRContext>(
        mlir::MLIRContext::Threading::DISABLED);
    mlir::DialectRegistry registry;
    // TODO(chokobole): Uncomment this. Dependency: zkx::RegisterAllHloDialects
    // zkx::RegisterAllHloDialects(registry);
