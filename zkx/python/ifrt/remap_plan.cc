...
bool CheckOneInputForOneOutput(const RemapPlan& plan) {
...
absl::Status RemapPlan::Validate() const {
  ...
  if (!mappings || mappings->empty()) {
    return absl::InvalidArgumentError("Must have at least one mapping");
  }
  ...
  for (int64_t i = 0; i < mappings->size(); ++i) {
    ...
    const std::shared_ptr<const zkx::PjRtLayout>& in_layout =
        input_specs[mapping.in_array].layout;
    const std::shared_ptr<const zkx::PjRtLayout>& out_layout =
    ...
    for (int s = 0; s < mapping.from.size(); ++s) {
      ...
      while (in_shard < in_interval.end) {
        if (in_used_buffers[in_shard]) {
          return absl::InvalidArgumentError(
              absl::StrFormat("Input array %d shard %d is already used",
        ...
        if (out_assigned_devices[out_shard] != nullptr) {
          return absl::InvalidArgumentError(
              absl::StrFormat("Output array %d shard %d is already assigned",
  ...
  for (int i = 0; i < num_outputs; ++i) {
    for (int out_shard = 0;
      ...
      if (out_assigned_devices_list[i][out_shard] == nullptr) {
        return absl::InvalidArgumentError(absl::StrFormat(
            "Output array %d shard %d is unassigned", i, out_shard));
      }
    }
...
absl::Status RemapPlan::CheckArrayCopySemantics(
    ArrayCopySemantics semantics) const {
