...
#include "zkx/python/ifrt/mock.h"
...
MockArray::MockArray(ArrayRef delegated) : delegated_(std::move(delegated)) {
  ...
  ON_CALL(*this, pjrt_layout)
      .WillByDefault(
          [this]() -> absl::StatusOr<std::shared_ptr<const zkx::PjRtLayout>> {
...
MockClient::MockClient(std::unique_ptr<Client> delegated)
  ...
  ON_CALL(*this, MakeDeviceList)
      .WillByDefault([this](absl::Span<Device* const> devices) {
  ...
  ON_CALL(*this, GetDefaultPjRtLayout)
      .WillByDefault(
          [this](DType dtype, absl::Span<const int64_t> dims, Device* device,
