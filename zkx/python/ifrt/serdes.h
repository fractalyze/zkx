...
#ifndef ZKX_PYTHON_IFRT_SERDES_H_
#define ZKX_PYTHON_IFRT_SERDES_H_
...
#include "zkx/python/ifrt/serdes.pb.h"
...
class Serializable : public llvm::RTTIExtends<Serializable, llvm::RTTIRoot> {
 public:
  ...
  using SerializeOptions = ::zkx::ifrt::SerializeOptions;
  using DeserializeOptions = ::zkx::ifrt::DeserializeOptions;
};
...
void RegisterSerDes(std::unique_ptr<SerDes> serdes) {
  static_assert(std::is_base_of_v<Serializable, T>,
                "Types must implement `zkx::ifrt::Serializable` to have a "
...
#endif  // ZKX_PYTHON_IFRT_SERDES_H_
