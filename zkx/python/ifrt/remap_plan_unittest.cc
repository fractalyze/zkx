  ...
  plan.input_specs.push_back(
      ArraySpec{/*dtype=*/DType(DType::kS32),
                ...
                ConcreteEvenSharding::Create(GetDevices({0}), MemoryKind(),
                                             ...
                                             /*shard_shape=*/Shape({2, 3}))});
  plan.input_specs.push_back(
      ArraySpec{/*dtype=*/DType(DType::kU32),  // dtype differs
                /*shape=*/Shape({2, 3}),
                /*sharding=*/
                ConcreteEvenSharding::Create(GetDevices({0}), MemoryKind(),
                                             /*shape=*/Shape({2, 3}),
...
TEST_P(RemapPlanTest, MixedDtype) {
  ...
  plan.input_specs.push_back(array_spec_u32);
  plan.output_specs.push_back(array_spec_u32);
...
TEST_P(RemapPlanTest, InvalidLayout) {
  ...
  plan.input_specs.push_back(ArraySpec{
      ...
      ConcreteEvenSharding::Create(GetDevices({0}), MemoryKind(),
                                   ...
                                   /*shard_shape=*/Shape({2, 3})),
      /*layout=*/
      std::make_shared<zkx::PjRtLayout>(
          zkx::LayoutUtil::MakeDescendingLayout(2)),
  });
  plan.output_specs.push_back(ArraySpec{
      ...
      /*layout=*/
      std::make_shared<zkx::PjRtLayout>(
          zkx::LayoutUtil::MakeAscendingLayout(2)),  // layout differs
  });
  plan.mappings = std::make_shared<std::vector<RemapPlan::Mapping>>();
  plan.mappings->push_back(
      RemapPlan::Mapping{/*in_array=*/0,
...
TEST_P(RemapPlanTest, CheckOneInputToOneOutput) {
  ...
  *plan.mappings = {RemapPlan::Mapping{/*in_array=*/0,
  ...
  TF_EXPECT_OK(plan.CheckArrayCopySemantics(ArrayCopySemantics::kReuseInput));
  TF_EXPECT_OK(plan.CheckArrayCopySemantics(ArrayCopySemantics::kDonateInput));
}
...
TEST_P(RemapPlanTest, CheckMultipleInputsToOneOutput) {
  ...
  *plan.mappings = {RemapPlan::Mapping{/*in_array=*/0,
  ...
  EXPECT_THAT(
      plan.CheckArrayCopySemantics(ArrayCopySemantics::kReuseInput),
      StatusIs(absl::StatusCode::kInvalidArgument,
...
TEST_P(RemapPlanTest, InvalidInputDevicesForOutputMap) {
  ...
  *plan.mappings = {RemapPlan::Mapping{/*in_array=*/0,
  ...
  EXPECT_THAT(plan.Validate(), StatusIs(absl::StatusCode::kInvalidArgument,
...
TEST_P(RemapPlanSerDesTest, ToFromProto) {
  ...
  plan.input_specs.push_back(ArraySpec{/*dtype=*/DType(DType::kU32),
  ...
  plan.output_specs.push_back(ArraySpec{
      /*dtype=*/DType(DType::kU32), /*shape=*/shape, /*sharding=*/sharding});
  plan.output_specs.push_back(ArraySpec{
      /*dtype=*/DType(DType::kU32), /*shape=*/shape, /*sharding=*/sharding});
  ...
  for (const auto& spec : plan_copy.input_specs) {
    EXPECT_EQ(spec.dtype, DType(DType::kU32));
