...
#include "zkx/python/ifrt/user_context_status_util.h"
...
#include "zkx/python/ifrt/user_context_registry.h"

namespace zkx::ifrt {

namespace {

constexpr std::string_view kIfrtUserContextPayloadUrl =
...
absl::Status AttachUserContextRef(absl::Status status,
  ...
  std::string_view user_context_id_str_view = *user_context_id_str_holder;
...
absl::Status ReattachUserContextRefs(absl::Status status) {
  ...
  std::string_view user_context_id_str_view = payload->Flatten();
  uint64_t user_context_id;
  if (!absl::SimpleAtoi(user_context_id_str_view, &user_context_id)) {
    tsl::errors::AppendToMessage(&status,
                                 "\n(failed to parse a user context ID: ",
                                 user_context_id_str_view, ")");
    return status;
  }
  TrackedUserContextRef tracked_user_context =
  ...
  auto user_context_id_str_holder =
      std::make_unique<std::string>(user_context_id_str_view);
  user_context_id_str_view = *user_context_id_str_holder;
  status.SetPayload(
      kIfrtUserContextPayloadUrl,
      absl::MakeCordFromExternal(
          user_context_id_str_view,
