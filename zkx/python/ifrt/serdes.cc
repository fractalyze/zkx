...
struct Registry {
  ...
  absl::flat_hash_map<std::string_view, SerDes*> name_to_serdes
      ABSL_GUARDED_BY(mu);
};

Registry* registry() {
  static auto* const r = absl::IgnoreLeak(new Registry());
  return r;
}
...
void RegisterSerDes(const void* type_id, std::unique_ptr<SerDes> serdes) {
  ...
  CHECK(r->type_id_to_serdes.insert({type_id, serdes.get()}).second)
      << "zkx::ifrt::SerDes cannot be registered more than once for the same "
         "type id: "
      << type_id;

  const std::string_view name = serdes->type_name();
