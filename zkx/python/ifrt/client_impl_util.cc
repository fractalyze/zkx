...
absl::Status CheckHostBuffer(
    ...
    const std::optional<Shape>& shard_shape) {
...
std::optional<Shape> TryToGetShardShape(const ArraySpec& array_spec) {
...
absl::StatusOr<std::vector<ArrayRef>> ClientMakeArraysFromHostBufferShards(
  ...
  for (Client::MakeArraysFromHostBufferShardsSpec& spec : specs) {
    std::optional<Shape> shard_shape = TryToGetShardShape(spec.array_spec);
    ...
    absl::Span<Device* const> addressable_devices =
    ...
    for (const auto& [addressable_shard_indices, host_buffer] : spec.buffers) {
      ...
      if (host_buffer.on_done != nullptr) {
        ...
        on_done_with_host_buffer_per_device = [on_done = host_buffer.on_done,
                                               count]() mutable {
