...
#ifndef ZKX_PYTHON_IFRT_EXECUTABLE_H_
#define ZKX_PYTHON_IFRT_EXECUTABLE_H_
...
#include "zkx/python/ifrt/execute_options.pb.h"
...
class Executable : public llvm::RTTIExtends<Executable, llvm::RTTIRoot> {
 public:
  ...
  // The following APIs are taken from `zkx::PjRtExecutable` for fast
  // prototyping.
  // TODO(hyeontaek): Factor some of them out as `ZkxCompatibleExecutable`.
  ...
  // TODO(hyeontaek): Move the following ZKX-specific methods to
  // pjrt_executable.h and put it in an `ZkxCompatibleExecutable`.
  ...
  virtual absl::StatusOr<std::vector<std::shared_ptr<const zkx::PjRtLayout>>>
...
class LoadedExecutable
 ...
 public:
  ...
  // The following APIs are taken from `zkx::PjRtLoadedExecutable` for fast
  // prototyping.

  // TODO(hyeontaek): Move the following to pjrt_executable.h and put it in an
  // `ZkxCompatibleExecutable`.
  ...
  using ExecuteOptions = ::zkx::ifrt::ExecuteOptions;
...
#endif  // ZKX_PYTHON_IFRT_EXECUTABLE_H_
