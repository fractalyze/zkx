...
#ifndef ZKX_PYTHON_IFRT_CLIENT_H_
#define ZKX_PYTHON_IFRT_CLIENT_H_
...
using PlatformId = ::zkx::PjRtPlatformId;

// TODO(hyeontaek): Generalize DeviceAssignment or hide it from the top-level
// API.
using DeviceAssignment = ::zkx::DeviceAssignment;
...
class Client : public llvm::RTTIExtends<Client, llvm::RTTIRoot> {
 public:
  ...
  // by default.
  using HostBufferSemantics = ::zkx::PjRtClient::HostBufferSemantics;
  ...
  virtual absl::StatusOr<std::vector<ArrayRef>> RemapArrays(
  ...
  virtual std::string_view runtime_type() const = 0;

  // The following APIs are taken from `zkx::PjRtClient` for fast prototyping.
  ...
  // the primary devices.
  virtual absl::Span<Device* const> GetAllDevices() const = 0;
  ...
  virtual absl::StatusOr<std::shared_ptr<const zkx::PjRtLayout>>
  GetDefaultPjRtLayout(DType dtype, absl::Span<const int64_t> dims,
                       Device* device, MemoryKind memory_kind) const = 0;
...
#endif  // ZKX_PYTHON_IFRT_CLIENT_H_
