...
#include "zkx/python/ifrt/client_impl_util.h"
#include "zkx/python/pjrt_ifrt/basic_string_array.h"
...
#include "zkx/python/pjrt_ifrt/pjrt_remap.h"
#include "zkx/python/pjrt_ifrt/pjrt_topology.h"
#include "zkx/python/pjrt_ifrt/pjrt_tuple.h"
...
AttributeMap MakeAttributeMap(zkx::PjRtClient* pjrt_client) {
...
struct GlobalTopology {
  ...
  absl::flat_hash_map<DeviceId, zkx::PjRtGlobalDeviceId>
...
absl::StatusOr<GlobalTopology> MakeGlobalTopologyFromPjRtClient(
    zkx::PjRtClient* pjrt_client, const PjRtClient::CreateOptions& options) {
  ...
  if (options.global_device_mapping.has_value()) {
    ...
    if (num_processes == 0) {
      return absl::InvalidArgumentError(
          "global device mapping contains no processes");
    }
    ...
    std::vector<zkx::PjRtDevice*> pjrt_devices(pjrt_client->devices().begin(),
                                               pjrt_client->devices().end());
    absl::c_sort(pjrt_devices, [](zkx::PjRtDevice* a, zkx::PjRtDevice* b) {
      return a->global_device_id() < b->global_device_id();
    });

    for (zkx::PjRtDevice* pjrt_device : pjrt_devices) {
      const zkx::PjRtGlobalDeviceId pjrt_global_device_id =
    ...
    num_processes = 1;
    for (zkx::PjRtDevice* pjrt_device : pjrt_client->devices()) {
      if (pjrt_device->IsAddressable()) {
        if (my_process_index.has_value()) {
...
LocalTopologyProto MakeLocalTopologyFromPjRtClient(
  ...
  // other process has.
  for (zkx::PjRtDevice* device : pjrt_client->addressable_devices()) {
    ...
    for (const DeviceProto& device_proto : node.devices()) {
      ...
      ifrt_device_id_to_pjrt_global_device_id.insert(
          {DeviceId(device_proto.global_device_id()),
           zkx::PjRtGlobalDeviceId(device_proto.global_device_id())});
    }
  }

  if (!my_process_index.has_value()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Could not determine process_index for this process"));
  }
  return GlobalTopology{std::move(global_topology_proto), *my_process_index,
                        std::move(ifrt_device_id_to_pjrt_global_device_id)};
}
  ...
  for (int process_index = 0;
    ...
    for (const DeviceProto& device_proto : node.devices()) {
      ...
      if (partition_index != -1) {
        ...
        attributes.insert(
            {"partition_index",
             zkx::PjRtDeviceAttribute(static_cast<int64_t>(partition_index))});
...
absl::StatusOr<std::unique_ptr<PjRtClient>> PjRtClient::Create(
  ...
  zkx::PjRtClient* pjrt_client = client->pjrt_client();
  ...
  for (auto& ifrt_device : devices) {
    ...
    TF_RET_CHECK(
        client->device_id_map_.emplace(ifrt_device->Id(), ifrt_device.get())
            .second);
    zkx::PjRtDevice* pjrt_device = ifrt_device->pjrt_device();
    if (pjrt_device) {
      TF_RET_CHECK(
  ...
  for (zkx::PjRtMemorySpace* memory_space : pjrt_client->memory_spaces()) {
  ...
  for (Device* ifrt_device : client->addressable_devices_) {
    ...
    for (zkx::PjRtMemorySpace* pjrt_memory_space :
...
std::unique_ptr<PjRtClient> PjRtClient::Create(
    std::shared_ptr<zkx::PjRtClient> pjrt_client) {
  PjRtClient::CreateOptions options;
  options.pjrt_client = std::move(pjrt_client);
  return *Create(std::move(options));
}

PjRtClient::PjRtClient(std::shared_ptr<zkx::PjRtClient> pjrt_client)
...
absl::StatusOr<PjRtCompatibleDevice*> PjRtClient::LookupPjRtDevice(
    zkx::PjRtDevice* pjrt_device) const {
  auto it = device_map_.find(pjrt_device);
  if (it == device_map_.end()) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "PjRtDevice not found: %s", pjrt_device->DebugString()));
  }
  return it->second;
}

absl::StatusOr<PjRtCompatibleMemory*> PjRtClient::LookupPjRtMemory(
    zkx::PjRtMemorySpace* pjrt_memory) const {
  auto it = memory_map_.find(pjrt_memory);
  if (it == memory_map_.end()) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "PjRtMemorySpace not found: %s", pjrt_memory->DebugString()));
  }
  return it->second;
}

absl::StatusOr<zkx::PjRtGlobalDeviceId> PjRtClient::GetPjRtGlobalDeviceId(
  ...
  if (it == ifrt_device_id_to_pjrt_global_device_id_.end()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Unknown PjRt global device ID for IFRT device ID %d",
                        device_id.value()));
  }
  return it->second;
}
...
absl::StatusOr<Device*> PjRtClient::LookupDevice(DeviceId device_id) const {
  ...
  return absl::InvalidArgumentError(absl::StrFormat(
      "No matching device found for device_id %d", device_id.value()));
}

absl::StatusOr<Device*> PjRtClient::LookupAddressableDevice(
    int local_hardware_id) const {
  DCHECK(this);
  TF_ASSIGN_OR_RETURN(zkx::PjRtDevice * pjrt_device,
                      pjrt_client_->LookupAddressableDevice(
                          zkx::PjRtLocalDeviceId(local_hardware_id)));
...
absl::StatusOr<ArrayRef> PjRtClient::MakeArrayFromHostBuffer(
  ...
  absl::Span<Device* const> ifrt_addressable_devices =
  ...
  if (ifrt_addressable_devices.empty()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Cannot copy array to non-addressable device: %s",
  ...
  for (Device* const device : ifrt_addressable_devices) {
    ...
    if (sharding->memory_kind().memory_kind().has_value()) {
      ...
      TF_ASSIGN_OR_RETURN(zkx::PjRtMemorySpace * memory_space,
  ...
  std::shared_ptr<const zkx::PjRtLayout> layout = nullptr;
...
absl::StatusOr<std::vector<ArrayRef>> PjRtClient::CopyArraysForCrossHost(
  ...
  for (int i = 0; i < dst_devices->size(); ++i) {
    ...
    if (src_devices->devices()[i]->IsAddressable()) {
      ...
      std::vector<zkx::Shape> recv_shapes;
      ...
      for (const ArrayRef& array : arrays) {
        if (auto* const pjrt_array = llvm::dyn_cast<PjRtArray>(array.get())) {
          ...
          zkx::Shape recv_shape =
              zkx::ShapeUtil::MakeShape(dtype, shard_shape.dims());
...
absl::Status PjRtClient::WatchGlobalProcessInfo(
  ...
  while (true) {
    ...
    // clang-format off
    // TODO(chokobole): Uncomment this. Dependency: WatchJobStateAsync, PjRtClient::UpdateGlobalProcessInfo.
    // clang-format on
    return absl::UnimplementedError(
        "WatchGlobalProcessInfo is not implemented");
  }
}
...
absl::Status PjRtClient::CrossHostSendBuffers(
  ...
  for (int i = 0; i < keys.size(); ++i) {
    auto promise = Future<std::string>::CreatePromise();
    auto descriptor_future = Future<std::string>(promise);
...
absl::StatusOr<PjRtArray::PjRtBuffers> PjRtClient::CrossHostReceiveBuffers(
    absl::Span<const zkx::Shape> shapes, zkx::PjRtDevice* device,
    std::vector<int64_t> keys) {
  auto notifier = [this, keys = std::move(keys)](
                      absl::StatusOr<zkx::PjRtCrossHostRecvState> recv_state) {
    if (!recv_state.ok()) {
      LOG(FATAL) << "Invalid PjRtCrossHostRecvState passed to "
                    "zkx::PjRtCrossHostRecvNotifier callback in "
                    "zkx::PjRtClient::MakeCrossHostReceiveBuffers: "
                 << recv_state.status();
    }
    auto on_canceled = [](const absl::Status& status) {
      if (!status.ok()) {
        LOG(FATAL) << "Invalid status passed to `on_canceled` callback in "
                      "zkx::PjRtCrossHostSendCancelNotifier in "
                      "zkx::PjRtClient::MakeCrossHostReceiveBuffers: "
                   << status;
      }
    };
...
absl::StatusOr<std::shared_ptr<Topology>> PjRtClient::GetTopologyForDevices(
    const DeviceListRef& devices) const {
  ...
  return std::make_shared<PjRtTopology>(
      std::shared_ptr<const zkx::PjRtTopologyDescription>(pjrt_client_,
                                                          topology));
}

absl::StatusOr<std::shared_ptr<const zkx::PjRtLayout>>
PjRtClient::GetDefaultPjRtLayout(DType dtype, absl::Span<const int64_t> dims,
  ...
  auto layout =
    ...
    if (memory_kind == kUnpinnedHostMemoryKind) {
      return std::make_shared<zkx::PjRtLayout>(
    ...
    TF_ASSIGN_OR_RETURN(zkx::Layout layout,
                        pjrt_client_->GetDefaultLayout(element_type, dims));
    return std::make_shared<zkx::PjRtLayout>(std::move(layout));
  }();
  {
    absl::MutexLock lock(&default_layout_cache_mu_);
...
absl::Status PjRtClient::TransferToInfeed(PjRtDevice* device,
  ...
  if (!device->IsAddressable()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Infeed is only supported on addressable devices "
...
absl::Status PjRtClient::TransferFromOutfeed(PjRtDevice* device,
  ...
  if (!device->IsAddressable()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Outfeed is only supported on addressable devices "
                        "but device %s is not addressable",
                        device->DebugString()));
  }
