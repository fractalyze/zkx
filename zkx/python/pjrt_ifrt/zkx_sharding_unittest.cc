...
#include "zkx/python/ifrt/device_test_util.h"
...
TEST_P(HloShardingTest, CreateWithBadDeviceList) {
  auto zkx_hlo_sharding = zkx::HloSharding::Replicate();
  EXPECT_DEATH(
      HloSharding::Create(DeviceListRef(), MemoryKind(), zkx_hlo_sharding), "");
...
TEST_P(HloShardingTest, IsFullyReplicated) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  {
    ...
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
  ...
  }
  {
    ...
    auto zkx_hlo_sharding = zkx::HloSharding::IotaTile({1, 1});
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
    EXPECT_TRUE(sharding->IsFullyReplicated());
  }
  {
    ...
    auto zkx_hlo_sharding = zkx::HloSharding::PartialTile(
        zkx::TileAssignment(zkx::IotaTileAssignment::Create({1, 6})));
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
    EXPECT_TRUE(sharding->IsFullyReplicated());
  }
  {
    ...
    auto zkx_hlo_sharding = zkx::HloSharding::PartialTile(
        zkx::TileAssignment(zkx::IotaTileAssignment::Create({2, 3})));
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
  ...
  }
  {
    ...
    // replicated.
    auto zkx_hlo_sharding = zkx::HloSharding::IotaTile({1, 6});
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
    EXPECT_FALSE(sharding->IsFullyReplicated());
  }
  {
    ...
    auto device_list = GetDevices({0});  // This sharding uses 1 device.
    auto zkx_hlo_sharding = zkx::HloSharding::AssignDevice(/*device_id=*/0);
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
    EXPECT_TRUE(sharding->IsFullyReplicated());
  }
  ...
  }
  {
    ...
    auto zkx_hlo_sharding = zkx::HloSharding::Manual();
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
    EXPECT_FALSE(sharding->IsFullyReplicated());
  }
  {
    ...
    auto zkx_hlo_sharding = zkx::HloSharding::Unknown();
    std::shared_ptr<const HloSharding> sharding =
        HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
    EXPECT_FALSE(sharding->IsFullyReplicated());
  }
...
TEST_P(HloShardingTest, GetShardShape) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  auto zkx_hlo_sharding = zkx::HloSharding::IotaTile({2, 3});
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
TEST_P(HloShardingTest, HasSamePartitioning) {
  ...
  {
    auto device_list1 = GetDevices({3, 4, 5, 0, 1, 2});
    auto zkx_hlo_sharding1 = zkx::HloSharding::IotaTile({3, 2});
    std::shared_ptr<const HloSharding> sharding1 =
        HloSharding::Create(device_list1, MemoryKind(), zkx_hlo_sharding1);
    EXPECT_FALSE(sharding0->HasSamePartitioning(*sharding1));
  }
  ...
  // Replicated sharding with different numbers of devices.
  {
    ...
    std::shared_ptr<const HloSharding> hlo_sharding0 = HloSharding::Create(
        device_list0, MemoryKind(), zkx::HloSharding::Replicate());
    std::shared_ptr<const HloSharding> hlo_sharding1 = HloSharding::Create(
        device_list1, MemoryKind(), zkx::HloSharding::Replicate());
...
TEST_P(HloShardingTest, DISABLED_IndexDomainsWithTile) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  // 6-way sharded along axis 0, 1-way sharded along axis 1.
  auto zkx_hlo_sharding = zkx::HloSharding::Tile(zkx::TileAssignment({6, 1}));
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
// TODO(chokobole): Enable this test.
TEST_P(HloShardingTest, DISABLED_IndexDomainsWithUnevenTile) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  // 6-way sharded along axis 0, 1-way sharded along axis 1.
  auto zkx_hlo_sharding = zkx::HloSharding::Tile(zkx::TileAssignment({6, 1}));
  std::shared_ptr<const HloSharding> sharding =
...
// TODO(chokobole): Enable this test.
TEST_P(HloShardingTest, DISABLED_DisassembleWithUnevenTile) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  // 6-way sharded along axis 0, 1-way sharded along axis 1.
  auto zkx_hlo_sharding = zkx::HloSharding::Tile(zkx::TileAssignment({6, 1}));
  std::shared_ptr<const HloSharding> sharding =
...
// TODO(chokobole): Enable this test.
TEST_P(HloShardingTest, DISABLED_IndexDomainsWithPartialTile) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  // 2-way sharded along axis 0, 1-way sharded along axis 1, each shard
  // replicated by 3 times.
  auto zkx_hlo_sharding =
      zkx::HloSharding::PartialTile(zkx::TileAssignment({2, 1, 3}));
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
// TODO(chokobole): Enable this test.
TEST_P(HloShardingTest, DISABLED_IndexDomainsWithSubgroupReplicated) {
  auto device_list = GetDevices({0, 1, 2, 3, 4, 5});
  // 2-way sharded along axis 0, 1-way sharded along axis 1, each shard
  // replicated by 3 times.
  auto zkx_hlo_sharding = zkx::HloSharding::Subgroup(
      zkx::TileAssignment({2, 1, 3}), {zkx::OpSharding::REPLICATED});
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
TEST_P(HloShardingTest, IndexDomainsWithSubgroupMaximalSlowPath) {
  ...
  auto zkx_hlo_sharding = zkx::HloSharding::Subgroup(
      zkx::TileAssignment({2, 1, 3}), {zkx::OpSharding::MAXIMAL});
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
// TODO(chokobole): Enable this test.
TEST_P(HloShardingTest, DISABLED_IndexDomainsWithTileTranspose) {
  auto device_list = GetDevices({0, 1, 2, 3});
  auto zkx_hlo_sharding =
      zkx::HloSharding::IotaTile(/*tile_assignment_dims=*/{2, 2},
                                 /*reshape_dims=*/{2, 2},
                                 /*transpose_perm=*/{1, 0});
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
TEST_P(HloShardingTest, DisassembleFailsWithInvalidDeviceCount) {
  ...
  auto zkx_hlo_sharding = zkx::HloSharding::Tile(zkx::TileAssignment({2, 1}));
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
TEST_P(HloShardingTest, DisassembleFailsWithDynamicShape) {
  auto device_list = GetDevices({0, 1});
  auto zkx_hlo_sharding = zkx::HloSharding::Tile(zkx::TileAssignment({2}));
  std::shared_ptr<const HloSharding> sharding =
      HloSharding::Create(device_list, MemoryKind(), zkx_hlo_sharding);
...
TEST_P(HloShardingTest, Hash) {
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      HloSharding::Create(GetDevices({0, 1, 2, 3, 4, 5}), MemoryKind(),
                          zkx::HloSharding::Replicate()),
      HloSharding::Create(GetDevices({0}), MemoryKind(),
                          zkx::HloSharding::Replicate()),
      HloSharding::Create(GetDevices({0}), MemoryKind("pinned_host"),
                          zkx::HloSharding::Replicate()),
      HloSharding::Create(GetDevices({0, 1, 2, 3, 4, 5}), MemoryKind(),
                          zkx::HloSharding::AssignDevice(/*device_id=*/0)),
      HloSharding::Create(GetDevices({0, 1, 2, 3, 4, 5}), MemoryKind(),
                          zkx::HloSharding::PartialTile(zkx::TileAssignment(
                              zkx::IotaTileAssignment::Create({2, 3})))),
  }));
}
