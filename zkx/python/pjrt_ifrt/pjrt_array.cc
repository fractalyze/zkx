...
const MemoryKind kPinnedHostMemoryKind(PinnedHostMemorySpace::kKind);
...
absl::Status ValidateArrayCreationInput(
  ...
  if (sharding_devices.size() != pjrt_buffers.size()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("device and buffer counts mismatch: %d vs. %d",
  ...
  for (int i = 0; i < sharding_devices.size(); ++i) {
    ...
    if (!device) {
      return absl::InvalidArgumentError(
          absl::StrFormat("Sharding device %d is not a PjRtDevice", i));
    }
...
absl::StatusOr<tsl::RCReference<PjRtArray>> PjRtArray::Create(
    ...
    bool has_custom_layout) {
  if (pjrt_buffers.empty()) {
    return absl::InvalidArgumentError("PjRtBuffers must be non-empty.");
  }
  TF_ASSIGN_OR_RETURN(auto dtype,
                      ToDType(pjrt_buffers.front()->element_type()));
...
Future<> PjRtArray::CopyToHostBuffer(
  ...
  std::unique_ptr<MutableBorrowingLiteral> literal;
  if (byte_strides.has_value()) {
    auto zkx_shape =
        MakeShapeWithTrivialByteStrides(*dtype, dims, *byte_strides);
    if (!zkx_shape.ok()) {
      return Future<>(std::move(zkx_shape).status());
    }
    literal = std::make_unique<MutableBorrowingLiteral>(
        static_cast<char*>(data), *zkx_shape);
  } else {
    auto zkx_shape = ShapeUtil::MakeShapeWithDescendingLayout(*dtype, dims);
    literal = std::make_unique<MutableBorrowingLiteral>(
        static_cast<char*>(data), zkx_shape);
  }
...
absl::StatusOr<ArrayRef> PjRtArray::Copy(std::optional<DeviceListRef> devices,
...
std::string PjRtArray::DebugString() const {
  DCHECK(this);
  absl::StatusOr<std::shared_ptr<const zkx::PjRtLayout>> layout_ptr =
      pjrt_layout();
...
absl::StatusOr<std::shared_ptr<const zkx::PjRtLayout>> PjRtArray::pjrt_layout()
    const {
#ifndef NDEBUG
  for (int i = 1; i < pjrt_buffers_.size(); ++i) {
    std::shared_ptr<const zkx::PjRtLayout> layout_i =
