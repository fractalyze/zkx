...
#ifndef ZKX_PYTHON_PJRT_IFRT_PJRT_CLIENT_H_
#define ZKX_PYTHON_PJRT_IFRT_PJRT_CLIENT_H_
...
#include "xla/tsl/platform/unbounded_work_queue.h"
...
#include "zkx/python/pjrt_ifrt/transfer_server_interface.h"
...
class PjRtCompatibleClient
 ...
 public:
  ...
  // APIs that allow direct access to `zkx::PjRtClient` for PjRt-only
  // operations.
  virtual zkx::PjRtClient* pjrt_client() = 0;
  virtual std::shared_ptr<zkx::PjRtClient> shared_ptr_pjrt_client() = 0;
  ...
  virtual absl::StatusOr<PjRtCompatibleDevice*> LookupPjRtDevice(
      zkx::PjRtDevice* pjrt_device) const = 0;
  virtual absl::StatusOr<PjRtCompatibleMemory*> LookupPjRtMemory(
      zkx::PjRtMemorySpace* pjrt_memory) const = 0;

  static char ID;
};

// `Client` implementation that wraps `zkx::PjRtClient`.
class PjRtClient final
 ...
 public:
  struct CreateOptions {
    ...
    std::shared_ptr<zkx::DistributedRuntimeClient> distributed_client = nullptr;
    ...
    std::shared_ptr<zkx::KeyValueStoreInterface> kv_store = nullptr;
  ...
  zkx::PjRtClient* pjrt_client() override { return pjrt_client_.get(); }
  std::shared_ptr<zkx::PjRtClient> shared_ptr_pjrt_client() override {
    return pjrt_client_;
  }
  absl::StatusOr<tsl::RCReference<PjRtCompatibleArray>> CreatePjRtArray(
  ...
  absl::StatusOr<std::vector<ArrayRef>> RemapArrays(
  ...
  std::string_view runtime_type() const override { return "pjrt_ifrt"; }
  ...
  absl::StatusOr<std::shared_ptr<const zkx::PjRtLayout>> GetDefaultPjRtLayout(
  ...
  absl::StatusOr<PjRtCompatibleDevice*> LookupPjRtDevice(
      zkx::PjRtDevice* pjrt_device) const override;
  absl::StatusOr<PjRtCompatibleMemory*> LookupPjRtMemory(
      zkx::PjRtMemorySpace* pjrt_memory) const override;
  ...
  // device IDs, unlike `zkx::ifrt::PjRtDevice` that does not keep
  // `zkx::PjRtDevice` around for non-addressable devices.
  //
  ...
  absl::StatusOr<zkx::PjRtGlobalDeviceId> GetPjRtGlobalDeviceId(
 ...
 private:
  explicit PjRtClient(std::shared_ptr<zkx::PjRtClient> pjrt_client);

  std::shared_ptr<zkx::PjRtClient> pjrt_client_;
  ...
  absl::flat_hash_map<zkx::PjRtDevice*, PjRtDevice*> device_map_;
  absl::flat_hash_map<zkx::PjRtMemorySpace*, PjRtMemory*> memory_map_;
  ...
  std::shared_ptr<zkx::DistributedRuntimeClient> distributed_client_;
  std::shared_ptr<zkx::KeyValueStoreInterface> kv_store_;
...
#endif  // ZKX_PYTHON_PJRT_IFRT_PJRT_CLIENT_H_
