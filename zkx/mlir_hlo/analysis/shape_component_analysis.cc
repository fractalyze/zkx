...
struct ShapeVisitor {
  ...
  void visit(ShapeOrValueInfo requestedInfo) {
    ...
    while (!backwardsWorklist.empty()) {
      ...
      if (!ty.isIntOrIndexOrFloat() && !isa<RankedTensorType>(ty))
        continue;
...
private:
  ...
  void forwardNumElements(shape::NumElementsOp op) {
    ...
    for (auto &it : in) {
      ...
      // Simply copy the first symbolic factor.
      if (!dim.expr) {
        dim = it;
        continue;
      }
...
bool SymbolicExpr::isKnownNotNegativeOne() const {
  ...
  auto isGoodSymbol = [](const Symbol &symbol) {
    ...
    if (op == nullptr)
      return false;
    return isa<shape::ShapeOfOp, shape::NumElementsOp>(op);
  };
