...
Operation *createReturn(OpBuilder &builder, Dialect *dialect, Location loc,
  ...
  Twine returnOpName = dialect->getNamespace() + ".return";
...
bool isCommutativeNoRegionMatchingDialect(OperationName innerOp,
  ...
  Dialect *innerOpDialect = innerOp.getDialect();
...
//     return.
bool isReduceEligibleForCompactPrint(Operation *op, ValueRange inputs,
                                     Region &body) {
  // Check E1.
  ...
  if (retOp->getName().stripDialect() != "return")
    return false;
...
ParseResult parseReduceOp(
  ...
  auto parseEle = [&]() -> ParseResult {
    ...
    if (failed(parser.parseOperand(operands.emplace_back())) ||
        failed(parser.parseKeyword("init")) || failed(parser.parseColon()) ||
        failed(parser.parseOperand(initOperands.emplace_back())) ||
  ...
  if (failed(parser.parseOptionalKeyword("applies"))) {
    ...
    auto parseDim = [&]() -> ParseResult {
      if (failed(parser.parseInteger(dimensions.emplace_back())))
        return failure();
      return success();
    };

    FunctionType reduceOpFnType;
    if (failed(parser.parseKeyword("across")) ||
        failed(parser.parseKeyword("dimensions")) ||
        failed(parser.parseEqual()) ||
        failed(parser.parseCommaSeparatedList(AsmParser::Delimiter::Square,
                                              parseDim)) ||
        failed(parser.parseOptionalAttrDict(result.attributes)) ||
        failed(parser.parseColon()) ||
        failed(parser.parseType(reduceOpFnType)) ||
        failed(parser.parseKeyword("reducer")))
      return failure();
    ...
    auto parseBlockOperand =
      ...
      if (failed(parser.parseOperand(operands.emplace_back(),
          ...
          failed(parser.parseColon()) ||
          failed(parser.parseType(types.emplace_back())) ||
          failed(parser.parseOptionalLocationSpecifier(locs.emplace_back())))
        return failure();
      return success();
    };
    while (succeeded(parser.parseOptionalLParen())) {
      if (failed(
              parseBlockOperand(reducerOperands, reducerTypes, reducerLocs)) ||
          failed(parser.parseComma()) ||
          failed(parseBlockOperand(reducerInitOperands, reducerInitTypes,
                                   reducerInitLocs)) ||
          failed(parser.parseRParen()))
        return failure();
    }
    ...
    if (failed(parser.resolveOperands(operands, reduceOpFnType.getInputs(), loc,
                                      result.operands)) ||
        failed(parser.parseRegion(*result.addRegion(), reducerArgs)) ||
        failed(parser.parseOptionalLocationSpecifier(trailingLoc)))
      return failure();
  ...
  if (!isCommutativeNoRegionMatchingDialect(*innerOpNameInfo, reduceOpDialect))
    return parser.emitError(
        loc, "expected the inner-op to be a commutative binary-op from "
             "the " +
                 reduceOpDialect +
  ...
  if (failed(parser.parseKeyword("across")) ||
      ...
      failed(parser.parseCommaSeparatedList(AsmParser::Delimiter::Square,
                                            parseDim)) ||
      failed(parser.parseOptionalAttrDict(result.attributes)) ||
      failed(parser.parseColon()) || failed(parser.parseType(reduceOpFnType)) ||
      failed(parser.parseOptionalLocationSpecifier(explicitLoc)))
    return failure();
  ...
  BlockArgument lhs = block.addArgument(innerOpType, reduceOpLoc);
  BlockArgument rhs = block.addArgument(innerOpType, reduceOpLoc);
...
ParseResult parseSelectOpType(OpAsmParser &parser, Type &pred, Type &onTrue,
  ...
  // Fail if the parsed type list does not contain either two types or a single
  // function type.
  bool isValidType =
...
ParseResult parseWhileOp(OpAsmParser &parser, OperationState &result) {
  ...
  do {
    ...
    if (failed(parser.parseOperand(iterArg)) || failed(parser.parseEqual()) ||
        failed(parser.parseOperand(operand)))
      return failure();
  ...
  if (!operands.empty()) {
    if (failed(parser.parseColon()) ||
        failed(parser.parseTypeList(result.types)))
      return failure();
  }
  SmallVector<OpAsmParser::Argument> args;
  createArgs(iterArgs, result.types, args);
  if (failed(parser.resolveOperands(operands, result.types, loc,
                                    result.operands)) ||
      failed(parser.parseOptionalAttrDictWithKeyword(result.attributes)) ||
      failed(parser.parseKeyword("cond")) ||
      failed(parser.parseRegion(*result.addRegion(), args)) ||
