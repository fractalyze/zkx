...
class DistributedRuntimeCoordinationServiceClient
 ...
 public:
  ...
  absl::StatusOr<std::string> KeyValueTryGet(std::string_view key) override;
...
DistributedRuntimeCoordinationServiceClient::
  ...
  config.set_service_leader("/job:zkx_worker/task:0");
  ...
  const absl::Status status =
      coord_agent_->Initialize(options.env, "zkx_worker", options.node_id,
...
absl::Status DistributedRuntimeCoordinationServiceClient::Connect() {
  ...
  if (s.ok()) {
    LOG(INFO) << "Connected to distributed ZKX controller";
  } else if (absl::IsDeadlineExceeded(s)) {
    LOG(ERROR)
        << "Failed to connect to distributed ZKX controller: waited too "
    ...
    LOG(ERROR) << "Failed to connect to distributed ZKX controller: " << s;
  }
  return s;
}
...
std::unique_ptr<DistributedRuntimeClient> GetDistributedRuntimeClient(
  ...
  return std::make_unique<DistributedRuntimeCoordinationServiceClient>(channel,
                                                                       options);
}
