...
class ClientServerTest : public testing::Test {
 public:
  std::shared_ptr<DistributedRuntimeClient> GetClient(
    ...
    if (channel == nullptr) {
      channel =
          coord_service_->server()->InProcessChannel(grpc::ChannelArguments());
    }
...
TEST_F(ClientServerTest, ConnectAndEnumerateDevices) {
  ...
  *node1 = locals[1];
  ...
  auto thread0_fn = [&]() -> absl::Status {
    ...
    TF_RET_CHECK(protobuf_util::ProtobufEquals(topology, expected_topology))
...
TEST_F(ClientServerTest, ClientsTerminateIfServiceGoesAway) {
  ...
  for (int i = 0; i < num_nodes; ++i) {
    EXPECT_EQ(statuses[i].code(), absl::StatusCode::kFailedPrecondition);
  }
}
...
TEST_F(ClientServerTest, ConnectEventuallyTimesOutIfAClientDoesNotShowUp) {
  ...
  for (int i = 0; i < num_nodes - 1; ++i) {
    EXPECT_EQ(statuses[i].code(), absl::StatusCode::kDeadlineExceeded);
  }
}
...
TEST_F(ClientServerTest,
  ...
  std::vector<absl::Status> statuses(num_nodes);
  {
    ...
    for (int i = 0; i < num_nodes; ++i) {
      EXPECT_EQ(statuses[i].code(), absl::StatusCode::kInvalidArgument)
...
TEST_F(ClientServerTest, KeyValueDelete) {
  ...
  EXPECT_EQ(deleted_kv.status().code(), absl::StatusCode::kDeadlineExceeded);
