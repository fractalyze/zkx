...
#ifndef ZKX_PJRT_DISTRIBUTED_KEY_VALUE_STORE_INTERFACE_H_
#define ZKX_PJRT_DISTRIBUTED_KEY_VALUE_STORE_INTERFACE_H_
...
// TODO(chokobole): Update the comment below if we use different term for
// PjRtClient.
//
...
class KeyValueStoreInterface {
 public:
  ...
  // There are no concurrency guarantees. To avoid a race / impose an ordering
  // on potentially concurrent ops (e.g. set, delete), use WaitAtBarrier().
  virtual absl::StatusOr<std::string> TryGet(std::string_view key) = 0;
...
#endif  // ZKX_PJRT_DISTRIBUTED_KEY_VALUE_STORE_INTERFACE_H_
