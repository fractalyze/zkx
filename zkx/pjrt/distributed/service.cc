...
std::unique_ptr<tsl::CoordinationServiceInterface> EnableCoordinationService(
  ...
  const std::string job_name = "zkx_worker";
...
DistributedRuntimeService::Get(
  ...
  if (!service->server_) {
    return absl::UnknownError("Failed to start RPC server");
  }
  LOG(INFO) << "ZKX service listening on " << address;
  return service;
}
...
void DistributedRuntimeService::Shutdown() {
  if (server_) {
    LOG(INFO) << "ZKX service shutting down";
    server_->Shutdown(absl::ToChronoTime(absl::Now() + absl::Seconds(5)));
    server_->Wait();
  }

  // Explicitly destroy coordination service before the gRPC server. This
  // clears all pending RPCs before the gRPC server is destroyed.
  coord_impl_ = nullptr;
  server_ = nullptr;
}
