...
#ifndef ZKX_PJRT_DISTRIBUTED_CLIENT_H_
#define ZKX_PJRT_DISTRIBUTED_CLIENT_H_
...
class DistributedRuntimeClient {
 public:
  struct Options {
    ...
    std::function<void(absl::Status)> missed_heartbeat_callback =
        [](const absl::Status& status) {
          LOG(QFATAL) << "Terminating process because the ZKX distributed "
  ...
  virtual absl::StatusOr<std::string> KeyValueTryGet(std::string_view key) = 0;
...
#endif  // ZKX_PJRT_DISTRIBUTED_CLIENT_H_
