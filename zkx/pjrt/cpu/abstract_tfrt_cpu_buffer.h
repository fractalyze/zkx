...
#ifndef ZKX_PJRT_CPU_ABSTRACT_TFRT_CPU_BUFFER_H_
#define ZKX_PJRT_CPU_ABSTRACT_TFRT_CPU_BUFFER_H_
...
#include "zkx/pjrt/cpu/tracked_tfrt_cpu_device_buffer.h"
#include "zkx/pjrt/pjrt_client.h"
#include "zkx/pjrt/pjrt_future.h"
#include "zkx/pjrt/transpose.h"
#include "zkx/service/cpu/cpu_event.h"
#include "zkx/shape.h"
...
class AsyncWorkRunner {
 public:
  ...
  // `work` enqueued by `Schedule` may run on the calling thread.
...
class AbstractTfrtCpuBuffer : public PjRtBuffer {
 public:
  ...
  PjRtFuture<> CopyRawToHost(void* dst, int64_t offset,
    ...
    return PjRtFuture<>(
        absl::UnimplementedError("CopyRawToHost not implemented"));
  }

  absl::StatusOr<std::unique_ptr<PjRtBuffer>> CopyToMemorySpace(
      PjRtMemorySpace* dst_memory_space) override {
    return absl::UnimplementedError("CopyToMemorySpace not implemented");
  }
  ...
  void CopyToRemoteDevice(PjRtFuture<std::string> serialized_descriptor,
    ...
    on_done(absl::UnimplementedError("CopyToRemoteDevice not implemented."),
  ...
  void CopyToRemoteDeviceScattered(
      ...
      const PjRtBuffer::ScatterDetails& scatter_details) override {
    for (const auto& on_done : callbacks) {
      on_done(
          absl::UnimplementedError("Implement CopyToRemoteDeviceScattered."),
          /*sends_were_enqueued=*/false);
    }
  }
 ...
 protected:
  virtual std::string_view buffer_name() const = 0;
...
#endif  // ZKX_PJRT_CPU_ABSTRACT_TFRT_CPU_BUFFER_H_
