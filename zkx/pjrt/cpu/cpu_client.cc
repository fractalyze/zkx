...
TfrtCpuClient::TfrtCpuClient(
    ...
    : process_index_(process_index),
      ...
      eigen_intraop_pool_(new tsl::thread::ThreadPool(
          tsl::Env::Default(), GetThreadOptions(), "ZKXEigen",
      ...
      topology_(CpuTopologyDescription::Create(
          ...
          GetPjRtDeviceSpan(owned_devices_),
          cpu::DetectMachineAttributes(std::nullopt).features)),
  ...
  for (int idx = 0; idx < addressable_devices_.size(); ++idx) {
    ...
    const int id = device->global_device_id().value();
...
absl::StatusOr<PjRtDevice*> TfrtCpuClient::LookupDevice(
  ...
  return absl::InvalidArgumentError(absl::StrFormat(
      "No matching device found for device_id %d", global_device_id.value()));
}
...
absl::StatusOr<PjRtDevice*> TfrtCpuClient::LookupAddressableDevice(
  ...
  return absl::InvalidArgumentError(
      absl::StrFormat("No matching device found for local_device_id %d",
  ...
  if (num_partitions * num_replicas <= addressable_devices().size()) {
    ...
    for (int i = 0; i < num_replicas; ++i) {
      for (int j = 0; j < num_partitions; ++j) {
        assignment(i, j) = addressable_devices()
                               .at(i * num_partitions + j)
                               ->global_device_id()
                               .value();
      }
    }
...
TfrtCpuClient::GetHloCostAnalysis() const {
  // TODO(chokobole): Uncomment this. Dependency: HloCostAnalysis
  // return
...
TfrtCpuClient::DeserializeExecutable(std::string_view serialized,
...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>>
TfrtCpuClient::LoadSerializedExecutable(std::string_view serialized,
                                        std::optional<CompileOptions> options,
...
static absl::StatusOr<std::unique_ptr<Executable>> JitCompile(
    ...
    const Compiler::CompileOptions& compile_options, int num_threads,
    std::function<void(HloModuleConfig&)> customize_hlo_module_config) {
...
TfrtCpuClient::CreateBuffersForAsyncHostToDevice(
    ...
    std::optional<absl::Span<const std::optional<Layout>>> device_layouts,
    PjRtMemorySpace* memory_space) {
  TF_ASSIGN_OR_RETURN(std::vector<Shape> device_shapes,
...
absl::StatusOr<PjRtLoadedExecutable::Result> TfrtCpuExecutable::ExecuteHelper(
  ...
  std::shared_ptr<DeviceAssignment> device_assignment;
  if (device == nullptr) {
    CHECK_NE(device_assignment_, nullptr);
  ...
  if (input_deps.empty() && execute_inline) {
    ...
    // Execution status for ZKX:CPU "classic" runtime or thunks.
    ZkxCustomCallStatus compute_function_status;
    ...
    if (cpu_executable->has_compute_function()) {
      // Call jit-compiled function implementing ZKX executable.
      ...
      // Call interpreted thunk sequence implementing ZKX executable.
      ...
      return absl::InternalError(
          "CpuExecutable has no compute function or thunks.");
    }
    ...
    if (cpu_executable->has_compute_function()) {
      if (auto error_message =
              CustomCallStatusGetMessage(&compute_function_status)) {
        return absl::InternalError(
            absl::StrFormat("Generated function failed: %s", *error_message));
      }
         ...
         eigen_device = client()->eigen_intraop_device()]() mutable {
          ...
          for (const auto& av : input_deps_avs) {
            if (auto* error = av->GetErrorIfPresent()) {
              execute_event.SetError(absl::InternalError(absl::StrFormat(
          ...
          for (const auto& buffer_info : buffer_table) {
            CHECK(buffer_info.buffer.IsAvailable());
            if (buffer_info.buffer.IsError()) {
              execute_event.SetError(absl::InternalError(
                  absl::StrFormat("Error preparing computation: %s",
...
TfrtCpuExecutable::Execute(
  ...
  if (num_addressable_devices == 1) {
    ...
    // TODO(chokobole): Uncomment this. Dependency: MaybeDumpHloSnapshot
...
TfrtCpuExecutable::ExecutePortable(
  ...
  if (device == nullptr) {
    return absl::InvalidArgumentError(
        "ExecutePortable expects a device to be specified");
  }
