...
#ifndef ZKX_PJRT_PJRT_CLIENT_H_
#define ZKX_PJRT_PJRT_CLIENT_H_
...
class PjRtDevice {
 public:
  ...
  virtual std::string_view device_kind() const {
    return description().device_kind();
  }

  // Debug string suitable for logging when errors occur. Should be verbose
  // enough to describe the current device unambiguously.
  virtual std::string_view DebugString() const {
    return description().DebugString();
  }

  // Debug string suitable for reading by end users, should be reasonably terse,
  // for example: "CpuDevice(id=0)".
  virtual std::string_view ToString() const { return description().ToString(); }
  ...
  virtual absl::StatusOr<PjRtMemorySpace*> memory_space_by_kind(
      std::string_view memory_space_kind) const {
...
using PjRtCrossHostSendCancelNotifier = std::function<void(
    std::string_view serialized_descriptor, absl::Status reason,
...
// Encapsulates the state of Python session with ZKX.
//
...
class PjRtClient {
 public:
  ...
  virtual std::string_view platform_version() const = 0;
  ...
  virtual absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> Compile(
    ...
    return absl::UnimplementedError("Compile with options is not supported.");
  }
  ...
  DeserializeExecutable(std::string_view serialized,
                        std::optional<CompileOptions> options) {
    return absl::UnimplementedError("Deserialize is not supported.");
  }
  ...
  class AsyncHostToDeviceTransferManager {
   public:
    ...
    virtual absl::Status TransferRawDataToBuffer(
        int buffer_index, std::string_view data,
        absl::AnyInvocable<void() &&> on_done) = 0;
  ...
  virtual absl::Status Defragment() {
    return absl::UnimplementedError("Defragment is not implemented.");
  }
 ...
 public:
  ...
  class ExternalReference {
   public:
    ...
    virtual absl::Status WaitUntilBufferReadyOnStream(intptr_t stream) {
      return absl::UnimplementedError(
  ...
  // `serialized_descriptors` is fulfilled. This method requires that both
  // `callbacks.size()` and (if Ok) `serialized_descriptors.size()` match the
...
class PjRtLoadedExecutable : public PjRtExecutable {
 public:
  ...
  // when the device execution is successfully launched. Concurrent donations
