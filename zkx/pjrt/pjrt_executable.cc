...
absl::StatusOr<ExecuteOptionsProto> ExecuteOptions::ToProto() const {
  ...
  if (!send_callbacks.empty() || !recv_callbacks.empty()) {
    return absl::UnimplementedError(
        "ExecuteOptions with send/recv callbacks is not serializable");
  }
  ...
  for (auto& alloc : allocs) {
    ...
    for (const auto& [value, _] : alloc.assigned_buffers()) {
      ...
      if (alloc_memory_space == -1) {
        ...
        CHECK_EQ(alloc_memory_space, memory_space)
...
PjRtExecutable::GetOutputElementTypes() const {
  ...
  for (int i = 0; i < output_shapes.size(); ++i) {
    ...
    if (output_shape.IsTuple()) {
      const std::vector<Shape>& tuple_shapes = output_shape.tuple_shapes();
...
PjRtExecutableUtil::RunHloCostAnalysis(const PjRtExecutable& executable,
  ...
  if (modules.empty()) {
    return absl::NotFoundError(
        absl::StrFormat("Executable '%s' did not have an HloModule to generate "
                        "cost analysis with.",
                        executable.name()));
  } else if (modules.size() > 1) {
    return absl::UnimplementedError(
        absl::StrFormat("GetCostAnalysis() doesn't support multiple program "
...
PjRtExecutableUtil::RunHloCostAnalysis(
  ...
  if (hlo_modules.empty()) {
    return absl::NotFoundError(
        "RunHloCostAnalysis called with empty hlo_modules");
  ...
  //     [&](std::string_view key, float val) { ret[key] = val; });
  // return ret;
  return absl::UnimplementedError("...");
}
  ...
  for (auto& env_option_override : env_option_overrides) {
    switch (env_option_override.second.value_case()) {
      ...
      case OptionOverrideProto::VALUE_NOT_SET:
        return absl::InternalError("OptionOverrideProto value not set.");
    }
  }
  return result;
}
...
absl::Status CompileOptions::ApplyOption(const std::string& key,
  ...
  if (auto* field = DebugOptions::descriptor()->FindFieldByName(key)) {
    ...
    if (!reflection) {
      return absl::InvalidArgumentError(
          "No reflection object associated with DebugOptions.");
    }
    if (field->type() == google::protobuf::FieldDescriptor::TYPE_BOOL &&
        std::holds_alternative<bool>(value)) {
      reflection->SetBool(&debug_options, field, std::get<bool>(value));
      return absl::OkStatus();
    } else if (std::holds_alternative<std::string>(value)) {
      TF_RETURN_IF_ERROR(
          ApplyOptionFromString(field, std::get<std::string>(value)));
      return absl::OkStatus();
    } else if (field->type() == google::protobuf::FieldDescriptor::TYPE_INT32 &&
               std::holds_alternative<int64_t>(value)) {
      reflection->SetInt32(&debug_options, field, std::get<int64_t>(value));
      return absl::OkStatus();
    } else if (field->type() == google::protobuf::FieldDescriptor::TYPE_INT64 &&
               std::holds_alternative<int64_t>(value)) {
      reflection->SetInt64(&debug_options, field, std::get<int64_t>(value));
      return absl::OkStatus();
    } else if (field->type() == google::protobuf::FieldDescriptor::TYPE_FLOAT &&
               std::holds_alternative<double>(value)) {
      reflection->SetFloat(&debug_options, field, std::get<double>(value));
      return absl::OkStatus();
    } else if (field->type() ==
                   google::protobuf::FieldDescriptor::TYPE_DOUBLE &&
               std::holds_alternative<double>(value)) {
      reflection->SetDouble(&debug_options, field, std::get<double>(value));
      return absl::OkStatus();
    } else if (field->type() == google::protobuf::FieldDescriptor::TYPE_ENUM) {
      if (std::holds_alternative<int64_t>(value)) {
        if (field->is_repeated()) {
          reflection->AddEnumValue(&debug_options, field,
                                   std::get<int64_t>(value));
        } else {
          reflection->SetEnumValue(&debug_options, field,
                                   std::get<int64_t>(value));
        }
      } else {
        auto enum_desc =
            field->enum_type()->FindValueByName(std::get<std::string>(value));
