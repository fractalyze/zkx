...
#ifndef ZKX_PJRT_STATUS_CASTERS_H_
#define ZKX_PJRT_STATUS_CASTERS_H_
...
//   m.def("my_func", []() { zkx::ThrowIfError(MyFunc()); }
// - Classes with a single argument:
//   py_class.def("delete", [](Buffer& self) {
//     zkx::ThrowIfError(self.Delete());
//   }
//
...
//
// m.def("my_func", zkx::ThrowIfErrorWrapper(MyFunc));
//
// Nonstatic member functions can be wrapped by passing a
// pointer-to-member-function:
// zkx::ThrowIfErrorWrapper(&MyClass::MyMethod)
...
struct ThrowIfErrorWrapper<absl::Status(Args...), absl::Status (&)(Args...)> {
  ...
  void operator()(Args... args) const {
    zkx::ThrowIfError(func(std::forward<Args>(args)...));
  }
...
struct ThrowIfErrorWrapper<absl::Status(Args...), C> {
  ...
  void operator()(C& instance, Args... args) const {
    zkx::ThrowIfError((instance.*ptmf)(std::forward<Args>(args)...));
  }
...
struct ValueOrThrowWrapper<absl::StatusOr<R>(Args...),
  ...
  R operator()(Args... args) const {
    return zkx::ValueOrThrow(func(std::forward<Args>(args)...));
  }
...
struct ValueOrThrowWrapper<absl::StatusOr<R>(Args...), C> {
  ...
  R operator()(C& instance, Args... args) const {
    return zkx::ValueOrThrow((instance.*ptmf)(std::forward<Args>(args)...));
  }
...
#endif  // ZKX_PJRT_STATUS_CASTERS_H_
