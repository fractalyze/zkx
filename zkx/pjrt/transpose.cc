...
static absl::Status ParseTilingSpecification(
  ...
  if (absl::c_find_if(tiling_spec, [](int64_t d) { return d < 1; }) !=
      tiling_spec.end()) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "Tiling sizes (%s) must be >= 1", absl::StrJoin(tiling_spec, ",")));
  }
  if (ndim == 1) {
...
absl::StatusOr<std::unique_ptr<TransposePlan>> TransposePlan::Create(
  ...
  if (absl::c_find_if(o.dims, is_negative) != o.dims.end()) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "dims must be non-negative, got %s", absl::StrJoin(o.dims, ",")));
  }
  ...
  if (!IsPermutation(o.permutation)) {
    return absl::InvalidArgumentError(
        absl::StrFormat("permutation argument is not valid, got: %s",
                        absl::StrJoin(o.permutation, ",")));
  }
  if (o.num_threads < 1) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "num_threads argument must be >= 1, got: %d", o.num_threads));
  }
  ...
  switch (o.elem_size_in_bytes) {
    ...
    default:
      return absl::InvalidArgumentError(absl::StrFormat(
          "Unsupported elem_size_in_bytes=%d", o.elem_size_in_bytes));
  }
  ...
  if (std::holds_alternative<Striding>(o.input_layout)) {
    ...
    auto cost = [&](int k) {
      ...
      return std::make_tuple(is_stride1, -std::abs(stride),
