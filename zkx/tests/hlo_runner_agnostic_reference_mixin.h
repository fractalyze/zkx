...
#ifndef ZKX_TESTS_HLO_RUNNER_AGNOSTIC_REFERENCE_MIXIN_H_
#define ZKX_TESTS_HLO_RUNNER_AGNOSTIC_REFERENCE_MIXIN_H_
...
class HloRunnerAgnosticReferenceMixin : public T {
 ...
 protected:
  ...
  ::testing::AssertionResult RunAndCompare(
    ...
    const absl::StatusOr<::testing::AssertionResult> result =
        RunAndCompareInternal(std::move(module), arguments,
  ...
  ::testing::AssertionResult RunAndCompare(
    ...
    return RunAndCompare(std::move(module), fake_argument_ptrs,
  ...
  ::testing::AssertionResult RunAndCompare(
      const std::string_view hlo_string,
      const std::function<void(HloModule*)>& reference_preprocessor = nullptr,
      const std::function<void(HloModule*)>& test_preprocessor = nullptr,
      const std::optional<int64_t> args_max_bits_of_precision = std::nullopt) {
    ...
    return RunAndCompare(*std::move(module), reference_preprocessor,
  ...
  ::testing::AssertionResult RunAndCompareNoHloPasses(
    ...
    return RunAndCompareNoHloPasses(*std::move(module), reference_preprocessor,
                                    test_preprocessor);
  }
 ...
 private:
  ...
  absl::StatusOr<::testing::AssertionResult> RunAndCompareInternal(
    ...
    return LiteralTestUtil::Equal(/*expected=*/reference, /*actual=*/test);
  }
...
#endif  // ZKX_TESTS_HLO_RUNNER_AGNOSTIC_REFERENCE_MIXIN_H_
