...
#ifndef ZKX_CORE_COLLECTIVES_COLLECTIVES_H_
#define ZKX_CORE_COLLECTIVES_COLLECTIVES_H_
...
// Collectives is a base class for host-initiated collective operations in ZKX.
//
// Host-initiated collective operations are collective operations that are
// initiated by the host runtime, i.e. in ZKX:GPU the default collectives
// implementation uses NCCL and Thunks initiate collective operations of the
// runtime-managed streams.
//
// IMPORTANT: ZKX also supports device-initiated collective operations, which
// are collective operations for communication between device kernels. In
// ZKX:GPU device-initiated collective operations are implemented using NVSHMEM.
class Collectives {
 public:
  virtual ~Collectives() = default;

  // A base class for the device that the collectives are running on, i.e. in
  // ZKX:GPU this is the GPU device (StreamExecutor).
  class Device {
   public:
...
#endif  // ZKX_CORE_COLLECTIVES_COLLECTIVES_H_
