...
#ifndef ZKX_PARSE_FLAGS_FROM_ENV_H_
#define ZKX_PARSE_FLAGS_FROM_ENV_H_
...
// Examples:
//
//  - ZKX_FLAGS="--foo=bar  --wombat='value with a space'"
//  - ZKX_FLAGS=/tmp/flagfile
//
...
void ParseFlagsFromEnvAndIgnoreUnknown(std::string_view envvar,
                                       const std::vector<tsl::Flag>& flag_list,
                                       bool reset_envvar = false);

// Used only for testing.  Not to be used by clients.
void ResetFlagsFromEnvForTesting(std::string_view envvar, int** pargc,
                                 std::vector<char*>** pargv);

}  // namespace zkx

#endif  // ZKX_PARSE_FLAGS_FROM_ENV_H_
