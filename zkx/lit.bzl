...
load("//xla/tsl:tsl.bzl", "if_cuda_tools")
...
def lit_test_suite(
    ...
    Args:
      ...
      tools: label list. Tools invoked in the lit RUN lines. These binaries will
        ...
        have unique basenames. Note that tools that are zkx_cc_binary targets
      ...
      hermetic_cuda_data_dir: string. If set, the tests will be run with a
        `--zkx_gpu_cuda_data_dir` flag set to the hermetic CUDA data directory.
...
def lit_script_with_zkx_gpu_cuda_data_dir(
        name,
        input_file,
        output_file,
        zkx_gpu_cuda_data_dir):
    """Adds a line to the LIT script to set the ZKX_FLAGS environment variable."""
    return native.genrule(
        name = name,
        srcs = [input_file],
        outs = [output_file],
        cmd = if_cuda_tools(
            """echo -e '// RUN: export ZKX_FLAGS=\"--zkx_gpu_cuda_data_dir={}\"' > $@;
cat $< >> $@;""".format(zkx_gpu_cuda_data_dir),
            "cat $< >> $@;",
        ),
    )
...
def lit_test(
    ...
    if hermetic_cuda_data_dir:
        output_file = "with_zkx_gpu_cuda_data_dir_{}".format(test_file)
        rule_name = "script_{}".format(output_file)
        lit_script_with_zkx_gpu_cuda_data_dir(
            rule_name,
            test_file,
            output_file,
