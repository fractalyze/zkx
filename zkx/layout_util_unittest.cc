...
TEST_F(LayoutUtilTest, CopyLayoutDenseArray) {
  ...
  Shape dst = MakeShapeWithLayout(U32, {2, 3}, {1, 0});
...
TEST_F(LayoutUtilTest, CopyLayoutNotCompatibleSameRank) {
  ...
  Shape dst = MakeShapeWithLayout(U32, {2, 3, 5}, {1, 0});
  ASSERT_TRUE(LayoutUtil::CopyLayoutBetweenShapes(src, &dst).ok());
  EXPECT_TRUE(LayoutUtil::LayoutsInShapesEqual(src, dst));
}
...
TEST_F(LayoutUtilTest, CopyLayoutBogusLayout) {
  ...
  Shape dst = ShapeUtil::MakeShape(U32, {2, 3});
...
//   // Layouts are trivially the same for token types and copying layouts
//   should
//   // be a nop.
//   EXPECT_TRUE(LayoutUtil::LayoutsInShapesEqual(src, dst));
...
// TODO(chokobole): Uncomment this. Dependency: ShapeUtil::MakeOpaqueShape
// TEST_F(LayoutUtilTest, CopyOpaqueLayout) {
//   Shape src = ShapeUtil::MakeOpaqueShape();
//   Shape dst = ShapeUtil::MakeOpaqueShape();

//   // Layouts are trivially the same for opaque types and copying layouts
//   should
//   // be a nop.
//   EXPECT_TRUE(LayoutUtil::LayoutsInShapesEqual(src, dst));
//   EXPECT_IS_OK(LayoutUtil::CopyLayoutBetweenShapes(src, &dst));
...
// TODO(chokobole): Uncomment this. Dependency: ShapeUtil::MakeOpaqueShape,
// ShapeUtil::MakeTokenShape, ShapeUtil::MakeTupleShape
...
//        MakeShapeWithLayout(U32, {42, 123}, {1, 0}),
//        ShapeUtil::MakeTokenShape(), ShapeUtil::MakeTupleShape(
//            {ShapeUtil::MakeOpaqueShape(), MakeShapeWithLayout(U32, {}, {}),
//             MakeShapeWithLayout(U32, {1, 2, 3}, {0, 2, 1})})});
//   Shape dst = ShapeUtil::MakeTupleShape(
//       {MakeShapeWithLayout(U32, {2, 3}, {1, 0}),
//        MakeShapeWithLayout(U32, {42, 123}, {1, 0}),
  ...
  EXPECT_TRUE(
      LayoutUtil::Equal(LayoutUtil::MakeLayout({4, 3, 2, 1, 0}),
                        LayoutUtil::GetDefaultLayoutForShape(
                            ShapeUtil::MakeShape(U32, {10, 20, 30, 15, 25}))));
}
...
TEST_F(LayoutUtilTest, ValidateLayout_MissingArrayLayout) {
  ...
  EXPECT_FALSE(status.ok());
  EXPECT_THAT(status.message(),
              ::testing::HasSubstr("shape u32[2,3] does not have a layout"));
  status = LayoutUtil::ValidateLayoutInShape(shape,
                                             /*allow_missing_layouts=*/true);
  EXPECT_TRUE(status.ok());
}
...
TEST_F(LayoutUtilTest, ValidateLayout_Sparse) {
  ...
  *shape.mutable_layout() = LayoutUtil::MakeLayout(
  ...
  shape.mutable_layout()->clear_tiles();
  EXPECT_TRUE(LayoutUtil::ValidateLayoutInShape(shape).ok());
  *shape.mutable_layout()->mutable_physical_shape() =
      ShapeUtil::MakeShape(U32, {6});
  EXPECT_TRUE(LayoutUtil::ValidateLayoutInShape(shape).ok());
  *shape.mutable_layout()
