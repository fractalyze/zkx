...
void ZkxDebugInfoManager::UnregisterModule(ModuleIdentifier module_id) {
  absl::MutexLock lock(&mutex_);
  ...
  if (!tracing_active_) {
    ...
    ZkxModuleEntry& m = it->second;
    m.active = false;
  }
}

void ZkxDebugInfoManager::StartTracing() {
  absl::MutexLock lock(&mutex_);
  tracing_active_ = true;
}

void ZkxDebugInfoManager::StopTracing(
    std::vector<std::unique_ptr<HloProto>>* module_debug_info) {
  std::vector<ZkxModuleEntry> modules_to_serialize;
  {
    absl::MutexLock lock(&mutex_);
...
bool ZkxDebugInfoManager::TracksModule(ModuleIdentifier module_id) const {
  absl::MutexLock lock(&mutex_);
