...
#ifndef ZKX_SERVICE_RENDEZVOUS_H_
#define ZKX_SERVICE_RENDEZVOUS_H_
...
// A little bit of compile time meta-programming to simplify the rendezvous
...
template <typename K>
void Rendezvous(std::string_view name, const K& key, size_t num_threads,
                absl::Duration warn_stuck_timeout = absl::InfiniteDuration(),
                absl::Duration terminate_timeout = absl::InfiniteDuration());
...
// `completed` state.
//
// In ZKX rendezvous are rare and used to guard costly shared state
...
template <typename R, typename K, typename Fn>
RendezvousResultType<R> Rendezvous(
    RendezvousFlag& flag, std::string_view name, const K& key,
...
// rendezvous.
template <typename K>
void Rendezvous(RendezvousFlag& flag, std::string_view name, const K& key,
                size_t num_threads,
                absl::Duration warn_stuck_timeout = absl::InfiniteDuration(),
                absl::Duration terminate_timeout = absl::InfiniteDuration());
...
struct RendezvousState : public RendezvousStateSynchronization {
  explicit RendezvousState(size_t num_treads)
      : RendezvousStateSynchronization(num_treads),
        values(num_treads, nullptr),
...
template <typename R, typename K, typename V, typename Fn>
RendezvousResultType<R> Rendezvous(std::string_view name, const K& key,
...
template <typename R, typename K, typename Fn>
RendezvousResultType<R> Rendezvous(RendezvousFlag& flag, std::string_view name,
...
#endif  // ZKX_SERVICE_RENDEZVOUS_H_
