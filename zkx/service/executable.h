...
#ifndef ZKX_SERVICE_EXECUTABLE_H_
#define ZKX_SERVICE_EXECUTABLE_H_
...
class ExecutionInput {
 public:
  explicit ExecutionInput(Shape shape) : buffers_(std::move(shape)) {
    SetHostShape(ShapeUtil::DeviceShapeToHostShape(buffers_.shape()));
  }
  // TODO(b/170310047): remove this overload.
  ExecutionInput(Shape shape, Shape host_shape) : buffers_(std::move(shape)) {
    SetHostShape(ShapeUtil::DeviceShapeToHostShape(buffers_.shape()));
  }
  ...
  // TODO(b/170310047): remove this overload.
  ExecutionInput(ShapeTree<MaybeOwningDeviceMemory> buffers, Shape host_shape)
      : buffers_(std::move(buffers)) {
    SetHostShape(ShapeUtil::DeviceShapeToHostShape(buffers_.shape()));
  }
 ...
 private:
  void SetHostShape(Shape host_shape) {
    if (shape() != host_shape) {
...
class ExecutionOutput {
 public:
  ...
  ~ExecutionOutput() {
    ...
    // released.
    for (const ShapeIndex& index : aliased_indices_) {
  ...
  std::vector<ShapeIndex> ConsumeAliasedIndices() {
    std::vector<ShapeIndex> aliased = std::move(aliased_indices_);
    aliased_indices_.clear();
    return aliased;
  }
 ...
 private:
  ...
  // donated input memory buffers that are not reused by ZKX as outputs.
...
class Executable {
 public:
  ...
  // considered donated and ZKX will potentially reuse it as output buffers. For
  // all donated inputs, ZKX is also responsible for freeing them.
  //
  // If an input is donated to ZKX but is not reused as output, it is returned
...
#endif  // ZKX_SERVICE_EXECUTABLE_H_
