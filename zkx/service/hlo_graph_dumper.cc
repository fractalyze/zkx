...
#include "xla/tsl/platform/base64.h"
...
std::string HtmlLikeStringSanitize(std::string_view s) {
...
class HloDotDumper {
 public:
  HloDotDumper(
      const HloComputation* computation, std::string_view label,
...
bool HloDotDumper::ShouldShowSubcomputation(const HloComputation* subcomp) {
  ...
  // TODO(batzor): Uncomment this. Dependency: WhileCallInstruction
  ...
  if (parent_instr->opcode() == HloOpcode::kFusion) {
    ...
    if (!highlight && (parent_instr->module_has_statistics() ||
      ...
      fillcolor = parent_instr->has_statistics()
                      ...
                      : "#f5f5f5";
      strokecolor = "#c2c2c2";
    } else if (debug_options_.zkx_hlo_graph_sharding_color() && !highlight) {
...
std::string HloDotDumper::DumpInstruction(const HloInstruction* instr) {
  ...
  if (hlo_render_options_.override_node_colors && color_map_.has_value()) {
    ...
    if (!debug_options_.zkx_hlo_graph_sharding_color()) {
  ...
  std::string extended_opcode =
      StrCat(HloOpcodeString(instr->opcode()),
             instr->opcode() != HloOpcode::kFusion
                 ? ""
                 : StrCat(":", zkx::ToString(instr->fusion_kind())));
...
// TODO(batzor): Add ExtractGemmBackendConfigProps
...
std::string HloDotDumper::GetInstructionNodeBackendConfig(
  ...
  // TODO(batzor): Uncomment this. Dependency: IsCustomCallToDnnConvolution,
  // IsCublasGemm if (gpu::IsCustomCallToDnnConvolution(*instr)) {
  ...
  //       instr->backend_config<gpu::GpuBackendConfig>();
  //   if (config.ok()) {
  //     // gemm strides are generally uninteresting (derived from the
  //     instruction
  //     // shape), so we hide them by default.
  //     props =
  //         ExtractGemmBackendConfigProps(config->gemm_backend_config(),
  //         instr);
  //   }
  // }
...
std::string HloDotDumper::GetInstructionNodeExtraInfo(
  ...
  if (instr->opcode() != HloOpcode::kFusion ||
    ...
    if (instr_shape.length() > kMaxShapeLen) {
      instr_shape = StrCat(
          std::string_view(instr_shape).substr(0, kMaxShapeLen - 3), "...");
    }
    lines.push_back(HtmlLikeStringSanitize(instr_shape));
  }
  if (debug_options_.zkx_hlo_graph_addresses()) {
...
std::function<absl::StatusOr<std::string>(std::string_view)>* url_renderer
...
struct FusionVisualizerProgress {
  ...
  void AddState(std::string_view dot, std::string_view explanation,
...
static auto& fusion_visualizer_states
    ABSL_GUARDED_BY(fusion_visualizer_state_mu) =
        *absl::IgnoreLeak(new absl::flat_hash_map<std::pair<int64_t, int64_t>,
                                                  FusionVisualizerProgress>());
...
static absl::StatusOr<std::string> CompressAndEncode(std::string_view input) {
  class WritableStringFile : public tsl::WritableFile {
   public:
    ...
    absl::Status Append(std::string_view data) override {
...
absl::StatusOr<std::string> WrapFusionExplorer(
    ...
    std::string_view graph_title) {
  if (visualizer_progress.frames.empty()) {
    return absl::InternalError("Empty");
  }
...
void RegisterGraphToURLRenderer(
  ...
  delete url_renderer;
  url_renderer =
      new std::function<absl::StatusOr<std::string>(std::string_view)>(
          std::move(renderer));
}

void RegisterFusionState(const HloComputation& computation,
                         std::string_view label, const HloInstruction& consumer,
