...
class LayoutAssignmentTest : public HloTestBase {
 protected:
  void AssignLayouts(HloModule* m, ComputationLayout* entry_computation_layout,
    ...
    TF_ASSERT_OK(layout_assignment.Run(m).status());
  }

  std::vector<int64_t> LayoutOf(HloModule* m, std::string_view name) {
...
TEST_F(LayoutAssignmentTest, FusionInstruction) {
  ...
  for (auto& minor_to_major : minor_to_majors) {
    auto builder = HloComputation::Builder(TestName());
    auto constant_literal1 = LiteralUtil::CreateR2WithLayout<int32_t>(
        {{1, 2}, {3, 4}}, LayoutUtil::MakeLayout(minor_to_major));
    auto constant_literal2 = LiteralUtil::CreateR2WithLayout<int32_t>(
        {{5, 6}, {7, 8}}, LayoutUtil::MakeLayout(minor_to_major));
...
// TODO(batzor): Uncomment this. Dependency: AlgebraicSimplifier
...
//   // second. This results in the conflict where the element of the
//   inner_tuple
...
//       ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 2}, {1, 0});
//   *ShapeUtil::GetMutableSubshape(&result_shape, /*index=*/{1, 0}) =
//       ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 2}, {0, 1});
...
//   // Layout assignment should have deep copied the result of the computation
//   to
...
TEST_F(LayoutAssignmentTest, ElementwiseAndReshape) {
  // param -> abs -> reshape -> negate
  auto builder = HloComputation::Builder(TestName());
  Shape ashape = ShapeUtil::MakeShape(S32, {1, 2, 3, 1});
  Shape bshape = ShapeUtil::MakeShape(S32, {3, 1, 2});
  auto param = builder.AddInstruction(
      HloInstruction::CreateParameter(0, ashape, "param"));
  auto abs = builder.AddInstruction(
      HloInstruction::CreateUnary(ashape, HloOpcode::kAbs, param));
  auto reshape =
      builder.AddInstruction(HloInstruction::CreateReshape(bshape, abs));
  auto negate = builder.AddInstruction(
      HloInstruction::CreateUnary(bshape, HloOpcode::kNegate, reshape));

  auto m = CreateNewVerifiedModule();
  HloComputation* computation = m->AddEntryComputation(builder.Build(negate));
  ...
  *computation_layout.mutable_result_layout() = ShapeLayout(bshape_with_layout);
  ...
  auto abs_minor_to_major = abs->shape().layout().minor_to_major();
  EXPECT_GT(PositionInContainer(abs_minor_to_major, 1),
            PositionInContainer(abs_minor_to_major, 2));
...
TEST_F(LayoutAssignmentTest, ElementwiseAndTranspose) {
  // param -> not -> transpose -> negate
  auto builder = HloComputation::Builder(TestName());
  Shape ashape = ShapeUtil::MakeShape(S32, {42, 12});
  Shape bshape = ShapeUtil::MakeShape(S32, {12, 42});
  auto param = builder.AddInstruction(
      HloInstruction::CreateParameter(0, ashape, "param"));
  auto abs = builder.AddInstruction(
      HloInstruction::CreateUnary(ashape, HloOpcode::kAbs, param));
  auto transpose = builder.AddInstruction(
      HloInstruction::CreateTranspose(bshape, abs, {1, 0}));
  auto negate = builder.AddInstruction(
      HloInstruction::CreateUnary(bshape, HloOpcode::kNegate, transpose));
  auto m = CreateNewVerifiedModule();
  auto computation = m->AddEntryComputation(builder.Build(negate));
  ...
  *computation_layout.mutable_result_layout() = ShapeLayout(bshape_with_layout);
  ...
  EXPECT_TRUE(
      LayoutUtil::Equal(ashape_with_layout.layout(), abs->shape().layout()));
  EXPECT_TRUE(LayoutUtil::Equal(bshape_with_layout.layout(),
                                transpose->shape().layout()));
  EXPECT_TRUE(
      LayoutUtil::Equal(bshape_with_layout.layout(), negate->shape().layout()));
}
...
TEST_F(LayoutAssignmentTest, BroadcastAndTranspose) {
  ...
  auto builder = HloComputation::Builder(TestName());
  Shape ashape = ShapeUtil::MakeShape(S32, {3, 4});
  Shape bshape = ShapeUtil::MakeShape(S32, {2, 3, 4});
  Shape cshape = ShapeUtil::MakeShape(S32, {4, 3, 2});
  auto param = builder.AddInstruction(
      HloInstruction::CreateParameter(0, ashape, "param"));
...
TEST_F(LayoutAssignmentTest, ReshapeOperandHasMultipleUsers) {
  ...
  // transpose is a bitcast. Furthermore, `negate` is expected to have the same
  // layout as `broadcast` (i.e. {0,1}) because `negate` is elementwise.
  Shape S32_4 = ShapeUtil::MakeShape(S32, {4});
  Shape S32_34 = ShapeUtil::MakeShape(S32, {3, 4});
  Shape S32_43 = ShapeUtil::MakeShape(S32, {4, 3});
  Shape S32_234 = ShapeUtil::MakeShape(S32, {2, 3, 4});

  auto builder = HloComputation::Builder(TestName());
  auto param = builder.AddInstruction(
      HloInstruction::CreateParameter(0, S32_4, "param"));
  auto broadcast = builder.AddInstruction(
      HloInstruction::CreateBroadcast(S32_34, param, {1}));
  auto transpose = builder.AddInstruction(
      HloInstruction::CreateTranspose(S32_43, broadcast, {1, 0}));
  auto negate = builder.AddInstruction(
      HloInstruction::CreateUnary(S32_34, HloOpcode::kNegate, broadcast));
  auto broadcast2 = builder.AddInstruction(
      HloInstruction::CreateBroadcast(S32_234, negate, {1, 2}));
  auto tuple = builder.AddInstruction(
  ...
  ComputationLayout computation_layout(computation->ComputeProgramShape());
  Shape param_shape_with_layout(S32_4);
  Shape transpose_shape_with_layout(S32_43);
  Shape broadcast2_shape_with_layout(S32_234);
  ...
  *computation_layout.mutable_result_layout() =
  ...
  EXPECT_THAT(negate->shape().layout().minor_to_major(), ElementsAre(0, 1));
}
...
TEST_F(LayoutAssignmentTest, MakeOperandsTheSame) {
  ...
  // param1   -^
  auto builder = HloComputation::Builder(TestName());
  Shape ashape = ShapeUtil::MakeShape(S32, {50, 1});
  Shape bshape = ShapeUtil::MakeShape(S32, {50, 2});
  Shape cshape = ShapeUtil::MakeShape(S32, {100});
  auto param0 = builder.AddInstruction(
      HloInstruction::CreateParameter(0, ashape, "param"));
  auto param1 = builder.AddInstruction(
  ...
  *computation_layout.mutable_parameter_layout(1) =
  ...
  TF_ASSERT_OK(layout_assignment.Run(m.get()).status());
...
TEST_F(LayoutAssignmentTest, TransposeToBitcastFromOperand) {
  ...
  Shape input_shape_with_layout =
      ShapeUtil::MakeShapeWithDenseLayout(S32, {3, 5, 6, 7}, {2, 0, 3, 1});
  auto param = builder.AddInstruction(
  ...
  auto transpose = builder.AddInstruction(HloInstruction::CreateTranspose(
      ShapeUtil::MakeShape(S32, {6, 7, 3, 5}), param, {2, 3, 0, 1}));
  auto m = CreateNewVerifiedModule();
  HloComputation* computation =
      m->AddEntryComputation(builder.Build(transpose));
  ComputationLayout computation_layout(computation->ComputeProgramShape());
...
TEST_F(LayoutAssignmentTest, TransposeToBitcastToUser) {
  auto builder = HloComputation::Builder(TestName());
  Shape input_shape = ShapeUtil::MakeShape(S32, {3, 5, 6, 7});
  auto constant = builder.AddInstruction(
      HloInstruction::CreateConstant(LiteralUtil::CreateR0<int32_t>(1)));
  auto broadcast = builder.AddInstruction(
      HloInstruction::CreateBroadcast(input_shape, constant, {}));
  auto transpose = builder.AddInstruction(HloInstruction::CreateTranspose(
      ShapeUtil::MakeShape(S32, {6, 7, 3, 5}), broadcast, {2, 3, 0, 1}));
  auto m = CreateNewVerifiedModule();
  HloComputation* computation =
      m->AddEntryComputation(builder.Build(transpose));
  ComputationLayout computation_layout(computation->ComputeProgramShape());
...
TEST_F(LayoutAssignmentTest, TransposeIsBitcastFail) {
  auto builder = HloComputation::Builder(TestName());
  Shape input_shape = ShapeUtil::MakeShape(S32, {2, 2, 2});
...
TEST_F(LayoutAssignmentTest, TransposeWithinFusionDoesNotCrash) {
  const char* module_str = R"(
    ...
    fused_computation {
      param_1 = s32[2,2,2]{2,1,0} parameter(1)
      transpose = s32[2,2,2]{2,1,0} transpose(param_1), dimensions={0,2,1}
      reduce_1 = s32[] parameter(0)
      broadcast_1 = s32[2,2,2]{2,1,0} broadcast(reduce_1), dimensions={}
      ROOT divide_1 = s32[2,2,2]{2,1,0} divide(transpose, broadcast_1)
    }

    ENTRY entry_computation {
      fusion.1 = s32[2,2,2]{2,1,0} parameter(1)
      reduce.1 = s32[] parameter(0)
      fusion.2 = s32[2,2,2]{2,1,0} fusion(reduce.1, fusion.1), kind=kLoop, calls=fused_computation
     ROOT tuple.1 = (s32[2,2,2]{2,1,0}) tuple(fusion.2)
    }
  )";
...
TEST_F(LayoutAssignmentTest, GTEInheritsLayoutFromOperand) {
  const char* module_str = R"(
    ...
    fused_computation {
      fparam = (s32[2,2,2], (s32[2,2,2], s32[2,2,2])) parameter(0)
      gte0 = s32[2,2,2] get-tuple-element(fparam), index=0
      gte1 = (s32[2,2,2], s32[2,2,2]) get-tuple-element(fparam), index=1
      gte1a = s32[2,2,2] get-tuple-element(gte1), index=0
      gte1b = s32[2,2,2] get-tuple-element(gte1), index=1
      add = s32[2,2,2] add(gte1a, gte1b)
      ROOT fresult = s32[2,2,2] add(gte0, add)
    }

    ENTRY entry_computation {
      param = (s32[2,2,2], (s32[2,2,2], s32[2,2,2])) parameter(0)
      ROOT fusion =
        s32[2,2,2] fusion(param), kind=kLoop, calls=fused_computation
    }
  )";
  ...
  Shape param_shape = ShapeUtil::MakeTupleShape(
      {ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 2, 2}, {0, 1, 2}),
       ShapeUtil::MakeTupleShape({
           ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 2, 2}, {1, 2, 0}),
           ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 2, 2}, {2, 0, 1}),
       })});
  TF_ASSERT_OK(
...
TEST_F(LayoutAssignmentTest, ConditionalAsymmetricLayout) {
  auto builder = HloComputation::Builder(TestName());
  auto m = CreateNewVerifiedModule();
  Shape shape = ShapeUtil::MakeShape(S32, {128, 8});
  ...
  {
    Shape xshape = ShapeUtil::MakeShapeWithDenseLayout(S32, {128, 8}, {0, 1});
...
TEST_F(LayoutAssignmentTest, LayoutAssignmentToTupleSiblingOperand) {
  ...
  true_branch {
    tparam = (s64[2,3], s64[2,3]) parameter(0)
    ROOT tgte = s64[2,3] get-tuple-element(tparam), index=1
  }

  false_branch {
    ROOT Arg = s64[2,3] parameter(0)
  }

  ENTRY entry {
    p0 = (s64[2,3], s64[2,3])  parameter(0)
    p1 = s64[2,3] parameter(1)
    constant = pred[] constant(true)
    ROOT conditional = s64[2,3] conditional(constant, p0, p1),
...
TEST_F(LayoutAssignmentTest, ChannelLayoutMismatch) {
  ...
  const char* module_str = R"(
    ...
    ENTRY entry_computation {
      param = (s32[2,2]) parameter(0)
      gte = s32[2,2] get-tuple-element(param), index=0
      token0 = token[] after-all()
      recv = (s32[2,2], u32[], token[]) recv(token0), channel_id=1
      recv-done = (s32[2,2], token[]) recv-done(recv), channel_id=1
      ROOT root = s32[2,2] get-tuple-element(recv-done), index=0
      send = (s32[2,2], u32[], token[]) send(gte, token0), channel_id=1
...
TEST_F(LayoutAssignmentTest, AllReduceSpmd) {
  ...
  const char* module_str = R"(
    ...
    ENTRY entry_computation {
      param = s32[2,2] parameter(0)
      ar = s32[2,2] all-reduce(param),
        channel_id=1, replica_groups={{0}}, to_apply=add
      p1 = s32[2] parameter(1)
      ar.1 = s32[2] all-reduce(p1),
        channel_id=1, replica_groups={{0}}, to_apply=add
      ROOT t = tuple(ar, ar.1)
    })";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> m,
                          ParseAndReturnVerifiedModule(module_str));
...
TEST_F(LayoutAssignmentTest, AllReduceLayoutMissmatch) {
  ...
  const char* module_str = R"(
    ...
    ENTRY entry_computation {
      param = (s32[2,2]) parameter(0)
      gte = s32[2,2] get-tuple-element(param), index=0
      ar = s32[2,2] all-reduce(gte),
        channel_id=1, replica_groups={{0}}, to_apply=add,
        sharding={maximal device=0}
      const = s32[2,2] constant({{0,1},{2,3}})
      ROOT ar.1 = s32[2,2] all-reduce(const),
        channel_id=1, replica_groups={{0}}, to_apply=add,
  ...
  EXPECT_THAT(LayoutOf(m.get(), "ar"), ElementsAre(0, 1));
...
TEST_F(LayoutAssignmentTest, CopySliceOperandToAvoidImplicitLayoutChange) {
  const char* module_str = R"(
    ...
    ENTRY CopySliceOperandToAvoidImplicitLayoutChange {
      par0 = s32[3,4]{1,0} parameter(0)
      par1 = s32[4,5]{0,1} parameter(1)
      slice0 = s32[3,4] slice(par1), slice={[1:4],[1:5]}
      ROOT add0 = s32[3,4]{1,0} add(par0,slice0)
    }
  )";
  ...
  Shape shape_copy = ShapeUtil::MakeShapeWithDenseLayout(S32, {4, 5}, {1, 0});
  EXPECT_THAT(
      root,
      GmockMatch(m::Add(
          m::Parameter(),
...
TEST_F(LayoutAssignmentTest, CopyDSliceOperandToAvoidImplicitLayoutChange) {
  const char* module_str = R"(
    ...
    ENTRY CopyDSliceOperandToAvoidImplicitLayoutChange {
      ...
      dslice0 = s32[3,4] dynamic-slice(par1, par2, par3), dynamic_slice_sizes={3,4}
      ROOT add0 = s32[3,4]{1,0} add(par0,dslice0)
    }
  )";
...
TEST_F(LayoutAssignmentTest, CopyConcatOperandToAvoidImplicitLayoutChange) {
  const char* module_str = R"(
    ...
    ENTRY CopyConcatOperandToAvoidImplicitLayoutChange {
      par0 = s32[3,8]{1,0} parameter(0)
      par1 = s32[3,5]{0,1} parameter(1)
      par2 = s32[3,3]{1,0} parameter(2)
      concat0 = s32[3,8] concatenate(s32[3,5] par1, s32[3,3] par2),
        dimensions={1}
      ROOT add0 = s32[3,8]{1,0} add(par0,concat0)
    }
  )";
  ...
  HloInstruction* root =
      compiled_module->entry_computation()->root_instruction();
  Shape shape_copy = ShapeUtil::MakeShapeWithDenseLayout(S32, {3, 5}, {1, 0});
  EXPECT_THAT(
      root,
      GmockMatch(m::Add(
          m::Parameter(),
...
TEST_F(LayoutAssignmentTest, PropagatingLayoutFromResultToOperand) {
  const char* module_str = R"(
    ...
    ENTRY PropagatingLayoutFromResultToOperand {
      par0 = s32[4,5]{1,0} parameter(0)
      ROOT slice0 = s32[3,4]{0,1} slice(par0), slice={[1:4],[1:5]}
    }
  )";
  ...
  HloInstruction* root =
      compiled_module->entry_computation()->root_instruction();
  Shape shape_copy = ShapeUtil::MakeShapeWithDenseLayout(S32, {4, 5}, {0, 1});
  EXPECT_THAT(root,
              GmockMatch(m::Slice(
...
TEST_F(LayoutAssignmentTest, TupleCopyOnLayoutMismatch) {
  ...
  const char* module_str = R"(
    ...
    condition.1 (tup: (s32[], token[], s32[512,1024]{0,1})) -> pred[] {
      tup.1 = (s32[], token[], s32[512,1024]{0,1}) parameter(0)
      counter.1 = s32[] get-tuple-element(tup.1), index=0
      five = s32[] constant(5)
      ROOT lt = pred[] compare(counter.1, five), direction=LT
    }

    body.2 (tup: (s32[], token[], s32[512,1024]{0,1})) -> (s32[], token[], s32[512,1024]{0,1}) {
      tup.2 = (s32[], token[], s32[512,1024]{0,1}) parameter(0)
      counter.2 = s32[] get-tuple-element(tup.2), index=0
      tok.2 = token[] get-tuple-element(tup.2), index=1

      ifeed.2 = (s32[512,1024]{1,0}, token[]) infeed(tok.2)
      next_tok = token[] get-tuple-element(ifeed.2), index=1
      next_buf = s32[512,1024]{1,0} get-tuple-element(ifeed.2), index=0

      one = s32[] constant(1)
      next_counter = s32[] add(counter.2, one)
      ROOT tup = (s32[], token[], s32[512,1024]{0,1}) tuple(next_counter, next_tok, next_buf)
    }

    ENTRY main () -> s32[512,1024]{0,1} {
      start_tok = token[] after-all()

      ifeed.3 = (s32[512,1024]{0,1}, token[]) infeed(start_tok)
      itok = token[] get-tuple-element(ifeed.3), index=1
      ibuf = s32[512,1024]{0,1} get-tuple-element(ifeed.3), index=0

      zero = s32[] constant(0)
      itup = (s32[], token[], s32[512,1024]{0,1}) tuple(zero, itok, ibuf)

      loop = (s32[], token[], s32[512,1024]{0,1}) while(itup), condition=condition.1, body=body.2
      ROOT result = s32[512,1024]{0,1} get-tuple-element(loop), index=2
    }
  )";
...
TEST_F(LayoutAssignmentTest, DISABLED_CustomCallNotLayoutConstrained) {
  const char* module_str = R"(
HloModule CustomCallNotLayoutConstrained

ENTRY %CustomCallWithNotLayoutConstrained (p: s32[42,2,3]) -> s32[1,2,3,4] {
  %p = s32[42,2,3] parameter(0)
  ROOT %custom-call = s32[1,2,3,4] custom-call(s32[42,2,3] %p), custom_call_target="baz"
}
)";
  ...
  // and result layout should match that of the computation.
  {
    ...
    *computation_layout.mutable_parameter_layout(0) = ShapeLayout(
        ShapeUtil::MakeShapeWithDenseLayout(S32, {42, 2, 3}, {0, 2, 1}));
    *computation_layout.mutable_result_layout() = ShapeLayout(
        ShapeUtil::MakeShapeWithDenseLayout(S32, {1, 2, 3, 4}, {3, 2, 0, 1}));
    AssignLayouts(m.get(), &computation_layout);
  ...
  }
  {
    ...
    ComputationLayout computation_layout = m->entry_computation_layout();
    *computation_layout.mutable_parameter_layout(0) = ShapeLayout(
        ShapeUtil::MakeShapeWithDenseLayout(S32, {42, 2, 3}, {0, 1, 2}));
    *computation_layout.mutable_result_layout() = ShapeLayout(
        ShapeUtil::MakeShapeWithDenseLayout(S32, {1, 2, 3, 4}, {0, 2, 3, 1}));
    AssignLayouts(m.get(), &computation_layout);
...
// TODO(batzor): Enable this test. Dependency: CustomCall
TEST_F(LayoutAssignmentTest, DISABLED_CustomCallLayoutConstrained) {
  const char* module_str = R"(
HloModule CustomCallLayoutConstrained

ENTRY %CustomCallWithLayoutConstraints (p0: s32[4,4], p1: s32[2,3]) -> s32[1,2,3,4] {
  %p0 = s32[4,4] parameter(0)
  %p1 = s32[2,3] parameter(1)
  ROOT %custom-call = s32[1,2,3,4]{3,2,0,1} custom-call(s32[4,4] %p0, s32[2,3] %p1), custom_call_target="baz", operand_layout_constraints={s32[4,4]{0,1}, s32[2,3]{1,0}}
}
)";
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> m,
  ...
  ComputationLayout computation_layout = m->entry_computation_layout();
  *computation_layout.mutable_parameter_layout(0) =
      ShapeLayout(ShapeUtil::MakeShapeWithDenseLayout(S32, {4, 4}, {1, 0}));
  *computation_layout.mutable_parameter_layout(1) =
      ShapeLayout(ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 3}, {1, 0}));
  *computation_layout.mutable_result_layout() = ShapeLayout(
      ShapeUtil::MakeShapeWithDenseLayout(S32, {1, 2, 3, 4}, {2, 1, 0, 3}));
  AssignLayouts(m.get(), &computation_layout);
...
TEST_F(LayoutAssignmentTest,
       DISABLED_CustomCallLayoutConstrainedAndElementwise) {
  const char* module_str = R"(
HloModule CustomCallLayoutConstrained
...
ENTRY %CustomCallWithLayoutConstraints (p0: s32[4,4], p1: s32[2,3]) -> s32[1,2,3,4] {
  p0 = s32[4,4] parameter(0)
  p1 = s32[2,3] parameter(1)
  cc = s32[1,2,3,4]{3,2,0,1} custom-call(s32[4,4] %p0, s32[2,3] %p1), custom_call_target="baz", operand_layout_constraints={s32[4,4]{0,1}, s32[2,3]{1,0}}
  ROOT e = s32[1,2,3,4] negate(cc)
}
)";
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> m,
  ...
  *computation_layout.mutable_result_layout() = ShapeLayout(
  ...
  ASSERT_THAT(
      ...
      GmockMatch(m::Copy(m::Negate(m::CustomCall(m::Copy(), m::Parameter())))));
...
// TODO(batzor): Enable this test. Dependency: CustomCall
TEST_F(LayoutAssignmentTest,
       DISABLED_CustomCallLayoutConstrainedAliasedOutput) {
  const char* module_str = R"(
HloModule customcall.4

ENTRY %customcall.4 (parameter.1: s32[8,128], parameter.2: s32[8,128]) -> s32[8,128] {
  %parameter.1 = s32[8,128]{1,0} parameter(0)
  %parameter.2 = s32[8,128]{1,0} parameter(1)
  ROOT %custom-call.3 = s32[8,128]{1,0} custom-call(s32[8,128]{1,0} %parameter.1, s32[8,128]{1,0} %parameter.2), custom_call_target="gpu_example_custom_call", operand_layout_constraints={s32[8,128]{1,0}, s32[8,128]{1,0}}, custom_call_has_side_effect=true, output_to_operand_aliasing={{}: (0, {})}
})";
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> m,
      ParseAndReturnVerifiedModule(module_str, GetModuleConfigForTest()));
  ComputationLayout computation_layout = m->entry_computation_layout();
  *computation_layout.mutable_parameter_layout(0) =
      ShapeLayout(ShapeUtil::MakeShapeWithDenseLayout(S32, {8, 128}, {1, 0}));
  *computation_layout.mutable_parameter_layout(1) =
      ShapeLayout(ShapeUtil::MakeShapeWithDenseLayout(S32, {8, 128}, {1, 0}));
  *computation_layout.mutable_result_layout() =
      ShapeLayout(ShapeUtil::MakeShapeWithDenseLayout(S32, {8, 128}, {1, 0}));
...
// TODO(batzor): Enable this test. Dependency: CustomCall
TEST_F(LayoutAssignmentTest, DISABLED_CustomCallLayoutConstrainedZeroOperands) {
  const char* module_str = R"(
HloModule CustomCallLayoutConstrainedZeroOperands

ENTRY %CustomCallLayoutConstrainedZeroOperands () -> s32[1,2,3,4] {
  ROOT %custom-call = s32[1,2,3,4]{3,2,0,1} custom-call(), custom_call_target="baz", operand_layout_constraints={}
}
)";
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> m,
...
// TODO(batzor): Enable this test. Dependency: CustomCall
TEST_F(LayoutAssignmentTest, DISABLED_CustomCallLayoutConstrainedTupleOperand) {
  const char* module_str = R"(
HloModule CustomCallLayoutConstrainedTupleOperand

ENTRY %CustomCallLayoutConstrainedTupleOperand (p0: s32[4,4], p1: s32[2,3]) -> s32[1,2,3,4] {
  %p0 = s32[4,4] parameter(0)
  %p1 = s32[2,3] parameter(1)
  %tuple = (s32[4,4], s32[2,3]) tuple(%p0, %p1)
  ROOT %custom-call = s32[1,2,3,4]{3,2,0,1} custom-call(%tuple), custom_call_target="baz", operand_layout_constraints={(s32[4,4]{1,0}, s32[2,3]{0,1})}
}
)";
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> m,
...
// TODO(batzor): Enable this test. Dependency: CustomCall
TEST_F(LayoutAssignmentTest, DISABLED_CustomCallLayoutConstrainedTupleResult) {
  const char* module_str = R"(
HloModule CustomCallLayoutConstrainedTupleResult

ENTRY %CustomCallLayoutConstrainedTupleResult (p0: s32[4,4]) -> (s32[4,4]{1,0}, s32[2,3]{0,1}) {
  %p0 = s32[4,4] parameter(0)
  ROOT %custom-call = (s32[4,4]{1,0}, s32[2,3]{0,1}) custom-call(%p0), custom_call_target="baz", operand_layout_constraints={s32[4,4]{1,0}}
}
)";
  // Try with a couple different layouts. In each case the custom calls operand
  // and result layout should match that of the computation.
  ...
  *computation_layout.mutable_result_layout() =
      ShapeLayout(ShapeUtil::MakeTupleShape(
          {ShapeUtil::MakeShapeWithDenseLayout(S32, {4, 4}, {1, 0}),
...
// TODO(batzor): Enable this test. Dependency: CustomCall
TEST_F(LayoutAssignmentTest, DISABLED_MemorySpaceRemoved) {
  const char* module_str = R"(
HloModule MixedHostDeviceResult

ENTRY %MixedHostDeviceResult {
  %p0 = s32[4,4] parameter(0)
  %d = s32[4,4]{1,0} custom-call(%p0), custom_call_target="MoveToDevice", metadata={}
  ROOT %tuple = (s32[4,4], s32[4,4]) tuple(%p0, %d)
}
)";
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<VerifiedHloModule> m,
...
TEST_F(LayoutAssignmentTest, OverwriteDiamondShapedConstraintsX) {
  ...
  Shape ashape = ShapeUtil::MakeShape(S32, {12, 8});
  Shape bshape = ShapeUtil::MakeShape(S32, {8, 12});
  auto param0 =
  ...
  Shape ashape_major =
      ShapeUtil::MakeShapeWithDenseLayout(S32, {12, 8}, {1, 0});
  Shape ashape_minor =
      ShapeUtil::MakeShapeWithDenseLayout(S32, {12, 8}, {0, 1});
...
ENTRY entry_computation {
  param = (s32[8,4]{0,1}, s32[16,2]{0,1}) parameter(0)
  gte0 = s32[8,4] get-tuple-element(param), index=0
  gte1 = s32[16,2] get-tuple-element(param), index=1
  crs = (s32[8,4]{0,1}, s32[16,2]{1,0}) all-reduce(gte0, gte1),
    replica_groups={}, constrain_layout=true, to_apply=add
  gte2 = s32[8,4] get-tuple-element(crs), index=0
  gte3 = s32[16,2] get-tuple-element(crs), index=1
  ROOT result = (s32[8,4]{1,0}, s32[16,2]{1,0}) tuple(gte2, gte3)
}
)";
...
ENTRY entry_computation {
  param = (s32[16,4]{0,1}, s32[16,4]{1,0}) parameter(0)
  gte0 = s32[16,4] get-tuple-element(param), index=0
  gte1 = s32[16,4] get-tuple-element(param), index=1
  alltoall = (s32[16,4]{1,0}, s32[16,4]{1,0}) all-reduce(gte0, gte1),
    replica_groups={{0,1}}, constrain_layout=true, to_apply=add
  gte2 = s32[16,4] get-tuple-element(alltoall), index=0
  gte3 = s32[16,4] get-tuple-element(alltoall), index=1
  ROOT concat = s32[16,8]{0,1} concatenate(gte2, gte3), dimensions={1}
}
)";
...
ENTRY entry_computation {
  param = s32[1,<=16]{0,1} parameter(0)
  ROOT abs = s32[1,<=16]{0,1} abs(param)
}
)";
...
call_1 {
  %arg_tuple.1 = (s32[93184,4]) parameter(0)
  %get-tuple-element.1 = s32[93184,4] get-tuple-element(%arg_tuple.1), index=0
  ROOT %reshape.8494 = s32[2,512,364]{2,1,0} reshape(s32[93184,4]{1,0}%get-tuple-element.1)
}

on_true {
  %arg_tuple.1 = (s32[93184,4]) parameter(0)
  %get-tuple-element.1 = s32[93184,4] get-tuple-element(%arg_tuple.1), index=0
  ROOT %reshape.8493 = s32[2,512,364]{2,1,0} reshape(s32[93184,4]{1,0}%get-tuple-element.1)
}

on_false {
  %arg_tuple.2 = (s32[93184,4]) parameter(0)
  %get-tuple-element.3 = s32[93184,4] get-tuple-element(%arg_tuple.2), index=0
  %reshape.9717 = s32[2,512,364]{2,1,0} reshape(s32[93184,4]{1,0}%get-tuple-element.3)
  ROOT %add = s32[2,512,364] add(%reshape.9717, %reshape.9717)
}

ENTRY main {
  pred.1 = pred[] parameter(0)
  arg.2 = s32[93184,4]{1,0} parameter(1)
  arg_tuple.11 = (s32[93184,4]{1,0}) tuple(arg.2)
  call.1 = s32[2,512,364] call(arg_tuple.11), to_apply=call_1
  conditional = s32[2,512,364] conditional(pred.1, arg_tuple.11, arg_tuple.11),
                     true_computation=on_true, false_computation=on_false
  ROOT add = s32[2,512,364] add(call.1, conditional)
}
  )";
  ...
  *computation_layout.mutable_parameter_layout(1) =
      ShapeLayout(ShapeUtil::MakeShapeWithDenseLayout(S32, {93184, 4}, {0, 1}));
  *computation_layout.mutable_result_layout() = ShapeLayout(
      ShapeUtil::MakeShapeWithDenseLayout(S32, {2, 512, 364}, {0, 1, 2}));
  ChannelLayoutConstraints channel_constraints;
  LayoutAssignment layout_assignment(
      &computation_layout,
      /*channel_constraints=*/&channel_constraints,
...
// TODO(batzor): Enable this test. Dependency: Gather
// Test the ability to propagate operand constraints across multiple operations.
TEST_F(LayoutAssignmentTest, DISABLED_PropagateOperandLayout2) {
  const char* module_str = R"(
 HloModule TensorFlowGather, entry_computation_layout={(s32[32,650]{1,0},s32[16,1,18]{0,1,2})->s32[16,1,18,32]{3,1,2,0}}

 ENTRY %main (operand: s32[32,650], indices: s32[16,1,18]) -> s32[16,1,18,32] {
   %operand = s32[32,650]{1,0} parameter(0)
   %transpose = s32[650,32]{0,1} transpose(s32[32,650]{1,0} %operand), dimensions={1,0}
   %indices = s32[16,1,18]{0,1,2} parameter(1)
   %reshape.1 = s32[288,1]{1,0} reshape(s32[16,1,18]{0,1,2} %indices)
   %gather.1 = s32[288,1,32]{2,1,0} gather(s32[650,32]{0,1} %transpose, s32[288,1]{1,0} %reshape.1), offset_dims={1,2}, collapsed_slice_dims={}, start_index_map={0}, index_vector_dim=1, slice_sizes={1,32}
   ROOT %reshape.3 = s32[16,1,18,32]{3,2,1,0} reshape(s32[288,1,32]{2,1,0} %gather.1)
 } )";
...
TEST_F(LayoutAssignmentTest, DISABLED_BreakBufferAliasAcrossInstructions) {
  const char* module_str = R"(
 HloModule break_alias_test, entry_computation_layout={(s32[256,8]{0,1})->s32[256,8]{1,0}}

called_computation {
  init = s32[256,8]{1,0:T(8)} parameter(0)
  one = s32[] constant(1)
  ones = s32[256,8] broadcast(one), dimensions={}
  ROOT add = s32[256,8] add(init, ones)
}

ENTRY main {
  init = s32[256,8] parameter(0)
  ROOT start = s32[256,8]{1,0} custom-call(init), custom_call_target="baz", to_apply=called_computation, custom_call_has_side_effect=true, output_to_operand_aliasing={{}: (0, {})}, metadata={}
}
)";
...
TEST_F(LayoutAssignmentTest, PartialEntryParameterLayout) {
  const char* module_str = R"(
 HloModule EntryLayout, entry_computation_layout={(s32[32,650]{1,0},s32[16,1,18]{0,1,2})->(s32[650,32]{1,0},s32[18,16,1]{0,1,2})}

 ENTRY %main {
   operand = s32[32,650] parameter(0)
...
TEST_F(LayoutAssignmentTest, TupleEntryParameterLayoutNoResultConstraint) {
  const char* module_str = R"(
 HloModule EntryLayout, entry_computation_layout={((s32[32,650]{0,1},s32[16,1,18]{0,1,2}))->(s32[208,100]{1,0},s32[2,144]{1,0})}

 ENTRY %main {
   p = (s32[32,650],s32[16,1,18]) parameter(0)
   operand = s32[32,650] get-tuple-element(p), index=0
   reshape = s32[208,100] reshape(operand)
...
TEST_F(LayoutAssignmentTest, AliasParameterAndOutput) {
 ...
 ENTRY %main {
   p0 = s32[65,65] parameter(0)
   p1 = s32[4225] parameter(1)
   r = s32[65,65] reshape(p1)
   a = add(p0,r)
...
condition  {
    p = (s32[100,100], (s32[100,100], u32[], token[])) parameter(0)
...
body {
    ...
    t1 = s32[100,100] get-tuple-element(p), index=0
    t = (s32[100,100], u32[], token[]) get-tuple-element(p), index=1
    sdone = token[] send-done(t), channel_id=1,
      frontend_attributes={
       _zkx_send_recv_pipeline="0"
      }
    tk = token[] after-all()
    snd = (s32[100,100], u32[], token[]) send(t1, tk), channel_id=1,
      frontend_attributes={
        _zkx_send_recv_pipeline="0"
      }
    a = add(t1, t1)
...
ENTRY %main {
    p0 = s32[100,100] parameter(0)
    tk = token[] after-all()
    snd = (s32[100,100], u32[], token[]) send(p0, tk), channel_id=1,
      frontend_attributes={
        _zkx_send_recv_pipeline="0"
      }
    t = tuple(p0, snd)
    loop = while(t), condition=condition, body=body
    ssend = (s32[100,100], u32[], token[]) get-tuple-element(loop), index=1
    sdone = token[] send-done(ssend), channel_id=1,
      frontend_attributes={
       _zkx_send_recv_pipeline="0"
      }
    ROOT result = s32[100,100] get-tuple-element(loop), index=0
})";
...
while_condition {
  tuple = (s32[5,16], u32[]) parameter(0)
...
while_body {
  tuple = (s32[5,16], u32[]) parameter(0)
  input = s32[5,16] get-tuple-element(tuple), index=0
  i = u32[] get-tuple-element(tuple), index=1
  c1 = u32[] constant(1)
  i_ = add(i, c1)
  output = add(input, input)
  ROOT tuple1 = (s32[5,16], u32[]) tuple(output, i_)
}

ENTRY main {
  input = s32[5,16] parameter(0)
  c0 = u32[] constant(0)
  tuple = (s32[5,16], u32[]) tuple(input, c0)
  tuple_ = (s32[5,16], u32[]) while(tuple), condition=while_condition, body=while_body
  ROOT output_ = s32[5,16] get-tuple-element(tuple_), index=0
})";
