...
std::string_view AsyncTracker::GetResourceName(int64_t resource_type) const {
...
std::string_view AsyncTracker::GetResourceUsageName(
    ResourceUsageType resource_usage_type) const {
  switch (resource_usage_type) {
    case ResourceUsageType::kNoResource:
      return "kNoResource";
    case ResourceUsageType::kResourceOccupy:
      return "kResourceOccupy";
    case ResourceUsageType::kResourceRelease:
...
std::string_view AsyncTracker::GetResourceUsageName(
  ...
  return GetResourceUsageName(
      static_cast<ResourceUsageType>(resource_usage_type));
}
...
void MemoryPressureTracker::Initialize(
  ...
  for (HloInstruction* instruction : computation->instructions()) {
    auto& output_values = output_buffers_[instruction];
    auto& defined_values = defined_buffers_[instruction];
    ShapeUtil::ForEachSubshape(
        instruction->shape(),
        [&](const Shape& subshape, const ShapeIndex& index) {
          for (const HloBuffer* buffer :
  ...
  if (!initial_live_buffers.empty()) {
    for (HloBuffer::Id id : initial_live_buffers) {
      const BufferInfoTracker::ValueInfo& buffer =
          buffer_tracker_.GetBufferInfo(id);
      if (buffer.value->values()[0]->shape().has_layout() &&
          buffer.value->values()[0]->shape().layout().memory_space() !=
              kDefaultMemorySpace) {
        continue;
      }
...
void MemoryPressureTracker::UpdateBuffers(const HloInstruction* instruction) {
  ...
  for (const HloComputation* called_comp : instruction->called_computations()) {
...
class ReadySetLt {
 public:
  ...
  DefaultSchedulerCore::CandidateResult operator()(
    ...
    // Schedule according to ForceDelay.
    if (auto value = DefaultSchedulerCore::ChooseBestCandidate(
    ...
    if (sched_state_.config.enable_release_start_policy) {
      // Prioritize scheduling ready "start" ops, to avoid useless extension of
 ...
 private:
  ...
  bool ShouldDelaySendHostDone(
    ...
    if (!gn_cand.estimated_connected_send_ready_time.has_value()) {
      ...
      for (const HloEdge& succ : start.GetSuccessors()) {
...
std::string_view SkipNodeReasonString(SkipNodeReason reason) {
  switch (reason) {
...
absl::StatusOr<HloGraphNode::TimeCost> DefaultSchedulerCore::ScheduleNode(
  ...
  for (const HloEdge& succ : n->GetSuccessors()) {
    const HloGraphNode::TimeCost time_from_edge =
        succ.Target().GetReadyTime() + succ.Latency();
    ...
    if (sched_state->config.resource_sharing) {
      ...
      std::vector<int64_t> occupied_resources =
          n->GetShareableResourcesOnEdge(succ);
  ...
  if (sched_state->config.resource_sharing) {
    ...
    for (HloEdge& succ : n->GetSuccessors()) {
      std::vector<int64_t> released_resources =
          n->GetShareableResourcesOnEdge(succ);
    ...
    for (HloEdge& pred : n->GetPredecessors()) {
      for (HloEdge& inverse_edge : pred.Target().GetSuccessors()) {
        if (&(inverse_edge.Target()) == n) {
          std::vector<int64_t> occupied_resources =
              pred.Target().GetShareableResourcesOnEdge(inverse_edge);
          for (const int64_t resource : occupied_resources) {
  ...
  for (HloEdge& pred : n->GetPredecessors()) {
    const int64_t current_outdegree = pred.Target().GetOutdegree();
    // Node is not ready yet. Decrease the outdegree and continue.
    if (current_outdegree != 1) {
      pred.Target().SetOutdegree(current_outdegree - 1);
      continue;
    }
    ...
    pred.Target().SetOutdegree(0);
    LatencyEstimator::TimeCost ready_time = current_time;
    for (const HloEdge& succ : pred.Target().GetSuccessors()) {
      ...
      if (sched_state->config.resource_sharing) {
        // Adjust the ready time if this edge uses shareable resources
        std::vector<int64_t> occupied_resources =
            pred.Target().GetShareableResourcesOnEdge(succ);
        for (const int64_t resource : occupied_resources) {
          auto occupiers = sched_state->shareable_resource_occupiers[resource];
          for (auto [occupier_edge, edge_pft] : occupiers) {
            if (occupier_edge == &succ) {
              VLOG(3) << "Ready time of predecessor "
                      << pred.Target().GetInstr().name()
                      << " before update with pft: " << edge_pft
    ...
    for (auto& resource : pred.Target().GetResources()) {
      if (resource.second == ResourceUsageType::kResourceOccupy) {
        ++(sched_state->resource_users_in_queue[resource.first]);
      }
    }
    pred.Target().SetReadyTime(ready_time);
    int64_t annotation = pred.Target().GetAnnotation();
    // We are adding the no-op instructions to a separate set so that we can
    // immediately schedule them when they are ready.
    if (IsNopInstruction(pred.Target().GetInstr()) && annotation == -1) {
      sched_state->nop_set.push_back(&pred.Target());
      continue;
    }
    ...
    if (annotation != -1) {
      ...
      if (annotation == sched_state->ongoing_annotation) {
        sched_state->annotation_ready.push_back(&pred.Target());
      }
    }
    if (pred.Target().GetReadyTime() > current_time) {
      sched_state->next_ready_stack.push_back(&pred.Target());
    ...
    if (sched_state->config.enable_selective_resources &&
        pred.Target().ReleasesSelectiveResource()) {
      sched_state->selective_resource_releasers.push_back(&pred.Target());
    }
  }
...
bool HloScheduleGraph::IsPredecessorTransitively(
    const HloGraphNode* node, const HloGraphNode* possible_predecessor) const {
  ...
  while (!to_visit_queue.empty()) {
    ...
    visited.insert(curr);
    for (const HloEdge& pred : curr->GetPredecessors()) {
      auto user_node_it = nodes_.find(&pred.Target().GetInstr());
...
HloScheduleGraph::HloScheduleGraph(
  ...
  for (const HloInstruction* instr : *post_order_instructions) {
    ...
    if (instr->opcode() == HloOpcode::kSendDone) {
      for (const HloInstruction* ctrl_pred : instr->control_predecessors()) {
        if (ctrl_pred->opcode() != HloOpcode::kRecvDone) {
          continue;
        }
        const HloInstruction* dependent_while_instr = nullptr;
        for (const HloInstruction* while_hlo : while_instrs) {
  ...
  while (!stack.empty()) {
    ...
    if (async_tracker->OccupiesSelectiveResource(node)) {
      ...
      int64_t closest_predecessor_distance =
          std::numeric_limits<int64_t>::max();
      for (const HloEdge& pred : node->GetPredecessors()) {
    ...
    for (HloEdge& succ : node->GetSuccessors()) {
...
absl::Status DefaultSchedulerCore::SchedulingStep(
  ...
  ZKX_VLOG_LINES(5, node->ToString());
  return absl::OkStatus();
}
...
DefaultSchedulerCore::ScheduleComputation(const HloComputation* computation) {
  ...
  ZKX_VLOG_LINES(5, sched_state.sched_graph.ToString(async_tracker_));
  ...
  std::vector<HloGraphNode*> roots = sched_state.sched_graph.FindBottomRoots();
  ...
  if (debug_options.zkx_dump_latency_hiding_schedule() &&
...
LatencyHidingScheduler::LatencyHidingStatistics(
  ...
  auto find_node_successor_edge =
      [](const HloGraphNode& graph_node,
         const HloGraphNode& successor_node) -> const HloEdge* {
  ...
  for (const HloInstruction* instr :
    ...
    if (async_tracker->IsSupportedAsyncStart(*instr)) {
      ...
      if (async_tracker->IsSupportedAsyncStart(*start_instr)) {
        ...
        const HloEdge* edge = find_node_successor_edge(start_node, instr_node);
        const double async_wasted_cycles =
            std::max(0.0, edge->Latency() - (current_time - std::get<1>(*it)));
...
void LatencyHidingScheduler::LogScheduleStatistics(
    const HloComputation* computation) {
  ZKX_VLOG_LINES(1, SchedulerStatisticsString(LatencyHidingStatistics(
...
absl::StatusOr<bool> LatencyHidingScheduler::Run(
  ...
  ZKX_VLOG_LINES(5, module->ToString());
