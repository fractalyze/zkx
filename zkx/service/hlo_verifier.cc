...
absl::Status ShapeVerifier::HandleDot(HloInstruction* dot) {
  ...
  for (int i = 0; i < sparsity.size(); ++i) {
    ...
    // clang-format off
    // TODO(chokobole): Uncomment this. Dependency: ShapeInference::InferSparseDotMetadataShape
    // clang-format on
...
absl::Status ShapeVerifier::HandleRaggedDot(HloInstruction* ragged_dot) {
  ...
  return absl::UnimplementedError("HandleRaggedDot not supported");
}
...
absl::Status ShapeVerifier::HandleRaggedAllToAll(HloInstruction* hlo) {
  ...
  for (int64_t i = kOffsetsSizesOperandsStart + 1; i < kNumRaggedOperands;
    ...
    if (operand_shapes[i - 1]->rank() != 1 &&
      ...
      return absl::InternalError(
          absl::StrFormat("RaggedAllToAll operand %d must be rank 1 or 2: %s",
                          i - 1, hlo->ToString()));
    }
...
absl::Status CheckBufferOffset(const Shape& buffer_shape,
  ...
  if (!buffer_offset_shape.IsTuple()) {
    return absl::InternalError("Buffer offset is not tuple.");
  }
  bool all_is_array =
  ...
  if (!all_is_array && !all_is_tuple) {
    return absl::InternalError(
        "Buffer offset should either be a tuple of arrays or  a tuple of "
        "tuples.");
  }
  ...
  if (all_is_tuple) {
    if (absl::c_any_of(buffer_offset_shape.tuple_shapes(),
      ...
      return absl::InternalError(
          "Buffer offset index should have the same number of elements as the "
          "buffer's rank.");
    }
  } else {
...
absl::Status CheckInplaceCollectivePermute(HloInstruction* collective_permute) {
  ...
  if (collective_permute->operand_count() != 4) {
    return absl::InternalError(
        absl::StrFormat("Unexpected number of operands: %d.",
  ...
  if (input_buffer_shape.IsArray() && output_buffer_shape.IsArray()) {
    ...
    if (ShapeUtil::TupleElementCount(input_buffer_shape) !=
      ...
      return absl::InternalError(
          "Unmatching input buffers and output buffers.");
    }
    if (!input_offset_shape.IsTuple() ||
      ...
      return absl::InternalError("Unmatching input buffers and input offset.");
    }
    ...
    if (!output_offset_shape.IsTuple() ||
        ShapeUtil::TupleElementCount(output_offset_shape) !=
            ShapeUtil::TupleElementCount(output_buffer_shape)) {
      return absl::InternalError(
          "Unmatching output buffers and output offset.");
    }
    ...
    return absl::InternalError("Unmatching input buffers and output buffers.");
  }
  return absl::OkStatus();
}
...
absl::Status CheckAsyncOpComputationThreadName(const HloInstruction* async_op) {
  std::string_view async_execution_thread = async_op->async_execution_thread();
...
absl::Status VerifyChannels(const HloModule& module,
  ...
  for (auto& [channel_id, instructions] : channel_instructions) {
    ...
    if (HloSendRecvInstruction::ClassOf(first)) {
