...
TEST_F(NameUniquerTest, AvoidKeywords) {
  ...
  EXPECT_EQ("u32_", uniquer.GetUniqueName("u32"));
  EXPECT_EQ("s64_", uniquer.GetUniqueName("s64"));
  EXPECT_EQ("pred_", uniquer.GetUniqueName("pred"));

  // Name prefix __zkx_ is preserved.
  EXPECT_NE(uniquer.GetUniqueName("__zkx_").find("__zkx_"), std::string::npos);
  ...
  EXPECT_EQ("U32", uniquer.GetUniqueName("U32"));
