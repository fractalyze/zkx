...
std::vector<PjRtBuffer*> BufferVecToPointerVec(
  ...
  for (const auto& buf : buffer) {
    argument_ptrs.push_back(buf.get());
  }
...
HloRunnerPjRt::CreateExecutable(HloModule* module,
                                CompileOptions compile_options) {
  ZkxComputation computation(module->ToProto());
...
absl::StatusOr<std::vector<Literal>> HloRunnerPjRt::ExecuteReplicatedImpl(
  ...
  if (has_infeed) {
    for (int64_t i = 0; i < options.num_replicas; ++i) {
      TF_RET_CHECK(options.infeed_values[i] != nullptr)
          << "Infeed literal for replica " << i << " is null.";
      pool->Schedule([device = replica_devices[i],
                      &infeed_literal = *options.infeed_values[i],
...
std::string_view HloRunnerPjRt::Name() const { return "HloRunnerPjRt"; }
