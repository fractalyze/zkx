...
HloComputation* MakeBinaryScalarComputation(HloOpcode binary_opcode,
                                            PrimitiveType dtype,
                                            HloInstruction* ctx,
                                            HloModule* module) {
...
absl::StatusOr<HloComputation*> ZkxComputationToHloComputation(
    ZkxComputation& src_comp, HloModule* dest_module) {
...
absl::StatusOr<std::unique_ptr<HloComputation>> CreateComputationWithSignature(
  ...
  for (const Shape* param_shape : domain) {
    b.AddInstruction(HloInstruction::CreateParameter(
        param_idx, *param_shape, absl::StrCat("param.", param_idx)));
    param_idx++;
  }
...
HloInstruction* ExpandDegenerateReshape(HloInstruction* inst) {
  ...
  if (reshape_degenerate.has_value()) {
    ...
    HloInstruction* current_hlo = inst->mutable_operand(0);
    for (const int64_t dim_to_delete : reshape_degenerate->deleted_dimensions) {
      current_hlo = CreateDegenerateRemovingReshape(current_hlo, dim_to_delete);
    }

    absl::c_reverse(reshape_degenerate->inserted_dimensions);
    for (const int64_t dim_to_add : reshape_degenerate->inserted_dimensions) {
      current_hlo = CreateDegenerateAddingReshape(current_hlo, dim_to_add);
    }
    return current_hlo;
  }
  return nullptr;
