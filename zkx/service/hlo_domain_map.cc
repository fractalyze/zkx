...
// static
absl::StatusOr<std::unique_ptr<HloDomainMap>> HloDomainMap::Create(
...
absl::Status HloDomainMap::PopulateDomainMetadataMap() {
  // TODO(jeong0982): Implement full domain metadata mapping once
  // HloDomainInstruction is ported. Requires
  // HloInstruction::user_side_metadata() and
  // HloInstruction::operand_side_metadata().
  //
  // For now, assign metadata id 0 to all instructions in domains without
  // enter/exit domains. Domains with enter/exit domains would require
  // HloDomainInstruction which is not yet implemented.
  for (auto& domain : instruction_domains_) {
    int64_t domain_metadata_id = 0;
    if (!domain->enter_domains.empty() || !domain->exit_domains.empty()) {
      return absl::UnimplementedError(
          "Domain metadata mapping for domains with enter/exit "
          "boundaries requires HloDomainInstruction which is not yet "
          "implemented");
    }
    ...
    if (domain->reach_set.insert(current_instruction).second) {
      ...
      for (HloInstruction* operand : current_instruction->operands()) {
        if (IsDomainInstruction(operand)) {
          ...
          // IOW the dataflow enters the domain through the kDomain
          // instruction.
          domain->enter_domains.insert(operand);
        } else {
...
bool HloDomainMap::IsDomainInstruction(
  ...
  if (!domain_kind_.empty()) {
    // TODO(jeong0982): Enable metadata kind check once HloDomainInstruction is
    // ported. Requires HloInstruction::user_side_metadata() and
    // HloInstruction::operand_side_metadata().
    LOG(FATAL) << "Domain kind filtering requires HloDomainInstruction which "
...
// static
std::vector<HloInstruction*> HloDomainMap::MakeNonDomainInstructions(
