...
#ifndef ZKX_SERVICE_HLO_CREATION_UTILS_H_
...
// in the same computation. `pred` is broadcasted up from a scalar if necessary.
  ...
  return primitive_util::PrimitiveTypeSwitch<std::unique_ptr<HloInstruction>>(
      [&](auto literal_constant) -> std::unique_ptr<HloInstruction> {
        if constexpr (primitive_util::IsIntegralType(literal_constant) ||
                      primitive_util::IsFieldType(literal_constant) ||
                      primitive_util::IsEcPointType(literal_constant)) {
        ...
        LOG(FATAL) << "Provided shape is not a int, field, or ec point type.";
      },
      shape.element_type());
}
...
#endif  // ZKX_SERVICE_HLO_CREATION_UTILS_H_
