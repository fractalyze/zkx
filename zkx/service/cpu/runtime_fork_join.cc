...
//
ABSL_ATTRIBUTE_NO_SANITIZE_MEMORY void __zkx_cpu_runtime_ParallelForkJoin(
  ...
  auto* run_options =
      static_cast<const zkx::ExecutableRunOptions*>(run_options_ptr);
  ...
  std::vector<ZkxCustomCallStatus> statuses(num_partitions);
  ...
  std::vector<std::pair<int32_t, std::string_view>> error_messages;
  for (int32_t i = 0; i < num_partitions; ++i) {
    std::optional<std::string_view> msg =
        zkx::CustomCallStatusGetMessage(&statuses[i]);
    if (msg) {
  ...
  if (!error_messages.empty()) {
    ...
    std::string error_message = absl::StrJoin(
        error_messages, "\n",
        [](std::string* out, std::pair<int32_t, std::string_view> p) {
          int32_t idx = p.first;
          std::string_view msg = p.second;
          absl::StrAppend(out,
                          absl::StrFormat("Partition %d error: %s", idx, msg));
        });
    ZkxCustomCallStatusSetFailure(
        reinterpret_cast<ZkxCustomCallStatus*>(status), error_message.data(),
