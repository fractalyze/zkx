...
bool IsNonBatchedMatrixVectorDot(const HloInstruction* hlo) {
  const Shape& hlo_shape = hlo->shape();
  return hlo->opcode() == HloOpcode::kDot && hlo_shape.dimensions_size() <= 1 &&
...
bool CanBeOutputFused(const HloInstruction* producer,
  ...
  return consumer->opcode() == HloOpcode::kAdd &&
         IsNonBatchedMatrixVectorDot(producer) && HasExactlyOneUse(*producer);
}
