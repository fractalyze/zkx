...
absl::StatusOr<MaybeOwningDeviceMemory> MemoryForAllocation(
...
absl::Status CpuExecutable::ExecuteComputeFunction(
  ...
  if (error_message) {
    return absl::InternalError(
        absl::StrFormat("CustomCall failed: %s", *error_message));
  }
...
absl::Status CpuExecutable::ExecuteThunks(
  ...
  VLOG(3) << "Executing ZKX:CPU thunks:";
  ...
  // Prepare for executing ZKX program collectively.
  ...
  // Prepare for executing ZKX custom calls.
  // clang-format off
  // TODO(chokobole): Uncomment this. Dependency: CustomCallExecuteParams
  // clang-format on
  // TF_ASSIGN_OR_RETURN(Thunk::CustomCallExecuteParams
    ...
    if (alias) {
      ...
      if (alias->must_alias() && !maybe_owning_memory->HasOwnership()) {
        return absl::InvalidArgumentError(
            absl::StrFormat("An input was configured to be must-alias at "
  ...
  struct AsyncRunTask {
    ...
    absl::Status operator()() {
      if (executable->has_compute_function()) {
        ...
        return absl::InternalError("No compute function or thunks found.");
      }
    }
  };
...
// static
int64_t CpuExecutable::ShapeSizeBytes(const Shape& shape) {
