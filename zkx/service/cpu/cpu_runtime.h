...
// zkx::cpu::SimpleResolver. It also defines a per-CpuExecutable context
...
#ifndef ZKX_SERVICE_CPU_CPU_RUNTIME_H_
#define ZKX_SERVICE_CPU_CPU_RUNTIME_H_
...
// 1. When using the JIT, the symbol resolver (zkx::cpu::RuntimeSymbolGenerator)
...
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemrefKoalabear);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemrefBabybear);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemrefMersenne31);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemrefGoldilocks);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemrefBn254Sf);
...
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemref1DKoalabear);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemref1DBabybear);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemref1DMersenne31);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemref1DGoldilocks);
DECLARE_MLIR_C_INTERFACE_FUNCTION(PrintMemref1DBn254Sf);
...
extern int __zkx_cpu_runtime_PrintfToStderr(const char* format, ...);

extern int64_t __zkx_cpu_runtime_TracingStart(
    const void* /* zkx::ExecutableRunOptions* */ run_options_ptr,
    const char* name, const char* hlo_module, int64_t program_id);
extern void __zkx_cpu_runtime_TracingEnd(
    const void* /* zkx::ExecutableRunOptions* */ run_options_ptr, int64_t id);
...
extern void* __zkx_cpu_runtime_AcquireInfeedBufferForDequeue(
...
// __zkx_cpu_runtime_AcquireInfeedBufferForDequeue. Once this call
// completes the data at buffer_ptr may no longer be
// accessed. buffer_length must match the length passed to the call to
// __zkx_cpu_runtime_AcquireInfeedBufferForDequeue that returned
...
extern void __zkx_cpu_runtime_ReleaseInfeedBufferAfterDequeue(
    const zkx::ExecutableRunOptions* run_options, int32_t buffer_length,
...
// returns it.
extern void* __zkx_cpu_runtime_AcquireOutfeedBufferForPopulation(
    const zkx::ExecutableRunOptions* run_options, int32_t buffer_length,
...
// use by the runtime.
extern void __zkx_cpu_runtime_ReleaseOutfeedBufferAfterPopulation(
    const zkx::ExecutableRunOptions* run_options, int32_t buffer_length,
    void* buffer_ptr, const void* shape_ptr, int32_t shape_length);
...
extern void __zkx_cpu_runtime_AllReduce(
...
extern void __zkx_cpu_runtime_CollectivePermute(
...
extern void __zkx_cpu_runtime_AllToAll(
    const zkx::ExecutableRunOptions* run_options, int32_t channel_id_present,
...
extern void __zkx_cpu_runtime_AllGather(
    const zkx::ExecutableRunOptions* run_options, int32_t channel_id_present,
...
extern void __zkx_cpu_runtime_PartitionId(
    const zkx::ExecutableRunOptions* run_options, void* output_buffer);
// Write the replica ID into the output buffer.
extern void __zkx_cpu_runtime_ReplicaId(
    const zkx::ExecutableRunOptions* run_options, void* output_buffer);

}  // extern "C"

#endif  // ZKX_SERVICE_CPU_CPU_RUNTIME_H_
