...
#ifndef ZKX_SERVICE_INSTRUCTION_FUSION_H_
#define ZKX_SERVICE_INSTRUCTION_FUSION_H_
...
class FusionDecision {
 public:
  ...
  static FusionDecision Forbid(std::string_view explanation) {
  ...
  // discarded.
  FusionDecision(bool condition, std::string_view explanation) {
    if (!condition) {
  ...
  FusionDecision operator<<(std::string_view explanation) const {
 ...
 private:
  ...
  explicit FusionDecision(std::string_view explanation)
...
#define RETURN_IF_NOT_FUSIBLE(...)                   \
  do {                                               \
    ::zkx::FusionDecision _decision = (__VA_ARGS__); \
    if (ABSL_PREDICT_FALSE(!_decision.CanFuse())) {  \
      return _decision;                              \
    }                                                \
  } while (0)
...
#endif  // ZKX_SERVICE_INSTRUCTION_FUSION_H_
