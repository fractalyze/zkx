...
std::unique_ptr<ComputationPlacer> CreateComputationPlacer() {
...
bool s_module_initialized = InitModule();
...
DeviceAssignment::LogicalIdForDevice(GlobalDeviceId device_id) const {
      ...
      if ((*this)(r, c) == device_id.value()) {
        if (logical_id.has_value()) {
          return absl::InternalError(
              absl::StrFormat("Device %d appears twice in DeviceAssignment: %s",
  ...
  if (logical_id.has_value()) {
    ...
    return absl::InternalError(
        absl::StrFormat("Device %d doesn't appear in DeviceAssignment: %s",
                        device_id.value(), ToString()));
  }
}
...
// static
absl::StatusOr<std::unique_ptr<DeviceAssignment>> DeviceAssignment::Deserialize(
  ...
  if (proto.replica_count() <= 0 || proto.computation_count() <= 0) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Invalid device assignment topology: replica_count=%d, "
                        "computation_count=%d",
...
std::string DeviceAssignment::ToString() const {
  std::string output = absl::StrCat("Computations: ", computation_count(),
                                    " Replicas: ", replica_count(), "\n");
  for (int computation = 0; computation < computation_count(); ++computation) {
    absl::StrAppend(&output, "Computation ", computation, ": ");
    for (int replica = 0; replica < replica_count(); ++replica) {
      absl::StrAppend(&output, operator()(replica, computation), " ");
    }
...
// static
absl::Mutex ComputationPlacer::platform_computation_placer_mutex_(
    absl::kConstInit);
