...
bool CompareHloValuesById(const HloValue* a, const HloValue* b) {
  return a->id() < b->id();
}

// Returns parameter instruction corresponding to the allocation or nullptr.
const HloInstruction* GetEntryParameterInstruction(
...
// nullptr.
const HloInstruction* GetOutputInstruction(const BufferAllocation& alloc) {
  for (const auto& p : alloc.assigned_buffers()) {
    const HloValue* value = p.first;
...
absl::Status GatherComputationsByAllocationType(
  ...
  absl::flat_hash_set<const HloComputation*> thread_local_set;
  absl::flat_hash_set<const HloComputation*> global_set;
        ...
        switch (instruction->opcode()) {
          ...
          default:
            return absl::InternalError(
                absl::StrFormat("Unexpected calling opcode: %s",
...
std::string BufferAllocation::ToShortString(bool human_readable_size) const {
  ...
  if (human_readable_size) {
    absl::StrAppendFormat(&output, "allocation %d: size %s", index_,
                          tsl::strings::HumanReadableNumBytes(size()));
  } else {
    absl::StrAppendFormat(&output, "allocation %d: size %d", index_, size());
  }
  if (color() != 0) {
    absl::StrAppend(&output, ", color ", color());
  }
  if (is_entry_computation_parameter()) {
    const HloInstruction* param = GetEntryParameterInstruction(*this);
    absl::StrAppend(&output, ", parameter ", parameter_number(), ", shape |",
  ...
  if (const HloInstruction* instr = GetOutputInstruction(*this)) {
    absl::StrAppend(&output, ", output shape is |",
                    instr->shape().ToString(/*print_layout=*/false), "|");
  }
  if (is_constant()) {
    absl::StrAppend(&output, ", constant");
  }
  if (is_thread_local()) {
    absl::StrAppend(&output, ", thread-local");
  }
  if (maybe_live_out()) {
    absl::StrAppend(&output, ", maybe-live-out");
  }
  if (IsPreallocatedTempBuffer()) {
    absl::StrAppend(&output, ", preallocated-temp");
  }
  ...
  for (const HloValue* buffer : sorted_buffers) {
    ...
    absl::StrAppend(
        ...
        absl::StrFormat(" value: %s (size=%d,offset=%d): %s\n",
                        buffer->ToShortString(), offset_size.size,
                        offset_size.offset,
        ...
        [&](const Shape& /*subshape*/,
          ...
          if (shape_slices.empty()) {
            return absl::InvalidArgumentError(
                "No slices assigned to part of instr.");
          }
...
void BufferAssignment::CombineTempAllocations(
  ...
  absl::flat_hash_map<BufferValue::Color, BufferAllocation*>
...
std::string BufferAssignment::StatsString(
  ...
  std::string s;
  absl::StrAppendFormat(&s, "BufferAssignment stats:\n");
  absl::StrAppendFormat(
      &s, "             parameter allocation: %10s\n",
      tsl::strings::HumanReadableNumBytes(stats_.parameter_allocation_bytes));
  absl::StrAppendFormat(
      &s, "              constant allocation: %10s\n",
      tsl::strings::HumanReadableNumBytes(stats_.constant_allocation_bytes));
  absl::StrAppendFormat(&s, "        maybe_live_out allocation: %10s\n",
                        tsl::strings::HumanReadableNumBytes(
                            stats_.maybe_live_out_allocation_bytes));
  absl::StrAppendFormat(&s, "     preallocated temp allocation: %10s\n",
                        tsl::strings::HumanReadableNumBytes(
  ...
  if (stats_.preallocated_temp_fragmentation_bytes >= 0) {
    ...
    absl::StrAppendFormat(&s,
                          "  preallocated temp fragmentation: %10s (%.2f%%)\n",
                          tsl::strings::HumanReadableNumBytes(
                              stats_.preallocated_temp_fragmentation_bytes),
                          percent);
  }
  absl::StrAppendFormat(
      &s, "                 total allocation: %10s\n",
      tsl::strings::HumanReadableNumBytes(stats_.total_allocation_bytes));
  if (report_total_fragmentation) {
    ...
    if (total_fragmentation_bytes.ok() && *total_fragmentation_bytes >= 0) {
      ...
      absl::StrAppendFormat(
          &s, "              total fragmentation: %10s (%.2f%%)\n",
          tsl::strings::HumanReadableNumBytes(*total_fragmentation_bytes),
          percent);
    }
  }
  return s;
...
std::string BufferAssignment::ToString() const {
  ...
  absl::StrAppend(&output, "\nTotal bytes used: ", total_size, " (",
                  tsl::strings::HumanReadableNumBytes(total_size), ")\n");
  absl::StrAppend(&output, "\nUsed values:\n");
...
std::string BufferAssignment::MemoryUsageReport(float percentile,
  ...
  for (auto& allocation : allocations) {
    ...
    absl::StrAppend(
        ...
        absl::StrFormat(
            "%10s(%3.0f%%); %10s; %s",
            tsl::strings::HumanReadableNumBytes(cumulative_size),
            100. * cumulative_size / total_size,
            tsl::strings::HumanReadableNumBytes(total_size - cumulative_size),
...
std::string BufferAllocation::MemoryUsageReport(const std::string& prefix,
  ...
  for (const auto& offset_info : sorted_offset_infos) {
    ...
    absl::StrAppendFormat(
        &output, "%s%9s(%3.0f%%); %10s; %12d; %16d; ", prefix,
        tsl::strings::HumanReadableNumBytes(cumulative_size),
        100. * cumulative_size / total_size,
        tsl::strings::HumanReadableNumBytes(offset_info.offset_size.size),
...
std::string BufferAssignment::ToVerboseString(
  ...
  for (size_t i = 0; i < std::min(max_buffers_to_show, peak_buffers.size());
    ...
    if (!instr->metadata().op_name().empty()) {
      buf_strs.push_back(absl::StrCat("\n\t\tOperator: ",
                                      OpMetadataToString(instr->metadata())));
    }
    if (instr->opcode() == HloOpcode::kParameter &&
        (instr->parent() == instr->GetModule()->entry_computation())) {
  ...
  for (const HloComputation* computation : computations) {
    ...
    if (has_sequential_order && buffers_to_assign_sequentially != nullptr) {
      ...
      buffers_to_assign_sequentially->emplace(
          computation, absl::flat_hash_set<const HloValue*>());
...
absl::flat_hash_map<LogicalBuffer::Color, absl::flat_hash_set<const HloValue*>>
...
absl::Status BufferAssigner::AssignBuffersWithSequentialOrdering(
  ...
  if (run_whole_module_heap_simulation) {
    ...
    HloSchedule schedule(&assignment->module());
    absl::flat_hash_set<const HloValue*> all_buffers_to_assign;
...
BufferAssigner::CreateAssignment(
  ...
  const int32_t multiheap_size_constraint_per_heap =
      module->config().debug_options().zkx_multiheap_size_constraint_per_heap();
