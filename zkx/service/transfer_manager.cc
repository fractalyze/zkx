...
// static
absl::Mutex TransferManager::platform_transfer_manager_mutex_(absl::kConstInit);
...
absl::StatusOr<ScopedShapedBuffer> TransferManager::AllocateScopedShapedBuffer(
  ...
  if (!LayoutUtil::HasLayout(on_host_shape)) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Shape must have a layout: %s",
...
Shape TransferManager::ChooseGoodInfeedLayout(const Shape& shape) const {
