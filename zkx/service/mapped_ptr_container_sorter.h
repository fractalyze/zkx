...
#ifndef ZKX_SERVICE_MAPPED_PTR_CONTAINER_SORTER_H_
#define ZKX_SERVICE_MAPPED_PTR_CONTAINER_SORTER_H_
  ...
  for (size_t index : result) {
    ...
    if (index >= unordered_container_size_) {
      return absl::InvalidArgumentError(
          absl::StrCat("invalid unordered_container index: ", index, " v size(",
                       unordered_container_size_, ")"));
    }
  }
...
MappedPtrContainerSorter<PointedToTy>::ComputeNewIndices(
  ...
  if (unordered_container.size() >= IndexBeforeMappedElements()) {
    return absl::InvalidArgumentError(
        "Unordered container is too large to sort.");
  }
...
#endif  // ZKX_SERVICE_MAPPED_PTR_CONTAINER_SORTER_H_
