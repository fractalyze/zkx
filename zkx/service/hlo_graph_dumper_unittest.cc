...
TEST_F(HloGraphDumperTest, NestedFusion) {
  ...
  auto shape = ShapeUtil::MakeShape(U32, {10, 100});
...
TEST_F(HloGraphDumperTest, TupleConstant) {
  Shape tuple_shape = ShapeUtil::MakeTupleShape(
      {ShapeUtil::MakeShape(U32, {3, 2}), ShapeUtil::MakeShape(S32, {4, 5})});
  HloComputation::Builder b("b");
  ...
  auto gte = b.AddInstruction(HloInstruction::CreateGetTupleElement(
      ShapeUtil::MakeShape(U32, {3, 2}), constant, 0));
  ...
  EXPECT_THAT(graph, HasSubstr("constant (u32[3,2], s32[4,5])"));
}
...
TEST_F(HloGraphDumperTest, Compare) {
  const char* hlo_string = R"(
    ...
    ENTRY comp {
      param.0 = u32[10] parameter(0)
      param.1 = u32[10] parameter(1)
...
// TODO(batzor): Uncomment this. Dependency: ParseAndReturnVerifiedModule
TEST_F(HloGraphDumperTest, HasStatisticsViz) {
  const char* hlo_string = R"(
    HloModule comp
//
    ENTRY comp {
      param.0 = u32[10] parameter(0), statistics={visualizing_index=0,stat-0=0.5}
      param.1 = u32[10] parameter(1), statistics={visualizing_index=1,stat-0=55.5,stat-1=44.4}
      ROOT lt = pred[10] compare(param.0, param.1), direction=LT
    })";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
...
%then_branch (empty: ()) -> u32[] {
  %empty = () parameter(0)
  ROOT %then = u32[] constant(1)
}

%else_branch (empty.1: ()) -> u32[] {
  %empty.1 = () parameter(0)
  ROOT %else = u32[] constant(2)
}

ENTRY %conditional_select (constant: pred[]) -> (u32[]) {
  %constant = pred[] parameter(0)
  %emptytuple = () tuple()
  %conditional = u32[] conditional(pred[] %constant, () %emptytuple, () %emptytuple), true_computation=%then_branch, false_computation=%else_branch
  ROOT %t = (u32[]) tuple(u32[] %conditional)
})";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnVerifiedModule(hlo_string));
  // Just check that it doesn't crash.
