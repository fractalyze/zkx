...
WidenWhileCondition(HloComputation* narrow_condition, const Shape& wide_shape) {
  ...
  HloComputation* wide_while_cond = [&]() {
    HloComputation::Builder builder(
        absl::StrCat("wide.", narrow_condition->name()));
...
WidenWhileBody(HloComputation* narrow_body, const Shape& wide_shape) {
  ...
  HloComputation* wide_while_body = [&]() {
    HloComputation::Builder builder(absl::StrCat("wide.", narrow_body->name()));
    builder.AddInstruction(HloInstruction::CreateParameter(
        0, wide_shape,
...
absl::StatusOr<std::unique_ptr<HloComputation>> MakeCountedLoopBodyComputation(
...
std::pair<std::unique_ptr<HloInstruction>, std::unique_ptr<HloInstruction>>
...
Shape MakeLoopStateShapeWithLayout(const WhileUtil::LoopStateTy& init_values) {
...
// static
absl::StatusOr<WhileUtil::OwningLoopStateTy> WhileUtil::MakeCountedLoop(
