...
#ifndef ZKX_SERVICE_PATTERN_MATCHER_H_
#define ZKX_SERVICE_PATTERN_MATCHER_H_
...
bool MatchAndLogIfFailed(HloInstruction* instr, std::string_view desc,
...
class LayoutPatternBaseImpl {
 public:
  bool Match(const ::zkx::Layout* layout, MatchOption option) const {
    if (layout == nullptr) {
...
class LayoutPatternEqualImpl {
 public:
  explicit constexpr LayoutPatternEqualImpl(const ::zkx::Layout* layout)
      : layout_(layout) {}
...
class LayoutPattern {
 private:
  ...
  auto AppendImpl(NewImpl new_impl) const {
    auto new_allof = AllOf<::zkx::Layout>(impl_, std::move(new_impl));
 ...
 public:
  ...
  // Returns true and captures the layout iff it matches the pattern.
  bool Match(::zkx::Layout* layout, MatchOption option) const {
    if (impl_.Match(layout, option)) {
      if (option.capture && matched_layout_) {
        *matched_layout_ = layout;
      }
  ...
  constexpr auto EqualTo(const ::zkx::Layout* layout) const {
...
// argument.
inline constexpr auto Layout(const ::zkx::Layout** matched_layout = nullptr) {
  return detail::LayoutPattern<const ::zkx::Layout,
...
// Creates a layout pattern that will capture the matched layout in the
// argument.
inline constexpr auto Layout(::zkx::Layout** matched_layout) {
  return detail::LayoutPattern<::zkx::Layout, detail::LayoutPatternBaseImpl>(
      detail::LayoutPatternBaseImpl(), matched_layout);
}
...
class ShapePatternBaseImpl {
 public:
  bool Match(const ::zkx::Shape* shape, MatchOption option) const {
    if (shape == nullptr) {
...
class ShapePatternEqualImpl {
 public:
  explicit constexpr ShapePatternEqualImpl(const ::zkx::Shape* shape)
      : shape_(shape) {}
...
class ShapePatternCompatibleImpl {
 public:
  explicit constexpr ShapePatternCompatibleImpl(const ::zkx::Shape* shape)
      : shape_(shape) {}
...
class ShapePatternLayoutImpl {
 public:
  ...
  bool Match(::zkx::Shape* shape, MatchOption option) const {
...
class ShapePatternSubshapeImpl {
 ...
 private:
  ::zkx::Shape* GetSubshape(::zkx::Shape* shape) const {
    return ShapeUtil::GetMutableSubshape(shape, index_);
  }
  const ::zkx::Shape* GetSubshape(const ::zkx::Shape* shape) const {
...
class ShapePattern {
 private:
  template <typename NewImpl>
  auto AppendImpl(NewImpl new_impl) const {
    auto new_all_of = AllOf<::zkx::Shape>(impl_, std::move(new_impl));
 ...
 public:
  ...
  // The layout must outlive the returned pattern.
  constexpr auto EqualTo(const ::zkx::Shape* shape) const {
  ...
  constexpr auto CompatibleTo(const ::zkx::Shape* shape) const {
  ...
  constexpr auto WithLayoutEqualTo(const ::zkx::Layout* layout) const {
  ...
  ShapePattern<ShapeType,
               AllOfPattern<::zkx::Shape, Impl,
                            ShapePatternSubshapeImpl<
                                const Shape,
                                AllOfPattern<::zkx::Shape, ShapePatternBaseImpl,
                                             ShapePatternEqualImpl>>>>
  WithSubshapeEqualTo(ShapeIndexView index, const ::zkx::Shape* shape) const {
    return WithSubshape(index,
                        ShapePattern<const ::zkx::Shape, ShapePatternBaseImpl>(
...
inline constexpr auto Shape(const ::zkx::Shape** matched_shape = nullptr) {
  return detail::ShapePattern<const ::zkx::Shape, detail::ShapePatternBaseImpl>(
      detail::ShapePatternBaseImpl(), matched_shape);
}

// Creates a shape pattern that will capture the matched layout in the argument.
inline constexpr auto Shape(::zkx::Shape** matched_shape) {
  return detail::ShapePattern<::zkx::Shape, detail::ShapePatternBaseImpl>(
      detail::ShapePatternBaseImpl(), matched_shape);
}
...
class HloInstructionPatternNameImpl {
 public:
  explicit HloInstructionPatternNameImpl(std::string_view name) : name_(name) {}
...
class HloInstructionPattern {
 ...
 public:
  ...
  // Returns true and captures the instruction iff it matches the pattern.
  bool Match(HloInstruction* inst, MatchOption option,
  ...
  auto WithName(std::string_view name) const {
  ...
  constexpr auto WithShapeEqualTo(const ::zkx::Shape* shape) const {
  ...
  // recursion bug.
  template <typename Dummy = void>
  constexpr auto WithShapeCompatibleTo(const ::zkx::Shape* shape) const {
  ...
  // clang-format off
  // TODO(chokobole): Uncomment this. Dependency: HloInstructionContractingDimsImpl
  // clang-format on
  ...
  auto WithSharding(std::string_view sharding) const {
    return AppendImpl(
...
// the argument.
inline constexpr auto Op(const HloInstruction** matched_inst = nullptr) {
  return detail::HloInstructionPattern<const HloInstruction,
...
// Creates an instruction pattern that will capture the matched instruction in
// the argument.
inline constexpr auto Op(HloInstruction** matched_inst) {
  return detail::HloInstructionPattern<HloInstruction,
                                       detail::HloInstructionPatternBaseImpl>(
      detail::HloInstructionPatternBaseImpl(), matched_inst);
}

// Helpers for nullary instructions.
#define ZKX_NULLOP_PATTERN(NAME)                                     \
...
#undef ZKX_NULLOP_PATTERN

// Helpers for unary instructions.
#define ZKX_UNOP_PATTERN(NAME)                                       \
  inline auto NAME() { return Op().WithOpcode(HloOpcode::k##NAME); } \
                                                                     \
  template <typename HloInstructionType>                             \
  inline auto NAME(HloInstructionType** matched_inst) {              \
...
#undef ZKX_UNOP_PATTERN

// Helpers for binary instructions.
#define ZKX_BINOP_PATTERN(NAME)                                               \
  inline auto NAME() { return Op().WithOpcode(HloOpcode::k##NAME); }          \
                                                                              \
...
#define ZKX_COMMUTATIVE_BINOP_PATTERN(NAME)                                \
  ZKX_BINOP_PATTERN(NAME)                                                  \
                                                                           \
  template <typename HloInstructionType, typename Lhs, typename Rhs>       \
...
ZKX_COMMUTATIVE_BINOP_PATTERN(Minimum)
...
#define ZKX_TERNOP_PATTERN(NAME)                                       \
  inline auto NAME() { return Op().WithOpcode(HloOpcode::k##NAME); }   \
                                                                       \
...
#define ZKX_VARIADIC_OP_PATTERN(NAME)                                         \
  inline auto NAME() { return Op().WithOpcode(HloOpcode::k##NAME); }          \
                                                                              \
...
ZKX_VARIADIC_OP_PATTERN(AfterAll);
...
ZKX_VARIADIC_OP_PATTERN(DynamicUpdateSlice)
ZKX_VARIADIC_OP_PATTERN(Fft);
ZKX_VARIADIC_OP_PATTERN(Fusion);
...
template <
    ...
    typename std::enable_if<
        !std::is_convertible<Arg0, std::string_view>::value &&
...
template <typename... Args>
auto CustomCall(absl::Span<const std::string_view> custom_call_targets,
                Args&&... args) {
...
template <typename HloInstructionType, typename Arg0, typename... Args,
          typename std::enable_if<!std::is_convertible<
              Arg0, std::string_view>::value>::type* = nullptr>
...
#define ZKX_COMPARE_PATTERN(NAME)                                             \
  inline auto NAME() {                                                        \
    return Op()                                                               \
...
#define ZKX_COMMUTATIVE_COMPARE_PATTERN(NAME)                              \
  ZKX_COMPARE_PATTERN(NAME)                                                \
                                                                           \
  template <typename HloInstructionType, typename Lhs, typename Rhs>       \
  inline auto NAME##AnyOrder(HloInstructionType** matched_inst, Lhs&& lhs, \
                             Rhs&& rhs) {                                  \
...
ZKX_COMMUTATIVE_COMPARE_PATTERN(Eq);
ZKX_COMMUTATIVE_COMPARE_PATTERN(Ne);
ZKX_COMPARE_PATTERN(Ge);
ZKX_COMPARE_PATTERN(Gt);
ZKX_COMPARE_PATTERN(Le);
ZKX_COMPARE_PATTERN(Lt);
...
class InstructionPatternInterface {
 public:
  ...
  virtual bool Match(HloInstruction* instr, MatchOption option) const = 0;
...
class TypedInstructionPattern : public InstructionPatternInterface {
 public:
  ...
  bool Match(HloInstruction* instr, MatchOption option) const override {
...
class HloInstructionPatternSharedImpl {
 public:
  ...
  bool Match(HloInstruction* instr, MatchOption option) const {
    return pattern_->Match(instr, option);
  }
  void DescribeTo(std::ostream* os, int64_t indent = 0) const {
    pattern_->DescribeTo(os, indent);
...
#endif  // ZKX_SERVICE_PATTERN_MATCHER_H_
