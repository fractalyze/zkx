...
ExecutionInput::~ExecutionInput() {
  for (auto& index : unowned_indices_) {
    std::optional<se::OwningDeviceMemory> buffer =
        buffers_.mutable_element(index)->Release();
    if (buffer) {
      buffer->Release();
    }
  }
...
absl::Status ExecutionInput::SetDynamicShape(Shape dynamic_shape) {
  ...
  if (!ShapeUtil::DynamicShapeIsCompatible(input_shape, dynamic_shape)) {
    return absl::InvalidArgumentError(
        absl::StrCat("Cannot set dynamic shape: ", input_shape.DebugString(),
                     " vs. ", dynamic_shape.DebugString()));
  }
