...
#ifndef ZKX_SERVICE_LATENCY_HIDING_SCHEDULER_H_
#define ZKX_SERVICE_LATENCY_HIDING_SCHEDULER_H_
...
class AsyncTracker {
 public:
  ...
  virtual std::string_view GetResourceName(int64_t resource_type) const;

  // Returns the name of the given resource usage
  std::string_view GetResourceUsageName(int64_t resource_usage_type) const;
  std::string_view GetResourceUsageName(
...
class AnnotationTracker {
 public:
  explicit AnnotationTracker(const HloModule* module) : module_(module) {
    for (const HloComputation* comp : module_->MakeNonfusionComputations()) {
      ...
      for (const HloInstruction* instr : comp->instructions()) {
        if (std::optional<int64_t> annotation = GetAnnotation(instr)) {
  ...
  const std::vector<int64_t>& GetAnnotations(const HloComputation* comp) const {
  ...
  std::optional<int64_t> GetAnnotation(const HloInstruction* instr) const {
    ...
    if (attrs.contains(kZkxSchedulingGroupIdAttr)) {
      return std::stoi(attrs.at(kZkxSchedulingGroupIdAttr));
    }
    return std::nullopt;
  }
  const std::vector<const HloInstruction*>& GetInstructions(
      const HloComputation* comp, int64_t annotation) const {
    return annotations_.at(annotation).at(comp);
  }
  int64_t GetNumInstructions(const HloComputation* comp, int64_t annotation) {
    return annotations_[annotation][comp].size();
  }
  void FindAnnotationRoots(const HloComputation* comp, int64_t annotation) {
  ...
  const std::vector<const HloInstruction*>& GetRootInstructions(
      const HloComputation* comp, int64_t annotation) {
 ...
 private:
  const HloModule* module_;  // not owned
...
class HloEdge {
 ...
 private:
  ...
  HloGraphNode* target_;  // not owned
};
...
class HloGraphNode {
 public:
  ...
  std::vector<int64_t> GetShareableResourcesOnEdge(const HloEdge& edge) const {
    const HloGraphNode& node = edge.Target();
 ...
 private:
  ...
  const HloInstruction* instr_;  // not owned
...
class BufferInfoTracker {
 public:
  struct ValueInfo {
    const HloBuffer* value = nullptr;                  // not owned
    const HloInstruction* first_definition = nullptr;  // not owned
...
class MemoryPressureTracker {
 ...
 private:
  static bool ShouldSkipBufferAllocations(
    ...
    // Make kGetTupleElement/kBitcast make alive only the tuple pointer if not
    // array shape.
  ...
  const HloAliasAnalysis* hlo_alias_analysis_;  // not owned
  ...
  // function of the scheduler querying the live_buffer_set_ object is too slow.
...
class DefaultSchedulerCore : public SchedulerCore {
 ...
 protected:
  ...
  const AsyncTracker* async_tracker_;          // not owned
  const LatencyEstimator* latency_estimator_;  // not owned
...
class LatencyHidingScheduler : public HloModulePass {
 public:
  struct SchedulerStatistics {
    const HloComputation* computation = nullptr;  // not owned
  ...
  LatencyHidingScheduler(
      ...
      : latency_estimator_(std::move(latency_estimator)),
        ...
        shape_size_bytes_(shape_size_bytes) {}
  constexpr static std::string_view kName = "latency-hiding-scheduler";
...
#endif  // ZKX_SERVICE_LATENCY_HIDING_SCHEDULER_H_
