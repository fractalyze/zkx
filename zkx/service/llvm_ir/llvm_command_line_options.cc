...
LLVMCommandLineOptionsLock::LLVMCommandLineOptionsLock(
  ...
  if (client_signature_ != active_client_signature_) {
    VLOG(1) << "ZKX (re)initializing LLVM with options fingerprint: "
            << client_signature_;
    VLOG(1) << "ZKX LLVM options:";
    ...
    for (std::string_view client_option : client_options) {
      VLOG(1) << absl::StrFormat("ZKX LLVM arg[%d]: %s", idx, client_option);
      fake_argv[idx] = client_option.data();
      ++idx;
    }
    for (std::string_view global_option : GetGlobalOptions()) {
      VLOG(1) << absl::StrFormat("ZKX LLVM arg[%d]: %s", idx, global_option);
    ...
    VLOG(1) << "ZKX skipping reinitializing LLVM with options signature: "
            << client_signature_;
  }
