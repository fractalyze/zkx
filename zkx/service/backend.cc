...
struct Backend::IntraOpThreadPool {
  explicit IntraOpThreadPool(const int num_threads)
      : pool(new tsl::thread::ThreadPool(tsl::Env::Default(), "ZKXEigen",
                                         num_threads)),
...
// static
absl::StatusOr<std::unique_ptr<Backend>> Backend::CreateBackend(
...
// static
absl::StatusOr<std::unique_ptr<Backend>> Backend::CreateDefaultBackend() {
...
absl::StatusOr<se::StreamExecutor*> Backend::stream_executor(
  ...
  return absl::InvalidArgumentError(absl::StrFormat(
      "device %s not supported by ZKX service", device_name(device_ordinal)));
}
