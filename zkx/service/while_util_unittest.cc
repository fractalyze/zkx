...
while_body {
  ROOT p_body = (s32[32,32]{1,0}, s32[32,32]{1,0}) parameter(0)
}

while_condition {
  p_cond = (s32[32,32]{1,0}, s32[32,32]{1,0}) parameter(0)
  ROOT result = pred[] constant(true)
}

ENTRY entry {
  p_entry_0 = s32[32,32]{1,0} parameter(0)
  p_entry_1 = s32[32,32]{1,0} parameter(1)
  p_entry_2 = s64[32,32]{1,0} parameter(2)
  while_init = (s32[32,32]{1,0}, s32[32,32]{1,0}) tuple(p_entry_0, p_entry_0)
  ROOT while = (s32[32,32]{1,0}, s32[32,32]{1,0}) while(while_init), condition=while_condition, body=while_body
}
)";
...
ENTRY main {
  init = (s32[], s32[]) parameter(0)
  to_make_live_in = s32[100] parameter(1)
  ROOT while = (s32[], s32[]) while(init), condition=cond, body=body
}
)";
...
// clang-format off
// TODO(chokobole): Enable this test. Dependency: CustomCall
// clang-format on
TEST_F(WhileUtilTest, DISABLED_TryIncrementSideEffecting) {
  constexpr std::string_view hlo = R"(
HloModule main
