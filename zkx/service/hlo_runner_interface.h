...
#ifndef ZKX_SERVICE_HLO_RUNNER_INTERFACE_H_
#define ZKX_SERVICE_HLO_RUNNER_INTERFACE_H_
...
// created by the same runner. We use this class to represent these
// executables when they leave the runner without exposing any details of the
// underlying implementation. See go/zkx-opaque-executable for more details.
...
class HloRunnerInterface {
 public:
  ...
  struct ReplicatedExecuteOptions {
    ...
    // Otherwise, executes using zkx::Executable::ExecuteOnStreams.
  ...
  static absl::StatusOr<std::unique_ptr<HloModule>> CreateModuleFromString(
      std::string_view hlo_string, const DebugOptions& debug_options);

  // Reads the proto file in zkx.HloProto format, creates and returns the
  // HloModule.
  static absl::StatusOr<std::unique_ptr<HloModule>>
  ReadModuleFromBinaryProtoFile(const std::string& filename,
                                const DebugOptions& debug_options);

  // Reads the proto file in zkx.HloModule format, creates and returns the
  // HloModule.
  static absl::StatusOr<std::unique_ptr<HloModule>>
...
#endif  // ZKX_SERVICE_HLO_RUNNER_INTERFACE_H_
