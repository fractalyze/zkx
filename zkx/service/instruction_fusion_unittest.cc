...
TEST_F(InstructionFusionTest, FuseInstructions) {
  ...
  ENTRY entry_computation {
    p0 = u32[4,3]{1,0} parameter(0)
    add = u32[4,3]{1,0} add(p0, p0)
    ROOT sub = u32[4,3]{1,0} subtract(add, p0)
  })")
                    .value();
...
TEST_F(InstructionFusionTest, FuseIntoFusionInstruction) {
  ...
  HloModule test_module
  fused_computation {
    p1 = s32[4,3] parameter(0)
    add = s32[4,3] add(p1, p1)
  }
  ENTRY entry_computation {
    p0 = s32[4,3] parameter(0)
    abs = s32[4,3] abs(p0)
    ROOT fusion = s32[4,3] fusion(abs), kind=kLoop, calls=fused_computation
  })")
                    .value();
...
TEST_F(InstructionFusionTest, FuseInstructionsIntoMultiOutput) {
  ...
  HloModule test_module
  ENTRY entry_computation {
    p0 = s32[4,3]{1,0} parameter(0)
    abs = s32[4,3]{1,0} abs(p0)
    negate = s32[4,3]{1,0} negate(abs)
    ROOT add = s32[4,3]{1,0} add(abs, negate)
  })")
                    .value();
  HloInstruction* root = module->entry_computation()->root_instruction();
  HloInstruction* abs = root->mutable_operand(0);
  HloInstruction* negate = root->mutable_operand(1);
  HloInstruction* fusion = InstructionFusionForTesting().FuseIntoMultiOutput(
      abs, negate, module->entry_computation());

  ASSERT_THAT(fusion, op::Fusion()) << module->ToString();
  EXPECT_THAT(fusion->fused_expression_root(),
              op::Tuple(op::Negate(), op::Abs()))
      << module->ToString();
}

TEST_F(InstructionFusionTest, AvoidDuplicationIfNotAllFusible) {
  HloComputation::Builder builder(TestName());
  auto shape = ShapeUtil::MakeShape(S32, {16, 16});
  auto param0 =
...
TEST_F(InstructionFusionTest, FuseCheapNonDuplicatableOps) {
  ...
  ENTRY OutputFusion {
    p0 = u32[4,3]{1,0} parameter(0)
    add = u32[4,3]{1,0} add(p0, p0)
    ROOT root = u32[4,3]{1,0} subtract(add, add)
  })")
                    .value();
...
//   // Make sure we do not duplicate the add, as we cannot fuse through the
//   rng.
//   //
//   // (p0, p1) -> add -------------------------> sub
//   //                 \-> abs1 -> rng -> abs2 -/
...
//     add = u32[] add(p0, p1)
//     abs1 = u32[] abs(add)
//     rng = u32[] rng(p1, abs1), distribution=rng_uniform
//     abs2 = u32[] abs(rng)
//     abs3 = u32[] abs(rng)
//     ROOT root = u32[] subtract(abs2, add)
//   })")
//                     .value();
...
//   ENTRY OutputFusion {
//     p0 = u32[4,3]{1,0} parameter(0)
//     p1 = u32[4,3]{1,0} parameter(1)
//     add = u32[4,3]{1,0} add(p0, p1)
//     abs1 = u32[4,3]{1,0} abs(add)
//     log = u32[4,3]{1,0} log(abs1)
//     token0 = token[] after-all()
//     send = u32[4,3]{1,0} send(log, token0), channel_id=1
//     send-done = token[] send-done(send), channel_id=1
//     abs2 = u32[4,3]{1,0} abs(log)
//     ROOT root = u32[4,3]{1,0} subtract(abs2, add)
//   })")
//                .value();
...
//   // Make sure we still fuse ops where one operand in the chain to the
//   producer
//   // can't be fused.
//   //
//   // (p0, p1) ---> add1 -----------> sub
...
//     p0 = u32[4,3]{1,0} parameter(0)
//     p1 = u32[4,3]{1,0} parameter(1)
//     add1 = u32[4,3]{1,0} add(p0, p1)
//     log = u32[4,3]{1,0} log(p0)
//     token0 = token[] after-all()
//     send = u32[4,3]{1,0} send(log, token0), channel_id=1
//     send-done = token[] send-done(send), channel_id=1
//     add2 = u32[4,3]{1,0} add(log, add1)
//     ROOT root = u32[4,3]{1,0} subtract(add1, add2)
//   })")
//                .value();
...
//   // A variant of the above that allows the algorithm to put add2 into the
//   set
//   // of unfusible ops to short-circuit the decision whether add1 should be
//   fused
//   // into sub2.
//   //
//   //             /---------------\
...
//     p1 = u32[4,3]{1,0} parameter(1)
//     add1 = u32[4,3]{1,0} add(p0, p1)
//     add2 = u32[4,3]{1,0} add(add1, p1)
//     log = u32[4,3]{1,0} log(add2)
//     token0 = token[] after-all()
//     send = u32[4,3]{1,0} send(log, token0), channel_id=1
//     send-done = token[] send-done(send), channel_id=1
//     sub1 = u32[4,3]{1,0} subtract(log, add2)
//     sub2 = u32[4,3]{1,0} subtract(add2, add1)
//     ROOT root = (u32[4,3]{1,0}, u32[4,3]{1,0}) tuple(sub1, sub2)
//   })")
//                .value();
...
TEST_F(InstructionFusionTest, AllowEffectiveUnaryDuplication) {
  auto shape = ShapeUtil::MakeShape(S32, {16, 16});
  auto small_shape = ShapeUtil::MakeShape(S32, {16});
  HloComputation::Builder builder(TestName());
...
//   // Make sure we do duplicate the add of the same values, even though we
//   cannot
...
//     add = u32[] add(p0, p0)
//     abs1 = u32[] abs(add)
//     rng = u32[] rng(p0, abs1), distribution=rng_uniform
//     abs2 = u32[] abs(rng)
//     abs3 = u32[] abs(rng)
//     ROOT root = u32[] subtract(abs2, add)
//   })")
...
TEST_F(InstructionFusionTest, FuseDiamondGraphsNoDuplication) {
  ...
  ENTRY Test {
    ...
    p1 = u32[100] parameter(1)
    add = u32[100] add(p0, p1)
    slice1 = u32[99] slice(add), slice={[0:99:1]}
    slice2 = u32[99] slice(add), slice={[1:100:1]}
    ROOT add2 = u32[99] add(slice1, slice2)
  })")
                    .value();
  EXPECT_TRUE(
...
TEST_F(InstructionFusionTest,
  ...
  HloModule test_module
  ENTRY Test {
    p0 = u16[100] parameter(0)
    c = u32[100] convert(p0)
    add = u32[100] add(c, c)
    ROOT mul = u32[100] multiply(c, c)
  })")
                    .value();
...
TEST_F(InstructionFusionTest, BroadcastsAreAlwaysDuplicableIntoConsumers) {
  ...
  ENTRY Test {
    p0 = u32[100] parameter(0)
    c = u32[100,100] broadcast(p0), dimensions={0}
    add = u32[100,100] add(c, c)
    ROOT mul = u32[100,100] multiply(c, c)
  })")
                    .value();
...
TEST_F(InstructionFusionTest,
  ...
  HloModule test_module
  ENTRY Test {
    parameter.1 = u32[8] parameter(0)
    slice.19 = u32[7] slice(parameter.1), slice={[0:7]}
    constant.7 = u32[] constant(1)
    broadcast.8 = u32[7] broadcast(constant.7), dimensions={}
    add.9 = u32[7] add(slice.19, broadcast.8)
    constant.10 = u32[] constant(1)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(parameter.1, add.9, constant.10)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
TEST_F(InstructionFusionTest,
  ...
  ENTRY Test {
    parameter = u32[8] parameter(0)
    slice = u32[5] slice(parameter), slice={[0:5]}
    one = u32[] constant(1)
    broadcast = u32[5] broadcast(one), dimensions={}
    add = u32[5] add(slice, broadcast)
    three = s32[] constant(3)
    update = u32[6] parameter(1)
    two = s32[] constant(2)
    dynamic-update-slice = u32[8] dynamic-update-slice(parameter, update, two)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(dynamic-update-slice, add, three)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
TEST_F(InstructionFusionTest, DynamicUpdateSliceShouldFuseWithElementwise) {
  ...
  ENTRY Test {
    ...
    add = u32[5] add(slice, broadcast)
    three = s32[] constant(3)
    parameter.1 = u32[1,8] parameter(1)
    bitcast = u32[8] bitcast(parameter.1)
    neg = u32[8] negate(bitcast)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(neg, add, three)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
TEST_F(InstructionFusionTest, InPlaceOpShouldFuseWithSliceSameIndex) {
  ...
  ENTRY Test {
    parameter.1 = u32[8] parameter(0)
    slice.19 = u32[7] slice(parameter.1), slice={[1:8]}
    constant.7 = u32[] constant(1)
    broadcast.8 = u32[7] broadcast(constant.7), dimensions={}
    add.9 = u32[7] add(slice.19, broadcast.8)
...
TEST_F(InstructionFusionTest, InPlaceOpShouldNotFuseWithUnknownDynamicSlice) {
  ...
  HloModule test_module
  ENTRY Test {
    parameter.0 = u32[8] parameter(0)
    parameter.1 = s32[] parameter(1)
    dynamic-slice = u32[7] dynamic-slice(parameter.0, parameter.1), dynamic_slice_sizes={7}
    constant.7 = u32[] constant(1)
    broadcast.8 = u32[7] broadcast(constant.7), dimensions={}
    add.9 = u32[7] add(dynamic-slice, broadcast.8)
    constant.10 = s32[] constant(1)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(parameter.0, add.9, constant.10)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
TEST_F(InstructionFusionTest, InPlaceOpShouldFuseWithSameDynamicSlice) {
  ...
  ENTRY Test {
    ...
    broadcast.8 = u32[7] broadcast(constant.7), dimensions={}
    add.9 = u32[7] add(dynamic-slice, broadcast.8)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(parameter.0, add.9, parameter.1)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
TEST_F(InstructionFusionTest, InPlaceOpShouldNotFuseWithSliceSameIndex) {
  ...
  ENTRY Test {
    ...
    broadcast.8 = u32[7] broadcast(constant.7), dimensions={}
    add.9 = u32[7] add(slice.19, broadcast.8)
    reverse = u32[7] reverse(add.9), dimensions={0}
    constant.10 = s32[] constant(1)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(parameter.1, reverse, constant.10)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
TEST_F(InstructionFusionTest,
  ...
  ENTRY Test {
    parameter.1 = u32[8] parameter(0)
    parameter.2 = u32[7] parameter(1)
    slice.19 = u32[7] slice(parameter.1), slice={[1:8]}
    reverse = u32[7] reverse(parameter.2), dimensions={0}
    add.9 = u32[7] add(slice.19, reverse)
    constant.10 = s32[] constant(1)
    ROOT dynamic-update-slice.1 = u32[8] dynamic-update-slice(parameter.1, add.9, constant.10)
  })")
                    .value();
...
// TODO(batzor): Uncomment this. Dependency: Scatter
...
//   // operand that has the same value as the in-place buffer should not be
//   fused. auto module = ParseAndReturnVerifiedModule(R"( HloModule test_module
//   update_s32 {
//     lhs = s32[] parameter(0)
...
//     ROOT scatter = s32[9] scatter(arg0, indices, arg0),
...
TEST_F(InstructionFusionTest, DontFuseAcrossRoot) {
  ...
  ENTRY entry_computation {
    p0 = u32[4,3]{1,0} parameter(0)
    mul = u32[4,3]{1,0} multiply(p0, p0)
    ROOT add = u32[4,3]{1,0} add(mul, p0)
    sub = u32[4,3]{1,0} subtract(p0, add)
  })")
                    .value();
  EXPECT_TRUE(
      InstructionFusion(InstructionFusion::IsExpensive, /*may_duplicate=*/false)
...
// FIXME(batzor): Figure out why this test is failing.
// TEST_F(InstructionFusionTest, DontFuseProducerIfInplaceConflict) {
//   auto module = ParseAndReturnVerifiedModule(R"(
//   HloModule test_module
//   func {
//     p0 = u32[200,200]{1,0} parameter(0)
//     p1 = u32[200,200]{1,0} parameter(1)
//     p2 = u32[72]{0} parameter(2)
//     bitcast0 = u32[200,200]{0,1} bitcast(p1)
//     bitcast1 = u32[72,1]{1,0} bitcast(p2)
//     gather = u32[72,1,200]{0,2,1} gather(bitcast0, bitcast1),
//     offset_dims={1,2}, collapsed_slice_dims={}, start_index_map={0},
//     index_vector_dim=1, slice_sizes={1,200} bitcast2 = u32[200,72]{1,0}
//     bitcast(gather) constant0 = u32[] constant(128) constant1 = u32[]
//     constant(0) ROOT dynamic-update-slice = u32[200,200]{1,0}
//     dynamic-update-slice(p0, bitcast2, constant1, constant0)
//   }
//   ENTRY entry_computation {
//     p0 = u32[200,200]{1,0} parameter(0)
//     p1 = u32[200,200]{1,0} parameter(1)
//     p2 = u32[72]{0} parameter(2)
//     add = u32[200,200]{1,0} add(p0, p1)
//     ROOT fusion = u32[200,200]{1,0} fusion(p0, add, p2), kind=kLoop,
//     calls=func
//   })")
//                     .value();
//   HloInstruction* root = module->entry_computation()->root_instruction();
