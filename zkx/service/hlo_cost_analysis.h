...
// during the traversal for the entire graph.
class HloCostAnalysis : public ConstDfsHloVisitor {
 public:
  static inline constexpr std::string_view kFlopsKey = "flops";
  static inline constexpr std::string_view kBytesAccessedKey = "bytes accessed";
  static inline constexpr std::string_view kOptimalSecondsKey =
      "optimal_seconds";
  static inline constexpr std::string_view kUtilizationKey = "utilization";

  // Key reserved for use by subclasses. This gets the same special "fast
  // path" treatment in Properties as the other keys above.
  static inline constexpr std::string_view kReserved0Key = "reserved0";
  ...
  // was, once. The problem is, this class is used *everywhere* within cost
  // analysis, and the hashtable lookups added up to the majority (!) of its
  // runtime.
  //
  ...
  // the hashtable-ness of this class is part of the API and so is hard
  // to change.
  //
  // So instead we end up with this hybrid. It *acts* like a hashtable, but
  // before falling back to the hashtable, it checks whether the string matches
  // one of a list of "known keys". If so, it returns that special value from
  // the struct.
  class Properties {
   public:
    Properties()
    ...
    float& operator[](std::string_view property) {
    ...
    float operator[](std::string_view property) const {
      if (property == kFlopsKey) {
        return flops_;
      }
      if (property == kBytesAccessedKey) {
    ...
    std::string ToString() const {
      return absl::StrFormat(
          ...
          " reserved0: %f\n"
          "}",
          flops_, bytes_accessed_, optimal_seconds_, utilization_,
          operand0_utilization_, operand1_utilization_,
   ...
   private:
    ...
    static inline constexpr std::string_view kOperand0UtilizationKey =
        "utilization0{}";
    static inline constexpr std::string_view kOperand1UtilizationKey =
        "utilization1{}";
    static inline constexpr std::string_view kOperand0BytesAccessedKey =
        "bytes accessed0{}";
    static inline constexpr std::string_view kOperand1BytesAccessedKey =
        "bytes accessed1{}";
    static inline constexpr std::string_view kOutputRootBytesAccessedKey =
        "bytes accessedout{}";
  ...
  // second rates of FLOPs, bytes per second (available bandwidth).
  struct Options {
    ...
    float per_second_rate(std::string_view key) const {
      return per_second_rates[key];
    }

    float min_latency_seconds(std::string_view key) const {
  ...
  float property(std::string_view key) { return properties_sum_[key]; }
 ...
 protected:
  ...
  virtual bool KeyToCopyFromSubcomputation(std::string_view key) const;
