...
class ScatterExpanderTest : public HloTestBase {
 protected:
  ...
  void ClearInstructionLayout(HloModule* module, std::string_view inst_name) {
...
TEST_F(ScatterExpanderTest, ScatterMultipleOperandsWithoutLayout) {
  const char* kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      operand1 = s32[5] constant({2,4,6,8,10})
      indices = s32[1] parameter(0)
      update0 = s32[] constant(0)
      update1 = s32[] constant(1)
      ROOT scatter = (s32[5]{0}, s32[5]{0}) scatter(operand0, operand1, indices, update0, update1),
        update_window_dims={}, inserted_window_dims={0},
        scatter_dims_to_operand_dims={0}, index_vector_dim=0,
        to_apply=scatter_computation
    })";
...
TEST_F(ScatterExpanderTest,
  ...
  const char* kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand0 = s32[5] iota(), iota_dimension=0
      operand1 = s32[5] iota(), iota_dimension=0
      indices = s32[1] parameter(0)
      update0 = s32[] constant(0)
      update1 = s32[] constant(0)
      ROOT scatter = (s32[5]{0}, s32[5]{0}) scatter(operand0, operand1, indices, update0, update1),
        update_window_dims={}, inserted_window_dims={0},
        scatter_dims_to_operand_dims={0}, index_vector_dim=0,
        to_apply=scatter_computation
