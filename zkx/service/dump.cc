...
struct CanonicalDebugOptions {
  explicit CanonicalDebugOptions(const DebugOptions& opts)
      : dump_to(opts.zkx_dump_to()),
        dump_as_text(opts.zkx_dump_hlo_as_text()),
        dump_as_proto(opts.zkx_dump_hlo_as_proto()),
        dump_as_dot(opts.zkx_dump_hlo_as_dot()),
        dump_as_html(opts.zkx_dump_hlo_as_html()),
        dump_as_url(opts.zkx_dump_hlo_as_url()),
        dump_fusion_visualization(opts.zkx_dump_fusion_visualization()),
        dump_snapshots(opts.zkx_dump_hlo_snapshots()),
        dump_unoptimized_snapshots(
            opts.zkx_gpu_dump_hlo_unoptimized_snapshots()),
        dump_include_timestamp(opts.zkx_dump_include_timestamp()),
        dump_max_hlo_modules(opts.zkx_dump_max_hlo_modules()),
        dump_module_metadata(opts.zkx_dump_module_metadata()),
        dump_compress_protos(opts.zkx_dump_compress_protos()),
        dump_hlo_metadata(!opts.zkx_dump_disable_metadata()),
        dump_fdo_profiles(opts.zkx_gpu_experimental_dump_fdo_profiles()),
        dump_as_long_text(opts.zkx_dump_hlo_as_long_text()),
        dump_mlir_pretty_form(opts.zkx_dump_enable_mlir_pretty_form()),
        dump_large_constants(opts.zkx_dump_large_constants()),
        syntax_sugar_async_ops(opts.zkx_syntax_sugar_async_ops()) {
    ...
    bool output_format_other_than_url_specified =
        opts.zkx_dump_hlo_as_text() || opts.zkx_dump_hlo_as_proto() ||
        opts.zkx_dump_hlo_as_dot() || opts.zkx_dump_hlo_as_html() ||
        opts.zkx_dump_hlo_snapshots() ||
        opts.zkx_gpu_dump_hlo_unoptimized_snapshots();
    bool output_format_specified =
        output_format_other_than_url_specified || opts.zkx_dump_hlo_as_url();
    ...
    // --zkx_dump_hlo_as_url, then don't dump to stdout, as that is likely noise
    // they don't want.
    if (opts.zkx_dump_to().empty() && output_format_other_than_url_specified) {
      dump_to = "-";
    }
    ...
    if (!opts.zkx_dump_hlo_module_re().empty()) {
      // RE2 object is not copyable, and we can't capture "by move", so we
      // resort to this hack.
      std::string pattern = opts.zkx_dump_hlo_module_re();
      should_dump_module = [pattern](std::string_view module_name) {
        return RE2::PartialMatch(module_name, pattern);
      };
    } else if (!opts.zkx_dump_hlo_pass_re().empty() ||
               !opts.zkx_dump_to().empty() || output_format_specified) {
      should_dump_module = [](std::string_view) { return true; };
    } else {
      should_dump_module = [](std::string_view) { return false; };
    }

    // Initialize should_dump_pass. This one is easy: We only dump per-pass
    // data if the user asked for it explicitly.
    if (!opts.zkx_dump_hlo_pass_re().empty()) {
      std::string pattern = opts.zkx_dump_hlo_pass_re();
      should_dump_pass = [pattern](std::string_view pass_name) {
        return RE2::PartialMatch(pass_name, pattern);
      };
    } else {
      should_dump_pass = [](std::string_view) { return false; };
    }
    ...
    // explicitly.
    if (!opts.zkx_dump_hlo_pipeline_re().empty()) {
      std::string pattern = opts.zkx_dump_hlo_pipeline_re();
      should_dump_pipeline = [pattern](std::string_view pipeline_name) {
        return RE2::PartialMatch(pipeline_name, pattern);
      };
    } else {
      should_dump_pipeline = [](std::string_view) { return true; };
    }
    ...
    if (dump_to_lower == "sponge" ||
      ...
      if (!tsl::io::GetTestUndeclaredOutputsDir(&dump_to)) {
        LOG(ERROR) << "--zkx_dump_to=" << opts.zkx_dump_to()
                   << ", but environment variable TEST_UNDECLARED_OUTPUTS_DIR "
                      "is not set, so cannot dump anywhere.";
        should_dump_module = [](std::string_view) { return false; };
        should_dump_pass = [](std::string_view) { return false; };
        should_dump_pipeline = [](std::string_view) { return false; };
      }
    }
  ...
  std::string dump_to;
  std::function<bool(std::string_view module_name)> should_dump_module;
  std::function<bool(std::string_view pass_name)> should_dump_pass;
  std::function<bool(std::string_view pipeline_name)> should_dump_pipeline;
...
absl::Status WriteStringToFile(tsl::Env* env, std::string_view fname,
  ...
  if (compressed) {
    // TODO(chokobole): Uncomment this. Dependency: gzip
...
std::vector<std::string> DumpHloModuleImpl(const HloModule& module,
  ...
  if (opts.dump_as_text) {
    ...
    file_paths.push_back(DumpToFileInDirOrStdoutImpl(
        absl::StrCat(filename, ".txt"), module.ToString(print_options), opts));
    if (buffer_assn) {
      ...
      file_paths.push_back(DumpToFileInDirOrStdoutImpl(
          absl::StrCat(filename, "-buffer-assignment.txt"), buffer_assignment,
          opts));
      DataProducer summary_report;
      summary_report.Append([&] { return buffer_assn->MemoryUsageReport(); });
      file_paths.push_back(DumpToFileInDirOrStdoutImpl(
          absl::StrCat(filename, "-memory-usage-report.txt"), summary_report,
          opts));
    }
  }
  ...
  if (opts.dump_as_dot) {
    // TODO(chokobole): Uncomment this. Dependency: RenderGraph
    // file_paths.push_back(DumpToFileInDirImpl(
  ...
  if (opts.dump_fusion_visualization) {
    // TODO(chokobole): Uncomment this. Dependency: WrapFusionExplorer
  ...
  if (opts.dump_fdo_profiles) {
    file_paths.push_back(
        DumpToFileInDirImpl(absl::StrFormat("%s.fdo_profile", filename),
  ...
  if (opts.dump_as_url) {
    // TODO(chokobole): Uncomment this. Dependency: RenderGraph
    // std::string url = RenderGraph(filename, module,
    // RenderedGraphFormat::kUrl); std::cout << filename << " --> " << url <<
    // std::endl; if (!opts.dumping_to_stdout()) {
    //   file_paths.push_back(
...
std::string TimestampFor(const HloModule& module) {
  if (!module.config().debug_options().zkx_dump_include_timestamp()) {
    return "";
  }
  absl::MutexLock lock(&mu);
...
std::string FilenameFor(int unique_id, std::string_view module_name,
...
#if !TSL_IS_IN_OSS
  std::string_view cl_number = BuildData::Changelist();
  if (!cl_number.empty()) {
...
void DumpPerModuleProtobufToFile(
    ...
    const DebugOptions& debug_options, std::string_view name,
    absl::AnyInvocable<absl::StatusOr<std::string>(
        tsl::Env*, const google::protobuf::Message&)>
        text_formatter) {
...
std::vector<std::string> DumpHloModuleProtoIfEnabled(
    const HloModuleProto& module_proto, std::string_view name) {
  auto config = HloModule::CreateModuleConfigFromProto(
      module_proto, GetDebugOptionsFromFlags());

  auto module =
      HloModule::CreateFromProto(module_proto, config.value()).value();
...
bool DumpingEnabledForHloModule(std::string_view hlo_module_name,
                                const DebugOptions& opts) {
  return CanonicalDebugOptions(opts).should_dump_module(hlo_module_name);
}

bool DumpingEnabledForHloPass(std::string_view hlo_pass_name,
                              const DebugOptions& opts) {
...
std::vector<std::string> DumpHloModuleBetweenPassesIfEnabled(
    std::string_view pipeline_name, std::string_view before_pass_name,
    std::string_view after_pass_name, const HloModule& module) {
  CanonicalDebugOptions opts(module.config().debug_options());
  if (!opts.should_dump_module(module.name())) {
    return {};
  }
  ...
  std::string filename_suffix =
      absl::StrFormat("%04d.%s.after_%s.before_%s", step_number, pipeline_name,
                      after_pass_name, before_pass_name);
  return DumpHloModuleImpl(module, /*buffer_assn=*/nullptr, timestamp,
                           filename_suffix, opts);
}

void DumpHloModuleDuringPassIfEnabled(std::string_view pass_name,
  ...
  std::string filename_suffix =
      absl::StrFormat("%04d.%s.%s", step_number, pass_name, step_name);
...
void DumpHloSnapshotIfEnabled(const HloModule& module,
  ...
  std::string filename = absl::StrCat(
      FilenameFor(module, std::to_string(timestamp),
                  absl::StrFormat("execution_%04d", execution_count)),
      ".hlo_snapshot.pb");
  if (opts.dumping_to_stdout()) {
...
void DumpHloSnapshotIfEnabled(const HloSnapshot& snapshot,
  ...
  std::string filename = absl::StrFormat(
      "module_%s.execution_%04d.hlo_snapshot.pb", name, execution_count);
...
void DumpIrIfEnabled(const HloModule& hlo_module,
                     ...
                     std::string_view filename_suffix) {
  ...
  DumpToFileInDirOrStdout(hlo_module, "", absl::StrCat(suffix, ".ll"),
                          llvm_ir::DumpToString(&llvm_module));
}
