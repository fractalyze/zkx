...
class AotCompilationResult {
 public:
  ...
  virtual absl::StatusOr<std::string> SerializeAsString() const {
    return absl::UnimplementedError("SerializeAsString unimplemented.");
  }

  virtual absl::StatusOr<std::unique_ptr<Executable>> LoadExecutable(
      Compiler* compiler, const se::StreamExecutor* executor) const {
    return absl::UnimplementedError("LoadExecutable unimplemented.");
  }
...
class Compiler {
 public:
  ...
  LoadAotCompilationResult(const std::string& serialized_aot_result) {
    return absl::UnimplementedError("LoadAotCompilationResult unimplemented.");
  }
  ...
  absl::StatusOr<std::vector<std::unique_ptr<Executable>>> Compile(
      ...
      std::vector<std::vector<se::StreamExecutor*>> executors,
      se::DeviceMemoryAllocator* device_allocator) {
    return Compile(std::move(module_group), executors,
                   CompileOptions{device_allocator});
  }
  ...
  virtual std::vector<std::unique_ptr<google::protobuf::Message>>
  ...
  // compilation stages.
  // TODO(chokobole): Uncomment this. Dependency: MetricsHookInterface
  // virtual std::unique_ptr<MetricsHookInterface> CreateMetricsHook(
  //     std::string_view filename_prefix) const;
...
class AotCompilationOptions {
 public:
  ...
  std::string_view cache_key() const { return cache_key_; }
  void set_cache_key(std::string_view cache_key) {
