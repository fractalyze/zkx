...
std::vector<HloInstruction*> GenerateExplicitBatchDimIndices(
  ...
  HloInstruction* dividend = induction_var;
  ...
  for (int64_t i = start_indices_shape.dimensions_size() - 1; i >= 0; i--) {
    ...
    if (num_batch_dims == 0) {
      if (it != start_indices_batching_dims.end()) {
        // Avoid generating a remainder that just returns the dividend itself.
    ...
    if (it != start_indices_batching_dims.end()) {
      explicit_batch_dim_indices[it - start_indices_batching_dims.begin()] =
          computation->AddInstruction(HloInstruction::CreateBinary(
              shape, HloOpcode::kRemainder, dividend, divisor));
    }

    dividend = computation->AddInstruction(HloInstruction::CreateBinary(
        shape, HloOpcode::kDivide, dividend, divisor));
  }
