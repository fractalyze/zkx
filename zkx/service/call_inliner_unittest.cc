...
TEST_F(CallInlinerTest, ControlDependenciesAreCarriedToCaller) {
  ...
  HloInstruction* zero = inner.AddInstruction(
      HloInstruction::CreateConstant(LiteralUtil::CreateR0<int32_t>(24)));
  ...
  outer.AddInstruction(
      HloInstruction::CreateCall(r0s32, {}, inner_computation));
  ...
  EXPECT_EQ(prior->literal().GetFirstElement<int32_t>(), 24);
}
...
TEST_F(CallInlinerTest, CallToOutfeedComputationIsInlined) {
  ...
  outfeeder.AddInstruction(
      HloInstruction::CreateOutfeed(s32, value, token, /*outfeed_config=*/""));
...
TEST_F(CallInlinerTest, InlineCompositeCall) {
  ...
  ENTRY %main () -> s32[] {
    %lhs = s32[] constant(42)
    ROOT %call = s32[] call(s32[] %lhs), to_apply=%add, is_composite=true, frontend_attributes={composite.attributes={n = 1 : i32, tensor = dense<1> : tensor<i32>},composite.name="foo.bar",composite.version="1"}
  })";
...
//     HloModule jit_f, entry_computation_layout={(s32[8,8]{1,0})->s32[8,8]{1,0}}

//     %prefix_shmap_body_suffix.4 (Arg_0.5: s32[1,8]) -> s32[1,8] {
//       %Arg_0.5 = s32[1,8]{1,0} parameter(0)
//       ROOT %add.6 = s32[1,8]{1,0} add(s32[1,8]{1,0} %Arg_0.5, s32[1,8]{1,0} %Arg_0.5), metadata={source_file="-" source_line=11}
//     }

//     ENTRY %main.10 (Arg_0.1: s32[8,8]) -> s32[8,8] {
//       %Arg_0.1 = s32[8,8]{1,0} parameter(0)
//       %custom-call.2 = s32[8,8]{1,0} custom-call(s32[8,8]{1,0} %Arg_0.1), custom_call_target="Sharding", sharding={devices=[8,1]<=[8]}, metadata={source_file="-" source_line=3}
//       %custom-call.3 = s32[1,8]{1,0} custom-call(s32[8,8]{1,0} %custom-call.2), custom_call_target="SPMDFullToShardShape", sharding={manual}, metadata={source_file="-" source_line=4}
//       %call.7 = s32[1,8]{1,0} call(s32[1,8]{1,0} %custom-call.3), to_apply=%prefix_shmap_body_suffix.4
//       %custom-call.8 = s32[1,8]{1,0} custom-call(s32[1,8]{1,0} %call.7), custom_call_target="Sharding", sharding={manual}, metadata={source_file="-" source_line=6}
//       ROOT %custom-call.9 = s32[8,8]{1,0} custom-call(s32[1,8]{1,0} %custom-call.8), custom_call_target="SPMDShardToFullShape", sharding={devices=[8,1]<=[8]}, metadata={source_file="-" source_line=7}
//     })";
...
//   HloInstruction* call = FindInstruction(module.get(), HloOpcode::kCall);
...
// Don't inline when the name starts with "zkx.sdy.manual_computation_body".
// clang-format off
// TODO(chokobole): Uncomment this. Dependency: Shardy
...
//     %zkx.sdy.manual_computation_body.4 (Arg_0.5: s32[1,8]) -> s32[1,8] {
//       %Arg_0.5 = s32[1,8]{1,0} parameter(0)
//       ROOT %add.6 = s32[1,8]{1,0} add(s32[1,8]{1,0} %Arg_0.5, s32[1,8]{1,0} %Arg_0.5), metadata={source_file="-" source_line=11}
//     }
...
//     ENTRY %main.10 (Arg_0.1: s32[8,8]) -> s32[8,8] {
//       %Arg_0.1 = s32[8,8]{1,0} parameter(0)
//       %custom-call.3 = s32[1,8]{1,0} custom-call(s32[8,8]{1,0} %Arg_0.1), custom_call_target="SPMDFullToShardShape", sharding={manual}, metadata={source_file="-" source_line=4}
//       %call.7 = s32[1,8]{1,0} call(s32[1,8]{1,0} %custom-call.3), to_apply=%zkx.sdy.manual_computation_body.4
//       ROOT %custom-call.9 = s32[8,8]{1,0} custom-call(s32[1,8]{1,0} %call.7), custom_call_target="SPMDShardToFullShape", sharding={devices=[8,1]<=[8]}, metadata={source_file="-" source_line=7}
//     })";
//   TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(hloString));
//   module->mutable_config().set_use_shardy_partitioner(true);
//   TF_ASSERT_OK_AND_ASSIGN(bool changed, CallInliner().Run(module.get()));
...
//   EXPECT_NE(call, nullptr);
//   EXPECT_TRUE(call->has_to_apply());
//   EXPECT_EQ(call->to_apply()->name(), "zkx.sdy.manual_computation_body.4");
// }
// clang-format on
...
//     %my_model.___call__.fwd.zkx.sdy.manual_computation_body_14.1234 (Arg_0.5: s32[1,8]) -> s32[1,8] {
//       %Arg_0.5 = s32[1,8]{1,0} parameter(0)
//       ROOT %add.6 = s32[1,8]{1,0} add(s32[1,8]{1,0} %Arg_0.5, s32[1,8]{1,0} %Arg_0.5), metadata={source_file="-" source_line=11}
//     }
...
//       %Arg_0.1 = s32[8,8]{1,0} parameter(0)
//       %custom-call.3 = s32[1,8]{1,0} custom-call(s32[8,8]{1,0} %Arg_0.1), custom_call_target="SPMDFullToShardShape", sharding={manual}, metadata={source_file="-" source_line=4}
//       %call.7 = s32[1,8]{1,0} call(s32[1,8]{1,0} %custom-call.3), to_apply=%my_model.___call__.fwd.zkx.sdy.manual_computation_body_14.1234
//       ROOT %custom-call.9 = s32[8,8]{1,0} custom-call(s32[1,8]{1,0} %call.7), custom_call_target="SPMDShardToFullShape", sharding={devices=[8,1]<=[8]}, metadata={source_file="-" source_line=7}
//     })";
//   TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(hloString));
//   module->mutable_config().set_use_shardy_partitioner(true);
...
//             "my_model.___call__.fwd.zkx.sdy.manual_computation_body_14.1234");
// }
// clang-format on
...
TEST_F(CallInlinerTest, DISABLED_DontInlineStreamAnnotationCall) {
  ...
  %sub (lhs: s32[]) -> s32[] {
    %lhs = s32[] parameter(0)
    %rhs = s32[] constant(1)
    ROOT %sub = s32[] subtract(s32[] %lhs, s32[] %rhs)
  }

  ENTRY %main () -> s32[] {
    %lhs = s32[] constant(42)
    %call1 = s32[] call(s32[] %lhs), to_apply=%sub, frontend_attributes={_zkx_stream_annotation="1"}
    ROOT %call2 = s32[] call(s32[] %call1), to_apply=%add
  })";

  auto debug_options = HloHardwareIndependentTestBase::GetDebugOptionsForTest();
  debug_options.set_zkx_gpu_experimental_stream_annotation(true);
  auto module = ParseAndReturnVerifiedModule(hlo_string).value();
  ...
  //CHECK: %lhs.2 = s32[] constant(42)
  //CHECK: %call1 = s32[] call(s32[] %lhs.2), to_apply=%sub, frontend_attributes={_zkx_stream_annotation="1"}
  //CHECK: %rhs.2 = s32[] constant(2)
  //CHECK: ROOT %add.1 = s32[] add(s32[] %call1, s32[] %rhs.2)
  )");
...
TEST_F(CallInlinerTest,
       DISABLED_ControlDepsPropagateToRootOfInlinedInstructions) {
  const char* hlo = R"(
  HloModule test
