...
#include "zkx/service/compile_time_cap.h"
...
bool IndicesToCopyForWhile(const HloDataflowAnalysis& dataflow,
                           const HloInstruction* zkx_while,
                           ShapeTree<bool>* indices_to_copy) {
  DCHECK(ShapeUtil::Compatible(indices_to_copy->shape(), zkx_while->shape()));

  bool any_copies = false;
  const HloInstruction* init = zkx_while->operand(0);
  for (auto& pair : *indices_to_copy) {
    ...
    if (IsSendRecvInInit(init, index)) {
      ...
      continue;
               ...
               dataflow.GetValueSet(zkx_while, index).values().size() > 1) {
      ...
      should_copy = dataflow.GetUniqueValueAt(zkx_while, index) !=
...
bool IndicesToCopyForConditional(const HloDataflowAnalysis& dataflow,
  ...
  for (auto& pair : *indices_to_copy) {
    ...
    CHECK_EQ(dataflow.GetValueSet(zkx_conditional, index).values().size(), 1);

    auto value = dataflow.GetValueSet(zkx_conditional, index).values()[0];
    // The conditional must be copied if the value is a phi.
    should_copy =
        value->is_phi() && value->defining_instruction() == zkx_conditional;
    any_copies |= should_copy;
  }
  return any_copies;
}
...
absl::Status AddCopiesForWhile(const HloAliasAnalysis& alias_analysis,
                               HloInstruction* zkx_while) {
  VLOG(2) << "Adding copies for kWhile instruction " << zkx_while->name();
  TF_RET_CHECK(zkx_while->opcode() == HloOpcode::kWhile);

  ShapeTree<bool> indices_to_copy(zkx_while->shape());
  if (!IndicesToCopyForWhile(alias_analysis.dataflow_analysis(), zkx_while,
                             &indices_to_copy)) {
  ...
  VLOG(2) << "Adding copies for " << zkx_while->name() << " at indices:";
  for (auto& pair : indices_to_copy) {
    if (pair.second) {
      VLOG(2) << "  " << pair.first;
    }
  ...
  // Deep copy init.
  HloInstruction* while_init = zkx_while->mutable_operand(0);
  TF_ASSIGN_OR_RETURN(
      HloInstruction * while_init_copy,
      zkx_while->parent()->DeepCopyInstruction(while_init, &indices_to_copy));
  TF_RETURN_IF_ERROR(while_init->ReplaceUseWith(zkx_while, while_init_copy));
  ...
  HloComputation* body = zkx_while->while_body();
 ...
 public:
  ...
  std::string ToString() const {
    ...
    for (const auto* computation : computation_vector_) {
      absl::StrAppend(&result, "computation: ", computation->name(), "\n");
      for (const auto& entry : computation_map_.at(computation)) {
        absl::StrAppend(&result, "  entry: ", entry.first->name(), ", ",
...
class ComputeRelativeLocation {
 public:
  ...
  Relation Compute(const InstructionEntry& entry1,
    ...
    //  branch_0 {
    //    exp = s64[] exp(...)
    //    ROOT tuple = (s64[], s64[]) tuple(exp, exp)
    //  }
    //
    //  fused_computation {
    //    abs = s64[] abs(...)
    //    negate = s64[] negate(...)
    //    ROOT tuple = (s64[], s64[]) tuple(abs, negate)
    //  }
    //
    //  branch_1 {
    //    ROOT fusion = (s64[], s64[]) fusion(...), calls=%fused_computation
    //  }
    //
    //  ENTRY main {
    //    ROOT root = (s64[], s64[]) conditional(...),
...
class CopyRemover {
 public:
  ...
  bool TryElideCopy(const HloInstruction* copy,
    ...
    *region_analysis_limit = 0;
    ...
    if (copy_node.src->next == copy_node.dest) {
      ...
      // the live ranges at and after s_{x+1}, and it is safe to order all uses
  ...
  std::string ValueListToString(const ValueNode* element) {
    ...
    auto VisitValueNode = [&](const ValueNode* node) {
      if (result == "{") {
        absl::StrAppend(&result, node->value->ToShortString());
      } else {
        absl::StrAppend(&result, ", ", node->value->ToShortString());
      }
    };
  ...
  std::string ToString() const {
    ...
    absl::StrAppend(&out, "  Def-use chains in each buffer:\n");
    for (const ValueNode* head : value_lists_) {
      absl::StrAppend(&out, "    Buffer defined by ",
                      head->value->ToShortString(), ":\n");
      const ValueNode* p = head;
      do {
        absl::StrAppend(&out, "      ", p->value->ToShortString(), ", uses: ",
                        absl::StrJoin(p->uses, "; ",
                                      [](std::string* s, const HloUse* use) {
                                        absl::StrAppend(s, use->ToString());
                                      }),
                        "\n");
