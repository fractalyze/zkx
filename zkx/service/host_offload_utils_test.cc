...
#include "zkx/service/host_offload_utils.h"
...
HloModule my_module
ENTRY main {
  data_param = s32[1,2048,2048] parameter(0)
  index_param = s32[] parameter(1)
  constant_s32_0 = s32[] constant(0)
  broadcast = s32[2,2048,2048] broadcast(constant_s32_0), dimensions={}
  offload_custom_call = s32[1,2048,2048] custom-call(data_param), custom_call_target="MoveToHost"
  dynamic_update_slice = s32[2,2048,2048] dynamic-update-slice(broadcast, offload_custom_call, index_param, constant_s32_0, constant_s32_0)
  dynamic_slice = s32[1,2048,2048] dynamic-slice(dynamic_update_slice, index_param, constant_s32_0, constant_s32_0), dynamic_slice_sizes={1,2048,2048}
  ROOT load_custom_call = s32[1,2048,2048] custom-call(dynamic_slice), custom_call_target="MoveToDevice"
}
)";
...
HloModule my_module
other_computation {
  param_0 = s32[2048] parameter(0)
  param_1 = s32[2048] parameter(1)
  ROOT tuple = (s32[2048], s32[2048]) tuple(param_0, param_1)
}
ENTRY main {
  data_param = s32[2048] parameter(0)
  other_param = s32[2048] parameter(1)
  offload_custom_call = s32[2048] custom-call(data_param), custom_call_target="MoveToHost"
  call = (s32[2048], s32[2048]) call(offload_custom_call, other_param), to_apply=other_computation
  gte_0 = s32[2048] get-tuple-element(call), index=0
  gte_1 = s32[2048] get-tuple-element(call), index=1
  ROOT load_custom_call = s32[2048] custom-call(gte_0), custom_call_target="MoveToDevice"
}
)";
