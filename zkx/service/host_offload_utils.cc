...
using ::zkx::host_memory_offload_annotations::kMoveToDeviceCustomCallTarget;
using ::zkx::host_memory_offload_annotations::kMoveToHostCustomCallTarget;
...
bool ComputeTypeIsHost(const HloInstruction* hlo_instruction) {
  ...
  return (frontend_attributes_map.find(kZkxComputeTypeAttr) !=
              frontend_attributes_map.end() &&
          frontend_attributes_map.find(kZkxComputeTypeAttr)->second ==
              kZkxComputeTypeHost);
}
