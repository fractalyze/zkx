...
class SubcomputationInsertionVisitor : public DfsHloVisitorWithDefault {
 ...
 private:
  ...
  HloInstruction* call_;   // not owned
  HloComputation* outer_;  // not owned
...
bool InlineStreamAnnotation(HloInstruction* instruction) {
  if (instruction->GetModule()
          ->config()
          .debug_options()
          .zkx_gpu_experimental_stream_annotation()) {
    if (instruction->frontend_attributes().map().contains(
            kZkxStreamAnnotationAttr)) {
      return false;
    }
  }
  return true;
...
// static
absl::StatusOr<CallInliner::InlinedInstructionMap> CallInliner::Inline(
    HloInstruction* call) {
  ...
  if (call->has_frontend_attributes()) {
    ...
    std::string has_fuse;
    if (call_attributes.map().contains("MUST_FUSE")) {
      has_fuse = "MUST_FUSE";
    } else if (call_attributes.map().contains("MAXIMAL_FUSE")) {
      has_fuse = "MAXIMAL_FUSE";
    }

    if (!has_fuse.empty()) {
      const auto& fuse_value = call_attributes.map().at(has_fuse);
      for (auto instruction : callee->instructions()) {
        ...
        if (instruction->IsFusible()) {
          ...
          frontend_attributes.mutable_map()->insert({has_fuse, fuse_value});
    ...
    for (HloInstruction* instruction :
      ...
      if (IsInlineableCallOp(instruction)) {
        ...
        if (!single_call_site_ || call_graph->GetNode(instruction->to_apply())
          ...
          if (update_domain_) {
            // clang-format off
            // TODO(chokobole): Uncomment this. Dependency: HloDomainIsolator
            // clang-format on
