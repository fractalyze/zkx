...
#include "zkx/hlo/transforms/collectives/async_collective_creator.h"
...
#include "zkx/service/gpu/transforms/collectives/collective_ops_utils.h"
...
#include "zkx/service/profile_guided_latency_estimator.h"
#include "zkx/shape.h"
...
bool ShouldScheduleSuccessor(const HloInstruction& successor,
                             const HloPredicate& is_scheduled) {
  return ShouldScheduleAsEarlyAsPossible(successor) &&
         absl::c_all_of(successor.operands(), is_scheduled) &&
         absl::c_all_of(successor.control_predecessors(), is_scheduled);
}
...
HloInstructionSequence PostprocessorToScheduleAsEarlyOrLateAsPossible(
  ...
  std::vector<HloInstruction*> earliest_scheduled;
  {
    ...
    auto add_to_schedule = [&](HloInstruction* instr) -> void {
  ...
  CHECK_EQ(input.instructions().size(), result.size())
...
HloInstructionSequence PostprocessorToScheduleSyncCollectives(
  ...
  auto is_sync_start = [](const HloInstruction* instr) -> bool {
...
SchedulerConfig MakeGPUSchedulerConfig(int64_t memory_limit,
  ...
  CHECK_LE(config.collective_broadcast_overlap_limit,
           config.parallel_collective_overlap_limit);
  CHECK_LE(config.all_to_all_overlap_limit,
           config.parallel_collective_overlap_limit);
  CHECK_LE(config.all_gather_overlap_limit,
           config.parallel_collective_overlap_limit);
  CHECK_LE(config.all_reduce_overlap_limit,
           config.parallel_collective_overlap_limit);
  CHECK_LE(config.reduce_scatter_overlap_limit,
           config.parallel_collective_overlap_limit);
...
ProfiledInstructionsProto FilterWithFingerprint(
    const ProfiledInstructionsProto& profile, std::string_view fingerprint) {
  ...
  for (const auto& cost : profile.costs()) {
    ...
    std::string_view cost_sep = "::";
    if (absl::StrContains(cost.name(), cost_sep)) {
      std::vector<std::string_view> split_names =
  ...
  auto strip_remat_suffix = [](std::string_view name) -> std::string_view {
    std::string_view suffix = ".remat";
    size_t index = name.rfind(suffix);
    if (index == std::string::npos) {
      return name;
    }
    std::string_view after_suffix = name.substr(index + suffix.size());
  ...
  absl::flat_hash_map<std::string_view, Data> costs;
...
std::optional<ProfiledInstructionsProto> ProfileFromConfig(
  ...
  std::string_view from_config = config.fdo_profile();
  ...
  if (google::protobuf::TextFormat::ParseFromString(
          // NOTE: ZKX Linux ARM64 breaks without this explicit conversion.
          std::string(from_config), &profile)) {
...
std::optional<ProfiledInstructionsProto> ReadProfileFromSources(
    const HloModuleConfig& config, std::string_view fingerprint) {
  if (auto profile = ProfileFromConfig(config); profile) {
    return profile;
  }

  const std::string& path =
      config.debug_options().zkx_gpu_pgle_profile_file_or_directory_path();
  if (path.empty()) {
    return std::nullopt;
  }
  ...
  // Try path as a file inferring the file type based on the extension.
...
std::optional<ProfiledInstructionsProto> ReadPGLEProfile(
  ...
  std::optional<ProfiledInstructionsProto> profile =
      ReadProfileFromSources(config, fingerprint);
...
absl::Status RunP2PSchedulePreparation(HloModule* module) {
  // TODO(chokobole): Uncomment this. Dependency: P2PSchedulePreparation
  // if (!module->config().debug_options().zkx_gpu_enable_pipelined_p2p()) {
...
std::unique_ptr<LatencyEstimator> GetLatencyEstimator(
    ...
    const se::DeviceDescription& gpu_device_info, std::string_view fingerprint,
  ...
  // TODO(chokobole): Uncomment this. Dependency: AnalyticalLatencyEstimator
  // if (options.zkx_gpu_enable_analytical_latency_estimator()) {
  ...
  // TODO(chokobole): Uncomment this. Dependency: SolLatencyEstimator
  // if (options.zkx_gpu_enable_analytical_sol_latency_estimator()) {
...
bool NeedAccuracyChecker(const DebugOptions& options,
  ...
  if (dynamic_cast<const ProfileGuidedLatencyEstimator*>(&latency_estimator) ==
      nullptr) {
    return false;
  }
  DebugOptions::PGLEStrictnessLevel level =
      options.zkx_gpu_pgle_accuracy_checker();
...
absl::Status RunLatencyHidingSchedulerPasses(
    HloModule* module, int pointer_size, std::string_view fingerprint,
  ...
  SchedulerConfig config = MakeGPUSchedulerConfig(
      memory_limit,
      options.zkx_gpu_experimental_parallel_collective_overlap_limit());
...
bool IsLHSEnabled(const HloModule& module, std::string_view fingerprint) {
  bool enable_lhs =
      module.config()
          .debug_options()
          .zkx_gpu_enable_latency_hiding_scheduler() ||
  ...
  if (!enable_lhs && HasValidPGLEProfile(module, fingerprint)) {
    LOG(WARNING)
        << "Profile data detected but "
           "`zkx_gpu_enable_latency_hiding_scheduler` unset. To use it "
...
absl::Status RunAsyncCollectivesConversionPasses(HloModule* module) {
  ...
  for (auto collective_op_type :
       module->config().debug_options().zkx_gpu_disable_async_collectives()) {
  ...
  auto convert_to_async =
      [&disabled_async_ops](const HloInstruction* inst) -> bool {
    switch (inst->opcode()) {
      case HloOpcode::kAllReduceStart:
...
absl::StatusOr<ScheduleMetadata> ScheduleGpuModule(
  ...
  // See `LatencyHidingScheduler::Run`.
