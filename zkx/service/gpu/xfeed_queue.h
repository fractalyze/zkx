...
#ifndef ZKX_SERVICE_GPU_XFEED_QUEUE_H_
#define ZKX_SERVICE_GPU_XFEED_QUEUE_H_
...
class XfeedQueue {
 public:
  ...
  BufferType BlockingGetNextDestination() {
    for (const std::function<void()>& callback :
         before_get_next_dest_callbacks_) {
      callback();
    }
    ...
    if (became_empty) {
      for (const std::function<void()>& callback : on_empty_callbacks_) {
        callback();
      }
    }
...
#endif  // ZKX_SERVICE_GPU_XFEED_QUEUE_H_
