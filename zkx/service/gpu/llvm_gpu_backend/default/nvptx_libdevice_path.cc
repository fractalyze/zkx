...
std::string GetLibdeviceDir(std::string_view zkx_gpu_cuda_data_dir) {
  for (const std::string& cuda_root :
       tsl::CandidateCudaRoots(std::string{zkx_gpu_cuda_data_dir})) {
...
std::string LibDevicePath(std::string_view zkx_gpu_cuda_data_dir) {
  ...
  static auto& libdevice_dir_path_cache ABSL_GUARDED_BY(libdevice_cache_mu) =
      *absl::IgnoreLeak(new absl::flat_hash_map<std::string, std::string>());
  std::string libdevice_dir_path = [&] {
    absl::MutexLock l(&libdevice_cache_mu);
    auto it = libdevice_dir_path_cache.find(zkx_gpu_cuda_data_dir);
    if (it != libdevice_dir_path_cache.end()) {
      return it->second;
    }
    auto [it2, inserted] = libdevice_dir_path_cache.emplace(
        zkx_gpu_cuda_data_dir, GetLibdeviceDir(zkx_gpu_cuda_data_dir));
    return it2->second;
  }();
