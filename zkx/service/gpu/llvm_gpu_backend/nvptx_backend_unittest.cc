...
TEST(UtilsTest, TestGetSmName) {
  ASSERT_EQ(GetSmName(se::CudaComputeCapability{9, 0}), "sm_90a");
  ASSERT_EQ(GetSmName(se::CudaComputeCapability{10, 0}), "sm_100a");
  ASSERT_EQ(GetSmName(se::CudaComputeCapability{10, 1}), "sm_101a");
  ASSERT_EQ(GetSmName(se::CudaComputeCapability{12, 0}), "sm_120a");
  ...
  ASSERT_EQ(GetSmName(se::CudaComputeCapability{12, 9}), "sm_120");
}
...
TEST_P(PtxVersionFromCudaVersionTest, VerifyMapping) {
  EXPECT_EQ(
      DetermineHighestSupportedPtxVersionFromCudaVersion(GetParam().first),
      GetParam().second);
}
