...
constexpr int64_t kCostlyAllReduceThreshold = 30 * 1024 * 1024;

// Multiplier which we apply to expand the base cost for the costly AR.
constexpr int64_t kCostlyAllReduceMultiplier = 4;
...
size_t CountOverlappingRanks(const std::vector<ReplicaGroup>& group,
  ...
  for (const ReplicaGroup& curr_replica_group : group) {
    absl::flat_hash_set<int> curr_replica_ids;
    for (int64_t curr_replica_id : curr_replica_group.replica_ids()) {
      curr_replica_ids.insert(curr_replica_id);
    }

    for (const ReplicaGroup& replica_group : other) {
...
bool IsPartiallyPipelinedSendRecvDone(const HloInstruction* instr) {
...
bool IsPartiallyPipelinedSendRecv(const HloInstruction* instr) {
...
void GpuAsyncTrackerBase::PostProcessScheduleGraph(
  ...
  const DebugOptions& debug_options = schedule_graph->GetOriginalInstrList()
                                          .front()
                                          ->GetModule()
                                          ->config()
                                          .debug_options();
  ...
  for (const HloInstruction* inst : schedule_graph->GetOriginalInstrList()) {
    ...
    if (debug_options.zkx_gpu_enable_pipelined_p2p() &&
...
std::string_view GpuAsyncTracker::GetResourceName(int64_t resource_type) const {
  CHECK_LT(resource_type,
           ResourceTypeToIndex(GpuResourceType::kGpuResourceTypeEnd));
  if (resource_type < GetTargetDefinedResourceTypeBegin()) {
...
ApproximateLatencyEstimator::TimeCost GpuLatencyEstimator::NodeCost(
  ...
  if (instr->opcode() == HloOpcode::kCustomCall) {
    // Consider custom calls as medium cost for now. Keeping the case
