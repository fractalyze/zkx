...
PersistentCliquesMap& GetPersistentCliquesMap() {
...
absl::Status ResourceRequests::AddClique(const GpuCliqueKey& clique_key,
  ...
  // ZKX compiler guarantees that all collective operations have the same
...
ResourceRequests::AcquireCollectiveCliques(
  ...
  for (const CliqueRequest& r : ordered_cliques) {
    ...
    // of ZKX, as we might observe partial state and some of the replicas will
    ...
    if (use_persistent_cliques) {
      PersistentCliquesMap& pc = GetPersistentCliquesMap();
      absl::MutexLock lock(&pc.mutex);
