...
TEST_F(PGLEAccuracyCheckerTest,
  ...
  const std::string_view kHloString = R"(
  HloModule m
  ...
  ENTRY ar {
    ...
    p1 = u32[32,32] parameter(1)
    p2 = u32[32,32] parameter(2)
    p3 = u32[32] parameter(3)

    dot0 = u32[32,32]{1,0} dot(p1, p2), lhs_contracting_dims={1}, rhs_contracting_dims={0}
    dot1 = u32[32,32]{1,0} dot(p1, p2), lhs_contracting_dims={1}, rhs_contracting_dims={0}
    add0 = u32[32,32] add(dot0, dot1)

    ar-start = u32[32] all-reduce-start(p0), to_apply=apply_op
    ar-done = u32[32] all-reduce-done(ar-start)

    ar-start1 = u32[32] all-reduce-start(p3), to_apply=apply_op
    ar-done1 = u32[32] all-reduce-done(ar-start1)

    ROOT _ = (u32[32],u32[32],u32[32,32]) tuple(ar-done, ar-done1, add0)
  })";
...
TEST_F(PGLEAccuracyCheckerTest,
  ...
  module->mutable_config()
      ...
      .set_zkx_gpu_pgle_accuracy_checker(
          DebugOptions::PGLE_STRICTNESS_LEVEL_ERROR);

  std::unique_ptr<ProfileGuidedLatencyEstimator> pgle_estimator =
      GetProfileGuidedLatencyEstimator(profile);
  PGLEAccuracyChecker pgle_accuracy_checker(*pgle_estimator);
