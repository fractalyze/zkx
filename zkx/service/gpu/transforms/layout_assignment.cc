  ...
  for (auto iterator = post_order.rbegin(); iterator != post_order.rend();
    ...
    if (HloPredicateIsOp<HloOpcode::kDot>(instruction)) {
      ...
      *shape.mutable_layout() =
      ...
      Shape shape = ars->shape();
      std::vector<int64_t> minor_to_major;
      for (int64_t i = shape.rank() - 1; i >= 0; --i) {
        if (i != ars->scatter_dimension()) {
          minor_to_major.push_back(i);
        }
      }
      minor_to_major.push_back(ars->scatter_dimension());
      *shape.mutable_layout() = LayoutUtil::MakeLayout(minor_to_major);
      TF_RETURN_IF_ERROR(SetInstructionLayout(shape, ars));
    } else if (HloPredicateIsOp<HloOpcode::kAllGather>(instruction)) {
      auto ag = Cast<HloAllGatherInstruction>(instruction);
      Shape shape = ag->shape();
      std::vector<int64_t> minor_to_major;
      for (int64_t i = shape.rank() - 1; i >= 0; --i) {
        if (i != ag->all_gather_dimension()) {
          minor_to_major.push_back(i);
        }
      }
      minor_to_major.push_back(ag->all_gather_dimension());
      *shape.mutable_layout() = LayoutUtil::MakeLayout(minor_to_major);
      TF_RETURN_IF_ERROR(SetInstructionLayout(shape, ag));
    } else if (HloPredicateIsOp<HloOpcode::kAllToAll>(instruction) &&
               instruction->shape().IsArray()) {
      auto* all_to_all = Cast<HloAllToAllInstruction>(instruction);
      if (all_to_all->split_dimension().has_value()) {
        Shape shape = all_to_all->shape();
        ...
        minor_to_major.push_back(split_dim);
        *shape.mutable_layout() = LayoutUtil::MakeLayout(minor_to_major);
        TF_RETURN_IF_ERROR(SetInstructionLayout(shape, all_to_all));
      }
...
absl::Status GpuLayoutAssignment::SetDotOperandLayout(
  ...
  // Always use major-to-minor layout ordering.
  // Note: MatrixLayout validation removed as it's cuBLAS-specific.
...
absl::Status GpuLayoutAssignment::SetDotLayout(
  ...
  // Try to use constraint from users if available.
  // Note: XLA's DotCanSupportShapeWithLayout validation was removed because
  // it relies on cuBLAS MatrixLayout which is not applicable to ZK workloads.
  // For ZK, we trust that the constraint layout is valid since there's no
  // cuBLAS compatibility requirement.
