...
class PriorityFusionQueue {
 ...
 public:
  ...
  // Returns true if there is the next producer to fuse, otherwise false.
  ...
  absl::Status UpdatePriorities() {
    // Revisit costs of all updated ops.
  ...
  // Prepares producer and consumer instruction to be fused.
  ...
  // Invalidates all cached value related to this instruction.
 ...
 private:
  ...
  Priority CalculateProducerPriority(HloInstruction* producer) {
    ...
    // well. Therefore, we fuse all of them at the end.
  ...
  FusionDecision CanFuse(HloInstruction* producer, HloInstruction* consumer) {
    ...
    // Check that our emitter can handle the fusion node.
  ...
  // The priority queue of producers.
  ...
  // The set of producers whose priorities need to be updated.
  ...
  // Proto with structured logs of fusion decisions.
  ...
  // Caches result of can_fuse for a (producer, consumer) pair.
  absl::flat_hash_map<
...
bool PriorityFusion::ConsumeFuel(HloInstruction* producer,
                                 HloInstruction* consumer) {
  return zkx::ConsumeFuel(name(), /*ran_out_of_fuel_msg=*/[&] {
...
absl::StatusOr<bool> PriorityFusion::Run(
  ...
  for (auto* computation : fusible_computations) {
    ...
    while (fusion_queue->DequeueNextProducer()) {
      ...
      for (auto* consumer : fusion_queue->current_consumers()) {
        // Don't fuse into single bitcasts.
        if (HloPredicateIsOp<HloOpcode::kBitcast>(consumer)) {
          continue;
        }
  ...
  // Clear cache for future modules.
...
HloInstruction::FusionKind PriorityFusion::ChooseKind(
  ...
  // Derive kInput/kLoop fusion kinds from fusion analysis.
