...
TEST_F(FusionAnalysisCacheTest, CachesAndInvalidates) {
  std::string_view hlo_string = R"(
    ...
    f {
      c0 = s32[] constant(0)
      b0 = s32[1000] broadcast(c0)
      ROOT n0 = s32[1000] negate(b0)
    }

    ENTRY e {
      ROOT r.1 = s32[1000] fusion(), kind=kLoop, calls=f
    })";
...
TEST_F(FusionAnalysisCacheTest, CachesAndInvalidatesProducerConsumerFusions) {
  std::string_view hlo_string = R"(
    ...
    f {
      c0 = s32[] constant(0)
      b0 = s32[1000] broadcast(c0)
      ROOT r0 = s32[] reduce(b0, c0), dimensions={0}, to_apply=add
    }

    ENTRY e {
      f0 = s32[] fusion(), kind=kInput, calls=f
      ROOT n0 = s32[] negate(f0)
    })";
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          ParseAndReturnUnverifiedModule(hlo_string));
