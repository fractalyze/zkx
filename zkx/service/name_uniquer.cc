...
// static
std::string NameUniquer::GetSanitizedName(std::string_view name) {
  if (name.empty()) {
    return "";
  }
  ...
  if (absl::StartsWith(result, "__") && !absl::StartsWith(result, "__zkx_")) {
    // Morph name prefix __ that is not __zkx_, to avoid using name prefixes
...
std::string NameUniquer::GetUniqueName(std::string_view prefix) {
  std::string root =
