...
HloModuleConfigProto::BoolList BoolVectorToProto(
...
void AssignStructFusionConfig(HloModuleConfig& config,
...
void AssignStructDotConfig(HloModuleConfig& config,
                           const HloModuleConfigProto& proto) {
...
void AssignStructLayoutConfig(HloModuleConfig& config,
                              const HloModuleConfigProto& proto) {
...
void AssignStructPhaseOrderingConfig(HloModuleConfig& config,
                                     const HloModuleConfigProto& proto) {
  std::vector<std::vector<bool>> module_config;
...
std::string HloModuleConfig::compilation_cache_key() const {
  ...
  if (entry_computation_layout_.has_value()) {
    ...
    absl::StrAppend(
        &key, absl::StrJoin(params, ", "), ") => ",
        entry_computation_layout_->result_shape().SerializeAsString());
  }
  if (seed() != 0) {
    static std::atomic<int> counter{0};
    absl::StrAppend(&key, "forcing recompile ", counter++);
  }
  absl::StrAppend(&key, "::exec_time_optimization_effort=",
                  exec_time_optimization_effort());
  absl::StrAppend(&key, "::memory_fitting_effort=", memory_fitting_effort());
  if (replica_count() != 1) {
    absl::StrAppend(&key, "::replica_count=", replica_count());
  }
  absl::StrAppend(&key, debug_options_.DebugString());
  if (intra_op_parallelism_threads() > 0) {
    absl::StrAppend(&key, "::intra_op_parallelism_threads=",
                    intra_op_parallelism_threads());
  }
  if (!device_type().empty()) {
    absl::StrAppend(&key, device_type());
  }
  absl::StrAppend(&key,
                  "::alias_passthrough_params=", alias_passthrough_params_);
  absl::StrAppend(
      &key, "::allow_spmd_sharding_propagation_to_parameters={",
      absl::StrJoin(allow_spmd_sharding_propagation_to_parameters_, ","), "}");
  absl::StrAppend(
      &key, "::allow_spmd_sharding_propagation_to_output={",
      absl::StrJoin(allow_spmd_sharding_propagation_to_output_, ","), "}");
  if (!fdo_profile().empty()) {
    absl::StrAppend(&key,
                    "::fdo_profile=", absl::BytesToHexString(fdo_profile()));
  }
  if (device_memory_size() != 0) {
    absl::StrAppend(&key, "::device_memory_size=", device_memory_size());
  }
  absl::StrAppend(&key, "::use_shardy_partitioner=", use_shardy_partitioner());
  return key;
}
...
void HloModuleConfig::AssignStructShardableValueUpdatePairs(
    ...
    const google::protobuf::RepeatedPtrField<ShardableValueUpdatePairProto>&
        pairs) {
