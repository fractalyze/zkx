...
TEST_F(FusionNodeIndexingEvaluationTest, FuseTwoInstructions) {
  ...
  ENTRY entry_computation {
    ...
    add = s32[4,3]{1,0} add(p0, p0)
    ROOT sub = s32[4,3]{1,0} subtract(add, p0)
  })")
                    .value();
...
TEST_F(FusionNodeIndexingEvaluationTest, FuseThreeInstructions) {
  ...
  ENTRY entry_computation {
    ...
    slice1 = s32[3]{0} slice(p0), slice={[0:3]}
    slice2 = s32[3]{0} slice(p0), slice={[0:3]}
    ROOT sub = s32[3]{0} subtract(slice1, slice2)
  })")
                    .value();
  HloInstruction* sub = module->entry_computation()->root_instruction();
...
TEST_F(FusionNodeIndexingEvaluationTest, ExponentialDuplicationPattern) {
  ...
  ENTRY entry_computation {
    ...
    add0 = s32[4]{0} add(p0, p1)
    slice1.0 = s32[3]{0} slice(add0), slice={[0:3]}
    slice1.1 = s32[3]{0} slice(add0), slice={[1:4]}
    add1 = s32[3]{0} add(slice1.0, slice1.1)
    slice2.0 = s32[2]{0} slice(add1), slice={[0:2]}
    slice2.1 = s32[2]{0} slice(add1), slice={[1:3]}
    ROOT add2 = s32[2]{0} add(slice2.0, slice2.1)
  })")
                    .value();
...
TEST_F(FusionNodeIndexingEvaluationTest, RecomputeCache) {
  ...
  auto module = ParseAndReturnVerifiedModule(R"(
HloModule test_module
%fused_computation (param_0.5: s32[4]) -> s32[2] {
  %param_0.5 = s32[4]{0} parameter(0)
  %slice1.2 = s32[3]{0} slice(s32[4]{0} %param_0.5), slice={[0:3]}
  %slice1.3 = s32[3]{0} slice(s32[4]{0} %param_0.5), slice={[1:4]}
  %add1.1 = s32[3]{0} add(s32[3]{0} %slice1.2, s32[3]{0} %slice1.3)
  %slice2.2 = s32[2]{0} slice(s32[3]{0} %add1.1), slice={[0:2]}
  %slice2.3 = s32[2]{0} slice(s32[3]{0} %add1.1), slice={[1:3]}
  ROOT %add2.1 = s32[2]{0} add(s32[2]{0} %slice2.2, s32[2]{0} %slice2.3)
}
...
ENTRY entry_computation {
  ...
  p1 = s32[4]{0} parameter(1)
  add0 = s32[4]{0} add(p0, p1)
  ROOT %fusion = s32[2]{0} fusion(add0), kind=kLoop, calls=%fused_computation
})")
                    .value();
...
ENTRY entry_computation {
  p0 = s32[] parameter(0)
  add = s32[] add(p0, p0)
  broadcast = s32[6]{0} broadcast(add), dimensions={}
  ROOT %fusion = s32[2]{0} fusion(broadcast), kind=kLoop, calls=%fused_computation
})")
                    .value();
  HloInstruction* fusion = module->entry_computation()->root_instruction();
  InstructionFusionForTesting instruction_fusion;
