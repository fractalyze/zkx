...
TEST_F(CallGraphTest, ComputationWithConditional) {
  ...
  HloComputation* true_computation =
      module->AddEmbeddedComputation(MakeScalarComputation(HloOpcode::kNegate));
  HloComputation* false_computation =
      module->AddEmbeddedComputation(MakeScalarComputation(HloOpcode::kAbs));
...
TEST_F(CallGraphTest, ComplexGraphNearestAncestors) {
  ...
  HloInstruction* entry_while;
  {
    ...
    entry_while = builder.AddInstruction(HloInstruction::CreateWhile(
        kScalarShape, cond_computation, a_computation, param0));
    std::ignore = module->AddEntryComputation(builder.Build());
  }
...
TEST_F(CallGraphTest, NearestCommonAncestorInstructions) {
  ...
  HloModule module
    ENTRY computation {
      ...
      mul.0 = s32[10] multiply(p.1, p.2)
      sub.0 = s32[10] subtract(add.0, mul.0)
      add.1 = s32[10] add(add.0, p.2)
      ROOT add.2 = s32[10] add(sub.0, add.1)
    }
  )";
...
TEST_F(CallGraphTest, VisitWithError) {
  ...
  absl::Status status = call_graph->VisitNodes([](const CallGraphNode&) {
    return absl::InternalError("Visitation failed");
  });
