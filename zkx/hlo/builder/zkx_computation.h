...
#ifndef ZKX_HLO_BUILDER_ZKX_COMPUTATION_H_
#define ZKX_HLO_BUILDER_ZKX_COMPUTATION_H_
...
// The computation graph that the user builds up with the ZkxBuilder.
class ZkxComputation {
 public:
  ZkxComputation() : unique_id_(-1) {}
  ZkxComputation(HloModuleProto proto)
      : unique_id_(proto.id()), proto_(std::move(proto)) {}

  ~ZkxComputation() = default;

  ZkxComputation(const ZkxComputation&) = delete;
  ZkxComputation& operator=(const ZkxComputation&) = delete;

  ZkxComputation(ZkxComputation&& from) = default;

  ZkxComputation& operator=(ZkxComputation&& from) = default;
 ...
 private:
  ZkxComputation(const int64_t unique_id) : unique_id_(unique_id) {}
...
#endif  // ZKX_HLO_BUILDER_ZKX_COMPUTATION_H_
