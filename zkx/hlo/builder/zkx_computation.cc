...
absl::StatusOr<ProgramShape> ZkxComputation::GetProgramShape() const {
  TF_RET_CHECK(proto_.has_host_program_shape());
  return ProgramShape(proto_.host_program_shape());
}

absl::StatusOr<std::unique_ptr<HloSnapshot>> ZkxComputation::Snapshot() const {
  if (IsNull()) {
    return absl::InvalidArgumentError("Computation is invalid.");
  }
