...
Shape TypeToShape(mlir::Type type) {
  PrimitiveType ptype = mlir_utils::MlirTypeToPrimitiveTypeWithSign(type);
  ...
  if (type.isIntOrFloat()) {
    ...
    PrimitiveType primitive_type =
        mlir_utils::MlirTypeToPrimitiveTypeWithSign(element_type);
    ...
    llvm::SmallVector<int64_t, 4> dimensions(m.getShape().begin(),
                                             m.getShape().end());
    return ShapeUtil::MakeShapeWithDenseLayout(primitive_type, dimensions,
                                               minor_to_major);
    ...
    PrimitiveType primitive_type =
        mlir_utils::MlirTypeToPrimitiveTypeWithSign(t.getElementType());
    if (primitive_type == PrimitiveType::PRIMITIVE_TYPE_INVALID) return {};
    ...
    if (auto sparse = mlir::sparse_tensor::getSparseTensorEncoding(type)) {
      ...
      auto sparse_shape = ShapeUtil::MakeShapeWithSparseLayout(
  ...
  // TODO(chokobole): Uncomment this. Dependency: stablehlo::TokenType
  // else if (mlir::isa<mlir::stablehlo::TokenType>(type)) {
  //   return ShapeUtil::MakeTokenShape();
  // }
  // TODO(chokobole): Uncomment this. Dependency: mhlo::AsyncBundleType
  ...
  // Return empty ZKX shape to signify error. No MLIR Type maps to a empty
  // Shape.
  return {};
}
