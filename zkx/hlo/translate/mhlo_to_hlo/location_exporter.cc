...
std::string GetNameFromLocImpl(Location loc) {
...
void SetSourceFileAndLine(Location loc, zkx::OpMetadata& metadata) {
  if (auto file_line_col_loc = dyn_cast<FileLineColLoc>(loc)) {
...
zkx::OpMetadata CreateOpMetadataFromLocation(
    Operation* op, StackFrameIndexBuilder* frame_index_builder) {
  zkx::OpMetadata metadata;
  Location loc = op->getLoc();
  if (isa<UnknownLoc>(loc)) return metadata;
...
std::string GetDebugNameFromLocation(Location loc) {
