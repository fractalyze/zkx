...
#ifndef ZKX_HLO_TRANSLATE_HLO_TO_MHLO_HLO_UTILS_H_
#define ZKX_HLO_TRANSLATE_HLO_TO_MHLO_HLO_UTILS_H_
...
// Converts the given ZKX shape for tensors to the template MLIR type.
template <typename TypeT>
static absl::StatusOr<TypeT> ConvertTensorShapeToType(const Shape& zkx_ty,
                                                      mlir::Builder builder) {
  auto element_type = mlir_utils::PrimitiveTypeToMlirTypeWithSign(
      zkx_ty.element_type(), builder.getContext());

  bool is_bounded_dynamic = false;
  int64_t rank = zkx_ty.rank();
  ...
  for (int64_t dim = 0; dim < rank; ++dim) {
    int64_t dim_size = zkx_ty.dimensions(dim);
    if (zkx_ty.is_dynamic_dimension(dim)) {
      if (!zkx_ty.is_unbounded_dynamic_dimension(dim)) {
        bounds[dim] = dim_size;
  ...
  //
  // For example, we wouldn't be able to represent the zkx type
  // `u32[4,<=4]{1,0:D(D,C)}`.
  if (zkx_ty.has_layout()) {
    auto layout = zkx_ty.layout();
    if (LayoutUtil::IsSparse(layout)) {
      ...
      for (size_t i = 0, e = layout.dim_level_types_size(); i < e; ++i) {
        ...
        switch (dlt) {
          ...
          case DimLevelType::DIM_LOOSE_COMPRESSED:
            ...
            break;
          default:
            return absl::InvalidArgumentError("Unknown DimLevelType from HLO");
        }
      }
      ...
      auto id_map = mlir::AffineMap::getPermutationMap(major_to_minor,
                                                       builder.getContext());
      // TODO(atondwal): support sizes other than 32 when ZKX does
      encoding = SparseTensorEncodingAttr::get(
          builder.getContext(), lts, id_map, mlir::AffineMap(), 32, 32);
    }
  }
  return TypeT::get(shape, element_type, encoding);
}
...
// Converts the given ZKX shape to the template MLIR type.
template <typename TypeT>
...
inline llvm::StringRef ToStringRef(std::string_view str) {
  return llvm::StringRef(str.data(), str.size());
}

}  // namespace zkx

#endif  // ZKX_HLO_TRANSLATE_HLO_TO_MHLO_HLO_UTILS_H_
