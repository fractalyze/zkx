...
constexpr std::string_view kConstantAdditionHloString = R"(
HloModule test
ENTRY main {
  zero = u32[] constant(0)
  five = u32[] constant(5)
...
TEST_F(HloQueryTest, GetUniqueGteTest) {
  ...
  ENTRY main {
    param.0 = (u32[32]{0}, u32[32]{0}, u32[32]{0}, u32[32]{0}) parameter(0)
    gte1 = u32[32]{0} get-tuple-element(param.0), index=0
    gte2 = u32[32]{0} get-tuple-element(param.0), index=1
    dup_gte2 = u32[32]{0} get-tuple-element(param.0), index=1
    gte3 = u32[32]{0} get-tuple-element(param.0), index=2
    ROOT gte4 = u32[32]{0} get-tuple-element(param.0), index=3
  })";
...
void FindInstructionsAndExpectEqual(const HloComputation* main,
                                    std::string_view name, HloOpcode opcode) {
