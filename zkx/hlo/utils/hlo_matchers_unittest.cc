...
TEST_F(HloMatchersTest, Test) {
  auto shape = ShapeUtil::MakeShape(S32, {1});
  ...
  EXPECT_THAT(
      ...
      Eq("(%add = s32[1]{0} add(s32[1]{0} %param, s32[1]{0} %multiply))"));
  EXPECT_THAT(
  ...
  EXPECT_THAT(
      ...
      Eq("(%add = s32[1]{0} add(s32[1]{0} %param, s32[1]{0} %multiply))"
         "\noperand 1:\n\t"
         "%multiply = s32[1]{0} multiply(s32[1]{0} %param, s32[1]{0} %param)\n"
         "doesn't match expected:\n\t"
         "parameter"
         ", (%multiply = s32[1]{0} multiply(s32[1]{0} %param, s32[1]{0} "
         "%param))"));
  EXPECT_THAT(
      Explain(add.get(),
      ...
      Eq("(%add = s32[1]{0} add(s32[1]{0} %param, s32[1]{0} %multiply))"
         ...
         "doesn't match expected:\n\t"
         "add, (%param = s32[1]{0} parameter(0))"));
}

// TEST_F(HloMatchersTest, CustomCallMatcher) {
//   auto c1 =
//       HloInstruction::CreateConstant(LiteralUtil::CreateR1<float>({1, 2,
//       3}));
//   auto c2 =
//       HloInstruction::CreateConstant(LiteralUtil::CreateR1<int32_t>({1, 2,
//       3}));
//   auto call = HloInstruction::CreateCustomCall(
//       ShapeUtil::MakeShape(S32, {1}), {c1.get(), c2.get()}, "foo_target");
...
//               "(%custom-call = s32[1]{0} custom-call(s32[3]{0} %constant, "
...
//               R"(custom-call with call target that is equal to
//               "foo_target")");
// }

TEST_F(HloMatchersTest, ShapeMatcher) {
  auto p0 = HloInstruction::CreateParameter(
      0, ShapeUtil::MakeShapeWithDenseLayout(S32, {5, 7}, {0, 1}), "param");

  EXPECT_THAT(p0.get(), op::Shape(ShapeUtil::MakeShape(S32, {5, 7})));
  EXPECT_THAT(p0.get(), op::Shape("s32[5,7]"));
  EXPECT_THAT(
      p0.get(),
      ::testing::Not(op::ShapeWithLayout(ShapeUtil::MakeShape(S32, {5, 7}))));
  EXPECT_THAT(p0.get(), ::testing::Not(op::ShapeWithLayout("s32[5,7]")));
  EXPECT_THAT(p0.get(),
              ::testing::Not(op::Shape(ShapeUtil::MakeShape(S32, {7, 5}))));
  EXPECT_THAT(p0.get(), ::testing::Not(op::Shape("s32[7,5]")));
  EXPECT_THAT(
      p0.get(),
      ::testing::Not(op::ShapeWithLayout(ShapeUtil::MakeShape(S32, {7, 5}))));
  EXPECT_THAT(p0.get(), ::testing::Not(op::ShapeWithLayout("s32[7,5]")));
  EXPECT_THAT(p0.get(), op::Shape(ShapeUtil::MakeShapeWithDenseLayout(
                            S32, {5, 7}, {0, 1})));
  EXPECT_THAT(p0.get(), op::Shape("s32[5,7]{0,1}"));
  EXPECT_THAT(p0.get(), op::ShapeWithLayout(ShapeUtil::MakeShapeWithDenseLayout(
                            S32, {5, 7}, {0, 1})));
  EXPECT_THAT(p0.get(), op::ShapeWithLayout("s32[5,7]{0,1}"));
  EXPECT_THAT(p0.get(),
              ::testing::Not(op::ShapeWithLayout(
                  ShapeUtil::MakeShapeWithDenseLayout(S32, {5, 7}, {1, 0}))));
  EXPECT_THAT(p0.get(), ::testing::Not(op::ShapeWithLayout("s32[5,7]{1,0}")));

  EXPECT_THAT(Explain(p0.get(), op::Shape(ShapeUtil::MakeShape(S32, {7, 5}))),
              "%param = s32[5,7]{0,1} parameter(0) has incorrect shape "
              "(expected: s32[7,5])");
  EXPECT_THAT(
      Explain(p0.get(), op::ShapeWithLayout(ShapeUtil::MakeShapeWithDenseLayout(
                            S32, {7, 5}, {1, 0}))),
      "%param = s32[5,7]{0,1} parameter(0) has incorrect shape "
      "(expected: s32[7,5]{1,0})");
}

// clang-format off
// TODO(chokobole): Enable this test. Dependency: HloInstruction::PrintWithCanonicalNameMap
// clang-format on
TEST_F(HloMatchersTest, DISABLED_ShardingMatcher) {
  auto p0 = HloInstruction::CreateParameter(0, ShapeUtil::MakeShape(S32, {5}),
                                            "param.0");
  p0->clear_sharding();
  auto p1 = HloInstruction::CreateParameter(1, ShapeUtil::MakeShape(S32, {7}),
                                            "param.1");
  p1->set_sharding(HloSharding::AssignDevice(1));

  auto tuple_shape = ShapeUtil::MakeTupleShape(
      {ShapeUtil::MakeShape(S32, {7}), ShapeUtil::MakeShape(S32, {9}),
       ShapeUtil::MakeShape(S32, {11})});
  ...
  EXPECT_THAT(Explain(p0.get(), op::Sharding(HloSharding::AssignDevice(1))),
              "%param.0 = s32[5]{0} parameter(0) has no sharding (expected: "
              "{maximal device=1})");
  EXPECT_THAT(Explain(p1.get(), op::NoSharding()),
              "%param.1 = s32[7]{0} parameter(1), sharding={maximal device=1} "
...
// TODO(chokobole): Enable this test. Dependency: HloInstruction::PrintWithCanonicalNameMap
// clang-format on
TEST_F(HloMatchersTest, DISABLED_DotMatcher) {
...
ENTRY DotOperationFusion_TransposeFusion {
  ...
  ROOT dot = s32[1,1024] dot(arg0, arg1), lhs_contracting_dims={1}, rhs_contracting_dims={0}
}
)";
  ...
  EXPECT_THAT(
      Explain(root, op::Dot(op::Parameter(0), op::Parameter(1),
                            ...
                            /*rhs_contracting_dim=*/0)),
      "(%dot = s32[1,1024]{1,0} dot(s32[1,256]{1,0} %arg0, s32[256,1024]{1,0} "
...
// TODO(chokobole): Enable this test. Dependency: HloInstruction::PrintWithCanonicalNameMap
// clang-format on
TEST_F(HloMatchersTest, DISABLED_ComparisonMatcher) {
  auto shape = ShapeUtil::MakeShape(S32, {1});
  ...
  EXPECT_THAT(Explain(eq.get(), op::Add()),
              Eq("(%compare = s32[1]{0} compare(s32[1]{0} %param.0, "
                 "s32[1]{0} %param.1), direction=EQ)"));
...
TEST_F(HloMatchersTest, AsyncCopyMatcher) {
  ...
  EXPECT_THAT(Explain(copy_start.get(), op::AsyncCopy(2, 1, op::Parameter(0))),
              Eq("(%copy-start = (s32[16]{0:S(2)}, s32[16]{0:S(1)}, u32[]) "
                 "copy-start(s32[16]{0:S(1)} %p0))"));
  EXPECT_THAT(Explain(copy_done.get(), op::AsyncCopy(3, 1, op::Parameter(0))),
              "(%copy-done = s32[16]{0:S(2)} copy-done((s32[16]{0:S(2)}, "
              "s32[16]{0:S(1)}, u32[]) "
              "%copy-start)) "
...
// TODO(chokobole): Enable this test. Dependency: HloInstruction::PrintWithCanonicalNameMap
// clang-format on
TEST_F(HloMatchersTest, DISABLED_ReplicaGroupsMatcher) {
  Shape shape = ShapeUtil::MakeShape(S32, {5, 7});
  ...
  EXPECT_THAT(Explain(p0.get(), op::ReplicaGroups({})),
              "%param = s32[5,7]{1,0} parameter(0) not a collective op");
  EXPECT_THAT(Explain(all_to_all.get(), op::ReplicaGroups({{0, 1}, {2, 3}})),
              "%all-to-all = s32[5,7]{1,0} all-to-all(s32[5,7]{1,0} %param), "
