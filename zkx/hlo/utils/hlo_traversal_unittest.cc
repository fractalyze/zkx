...
const char kTestModule[] = R"(
    accumulate {
      p0.0 = s32[] parameter(0)
      p1.0 = s32[] parameter(1)
      ROOT add = s32[] add(p0.0, p1.0)
    }

    computation1 {
      p0.1 = s32[] parameter(0)
      p1.1 = s32[128] parameter(1)
      mul = s32[128] multiply(p1.1, p1.1)
      ROOT reduce = s32[] reduce(mul, p0.1), dimensions={0}, to_apply=accumulate
    }

    computation2 {
      p0.2 = s32[] parameter(0)
      zero = s32[] constant(0)
    ...
    ENTRY entry {
      p0.3 = s32[] parameter(0)
      p1.3 = s32[128] parameter(1)
      sum = s32[128] add(p1.3, p1.3)
      not = s32[128] not(sum)
      negate = s32[128] negate(not)
      fusion1 = s32[] fusion(p0.3, negate), kind=kLoop, calls=computation1
      fusion2 = (pred[], pred[]) fusion(fusion1), kind=kLoop, calls=computation2
      gte = pred[] get-tuple-element(fusion2), index=0
      ROOT select = s32[] select(gte, fusion1, p0.3)
    })";
...
TEST_F(HloTraversalTest, AdaptorUsers) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
    computation1 {
      p0.1 = s32[] parameter(0)
      neg.1 = s32[] negate(p0.1)
      add.1 = s32[] add(p0.1, neg.1)
      ROOT tuple.1 = (s32[], s32[]) tuple(neg.1, add.1)
    }

    computation2 {
      p0.2 = s32[] parameter(0)
      mul = s32[] multiply(p0.2, p0.2)
      ROOT neg.2 = s32[] negate(mul)
    }

    ENTRY entry {
      p0.3 = s32[] parameter(0)
      fusion1 = (s32[], s32[]) fusion(p0.3), kind=kLoop, calls=computation1
      gte = s32[] get-tuple-element(fusion1), index=0
      add.3 = s32[] add(p0.3, gte)
      fusion2 = s32[] fusion(gte), kind=kLoop, calls=computation2
      not = s32[] not(fusion2)
      ROOT tuple.3 = (s32[], (s32[], s32[]), s32[], s32[]) tuple(add.3, fusion1, fusion2, not)
    }
  )"));
...
TEST_F(HloTraversalTest, FindArgumentsAfterFusion) {
  ...
  EXPECT_THAT(producers, ElementsAre("p0.3", "not.1"));
}
...
TEST_F(HloTraversalTest, FindAllMultiple) {
  const char kConverts[] = R"(
    ENTRY entry {
      ...
      p0.s16 = s16[128] convert(p0)
      p1.s8 = s8[128] convert(p1)
      p1.s16 = s16[128] convert(p1.s8)
      ROOT diff = s16[128] subtract(p0.s16, p1.s16)
    })";
  ...
  EXPECT_THAT(converts,
              ElementsAre(get("p0.s16"), get("p1.s16"), get("p1.s8")));
}
...
TEST_F(HloTraversalTest, FindAllNotFound) {
  const char kConverts[] = R"(
    ENTRY entry {
      p0 = s8[128] parameter(0)
      p1 = s16[128] parameter(1)
      p0s16 = s16[128] convert(p0)
      ROOT diff = s16[128] subtract(p0s16, p1)
    })";
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(kConverts));
  auto root = module->entry_computation()->GetInstructionWithName("diff");
  std::vector<const HloInstruction*> converts =
...
const char kTwoFusions[] = R"(
    ...
    computation1 {
      ...
      p1.1 = s32[128] parameter(1)
      mul = s32[128] multiply(p1.1, p1.1)
      ROOT reduce.1 = s32[] reduce(mul, p0.1), dimensions={0}, to_apply=accumulate
    }

    computation2 {
      p0.2 = s32[] parameter(0)
      p1.2 = s32[128] parameter(1)
      ROOT reduce.2 = s32[] reduce(p1.2, p0.2), dimensions={0}, to_apply=accumulate
    }
    ...
    ENTRY entry {
      ...
      p1.3 = s32[128] parameter(1)
      sum = s32[128] add(p1.3, p1.3)
      negate = s32[128] negate(sum)
      fusion1 = s32[] fusion(p0.3, negate), kind=kLoop, calls=computation1
      fusion2 = s32[] fusion(fusion1, negate), kind=kLoop, calls=computation2
      ROOT difference = s32[] subtract(fusion2, p0.3)
    })";
...
TEST_F(HloTraversalTest, FuseNonFusionConsumerAndProducer) {
  ...
  auto producer = module->entry_computation()->GetInstructionWithName("not.1");
  ...
  EXPECT_THAT(nodes, ElementsAre("negate", "not.1"));
}
...
TEST_F(HloTraversalTest, MultiOutputFusionDuplicateRoot) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
    computation {
      p0.1 = s32[128] parameter(0)
      p1.1 = s32[128] parameter(1)
      mul = s32[128] multiply(p0.1, p1.1)
      ROOT tuple = (s32[128], s32[128]) tuple(mul, mul)
    }

    ENTRY entry {
      p0.2 = s32[128] parameter(0)
      p1.2 = s32[128] parameter(1)
      ROOT fusion = (s32[128], s32[128]) fusion(p0.2, p1.2), kind=kLoop, calls=computation
    })"));
  auto fusion = HloFusionAdaptor::ForInstruction(
...
TEST_F(HloTraversalTest, MakeInstructionsPostOrder_TwoMultiOutputFusions) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
    ...
    computation1 {
      ...
      p1.1 = s32[128] parameter(1)
      mul = s32[128] multiply(p1.1, p1.1)
      reduce.1 = s32[] reduce(mul, p0.1), dimensions={0}, to_apply=accumulate
      ROOT tuple.1 = (s32[128], s32[]) tuple(mul, reduce.1)
    }

    computation2 {
      p0.2 = s32[] parameter(0)
      p1.2 = s32[128] parameter(1)
      neg = s32[128] negate(p1.2)
      reduce.2 = s32[] reduce(neg, p0.2), dimensions={0}, to_apply=accumulate
      ROOT tuple.2 = (s32[], s32[128]) tuple(reduce.2, neg)
    }
    ...
    ENTRY entry {
      ...
      sum = s32[128] add(p1.3, p1.3)
      negate = s32[128] negate(sum)
      fusion1 = (s32[128], s32[]) fusion(p0.3, negate), kind=kLoop, calls=computation1
      gte1 = s32[128] get-tuple-element(fusion1), index=0
      gte2 = s32[] get-tuple-element(fusion1), index=1
      fusion2 = (s32[], s32[128]) fusion(p0.3, gte1), kind=kLoop, calls=computation2
      gte3 = s32[] get-tuple-element(fusion2), index=0
      gte4 = s32[128] get-tuple-element(fusion2), index=1
      difference = s32[] subtract(gte3, p0.3)
      ROOT tuple.3 = (s32[], s32[128]) tuple(difference, gte4)
    })"));
  auto fusion = HloFusionAdaptor::ForProducerConsumer(
      module->entry_computation()->GetInstructionWithName("fusion1"),
      module->entry_computation()->GetInstructionWithName("fusion2"));
...
TEST_F(HloTraversalTest, GetRootsForProducerConsumerFusion) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
      computation1 {
        p0.1 = s32[10]{0} parameter(0)
        ROOT neg = s32[10]{0} negate(p0.1)
      }

      computation2 {
        p0.2 = s32[10]{0} parameter(0)
        ROOT add = s32[10]{0} add(p0.2, p0.2)
      }

      ENTRY entry {
        p0.3 = s32[10]{0} parameter(0)
        fusion1 = s32[10]{0} fusion(p0.3), kind=kLoop, calls=computation1
        fusion2 = s32[10]{0} fusion(fusion1), kind=kLoop, calls=computation2
        ROOT tuple = (s32[10]{0}, s32[10]{0}) tuple(fusion1, fusion2)
      }
  )"));
  auto producer_instr =
      module->entry_computation()->GetInstructionWithName("fusion1");
...
const char kTwoMultiOutputFusions[] = R"(
    computation1 {
      p0.1 = s32[10]{0} parameter(0)
      p1.1 = s32[10]{0} parameter(1)
      computation1.p2 = s32[10]{0} parameter(2)
      add = s32[10]{0} add(p0.1, p1.1)
      sub = s32[10]{0} subtract(p0.1, p1.1)
      ROOT tuple.1 = (s32[10]{0}, s32[10]{0}, s32[10]{0}, s32[10]{0}, s32[10]{0}) tuple(p1.1, add, sub, p0.1, computation1.p2)
    }

    computation2 {
      p0.2 = s32[10]{0} parameter(0)
      p1.2 = s32[10]{0} parameter(1)
      p2.2 = s32[10]{0} parameter(2)
      mul = s32[10]{0} multiply(p0.2, p1.2)
      div = s32[10]{0} divide(p0.2, p1.2)
      ROOT tuple.2 = (s32[10]{0}, s32[10]{0}, s32[10]{0}) tuple(mul, div, p2.2)
    }

    ENTRY entry {
      p0.3 = s32[10]{0} parameter(0)
      p1.3 = s32[10]{0} parameter(1)
      p2.3 = s32[10]{0} parameter(2)
      fusion1 = (s32[10]{0}, s32[10]{0}, s32[10]{0}, s32[10]{0}, s32[10]{0}) fusion(p0.3, p1.3, p2.3), kind=kLoop, calls=computation1
      gte0 = s32[10]{0} get-tuple-element(fusion1), index=0
      gte1 = s32[10]{0} get-tuple-element(fusion1), index=1
      gte2 = s32[10]{0} get-tuple-element(fusion1), index=2
      gte3 = s32[10]{0} get-tuple-element(fusion1), index=3
      gte4 = s32[10]{0} get-tuple-element(fusion1), index=4
      fusion2 = (s32[10]{0}, s32[10]{0}, s32[10]{0}) fusion(gte1, gte2, gte3), kind=kLoop, calls=computation2
      gte5 = s32[10]{0} get-tuple-element(fusion2), index=0
      gte6 = s32[10]{0} get-tuple-element(fusion2), index=1
      gte7 = s32[10]{0} get-tuple-element(fusion2), index=2
...
// TODO(chokobole): Enable this. Dependency: CustomCall
TEST_F(HloTraversalTest, DISABLED_HloFindUseChain) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
    computation {
      p0.1 = s32[] parameter(0)
      p1.1 = s32[] parameter(1)
      negate = s32[] negate(p0.1)
      not = s32[] not(p0.1)
      sum = s32[] add(p0.1, not)
      sign = s32[] sign(p1.1)
      ROOT call = s32[] custom-call(negate, sign, sum), custom_call_target="it"
    }

    ENTRY entry {
      p0.2 = s32[] parameter(0)
      p1.2 = s32[] parameter(1)
      ROOT fusion = s32[] fusion(p0.2, p1.2), kind=kLoop, calls=computation
    }
    )"));
  ...
  EXPECT_THAT(HloFindUseChain(p0, sum), ElementsAre(p0, not_instr, sum));
  EXPECT_THAT(HloFindUseChain(p1, sign), ElementsAre(p1, sign));
  EXPECT_THAT(HloFindUseChain(negate, sign), IsEmpty());
...
TEST_F(HloTraversalTest, DoNotResolveIntoNestedFusions) {
  TF_ASSERT_OK_AND_ASSIGN(auto module, ParseAndReturnVerifiedModule(R"(
    computation1 {
      p0.1 = s32[] parameter(0)
      ROOT mul = s32[] multiply(p0.1, p0.1)
    }

    computation2 {
      p0.2 = s32[] parameter(0)
      fusion.2 = s32[] fusion(p0.2), kind=kLoop, calls=computation1
      ROOT neg = s32[] negate(fusion.2)
    }

    ENTRY entry {
      p0.3 = s32[] parameter(0)
      ROOT fusion.3 = s32[] fusion(p0.3), kind=kLoop, calls=computation2
    }
  )"));
