...
#ifndef ZKX_HLO_EVALUATOR_HLO_EVALUATOR_H_
#define ZKX_HLO_EVALUATOR_HLO_EVALUATOR_H_
...
class HloEvaluator : public ConstDfsHloVisitorWithDefault {
 public:
  ...
  struct PrecomputedAnalyses {
    TuplePointsToAnalysis* tuple_points_to;  // not owned
    CallGraph* call_graph;                   // not owned
  };
  ...
  // Callback for each multiply-accumulate in each dot operation.
  using TraceMACHandler = std::function<void(
      int64_t result_index, int64_t lhs_index, int64_t rhs_index)>;

  // Sets a callback for each multiply-accumulate in each dot
  // operation.
 ...
 protected:
  ...
  absl::Status HandleBitReverse(const HloInstruction* bit_reverse) override;
  ...
  absl::Status HandleOutfeed(const HloInstruction* outfeed) override {
    return absl::UnimplementedError(
        "Outfeed HLO is unsupported by the evaluator.");
  }
...
extern const std::string_view kEvalErrorDetailUrl;
...
#endif  // ZKX_HLO_EVALUATOR_HLO_EVALUATOR_H_
