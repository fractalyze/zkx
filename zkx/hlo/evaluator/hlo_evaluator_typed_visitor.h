    ...
    TF_ASSIGN_OR_RETURN(
        ...
        ElementWiseBinaryOp(
            multiply, [](ElementwiseT lhs_elem, ElementwiseT rhs_elem) {
              if constexpr (math::IsEcPoint<ElementwiseT>) {
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        ElementWiseBinaryOp(
            subtract, [](ElementwiseT lhs_elem, ElementwiseT rhs_elem) {
              if constexpr (math::IsAffinePoint<ElementwiseT>) {
        ...
        ElementWiseBinaryOp(
            ...
            [](ElementwiseT lhs_elem, ElementwiseT rhs_elem) -> ElementwiseT {
              if constexpr (math::IsEcPoint<ElementwiseT>) {
                ABSL_UNREACHABLE();
                return ElementwiseT(0);
              } else if constexpr (math::IsPrimeField<ElementwiseT>) {
                if (rhs_elem.IsZero()) {
                  return ElementwiseT::Zero();
                }
