...
#ifndef ZKX_HLO_PARSER_HLO_LEXER_H_
#define ZKX_HLO_PARSER_HLO_LEXER_H_
...
class HloLexer {
 public:
  explicit HloLexer(std::string_view buf) : buf_(buf) {
  ...
  int64_t GetInt64Val() const {
    CHECK_EQ(GetKind(), TokKind::kInt) << TokKindToString(GetKind());
    return token_state_.int64_val;
  }
  double GetDecimalVal() const {
    CHECK_EQ(GetKind(), TokKind::kDecimal);
    return token_state_.decimal_val;
  }
  PrimitiveType GetPrimitiveTypeVal() const {
    CHECK_EQ(GetKind(), TokKind::kPrimitiveType);
  ...
  std::string_view GetLine(LocTy loc) const;
 ...
 private:
  ...
  std::string_view StringViewFromPointers(const char* begin,
  ...
  std::optional<int64_t> LexNanPayload(std::string_view& consumable);
...
#endif  // ZKX_HLO_PARSER_HLO_LEXER_H_
