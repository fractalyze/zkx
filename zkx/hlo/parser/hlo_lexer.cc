...
std::string_view HloLexer::StringViewFromPointers(const char* begin,
...
std::optional<int64_t> HloLexer::LexNanPayload(std::string_view& consumable) {
  ...
  if (absl::SimpleAtoi(slice, &payload_value)) {
...
#undef KEYWORD
  ...
  {
    std::string_view consumable = StringViewFromPointers(
...
TokKind HloLexer::LexJsonDict() {
  ...
  std::string_view orig = StringViewFromPointers(token_state_.token_start,
                                                 buf_.data() + buf_.size());
  std::string_view str = orig;
...
bool LexesAsJsonDict(std::string_view str) {
  HloLexer lexer(str);
