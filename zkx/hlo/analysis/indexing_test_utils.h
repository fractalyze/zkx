...
#ifndef ZKX_HLO_ANALYSIS_INDEXING_TEST_UTILS_H_
#define ZKX_HLO_ANALYSIS_INDEXING_TEST_UTILS_H_
...
class IndexingTestBase : public HloTestBase {
 public:
  HloInstruction* ParseAndGetRoot(std::string_view hlo_string);
...
HloInstructionIndexing ComputeOutputToInputIndexingForEntryComputation(
    ...
    std::string_view hlo_string, int output_id = 0,
    bool use_physical_layout = false);

HloInstructionIndexing ComputeInputToOutputIndexingForEntryComputation(
    HloTestBase* test_base, mlir::MLIRContext* mlir_context,
    std::string_view hlo_string, int input_id = 0,
    bool use_physical_layout = false);

mlir::AffineMap ParseAffineMap(std::string_view serialized_affine_map,
                               mlir::MLIRContext* context);

mlir::AffineExpr ParseAffineExpr(std::string_view serialized_affine_expr,
                                 mlir::MLIRContext* context);
...
#endif  // ZKX_HLO_ANALYSIS_INDEXING_TEST_UTILS_H_
