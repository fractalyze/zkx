...
enum class Delimiter { kParen, kBracket, kBrace };
...
class Parser {
 public:
  ...
  std::pair<Token::Kind, Token::Kind> GetDelimiterPair(Delimiter delimiter);
...
bool Parser::ParseCommaSeparatedVarList(
    Delimiter delimiter,
    llvm::function_ref<bool(Parser& parser)> parse_element_fn) {
  auto [left_delimiter, right_delimiter] = GetDelimiterPair(delimiter);
...
bool ParseVarNames(Parser& parser, Delimiter delimiter,
  ...
  return parser.ParseCommaSeparatedVarList(delimiter, parse_var_name_fn);
}
...
bool ParseAffineMapResults(Parser& parser,
  ...
  return parser.ParseCommaSeparatedVarList(Delimiter::kParen,
                                           parse_var_name_fn);
}
...
std::optional<IndexingMap> ParseIndexingMap(llvm::StringRef input,
  ...
  if (!ParseVarNames(parser, Delimiter::kParen, dim_var_names) ||
      (parser.GetCurrentToken().kind == Token::Kind::kLBracket &&
       !ParseVarNames(parser, Delimiter::kBracket, range_var_names)) ||
      (parser.GetCurrentToken().kind == Token::Kind::kLBrace &&
       !ParseVarNames(parser, Delimiter::kBrace, rt_var_names))) {
