...
#ifndef ZKX_HLO_ANALYSIS_INDEXING_MAP_H_
#define ZKX_HLO_ANALYSIS_INDEXING_MAP_H_
...
struct Interval {
  ...
  // considered to be possibly saturated, i.e. `{-2⁶³, 0} + {42, 42}`
  // returns `{-2⁶³, 42}`, not `{-2⁶³ + 42, 42}`.
...
// (d0, ..., d_{N - 1})[s0, ..., s_{M - 1}]{r0, ..., r_{K - 1}] -> f(d_i, s_j)
...
// ```
//   %p0 = u32[150, 20, 10, 50] parameter(0)
//   %reduce = u32[150, 10] reduce(%p0, %p0_init), dimensions={3, 1}
// ```
...
class IndexingMap {
 public:
  ...
  struct Variable {
    Variable() = default;
    explicit Variable(Interval bounds, std::string_view name = "")
        : bounds(bounds), name(name) {}
    Variable(int64_t lb, int64_t ub, std::string_view name = "")
  ...
  // Rename a dimension variable. Index must be valid.
  void RenameDimVar(int64_t id, std::string_view new_name);
  ...
  // 0 <= d0 mod 11 <= 0 for a dimension 1 <= d0 <= 50 then there is no d0
...
#endif  // ZKX_HLO_ANALYSIS_INDEXING_MAP_H_
