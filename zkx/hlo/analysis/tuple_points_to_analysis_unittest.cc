...
//   ENTRY entry {
//     p0 = u32[2,3] parameter(0)
//     async-start = ((u32[2,3]), u32[2,3], u32[]) custom-call-start(p0),
//     custom_call_target="foo" async-update = ((u32[2,3]), u32[2,3], u32[])
//     custom-call-update(async-start) ROOT async-done = u32[2,3]
//     custom-call-done(async-update)
//   }
// )";
//   TF_ASSERT_OK_AND_ASSIGN(
//       module_, ParseAndReturnVerifiedModule(hlo_str,
//       GetModuleConfigForTest()));

//   HloInstruction* param =
//       module_->entry_computation()->parameter_instruction(0);
//   HloInstruction* async_start = FindInstruction(module_.get(),
//   "async-start"); HloInstruction* async_update =
//   FindInstruction(module_.get(), "async-update"); HloInstruction* async_done
...
//       param, {}, {{param, {}}, {async_start, {0, 0}}, {async_update, {0,
//       0}}});
//   ExpectHasBufferAliases(
//       async_start, {1},
...
TEST_F(TuplePointsToAnalysisTest, RecvAndRecvDone) {
  ...
  auto recv = builder.AddInstruction(
      HloInstruction::CreateRecv(ShapeUtil::MakeShape(U32, {1, 2, 3}), token,
...
TEST_F(TuplePointsToAnalysisTest, PointsToTupleConstantElements) {
  ...
  auto builder = HloComputation::Builder(TestName());
  Literal elements[] = {LiteralUtil::CreateR2<uint32_t>({{1}, {2}}),
                        LiteralUtil::CreateR1<uint32_t>({2, 42})};
...
//   auto constant = builder.AddInstruction(
//       HloInstruction::CreateConstant(LiteralUtil::CreateR0<uint32_t>(1)));
//   Shape data_shape = ShapeUtil::MakeShape(U32, {});
...
class FusionPointsToAnalysisTest : public TuplePointsToAnalysisTest {
 protected:
  ...
  // Returns all users of 'fusion_param' at 'tuple_index'.
...
class PointsToAnalysisTestBase : public HloHardwareIndependentTestBase {
 protected:
  ...
  std::unique_ptr<HloModule> module_;
  HloComputation* computation_ = nullptr;  // not owned
  std::unique_ptr<TuplePointsToAnalysis> points_to_analysis_;
};
