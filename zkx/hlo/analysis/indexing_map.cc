...
class AffineExprSimplifier {
 ...
 private:
  ...
  RangeEvaluator* range_evaluator_;  // not owned
  AffineExpr zero_;
...
constexpr std::string_view kVarKindDefault = "default";
constexpr std::string_view kVarKindThreadX = "th_x";
constexpr std::string_view kVarKindThreadY = "th_y";
constexpr std::string_view kVarKindThreadZ = "th_z";
constexpr std::string_view kVarKindBlockX = "bl_x";
constexpr std::string_view kVarKindBlockY = "bl_y";
constexpr std::string_view kVarKindBlockZ = "bl_z";
constexpr std::string_view kVarKindWarp = "warp";
constexpr std::string_view kVarKindWarpThread = "th_w";
...
VariableKind ToVariableKind(std::string_view var_name) {
...
std::ostream& operator<<(std::ostream& out, VariableKind var_kind) {
  out << ToVariableName(var_kind);
  return out;
}
...
bool IndexingMap::ConstraintsSatisfied(
  ...
  CHECK_EQ(dim_const_exprs.size(), affine_map_.getNumDims());
  CHECK_EQ(symbol_const_exprs.size(), affine_map_.getNumSymbols());
  if (IsKnownEmpty()) {
    return false;
  }
...
SmallVector<int64_t, 4> IndexingMap::Evaluate(
    ArrayRef<AffineExpr> dim_const_exprs,
    ArrayRef<AffineExpr> symbol_const_exprs) const {
  CHECK_EQ(dim_const_exprs.size(), GetDimensionCount());
  CHECK_EQ(symbol_const_exprs.size(), GetSymbolCount());
...
void IndexingMap::RenameDimVar(int64_t id, std::string_view new_name) {
...
// Detects unused dimensions and symbols in the indexing map.
