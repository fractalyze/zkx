...
HloHardwareIndependentTestBase::CreateNewVerifiedModule(
  ...
  return std::make_unique<VerifiedHloModule>(
      ...
      ShapeUtil::ByteSizeOfElements, instruction_can_change_layout_func_);
}

absl::StatusOr<std::unique_ptr<VerifiedHloModule>>
HloHardwareIndependentTestBase::ParseAndReturnVerifiedModule(
    std::string_view hlo_text, int64_t replica_count,
...
absl::StatusOr<std::unique_ptr<VerifiedHloModule>>
HloHardwareIndependentTestBase::ParseAndReturnVerifiedModule(
    std::string_view hlo_text, const HloModuleConfig& config) const {
...
DebugOptions HloHardwareIndependentTestBase::GetDebugOptionsForTest() const {
  ...
  debug_options.add_zkx_disable_hlo_passes("constant_folding");
  debug_options.set_zkx_hlo_evaluator_use_fast_path(true);
  return debug_options;
}

void HloHardwareIndependentTestBase::RunAndFilecheckHloRewrite(
    std::string_view hlo, HloPassInterface&& hlo_pass,
...
void HloHardwareIndependentTestBase::RunAndFilecheckHloRewrite(
    std::string_view hlo_with_checks, HloPassInterface&& hlo_pass,
    std::function<void(HloModule*)> after_pass_checks,
    const HloModuleConfig* config) const {
...
void HloHardwareIndependentTestBase::RunAndFilecheckHloModuleGroupRewrite(
    absl::Span<const std::string_view> hlo_module_strs,
    HloPassInterface&& hlo_pass,
    std::optional<absl::Span<const std::string_view>> expected) const {
  std::vector<std::unique_ptr<HloModule>> modules;
  for (std::string_view hlo : hlo_module_strs) {
    TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<VerifiedHloModule> module,
                            ParseAndReturnVerifiedModule(hlo));
...
HloHardwareIndependentTestBase::RunAndCheckHloRewrite(
    std::string_view hlo_template, HloPassInterface* hlo_pass,
...
HloComputation* HloHardwareIndependentTestBase::FindComputation(
    HloModule* module, std::string_view name) {
