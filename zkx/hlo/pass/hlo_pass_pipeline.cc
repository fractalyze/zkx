...
absl::Status HloPassPipeline::RunInvariantCheckers(
    HloT* hlo, std::string_view after_pass_name,
  ...
  for (auto& invariant_checker : invariant_checkers_) {
    ...
    if (!changed_status.ok()) {
      ...
      ZKX_VLOG_LINES(2, hlo->ToString());
      return tsl::errors::internal::CreateWithUpdatedMessage(
...
absl::StatusOr<bool> HloPassPipeline::RunPassesInternal(
  ...
  // module group pass.
  const std::string& dump_regex = debug_options.zkx_dump_hlo_pass_re();
...
std::vector<HloPassInterface*> HloPassPipeline::GetEnabledPasses(
  ...
  if (debug_options.zkx_disable_all_hlo_passes()) {
    VLOG(1) << "*All* passes disabled by --zkx_disable_all_hlo_passes.";
    return {};
  }

  absl::flat_hash_set<std::string> disabled_pass_names(
      debug_options.zkx_disable_hlo_passes().begin(),
      debug_options.zkx_disable_hlo_passes().end());

  absl::flat_hash_set<std::string> enabled_pass_names(
      debug_options.zkx_enable_hlo_passes_only().begin(),
      debug_options.zkx_enable_hlo_passes_only().end());

  if (!disabled_pass_names.empty()) {
    VLOG(1) << "Passes disabled by --zkx_disable_hlo_passes: "
            << absl::StrJoin(disabled_pass_names, ", ");
  }

  if (!enabled_pass_names.empty()) {
    VLOG(1) << "Passes enabled by --zkx_enable_hlo_passes_only: "
...
void HloPassPipeline::MaybeDumpHloAndSaveFilenames(
    HloModule& module, std::string_view after_pass_name,
    std::string_view before_pass_name) {
...
void HloPassPipeline::MaybeDumpHloAndSaveFilenames(
    HloModuleGroup& module_group, std::string_view after_pass_name,
    std::string_view before_pass_name) {
  for (HloModule* module : module_group.modules()) {
