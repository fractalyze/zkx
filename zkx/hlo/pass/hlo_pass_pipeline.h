...
#ifndef ZKX_HLO_PASS_HLO_PASS_PIPELINE_H_
#define ZKX_HLO_PASS_HLO_PASS_PIPELINE_H_
...
class HloPassPipeline : public HloPassInterface {
 public:
  ...
  size_t PassesSize() const { return passes_.size(); }
  // Return reference to pass specified by index.
  HloPassInterface& GetPass(size_t index) { return *passes_[index]; }
 ...
 private:
  ...
  void MaybeDumpHloAndSaveFilenames(HloModuleGroup& module_group,
                                    std::string_view after_pass_name,
  ...
  CompilationStats* compilation_stats_;  // not owned
...
#endif  // ZKX_HLO_PASS_HLO_PASS_PIPELINE_H_
