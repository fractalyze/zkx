...
#ifndef ZKX_HLO_IR_HLO_MODULE_H_
#define ZKX_HLO_IR_HLO_MODULE_H_
...
#include "zkx/hlo/ir/hlo_module_metadata.h"
...
class HloModule {
 public:
  ...
  HloComputation* GetComputationWithName(std::string_view name);
  ...
  void SetAndUniquifyInstrName(HloInstruction* instr, std::string_view name) {
  ...
  struct CrossProgramPrefetchInfo {
    ...
    ShapeIndex index;
    // Offset into alt memory where the cross program prefetched buffer will be
    // stored.
  ...
  void set_autofdo_fingerprint(std::string_view fingerprint) {
 ...
 private:
  ...
  // Sharable copy-on-write instance.
...
#endif  // ZKX_HLO_IR_HLO_MODULE_H_
