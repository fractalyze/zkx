  ...
  switch (opcode) {
    case HloOpcode::kFft: {
      instruction = CreateFft(shape, operands(0), proto.fft_type(),
                              proto.fft_length(), proto.fft_bit_reverse());
      break;
    }
...
bool HloInstruction::fft_bit_reverse() const {
  return Cast<HloFftInstruction>(this)->fft_bit_reverse();
}
