  ...
  for (const auto& index_to_sharding : shape_tree.leaves()) {
    ...
    if (!status.ok()) {
      tsl::errors::AppendToMessage(
          &status,
          absl::StrCat("Note: While validating sharding tuple element ",
...
absl::Status HloSharding::Validate(const Shape& shape,
  ...
  if (!status.ok()) {
    tsl::errors::AppendToMessage(
        &status,
        absl::StrCat("Note: While validating sharding ", ToString(),
...
absl::Status HloSharding::ValidateNonTuple(
  ...
  if (!all_devices_seen) {
    return absl::InvalidArgumentError(
        absl::StrCat("tile_assignment should have ", *num_devices, " devices"));
  }
...
// static
absl::StatusOr<HloSharding> HloSharding::FromProto(const OpSharding& proto) {
