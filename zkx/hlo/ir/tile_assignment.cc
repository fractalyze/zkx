...
// static
IotaTileAssignment IotaTileAssignment::Create(absl::Span<const int64_t> dims) {
...
IotaTileAssignment IotaTileAssignment::Create(
  ...
  absl::InlinedVector<int, 6> canonicalized_perm(transpose_perm.begin(),
                                                 transpose_perm.end());
  absl::Span<int64_t> dims_span = absl::MakeSpan(canonicalized_dims);
  absl::Span<int> perm_span = absl::MakeSpan(canonicalized_perm);
...
std::optional<IotaTileAssignment> IotaTileAssignment::Transpose(
  ...
  absl::Span<const int64_t> dims = this->dims();
  ...
  absl::Span<const int64_t> reshape_dims = this->reshape_dims();
  absl::Span<const int> transpose_perm = this->transpose_perm();
  ...
  for (int i = 0; i < perm.size(); ++i) {
    ...
    absl::InlinedVector<int, 2>& group =
        grouped_reshape_dims[one_to_non_one[dim]];
...
[[nodiscard]] TileAssignment TileAssignment::Transpose(
  ...
  if (iota_) {
    std::optional<IotaTileAssignment> transposed = iota_->Transpose(perm);
    if (transposed) {
      return TileAssignment(std::move(*transposed));
    }
  }
  std::shared_ptr<Array<int64_t>> cloned_array = shared_array_clone();
