...
absl::StatusOr<ReplacedAsync> CreateAsyncAllGather(
  ...
  absl::c_transform(
      ag->operands(), std::back_inserter(operand_shapes),
...
absl::StatusOr<ReplacedAsync> CreateAsyncCollectivePermute(
  ...
  absl::c_transform(
      ...
      [](const HloInstruction* operand) { return &(operand->shape()); });
  TF_ASSIGN_OR_RETURN(Shape start_shape,
                      ShapeInference::InferCollectivePermuteStartShape(
                          operand_shapes, context_shapes, cp->inplace()));
  if (!cp->inplace()) {
    start = instruction->AddInstruction(
        HloInstruction::CreateCollectivePermuteStart(
            start_shape, cp->operands(), cp->source_target_pairs(),
            cp->channel_id()));
  } else {
    // TODO support grouped partial collective permutes
    if (cp->operand_count() != 4) {
      return absl::UnimplementedError(
          "In-place collective-permute with operand_count != 4 is not "
          "supported");
    }
    start = instruction->AddInstruction(
        HloInstruction::CreateCollectivePermuteStart(
            start_shape, cp->mutable_operand(0), cp->mutable_operand(1),
    ...
    switch (instruction->opcode()) {
      ...
      default:
        return absl::InternalError(absl::StrFormat(
            "Unexpected opcode %s", HloOpcodeString(instruction->opcode())));
    }
