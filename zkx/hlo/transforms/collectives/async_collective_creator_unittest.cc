...
TEST_F(AsyncCollectiveCreatorTest, SplitsSingleAllReduce) {
  ...
  ENTRY entry {
    p0 = u32[1024] parameter(0)
    ROOT ar = u32[1024] all-reduce(p0), to_apply=add
  }
  )";
...
TEST_F(AsyncCollectiveCreatorTest, SplitsSingleAllGather) {
  ...
  ENTRY entry {
    ...
    ROOT ag = u32[8] all-gather(p0), dimensions={0}, replica_groups={{0,1,2,3,4,5,6,7}}
  }
  )";
...
TEST_F(AsyncCollectiveCreatorTest, SplitsSingleCollectivePermute) {
  ...
  ENTRY entry {
    ...
    ROOT %collective-permute.1 = u32[8]{0} collective-permute(u32[8]{0} p0), source_target_pairs={{0,1},{1,2},{2,3}}, channel_id=1
  }
  )";
...
//     %custom-call = u32[4,4,128]{2,1,0:T(4,128)} custom-call(), custom_call_target="SomeCustomCall"
//     ROOT %collective-permute = u32[4,4,128]{2,1,0:T(4,128)} collective-permute(u32[4,4,128]{2,1,0:T(4,128)} %custom-call, u32[4,4,128]{2,1,0:T(4,128)} %custom-call, (u32[], u32[], u32[]) %tuple, (u32[], u32[], u32[]) %tuple.1), channel_id=958, source_target_pairs={{0,4},{4,0},{1,5},{5,1},{2,6},{6,2},{3,7},{7,3}}, slice_sizes={{2,4,128}}
//   }
//   )";
...
TEST_F(AsyncCollectiveCreatorTest, SplitsSingleCollectiveBroadcast) {
  ...
  HloModule test
  ENTRY entry {
    p0 = u32[8,16] parameter(0)
    ROOT cb = u32[8,16] collective-broadcast(p0), replica_groups={{7,0,1,2,3,4,5,6}}
  }
  )";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
                          ParseAndReturnVerifiedModule(hlo_string));
...
TEST_F(AsyncCollectiveCreatorTest, SplitsSingleAllToAll) {
  ...
  ENTRY entry {
    p0 = u32[8,16] parameter(0)
    ROOT ata = u32[8,16] all-to-all(p0), dimensions={0}, replica_groups={{0,1,2,3,4,5,6,7}}
  }
  )";
  ...
  TF_ASSERT_OK(AsyncCollectiveCreator(config).Run(hlo_module.get()).status());
  ZKX_VLOG_LINES(0, hlo_module->ToString());
...
TEST_F(AsyncCollectiveCreatorTest, SplitsSingleRaggedAllToAll) {
  ...
  ENTRY RA2A {
    input = u32[64,8,128]{2,1,0:T(8,128)} parameter(0)
    c0 = u32[] constant(0)
    output = u32[64,8,128]{2,1,0:T(8,128)S(1)} broadcast(c0), dimensions={}
    ...
    ROOT ra2a = u32[64,8,128]{2,1,0:T(8,128)S(1)} ragged-all-to-all(input, output, input_offsets, send_sizes, output_offsets, recv_sizes), replica_groups={{0,1,2,3,4,5,6,7}}
  }
  )";
...
TEST_F(AsyncCollectiveCreatorTest, ControlPredecessor) {
  ...
  HloModule test
  ENTRY entry {
    p0 = u32[128] parameter(0)
    ag = u32[1024] all-gather(p0), dimensions={0}, replica_groups={{0,1,2,3,4,5,6,7}}, control-predecessors={p0}
    p1 = u32[128] parameter(1), control-predecessors={ag}
    ROOT sum = add(ag, ag)
  }
  )";
...
TEST_F(AsyncCollectiveCreatorTest, PreserveFrontendAttributesAllGather) {
  ...
  ENTRY entry {
    p0 = u32[128] parameter(0)
    ROOT ag = u32[1024] all-gather(p0), dimensions={0}, replica_groups={{0,1,2,3,4,5,6,7}}, frontend_attributes={_scheduling_group_id="0"}
  }
  )";
  ...
  EXPECT_TRUE(
      done->frontend_attributes().map().contains(kZkxSchedulingGroupIdAttr));
  EXPECT_EQ(done->frontend_attributes().map().at(kZkxSchedulingGroupIdAttr),
            "0");
  EXPECT_TRUE(
      start->frontend_attributes().map().contains(kZkxSchedulingGroupIdAttr));
  EXPECT_EQ(start->frontend_attributes().map().at(kZkxSchedulingGroupIdAttr),
            "0");
}
...
TEST_F(AsyncCollectiveCreatorTest, PreserveFrontendAttributesAllReduce) {
  ...
  ENTRY entry {
    p0 = u32[1024] parameter(0)
    ROOT ar = u32[1024] all-reduce(p0), replica_groups={{0,1,2,3,4,5,6,7}}, to_apply=add, frontend_attributes={_scheduling_group_id="0"}
  }
  )";
...
TEST_F(AsyncCollectiveCreatorTest,
  ...
  ENTRY entry {
    p0 = u32[1024] parameter(0)
    ROOT cp = u32[1024] collective-permute(p0), source_target_pairs={{0,1},{1,2},{2,3},{3,0}}, frontend_attributes={_scheduling_group_id="0"}
  }
  )";
...
TEST_F(AsyncCollectiveCreatorTest, PreserveFrontendAttributesAllToAll) {
  ...
  ENTRY entry {
    p0 = u32[1024] parameter(0)
    ROOT a2a = u32[1024] all-to-all(p0), replica_groups={{0,1,2,3,4,5,6,7}}, dimensions={0}, frontend_attributes={_scheduling_group_id="0"}
  }
  )";
