...
absl::StatusOr<bool> CombineConstants(HloComputation* computation,
  ...
  // Map from the literal hash of a constant to all equivalent instructions.
  // This avoids extreme quadratic behavior with many scalar constants.
...
struct CseKey {
  ...
  friend H AbslHashValue(H h, const CseKey& key) {
    ...
    switch (instruction->opcode()) {
      ...
      // TODO(jeong0982): Add kReduceWindow case once that opcode and
      // HloInstruction::window() are ported.
    ...
    if (fusion->IsMultiOutputFusion()) {
      ...
      for (const HloInstruction* hlo : root->operands()) {
        if (root_to_unique_index.find(hlo) == root_to_unique_index.end()) {
          root_to_unique_index[hlo] = root_index;
        }
        ++root_index;
      }
