...
absl::StatusOr<bool> HloDCE::RunOnComputation(
  ...
  for (auto* instruction : computation->instructions()) {
    ...
    // TODO(chokobole): Uncomment this. Dependency: IsCustomCall
    // bool is_custom_call_sharding = instruction->IsCustomCall("Sharding");
    bool is_custom_call_sharding = false;
    if (instruction->IsDead() && computation->IsSafelyRemovable(instruction) &&
        (!is_custom_call_sharding ||
