...
TEST_F(HloSchedulingTest, LastUseScheduledFirst) {
  ...
  //   %sub = subtract(%add, %negate2)
  //
  // %add should be scheduled before %negate2 because %add is the last (and
  // only) use of %ab. Scheduling %add first then frees up %ab's buffer.
  const Shape vec = ShapeUtil::MakeShape(U32, {42});
  ...
  auto sub = builder.AddInstruction(
      HloInstruction::CreateBinary(vec, HloOpcode::kSubtract, add, negate2));
  ...
  EXPECT_TRUE(ordering.ExecutesBefore(add, negate2));
...
ENTRY entry {
  %p = u32[1000, 1000] parameter(0)
  %token.0 = token[] after-all()
  %send = (u32[1000, 1000], token[]) send(%p, %token.0),
    channel_id=1, is_host_transfer=true
  %n1 = u32[1000, 1000] negate(%p)
  %n2 = u32[1000, 1000] negate(%n1)
  %n3 = u32[1000, 1000] negate(%n2)
...
//   // ZKX executor to overlap the execution of independent additions and
...
//     ENTRY e {
//       p0 = u32[1,2,1,512,256] parameter(0)
//       c0 = u32[] constant(0)

//       c1 = u32[] constant(1)
//       bcast1 = u32[1,2,1,512,256] broadcast(c1), dimensions={}
//       add1 = u32[1,2,1,512,256] add(p0, bcast1)

//       c2 = u32[] constant(2)
//       bcast2 = u32[1,2,1,512,256] broadcast(c2), dimensions={}
//       add2 = u32[1,2,1,512,256] add(p0, bcast2)

//       c3 = u32[] constant(3)
//       bcast3 = u32[1,2,1,512,256] broadcast(c3), dimensions={}
//       add3 = u32[1,2,1,512,256] add(p0, bcast3)

//       c4 = u32[] constant(4)
//       bcast4 = u32[1,2,1,512,256] broadcast(c4), dimensions={}
//       add4 = u32[1,2,1,512,256] add(p0, bcast4)

//       c5 = u32[] constant(5)
//       bcast5 = u32[1,2,1,512,256] broadcast(c5), dimensions={}
//       add5 = u32[1,2,1,512,256] add(p0, bcast5)

//       r1 = u32[1,2] reduce(add1, c0), dimensions={2,3,4}, to_apply=add
//       r2 = u32[1,2] reduce(add2, c0), dimensions={2,3,4}, to_apply=add
//       r3 = u32[1,2] reduce(add3, c0), dimensions={2,3,4}, to_apply=add
//       r4 = u32[1,2] reduce(add4, c0), dimensions={2,3,4}, to_apply=add
//       r5 = u32[1,2] reduce(add5, c0), dimensions={2,3,4}, to_apply=add

//       out0 = u32[1,2] add(r1, r2)
//       out1 = u32[1,2] add(r3, r4)
//       out2 = u32[1,2] add(out0, out1)
//       ROOT out3 = u32[1,2] add(out2, r5)
//     }
//   )";
...
//     return std::distance(sequence.begin(), absl::c_find(sequence,
//     instruction));
//   };
