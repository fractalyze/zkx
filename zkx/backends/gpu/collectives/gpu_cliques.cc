...
#include "zkx/service/lockable.h"
...
// barrier at the same time) to guarantee that GPU communicators are used in a
// way that does not lead to a deadlock. This itself can create new deadlocks if
// thread pools are sized incorrectly.
//
// To make such issues easier to detect and debug, we set timeouts on
// rendezvous operations:
//   - If rendezvous takes too long, log a warning to indicate a potential
//     deadlock.
//   - If it still does not complete after a termination timeout, forcefully
//     terminate the process to avoid indefinite hangs.

absl::Duration WarnStuckTimeout() { return absl::Seconds(10); }

absl::Duration TerminateTimeout() {
  static const int64_t terminate_timeout =
      GetDebugOptionsFromFlags().zkx_gpu_nccl_termination_timeout_seconds();
  return (terminate_timeout >= 0) ? absl::Seconds(terminate_timeout)
                                  : absl::InfiniteDuration();
}

bool TerminateOnCollectivesError() {
  return GetDebugOptionsFromFlags().zkx_gpu_nccl_terminate_on_error();
}
...
ProcessGpuCliques& GetProcessGpuCliques() {
...
absl::Status CheckComm(Communicator* comm) {
...
void CheckClique(const GpuCliqueKey& clique_key,
  ...
  if (LockableGpuClique::Lock clique = lockable_clique.TryAcquire()) {
    ...
    clique->ForEachComm([](RankId rank, Communicator* comm) {
      absl::Status status = CheckComm(comm);
...
void GpuCliqueHeartBeatMonitorThread() {
...
void StartGpuCliqueHeartBeatMonitor() {
  std::ignore = absl::IgnoreLeak(tsl::Env::Default()->StartThread(
...
// and locking to guarantee that all collective operations in ZKX have a well
// defined order and do not deadlock inside underlying collective communication
// library.

auto DeviceRanksToString(absl::Span<const DeviceRank> ranks) {
...
absl::StatusOr<std::shared_ptr<LockableGpuClique::Lock>> InitializeGpuClique(
  ...
  static const int64_t nccl_init_rank_per_root_ratio =
      GetDebugOptionsFromFlags().zkx_gpu_nccl_init_max_rank_per_root_ratio();
  ...
  auto initialize = [&](absl::Span<const RendezvousArg* const> args)
    ...
    auto [it, inserted] = cliques.map.try_emplace(clique_key, clique_key,
                                                  clique_ids, std::move(comms));

    // We can have a race to create a clique for a given key, the winner
    // inserts it into a map and the loser destroys all communicators.
    if (inserted) {
      VLOG(3) << "Created new clique: " << it->second.DebugString();
    } else {
      VLOG(3) << "Clique already exists: " << it->second.DebugString();
    }
...
int32_t GetCommSplitColor(const GpuCliqueKey& clique_key) {
  ...
  // NOTE(chokobole): The static_cast<int32_t> can result in any int32_t value,
  // including INT_MIN. In C++, abs(INT_MIN) is undefined behavior because the
  // positive equivalent cannot be represented in a 32-bit signed integer.
  ...
  CHECK_NE(ret, std::numeric_limits<int32_t>::min());
  return abs(ret);
}
...
absl::StatusOr<std::shared_ptr<LockableGpuClique::Lock>> InitializeGpuClique(
  ...
  // Current approach for communicator splitting works because of ZKXs SPMD
  ...
  auto split = [&](absl::Span<const RankPair* const> rank_pairs)
    ...
    // Create a new clique with given clique key and communicators.
    auto [it, inserted] = cliques.map.try_emplace(
...
absl::StatusOr<std::shared_ptr<LockableGpuClique::Lock>> AcquireGpuClique(
  ...
  static const int64_t enable_nccl_comm_splitting =
      GetDebugOptionsFromFlags().zkx_gpu_enable_nccl_comm_splitting();
