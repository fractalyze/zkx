...
llvm::SmallVector<mlir::Value> ReplaceInductionVar(
  ...
  for (mlir::Value& index : indices) {
    ...
    mlir::Operation* op = index.getDefiningOp();
    ...
    mlir::SmallVector<mlir::Value> replaced_args = ReplaceInductionVar(
        induction_var, replacement, op->getOperands(), builder);
    index = builder
...
struct PipelineLoad : mlir::OpRewritePattern<Op> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    mlir::Value source = GetSource(op);
    ...
    mlir::SmallVector<mlir::Value> first_args =
...
int GetUnrollingFactor(mlir::scf::ForOp op) {
  ...
  if (std::optional<llvm::APInt> step = op.getConstantStep();
      !step || *step != 1) {
    return 1;
  }
  llvm::APInt lb, ub;
