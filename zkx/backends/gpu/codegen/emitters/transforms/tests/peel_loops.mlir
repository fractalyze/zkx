// RUN: emitters_opt -split-input-file %s -zkx-gpu-peel-loops \
// RUN: | FileCheck %s

#map = #zkx.indexing_map<"(d0)[s0, s1] -> (s0, s1), domain:"
  "d0 in [0, 3], s0 in [0, 7], s1 in [0, 10], d0 + s0 in [0, 9],"
  "d0 + s1 in [0, 12]">
func.func @peel_both_loops(%input: tensor<16x32xf32>,
    %init: f32, %dim: index) -> (f32) {
  %sum = zkx.loop (%dim)[%i, %j] -> (%r0, %r1)
...
// CHECK: #[[$PEELED_MAP:.*]] = #zkx.indexing_map<"(d0)[s0, s1] -> (s0, s1), domain: d0 in [0, 3], s0 in [0, 6], s1 in [0, 9]">
// CHECK: #[[$TAIL_MAP0:.*]] = #zkx.indexing_map<"(d0)[s0, s1] -> (7, s1), domain: d0 in [0, 2], s0 in [7, 7], s1 in [0, 9]">
// CHECK: #[[$TAIL_MAP1:.*]] = #zkx.indexing_map<"(d0)[s0, s1] -> (s0, 10), domain: d0 in [0, 2], s0 in [0, 7], s1 in [10, 10]">

// CHECK-LABEL: func.func @peel_both_loops(
// CHECK-SAME:      %[[INPUT:.*]]: tensor<16x32xf32>,
// CHECK-SAME:      %[[INIT:.*]]: f32, %[[DIM:.*]]: index)

// CHECK:      %[[PEELED:.*]] = zkx.loop (%[[DIM]])[%[[I:.*]], %[[J:.*]]] ->
// CHECK-SAME:     in #[[$PEELED_MAP]] iter_args(%[[INIT_:.*]] = %[[INIT]])
// CHECK:        tensor.extract %[[INPUT]][%[[I]], %[[J]]] : tensor<16x32xf32>
// CHECK:        arith.addf %[[INIT_]]

// CHECK:      %[[TAIL0:.*]] = zkx.loop (%[[DIM]])[%[[I:.*]], %[[J:.*]]]
// CHECK-SAME:     in #[[$TAIL_MAP0]] iter_args(%[[INIT_:.*]] = %[[PEELED]])
// CHECK:        tensor.extract %[[INPUT]][%[[I]], %[[J]]]
// CHECK:        arith.addf %[[INIT_]]

// CHECK:      %[[TAIL1:.*]] = zkx.loop (%[[DIM]])[%[[I:.*]], %[[J:.*]]]
