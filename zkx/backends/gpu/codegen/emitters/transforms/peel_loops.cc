...
struct PeelLoop : public OpRewritePattern<LoopOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    constexpr int kMaxCumulativeLoopSize = 64;
    ...
    SmallVector<IndexingMap> indexing_maps{std::move(indexing_map)};
    for (int sym_index = indexing_map.GetSymbolCount() - 1;
         sym_index >= 0 && cumulative_loop_size < kMaxCumulativeLoopSize;
         --sym_index) {
      const IndexingMap& indexing_map = indexing_maps.back();
      const Interval& bound = indexing_map.GetSymbolBound(sym_index);
      ...
      indexing_maps.push_back(std::move(tail_map));
      indexing_maps.push_back(std::move(peeled_map));
    }
    ...
    for (const IndexingMap& indexing_map : llvm::reverse(indexing_maps)) {
