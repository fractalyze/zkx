...
constexpr std::string_view kModule = R"(
    fused_computation {
      ROOT %p0 = s32[100] parameter(0)
    }

    ENTRY main {
      %p0 = s32[100] parameter(0)
      ROOT fusion = s32[100] fusion(%p0), kind=kLoop, calls=fused_computation
    })";
...
TEST_F(EmitterBaseTest, CreateMlirModule) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto filecheck_result, RunFileCheck(out, R"(
    ...
    // CHECK-SAME:     %[[IN:.*]]: tensor<100xi32> {zkx.slice_index = 0
    // CHECK-SAME:     %[[OUT:.*]]: tensor<100xi32> {zkx.slice_index = 1
...
TEST_F(EmitterBaseTest, CreateLLVMModule) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
    ...
    // CHECK:   %[[IN_PTR:.*]] = getelementptr inbounds i32, ptr %[[IN]], i32 %[[TID]]
    // CHECK:   %[[VAL:.*]] = load i32, ptr %[[IN_PTR]], align 4
    // CHECK:   %[[OUT_PTR:.*]] = getelementptr inbounds i32, ptr %[[OUT]], i32 %[[TID]]
    // CHECK:   store i32 %[[VAL]], ptr %[[OUT_PTR]], align 4
    // CHECK:   ret void
  )",
