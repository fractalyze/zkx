...
#ifndef ZKX_BACKENDS_GPU_RUNTIME_THUNK_H_
#define ZKX_BACKENDS_GPU_RUNTIME_THUNK_H_
...
// different devices (stream executors). For partitioned ZKX programs the
...
class Thunk {
 public:
  ...
  // at prepare stage. ZKX executable is responsible for allocating them before
  ...
  struct CollectiveExecuteParams {
    ...
    // ZKX execution run id allows us to distinguish collective operations
  ...
  struct ExecuteParams {
    ...
    // Send/Recv callbacks passed to ZKX from PjRt.
  ...
  static std::string_view KindToString(Thunk::Kind kind);
  ...
  static absl::StatusOr<GpuCollectives*> GetGpuCollectives(
    ...
    if (params.collective_params == nullptr) {
      return absl::InternalError("Collective params are not provided");
    }
    if (params.collective_params->collectives == nullptr) {
      return absl::InternalError("Collectives API is not provided");
    }
...
#endif  // ZKX_BACKENDS_GPU_RUNTIME_THUNK_H_
