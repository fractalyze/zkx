...
absl::StatusOr<se::StreamExecutor*> GpuExecutor() {
  auto name_status = PlatformUtil::CanonicalPlatformName("gpu");
  if (!name_status.ok()) {
    return name_status.status();
  }
  std::string name = absl::AsciiStrToUpper(name_status.value());
  auto platform_status = se::PlatformManager::PlatformWithName(name);
  if (!platform_status.ok()) {
    return platform_status.status();
  }
  auto executor_status = platform_status.value()->ExecutorForDevice(0);
  if (!executor_status.ok()) {
    return executor_status.status();
  }
  return executor_status.value();
}
...
TEST(CommandBufferCmdTest, MemcpyCmd) {
  auto executor_status = GpuExecutor();
  if (!executor_status.ok()) {
    GTEST_SKIP() << "Skipping: " << executor_status.status();
  }
