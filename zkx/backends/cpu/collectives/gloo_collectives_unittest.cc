...
#include "zkx/backends/cpu/collectives/gloo_kv_store.h"
...
absl::StatusOr<std::unique_ptr<Communicator>> GetCommunicator(
    size_t num_participants, absl::Span<const GlobalDeviceId> global_devices,
    const std::shared_ptr<KeyValueStoreInterface>& kv_store, int rank) {
  auto collectives = std::make_shared<cpu::GlooCollectives>(
      std::make_unique<GlooKeyValueStore>(kv_store),
#if defined(__linux__)
...
RendezvousKey MakeRendezvousKey(std::vector<GlobalDeviceId> global_devices) {
  return RendezvousKey(RunId(0), std::move(global_devices), kNumParticipants,
...
absl::StatusOr<std::vector<uint8_t>> AllReduce(
  ...
  TF_RETURN_IF_ERROR(communicator->AllReduce(
      ...
      PrimitiveType::U8, kBufferSize, ReductionKind::kSum, executor));
...
TEST(GlooCollectives, AllReduce) {
  ...
  for (int rank = 0; rank < kNumParticipants; ++rank) {
    ASSERT_TRUE(output_buffers[rank].status().ok());
  }
