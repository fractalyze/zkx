...
absl::Status ReduceScatter(ReductionKind reduction_kind,
  ...
  // Extension fields don't support min/max operations.
  if constexpr (primitive_util::IsExtensionFieldType(PT)) {
    if (reduction_kind == ReductionKind::kMin ||
        reduction_kind == ReductionKind::kMax) {
      return absl::InvalidArgumentError(
          "Min/Max reduction is not supported for extension field types");
    }
  }
