...
#ifndef ZKX_BACKENDS_CPU_RUNTIME_KERNEL_C_API_H_
#define ZKX_BACKENDS_CPU_RUNTIME_KERNEL_C_API_H_
...
// backend and a runtime. ZKX:CPU backend compiles fusion regions to native
...
typedef struct ZKX_CPU_KernelDim3 {
  uint64_t x;
  uint64_t y;
  uint64_t z;
} ZKX_CPU_KernelDim3;
...
typedef struct ZKX_CPU_KernelDim3 ZKX_CPU_KernelThreadDim;
...
typedef struct ZKX_CPU_KernelCallFrame {
  const ZKX_CPU_KernelThreadDim* thread_dims;
  const ZKX_CPU_KernelThread* thread;

  size_t num_args;
  const ZKX_CPU_KernelArg* args;
} ZKX_CPU_KernelCallFrame;

// Error reporting for host kernels. NULL means success.
typedef struct ZKX_CPU_KernelError ZKX_CPU_KernelError;

// Host kernel API.
typedef ZKX_CPU_KernelError* ZKX_CPU_Kernel(
    const ZKX_CPU_KernelCallFrame* call_frame);

#ifdef __cplusplus
}
#endif

#endif  // ZKX_BACKENDS_CPU_RUNTIME_KERNEL_C_API_H_
