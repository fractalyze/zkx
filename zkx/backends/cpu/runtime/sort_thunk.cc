...
absl::Status VerifySortInputs(absl::Span<const SortThunk::Input> inputs,
                              int64_t dimension) {
  // We should have at least one input buffer.
  if (inputs.empty()) {
    return absl::InternalError("Inputs must not be empty");
  }
  ...
  for (const SortThunk::Input& input : inputs) {
    if (!equal(shape, input.shape)) {
      return absl::InternalError("Inputs must have the same shape");
    }
  }
...
// The size of the largest element we support (bn254::Fr).
constexpr size_t kMaxElementSize = 32;
...
// We know that we can only copy up to 32 bytes for the largest element type
// and can specialize `std::memcpy` to allow LLVM to inline it with statically
// known sizes.
ABSL_ATTRIBUTE_ALWAYS_INLINE void Memcpy(void* __restrict dest,
...
ABSL_ATTRIBUTE_ALWAYS_INLINE void Swap(void* __restrict a, void* __restrict b,
                                       size_t n) {
  ...
  switch (n) {
    ...
    case 16:
      ...
      break;
    case 32:
      std::memcpy(tmp.data(), a, 32);
      std::memcpy(a, b, 32);
      std::memcpy(b, tmp.data(), 32);
      break;
    default:
...
SortDims GetSortDims(const Shape& shape, int64_t dimension) {
  int64_t sort_dimension =
      dimension >= 0 ? dimension : shape.rank() + dimension;
...
void Sort1DArrInplace(int64_t sort_dims_size, int64_t offset, Iterator begin,
...
void Sort1DArrInplace(const SortDims& sort_dims, int64_t offset,
...
template <size_t n>
void SortInplace(const SortDims& sort_dims, int64_t offset,
...
void DSortInplace(const SortDims& sort_dims, int64_t offset,
                  absl::Span<se::DeviceMemoryBase> data,
                  absl::Span<const Shape> shapes, bool is_stable,
...
absl::Status SortInplace(absl::Span<se::DeviceMemoryBase> data,
                         absl::Span<const Shape> shapes, int64_t dimension,
                         bool is_stable, SortThunk::LessThan* less_than,
  ...
  for (int64_t i = 0; i < sort_dims.num_iterations; ++i) {
    ...
    auto builtin_sort = [&](PrimitiveType type,
      ...
      primitive_util::ArrayTypeSwitch<void>(
          [&](auto cst_type) {
            if constexpr ((primitive_util::IsPrimeFieldType(cst_type) ||
...
tsl::AsyncValueRef<SortThunk::ExecuteEvent> SortThunk::Execute(
  ...
  absl::call_once(less_than_init_flag_, [&]() {
    ...
    if (ABSL_PREDICT_TRUE(comparator.ok())) {
      less_than_ = [comparator](const void** data) -> bool {
