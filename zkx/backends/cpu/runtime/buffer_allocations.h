...
#ifndef ZKX_BACKENDS_CPU_RUNTIME_BUFFER_ALLOCATIONS_H_
#define ZKX_BACKENDS_CPU_RUNTIME_BUFFER_ALLOCATIONS_H_
...
// particular ZKX execution. Buffers are indexed by the buffer allocation index.
...
BufferAllocations::GetDeviceAddress(BufferAllocation::Slice slice) const {
  ...
  if (ABSL_PREDICT_FALSE(offset < 0)) {
    return absl::InvalidArgumentError(
        absl::StrFormat("Buffer slice offset %d must be non-negative", offset));
  }
...
#endif  // ZKX_BACKENDS_CPU_RUNTIME_BUFFER_ALLOCATIONS_H_
