...
// If ZKX:CPU compiled with `-DZKX_CPU_USE_BLOCKING_THUNK_EXECUTOR` we'll run
...
absl::StatusOr<ThunkExecutor> ThunkExecutor::Create(
  ...
  if (thunk_sequence.size() > std::numeric_limits<NodeId>::max()) {
    return absl::InternalError(
        absl::StrFormat("Can't create ThunkExecutor with more than %d thunks",
...
tsl::AsyncValueRef<ThunkExecutor::ExecuteEvent> ThunkExecutor::Execute(
  ...
  // ZKX programs with many cheap operations.
  if (is_sequential_) {
