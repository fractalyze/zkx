...
#ifndef ZKX_BACKENDS_CPU_RUNTIME_THUNK_H_
#define ZKX_BACKENDS_CPU_RUNTIME_THUNK_H_
...
// TODO(chokobole): Update the comment below if the implementation approach for
// ZKX differs significantly from XLA, update this comment accordingly to
// reflect the new design decisions.

// WARNING: This is under construction. Long term plan for ZKX is to unify
// runtimes between different backends and have a shared Thunk interface,
// however for now we chose to have separate Thunk implementations in zkx::cpu
// and zkx::gpu namespaces with a plan to unify them in the future.

// Thunk is the basic unit of execution for the ZKX CPU runtime.
//
...
// different devices. For partitioned ZKX programs the expectation is that all
// local participants execute simultaneously on different threads and coordinate
// resource acquisition via rendezvous.
//
// This is ZKX CPU's counterpart of the ZKX GPU runtime Thunk.
class Thunk {
 public:
  enum class Kind {
    kUnknown,
  ...
  // pool. By default ZKX:CPU uses task runner that shares underlying thread
  ...
  class ExecuteSession {
   public:
    ...
    // configurable with ZKX_FLAGS. Also, we should find representative
 ...
 private:
  ...
  // TODO(chokobole): Figure out why is this not a static member variable.
...
#endif  // ZKX_BACKENDS_CPU_RUNTIME_THUNK_H_
