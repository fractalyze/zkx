...
KernelThunk<num_arguments, num_results>::KernelThunk(
  ...
  for (size_t i = 0; i < arguments_buffers.size(); ++i) {
    kernel_args_[i] = ZKX_CPU_KernelArg{
        .data = nullptr,
        .size = static_cast<size_t>(arguments_buffers_[i].size())};
  }
  for (size_t i = 0; i < results_buffers.size(); ++i) {
    kernel_args_[arguments_buffers_.size() + i] = ZKX_CPU_KernelArg{
        .data = nullptr,
        .size = static_cast<size_t>(results_buffers_[i].size())};
  }
}
...
KernelThunk<num_arguments, num_results>::ExecuteInternal(
  ...
  ZKX_CPU_KernelArg* kernel_args_ptr = kernel_args.data();
...
KernelThunk<num_arguments, num_results>::CheckInvariantBuffersMemory(
  ...
  auto results = absl::Span<const ZKX_CPU_KernelArg>(
  ...
  for (int64_t i = 0; i < arguments.size(); ++i) {
    const ZKX_CPU_KernelArg& argument = arguments[i];
    if (invariant_arguments_->contains(i)) {
      ...
      if (Contains(results, argument)) {
        return absl::InternalError("Mismatch in invariant buffers metadata");
      }
    } else {
...
absl::StatusOr<std::unique_ptr<Thunk>> KernelThunk::Create(
  ...
  if (min_alignment.has_value() && !absl::has_single_bit(*min_alignment)) {
    return absl::InternalError(absl::StrFormat(
        "Host kernel %s minimum alignment %d is not a power of 2", info.op_name,
        *min_alignment));
  }
