...
TEST(JitCompilerTest, Compile) {
  ...
  ASSERT_TRUE(add_module(add_in_place_ir, "AddInplace", 0).ok());
  ASSERT_TRUE(add_module(mul_in_place_ir, "MulInplace", 1).ok());
...
TEST(JitCompilerTest, ExternalDefinitionGenerator) {
  ...
  ASSERT_TRUE(compiler.AddModule(std::move(tsm)).ok());
