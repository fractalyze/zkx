...
absl::Status ObjectLoader::AddObjFile(
  ...
  if (dylib_index >= num_dylibs()) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "Invalid dylib index %d (num dylibs: %d))", dylib_index, num_dylibs()));
  }

  if (!obj_file) {
    return absl::InvalidArgumentError("Failed to create memory buffer");
  }
...
std::function<std::string(std::string_view)> ObjectLoader::GetMangler() {
  // Mangle symbol names for the target machine data layout.
  auto mangle = [this](std::string_view name) {
...
ObjectLoader::CreateFunctionLibrary(absl::Span<const Symbol> symbols,
  ...
  for (const Symbol& symbol : symbols) {
    llvm::orc::SymbolStringPtr symbol_name =
        execution_session()->intern(mangle(symbol.name));
