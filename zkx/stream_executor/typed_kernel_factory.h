...
#ifndef ZKX_STREAM_EXECUTOR_TYPED_KERNEL_FACTORY_H_
#define ZKX_STREAM_EXECUTOR_TYPED_KERNEL_FACTORY_H_
...
class TypedKernelFactory {
 public:
  ...
  static absl::StatusOr<TypedKernel<Params...>> Create(
      StreamExecutor *executor, std::string_view kernel_name,
      std::string_view ptx, absl::Span<const uint8_t> cubin_data) {
  ...
  static absl::StatusOr<TypedKernel<Params...>> Create(
      StreamExecutor *executor, std::string_view kernel_name, void *symbol) {
    MultiKernelLoaderSpec loader_spec(
        TypedKernel<Params...>::kNumberOfParameters);
...
#endif  // ZKX_STREAM_EXECUTOR_TYPED_KERNEL_FACTORY_H_
