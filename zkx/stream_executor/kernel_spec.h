...
#ifndef ZKX_STREAM_EXECUTOR_KERNEL_SPEC_H_
#define ZKX_STREAM_EXECUTOR_KERNEL_SPEC_H_
...
class KernelLoaderSpec {
 ...
 protected:
  explicit KernelLoaderSpec(std::string_view kernel_name);
...
class CudaPtxInMemory : public KernelLoaderSpec {
 public:
  ...
  typedef std::tuple<int, int, std::string_view> PtxSpec;
  ...
  //
  // Warning: the string backing the provided std::string_view ptx must outlive
  // this instance.
  CudaPtxInMemory(std::string_view ptx, std::string_view kernel_name);
...
class LlvmHostKernel : public KernelLoaderSpec {
 public:
  LlvmHostKernel(std::string_view ir, std::string_view entrypoint,
                 std::string_view kernel_name, absl::Span<std::string> options);

  std::string_view ir() const { return ir_; }
  std::string_view entrypoint() const { return entrypoint_; }
...
class MultiKernelLoaderSpec {
 public:
  ...
  MultiKernelLoaderSpec* AddCudaPtxInMemory(std::string_view ptx,
                                            std::string_view kernel_name);
  MultiKernelLoaderSpec* AddLlvmHostKernel(std::string_view ir,
...
#endif  // ZKX_STREAM_EXECUTOR_KERNEL_SPEC_H_
