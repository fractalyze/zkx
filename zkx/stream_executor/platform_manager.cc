...
class PlatformManagerImpl {
 public:
  ...
  absl::StatusOr<Platform*> PlatformWithName(std::string_view target)
      ABSL_LOCKS_EXCLUDED(mu_);
 ...
 private:
  ...
  // mutex is held.
  absl::StatusOr<Platform*> LookupByNameLocked(std::string_view target)
...
absl::Status PlatformManagerImpl::RegisterPlatform(
  ...
  if (name_map_.find(key) != name_map_.end()) {
    return absl::InternalError(absl::StrFormat(
        "platform is already registered with name: \"%s\"", platform->Name()));
  }
...
PlatformManagerImpl::InitializedPlatformNamesWithFilter(
  ...
  for (const auto& entry : id_map_) {
    ...
    if (filter(platform)) {
      if (platform->Initialized()) {
        initialized_platforms_names.push_back(std::string(platform->Name()));
      }
    }
  }
