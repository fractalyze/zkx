...
#ifndef ZKX_STREAM_EXECUTOR_STREAM_COMMON_H_
#define ZKX_STREAM_EXECUTOR_STREAM_COMMON_H_
...
class StreamCommon : public Stream {
 public:
  ...
  void ReturnSubStream(Stream* sub_stream) override ABSL_LOCKS_EXCLUDED(mu_);
 ...
 protected:
  bool InErrorState() const ABSL_LOCKS_EXCLUDED(mu_) {
  ...
  void CheckError(bool operation_retcode) ABSL_LOCKS_EXCLUDED(mu_);

  // Checks the status and logs the error message, if any.
  void CheckStatus(absl::Status status) ABSL_LOCKS_EXCLUDED(mu_);
...
#endif  // ZKX_STREAM_EXECUTOR_STREAM_COMMON_H_
