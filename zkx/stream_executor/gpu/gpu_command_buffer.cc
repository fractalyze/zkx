...
absl::Status UnsupportedStateError(State state) {
  return absl::InternalError(
      absl::StrCat("Unsupported command buffer state: ", StateToString(state)));
}
...
std::atomic<int64_t> g_allocated_execs(0);
std::atomic<int64_t> g_alive_execs(0);

}  // namespace

// static
int64_t GpuCommandBuffer::NotifyExecCreated() {
  g_alive_execs.fetch_add(1, std::memory_order_relaxed);
  return g_allocated_execs.fetch_add(1, std::memory_order_relaxed);
}

// static
int64_t GpuCommandBuffer::NotifyExecDestroyed() {
  DCHECK_GE(g_alive_execs.load(std::memory_order_relaxed), 1);
  return g_alive_execs.fetch_sub(1, std::memory_order_relaxed) - 1;
}

// static
int64_t GpuCommandBuffer::AliveExecs() {
  return g_alive_execs.load(std::memory_order_relaxed);
}
...
GpuCommandBuffer::Dependencies GpuCommandBuffer::GetBarrierDependencies(
  ...
  // Collect nodes that will become new barrier dependencies starting from
...
absl::Status GpuCommandBuffer::Case(ExecutionScopeId execution_scope_id,
  ...
  while (batch_offset < branches.size()) {
    // Conditionals will by default run branches[branches.size()-1] if index is
  ...
  if (mode_ == Mode::kPrimary && state_ == State::kCreate) {
    ...
    if (instantiated.code() == absl::StatusCode::kResourceExhausted) {
      return absl::ResourceExhaustedError(absl::StrFormat(
          ...
          "driver by reducing ZKX_CLIENT_MEM_FRACTION (b) Disable "
          "command buffers with 'ZKX_FLAGS=--zkx_gpu_enable_command_buffer=' "
