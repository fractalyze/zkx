...
#include "zkx/stream_executor/gpu/gpu_command_buffer.h"
...
#include "zkx/stream_executor/typed_kernel_factory.h"
...
GpuCommandBuffer* CastToGpuCommandBuffer(CommandBuffer* command_buffer) {
  return static_cast<GpuCommandBuffer*>(command_buffer);
}

Platform* GpuPlatform() {
  auto name = absl::AsciiStrToUpper(
      zkx::PlatformUtil::CanonicalPlatformName("gpu").value());
...
constexpr auto kNested = CommandBuffer::Mode::kNested;
constexpr auto kPrimary = CommandBuffer::Mode::kPrimary;

// Some of the tests rely on CUDA 12.3+ features.
bool IsAtLeastCuda12300(const stream_executor::StreamExecutor* executor) {
...
// Some of the tests rely on CUDA 12.4+ features.
bool IsAtLeastCuda12400(const stream_executor::StreamExecutor* executor) {
  if (executor->GetPlatform()->id() != cuda::kCudaPlatformId) {
    return false;
  }
  if (std::min({executor->GetDeviceDescription().runtime_version(),
...
TEST(GpuCommandBufferTest, LaunchSingleKernel) {
  ...
  auto cmd_buffer = executor->CreateCommandBuffer(kPrimary).value();
...
TEST(GpuCommandBufferTest, LaunchNestedCommandBuffer) {
  ...
  // Create a command buffer with a single kernel launch.
  auto primary_cmd = executor->CreateCommandBuffer(kPrimary).value();
  auto nested_cmd = executor->CreateCommandBuffer(kNested).value();
  ...
  // command buffer.
  nested_cmd = executor->CreateCommandBuffer(kNested).value();
...
TEST(GpuCommandBufferTest, Barriers) {
  ...
  auto transfer_buffers = [&]() -> std::vector<int32_t> {
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
      CHECK_OK(stream->Memcpy(dst.data() + i, buffers[i], sizeof(int32_t)));
    }
    return dst;
  };
...
TEST(GpuCommandBufferTest, ConditionalIfWithMemset) {
  ...
  if (!IsAtLeastCuda12400(executor)) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto cmd_buffer,
                          executor->CreateCommandBuffer(kPrimary));
  TF_ASSERT_OK(cmd_buffer->If(pred, then_builder));
  TF_ASSERT_OK(cmd_buffer->Finalize());
