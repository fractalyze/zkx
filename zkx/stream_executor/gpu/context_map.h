...
#ifndef ZKX_STREAM_EXECUTOR_GPU_CONTEXT_MAP_H_
#define ZKX_STREAM_EXECUTOR_GPU_CONTEXT_MAP_H_
...
class ContextMap {
 public:
  ...
  void Remove(GpuContext context) {
    ...
    auto ordinal_it = ordinal_to_type_map_.find(device_ordinal);
    if (ordinal_it != ordinal_to_type_map_.end()) {
      auto& contexts = ordinal_it->second;
      auto context_it = std::find(contexts.begin(), contexts.end(), context);
      if (context_it != contexts.end()) {
        contexts.erase(context_it);
        if (contexts.empty()) {
          ordinal_to_type_map_.erase(ordinal_it);
        }
      }
    }
  }
...
#endif  // ZKX_STREAM_EXECUTOR_GPU_CONTEXT_MAP_H_
