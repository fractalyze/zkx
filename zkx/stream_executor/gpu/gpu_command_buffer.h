...
#ifndef ZKX_STREAM_EXECUTOR_GPU_GPU_COMMAND_BUFFER_H_
#define ZKX_STREAM_EXECUTOR_GPU_GPU_COMMAND_BUFFER_H_
...
class GpuCommandBuffer : public CommandBuffer {
 ...
 protected:
  ...
  // allocates resources on GPU devices (rule of thumb is ~8kb per node), so
 ...
 private:
  ...
  // kernel nodes, however a large number of no-op kernels impacts performance.
 ...
 private:
  ...
  virtual absl::Status WriteGraphToDotFile(std::string_view path) = 0;
...
#endif  // ZKX_STREAM_EXECUTOR_GPU_GPU_COMMAND_BUFFER_H_
