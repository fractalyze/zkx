...
std::string_view ToString(nvPTXCompileResult status) {
  switch (status) {
...
absl::StatusOr<std::vector<uint8_t>> CompileGpuAsmUsingLibNvPtxCompiler(
    ...
    const GpuAsmOpts& old_options, bool cancel_if_reg_spill) {
  GpuAsmOpts options = old_options;
  ...
  if (compile_result != NVPTXCOMPILE_SUCCESS) {
    ...
    RETURN_IF_NVPTXCOMPILER_ERROR(
        nvPTXCompilerGetErrorLog(compiler_handle, error_log_buffer.data()));
    std::string error_log(error_log_buffer.data());
  ...
  RETURN_IF_NVPTXCOMPILER_ERROR(nvPTXCompilerGetCompiledProgram(
      compiler_handle, reinterpret_cast<char*>(cubin.data())));
