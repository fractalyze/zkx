...
absl::StatusOr<std::string> GetToolVersionString(std::string_view binary_path) {
...
absl::StatusOr<SemanticVersion> GetToolVersionImpl(std::string_view tool_path) {
  ...
  return SemanticVersion{vmaj, vmin, vdot};
}
...
absl::StatusOr<SemanticVersion> GetToolVersion(std::string_view tool_path) {
  ...
  static absl::Mutex s_mutex(absl::kConstInit);
  static absl::flat_hash_map<std::string, absl::StatusOr<SemanticVersion>>
      s_cache ABSL_GUARDED_BY(s_mutex);

  absl::MutexLock lock(&s_mutex);
  auto it = s_cache.find(tool_path);
  if (it != s_cache.end()) {
    return it->second;
  }

  return s_cache.try_emplace(tool_path, GetToolVersionImpl(tool_path))
      .first->second;
}
...
void LogPtxasTooOld(const std::string& ptxas_path, int cc_major, int cc_minor) {
  using AlreadyLoggedSetTy =
      absl::flat_hash_set<std::tuple<std::string, int, int>>;

  static absl::Mutex s_mutex(absl::kConstInit);
  static AlreadyLoggedSetTy s_already_logged ABSL_GUARDED_BY(s_mutex);

  absl::MutexLock lock(&s_mutex);
  if (s_already_logged.insert(std::make_tuple(ptxas_path, cc_major, cc_minor))
          .second) {
...
void AppendArgsFromOptions(const GpuAsmOpts& options,
...
absl::StatusOr<std::vector<uint8_t>> CompileGpuAsmUsingPtxAs(
  ...
  absl::Cleanup ptx_cleaner = [&ptx_path] {
    tsl::Env::Default()->DeleteFile(ptx_path).IgnoreError();
  };
...
absl::StatusOr<std::vector<uint8_t>> BundleGpuAsmUsingFatbin(
  ...
  for (const CubinOrPTXImage& img : images) {
    ...
    TF_RETURN_IF_ERROR(tsl::WriteStringToFile(
        env, img_path,
        std::string_view(reinterpret_cast<const char*>(img.bytes.data()),
                         img.bytes.size())));
...
absl::StatusOr<std::vector<uint8_t>> LinkUsingNvlink(
    CudaComputeCapability cc, std::string_view preferred_cuda_dir,
...
absl::StatusOr<std::vector<uint8_t>> LinkUsingNvlink(
    std::string_view nvlink_path, CudaComputeCapability cc,
    absl::Span<const std::vector<uint8_t>> images) {
  ...
  absl::Cleanup cleaners = [&] {
    for (auto& f : temp_files) {
      tsl::Env::Default()->DeleteFile(f).IgnoreError();
    }
  };
  for (int i = 0; i < images.size(); i++) {
    ...
    TF_RETURN_IF_ERROR(tsl::WriteStringToFile(
        env, temp_files.back(),
        std::string_view(reinterpret_cast<const char*>(images[i].data()),
                         images[i].size())));
  }
  ...
  std::string_view extension = ShouldUsePtxExtension(cc) ? "a" : "";
