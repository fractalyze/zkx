...
absl::StatusOr<std::vector<uint8_t>> CompileHelper(
    CudaComputeCapability cc, const char* const ptx_input,
    bool disable_gpuasm_optimizations = false, bool cancel_if_reg_spill = false,
    std::vector<std::string> extra_flags = {}) {
  GpuAsmOpts options(disable_gpuasm_optimizations,
                     /*preferred_cuda_dir=*/"", extra_flags);

  return CompileGpuAsmUsingLibNvPtxCompiler(cc, ptx_input, options,
                                            cancel_if_reg_spill);
}
