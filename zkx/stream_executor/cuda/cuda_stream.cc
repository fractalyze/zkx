...
absl::Status WaitStreamOnEvent(StreamExecutor* executor, CUstream stream,
  ...
  return cuda::ToStatus(cuStreamWaitEvent(stream, event, /*flags=*/0));
}
...
int GetGpuStreamPriority(StreamPriority stream_priority) {
  if (stream_priority == StreamPriority::Default) {
    return 0;
  }
  int lowest, highest;
  ...
  return stream_priority == StreamPriority::Highest ? highest : lowest;
}
...
absl::Status LaunchCudaKernel(StreamExecutor* executor,
                              std::string_view kernel_name, CUfunction function,
...
absl::Status LaunchCudaKernel(
    StreamExecutor* executor, std::string_view kernel_name, CUfunction function,
...
absl::Status CudaStream::LaunchKernel(
    ...
    std::string_view name, void** args, int64_t shmem_bytes) {
