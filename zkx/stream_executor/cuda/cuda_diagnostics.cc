...
#if !defined(PLATFORM_WINDOWS)
constexpr char kDriverVersionPath[] = "/proc/driver/nvidia/version";
#else
constexpr char kDriverVersionPath[] = "NO NVIDIA DRIVER VERSION FILE";
#endif
...
// static
void Diagnostician::LogDriverVersionInformation() {
...
absl::StatusOr<DriverVersion> Diagnostician::FindDsoVersion() {
  ...
  auto iterate_phdr = [](struct dl_phdr_info *info, size_t size,
    ...
    std::string_view resolved_path(resolved_path_buf);
    VLOG(1) << "found DLL info with resolved path: " << resolved_path;
    size_t slash = resolved_path.rfind('/');
    if (slash == std::string_view::npos) {
      return 0;
    }
    std::string_view so_suffix = ".so.";
    size_t dot = resolved_path.find(so_suffix, slash);
    if (dot == std::string_view::npos) {
      return 0;
    }

    std::string_view dso_version = resolved_path.substr(dot + so_suffix.size());
    std::string_view stripped_dso_version =
