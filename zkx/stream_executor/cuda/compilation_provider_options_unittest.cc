...
TEST(CompilationProviderOptionsTest, FromDebugOptions) {
  zkx::DebugOptions debug_options;
  debug_options.set_zkx_gpu_libnvjitlink_mode(
      zkx::DebugOptions::LIB_NV_JIT_LINK_MODE_ENABLED);
  debug_options.set_zkx_gpu_enable_libnvptxcompiler(true);
  debug_options.set_zkx_gpu_enable_llvm_module_compilation_parallelism(true);
  debug_options.set_zkx_gpu_unsafe_fallback_to_driver_on_ptxas_not_found(true);
  debug_options.set_zkx_gpu_cuda_data_dir("/usr/local/cuda");
  ...
  debug_options.set_zkx_gpu_libnvjitlink_mode(
      zkx::DebugOptions::LIB_NV_JIT_LINK_MODE_DISABLED);
  EXPECT_EQ(CompilationProviderOptions::FromDebugOptions(debug_options),
            CompilationProviderOptions(
  ...
  debug_options.set_zkx_gpu_libnvjitlink_mode(
      zkx::DebugOptions::LIB_NV_JIT_LINK_MODE_AUTO);
  EXPECT_EQ(CompilationProviderOptions::FromDebugOptions(debug_options),
            CompilationProviderOptions(
                CompilationProviderOptions::NvJitLinkMode::kAuto, true, true,
                true, "/usr/local/cuda"));

  debug_options.set_zkx_gpu_enable_libnvptxcompiler(false);
  EXPECT_EQ(CompilationProviderOptions::FromDebugOptions(debug_options),
            CompilationProviderOptions(
                CompilationProviderOptions::NvJitLinkMode::kAuto, false, true,
                true, "/usr/local/cuda"));

  debug_options.set_zkx_gpu_enable_llvm_module_compilation_parallelism(false);
  EXPECT_EQ(CompilationProviderOptions::FromDebugOptions(debug_options),
            CompilationProviderOptions(
                CompilationProviderOptions::NvJitLinkMode::kAuto, false, false,
                true, "/usr/local/cuda"));

  debug_options.set_zkx_gpu_unsafe_fallback_to_driver_on_ptxas_not_found(false);
  EXPECT_EQ(CompilationProviderOptions::FromDebugOptions(debug_options),
            CompilationProviderOptions(
                CompilationProviderOptions::NvJitLinkMode::kAuto, false, false,
                false, "/usr/local/cuda"));

  debug_options.set_zkx_gpu_cuda_data_dir("");
  EXPECT_EQ(CompilationProviderOptions::FromDebugOptions(debug_options),
            CompilationProviderOptions(
                CompilationProviderOptions::NvJitLinkMode::kAuto, false, false,
                false, ""));
