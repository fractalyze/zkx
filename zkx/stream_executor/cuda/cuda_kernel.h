...
#ifndef ZKX_STREAM_EXECUTOR_CUDA_CUDA_KERNEL_H_
#define ZKX_STREAM_EXECUTOR_CUDA_CUDA_KERNEL_H_
...
class CudaKernel : public Kernel {
 ...
 private:
  ...
  StreamExecutor* executor_ = nullptr;  // not owned

  // wrapped CUDA kernel handle
  CUfunction gpu_function_ = nullptr;  // not owned
  unsigned arity_ = 0;  // number of formal parameters the kernel takes
};

}  // namespace stream_executor::gpu

#endif  // ZKX_STREAM_EXECUTOR_CUDA_CUDA_KERNEL_H_
