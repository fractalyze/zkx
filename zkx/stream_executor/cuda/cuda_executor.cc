...
#include "zkx/stream_executor/cuda/cuda_command_buffer.h"
#include "zkx/stream_executor/cuda/cuda_context.h"
#include "zkx/stream_executor/cuda/cuda_event.h"
#include "zkx/stream_executor/cuda/cuda_kernel.h"
#include "zkx/stream_executor/cuda/cuda_platform_id.h"
#include "zkx/stream_executor/cuda/cuda_status.h"
#include "zkx/stream_executor/cuda/cuda_stream.h"
#include "zkx/stream_executor/cuda/cuda_timer.h"
...
#include "zkx/stream_executor/generic_memory_allocator.h"
#include "zkx/stream_executor/gpu/context.h"
#include "zkx/stream_executor/gpu/read_numa_node.h"
#include "zkx/stream_executor/gpu/scoped_activate_context.h"
...
tsl::thread::ThreadPool* GetDriverExecutor() {
  static tsl::thread::ThreadPool* thread_pool =
      absl::IgnoreLeak(new tsl::thread::ThreadPool(
          tsl::Env::Default(), tsl::ThreadOptions(), "cuda_driver", 1));
  return thread_pool;
}

// Loads ptx_contents with the CUDA driver's PTX JIT and returns the resulting
...
absl::StatusOr<CUmodule> LoadPtx(Context* context, const char* ptx_contents) {
  ...
  GetDriverExecutor()->Schedule(
      [context, ptx_contents, &module, &returned_status, &notification]() {
        ...
        constexpr unsigned int kLogBufferBytesLimit = 1024;
        ...
        CHECK_EQ(std::size(options), std::size(option_values));

        absl::Status status;
        status = cuda::ToStatus(cuModuleLoadDataEx(
            &module, ptx_data, std::size(options), options, option_values));
...
template <typename T>
absl::StatusOr<T> GetSimpleAttribute(CUdevice device,
...
absl::Status EnablePeerAccess(Context* from, Context* to) {
  ...
  CUresult result = cuCtxEnablePeerAccess(
      tensorflow::down_cast<CudaContext*>(to)->context(), /*flags=*/0);
...
CUdeviceptr AsCudaDevicePtr(const DeviceMemoryBase& gpu_mem) {
  return reinterpret_cast<CUdeviceptr>(gpu_mem.opaque());
}

// See description on const version above.
CUdeviceptr AsCudaDevicePtr(DeviceMemoryBase* gpu_mem) {
  ...
  if (type == MemoryType::kUnified) {
    ...
    return std::make_unique<GenericMemoryAllocator>(
        [](uint64_t size) -> absl::StatusOr<std::unique_ptr<MemoryAllocation>> {
          // clang-format off
          // TODO(chokobole): Uncomment this. Dependency: CudaCollectives::CollectiveMemoryAllocate
          // clang-format on
          //   TF_ASSIGN_OR_RETURN(
          //       void* ptr, CudaCollectives::CollectiveMemoryAllocate(this,
          //       size));
          //   VLOG(2) << "allocated " << ptr << " for context " <<
          //   cuda_context_
          //           << " of " << size << " bytes of collective memory";
          //   return std::make_unique<GenericMemoryAllocation>(
          //       ptr, size, [this](void* location, uint64_t size) {
          //         auto status =
          //             CudaCollectives::CollectiveMemoryDeallocate(this,
          //             location);
          //         if (!status.ok()) {
          //           LOG(ERROR) << "failed to free collective memory at "
          //                      << location << "; result: " << status;
          //         } else {
          //           VLOG(2) << "deallocated collective memory at " <<
          //           location
          //                   << " for context " << cuda_context_;
          //         }
          //       });
...
void CudaExecutor::UnloadKernel(const Kernel* kernel) {
  ...
  if (kernel_to_gpu_binary_.end() == gpu_binary_it) {
    // We might never see kernel being explicitly loaded if it was resolved
    // from in process symbol pointer (CUDA C++ device function pointer).
...
absl::uint128 Fingerprint128(const std::string_view s) {
...
CudaExecutor::CreateOrShareConstant(Stream* stream,
  ...
  // (highly unlikely) event of a hash collision, the program will likely
  // crash (because the cached constant that will be returned by mistake is
  // unlikely to have the correct size).
  absl::uint128 fingerprint = Fingerprint128(std::string_view(
...
CudaExecutor::CreateDeviceDescription(int device_ordinal) {
  ...
  {
    // TODO(b/381052076): Return an error instead of silent failure once TF
  ...
  desc.set_model_str(absl::StrFormat(
      "sm_%d.%d with %dB RAM, %d cores, %dKHz clock, %dKHz mem clock, %dB "
      "L2$",
