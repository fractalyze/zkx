...
static void TestParseFlagsFromEnv(const char* msg) {
  ...
  ParseFlagsFromEnvAndDieIfUnknown("ZKX_FLAGS", flag_list);
  ASSERT_EQ(*pargc, 1) << msg;
  const std::vector<char*>& argv_second = *pargv;
  ASSERT_NE(argv_second[0], nullptr) << msg;
  ASSERT_EQ(argv_second[1], nullptr) << msg;
  ASSERT_TRUE(simple) << msg;
  ASSERT_EQ(with_value, "a_value") << msg;
  ASSERT_EQ(embedded_quotes, "single'double\"") << msg;
  ASSERT_EQ(single_quoted, "single quoted \\\\ \n \"") << msg;
  ASSERT_EQ(double_quoted, "double quoted \\ \n '\"") << msg;
}
...
TEST(ParseFlagsFromEnv, Basic) {
  // Prepare environment.
  tsl::setenv("ZKX_FLAGS", kTestFlagString, true /*overwrite*/);
...
TEST(ParseFlagsFromEnv, File) {
  ...
  for (int i = 0; i != std::size(kTempVars) && tmp_dir == nullptr; i++) {
  ...
  ASSERT_NE(fp, nullptr) << "can't write to " << tmp_file;
  for (int i = 0; kTestFlagString[i] != '\0'; i++) {
    putc(kTestFlagString[i], fp);
  }
  fflush(fp);
  ASSERT_EQ(ferror(fp), 0) << "writes failed to " << tmp_file;
  fclose(fp);
  // Prepare environment.
  tsl::setenv("ZKX_FLAGS", tmp_file.c_str(), true /*overwrite*/);
...
TEST(ParseFlagsFromEnv, EnvAndFlag) {
  ...
  for (int i = 0; i != std::size(test); i++) {
    if (test[i].env == nullptr) {
      ...
      tsl::unsetenv("ZKX_FLAGS");
    } else {
      tsl::setenv("ZKX_FLAGS", test[i].env, /*overwrite=*/true);
    }
...
TEST(ParseFlagsFromEnv, ErrorOutOnFlagFailure) {
  ...
  if (env == nullptr) {
    ...
    tsl::setenv("ZKX_FLAGS", env, /*overwrite=*/true);
  }
  tsl::SubProcess child;
  std::vector<std::string> argv;
  argv.push_back(binary_name);
...
TEST(ParseFlagsFromEnv, UknownFlagErrorMessage) {
  ...
  EXPECT_THAT(
      stderr_str,
      ::testing::EndsWith("Unknown flags in ZKX_FLAGS: "
...
int main(int argc, char* argv[]) {
  ...
  zkx::ParseFlagsFromEnvAndDieIfUnknown("ZKX_FLAGS", flag_list);
